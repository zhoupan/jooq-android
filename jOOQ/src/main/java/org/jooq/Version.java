...
public interface Version extends Node<Version> {
   ...
   *
   * <p>In jOOQ's commercial distributions, this method allows for migrating between versions in any
   * direction, regardless of which version was "first" in a version graph, or if the two versions
   * are on different branches. The resulting queries are potentially destructive in such a case.
   * Such destructive queries ("UNDO" migrations) are prevented by default, and can be turned on
   * using {@link Settings#isMigrationAllowsUndo()}.
   *
   * <p>In jOOQ's Open Source Edition, this method only allows for migrating "forward".
   */
  @NotNull
   ...
   *
   * <p>This calculates a migration path using {@link Meta#migrateTo(Meta)}.
   */
  @NotNull
