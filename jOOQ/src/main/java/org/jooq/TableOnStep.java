...
/**
 * An intermediate type for the construction of a <code>JOIN</code> clause, where there must be a
 * join criteria added using an <code>ON</code> clause (with a {@link Condition}), or using a <code>
 * USING</code> clause (with a list of {@link Field}).
 *
 * <p>
 *
...
public interface TableOnStep<R extends Record> {
  ...
  /**
   * Add an <code>ON</code> clause to the <code>JOIN</code>, connecting them with each other with
   * {@link Operator#AND}.
   */
  @NotNull
  @Support
   ...
   *
   * <p>See {@link #onKey(ForeignKey)} for examples.
   *
