 ...
 *
 * <p>This type is the model API representation of a {@link Delete} statement, which can be mutated
 * after creation. The advantage of this API compared to the DSL API is a more simple approach to
 * writing dynamic SQL.
 *
 * <p>Instances can be created using {@link DSLContext#deleteQuery(Table)}.
 *
...
public interface DeleteQuery<R extends Record> extends ConditionProvider, Delete<R> {
   ...
   *
   * <p>If the <code>DELETE</code> statement returns several records, this is the same as calling
   * <code>getReturnedRecords().get(0)</code>
   *
   * <p>This implemented differently for every dialect:
   *
   * <ul>
   *   <li>Firebird and Postgres have native support for <code>DELETE .. RETURNING</code> clauses
   * </ul>
   *
