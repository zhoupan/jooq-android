...
/**
 * A contextual DSL providing "attached" implementations to the <code>org.jooq</code> interfaces.
 *
 * <p>Apart from the {@link DSL}, this contextual DSL is the main entry point for client code, to
 * access jOOQ classes and functionality that are related to {@link Query} execution. Unlike objects
 * created through the <code>DSL</code> type, objects created from a <code>DSLContext</code> will be
 * "attached" to the <code>DSLContext</code>'s {@link #configuration()}, such that they can be
 ...
 * </pre></code>
 *
 * <p>The <code>DSL</code> provides convenient constructors to create a {@link Configuration}, which
 * will be shared among all <code>Query</code> objects thus created. Optionally, you can pass a
 * reusable <code>Configuration</code> to the {@link DSL#using(Configuration)} constructor. Please
 * consider thread-safety concerns documented in {@link Configuration}, should you want to reuse the
 * same <code>Configuration</code> instance in various threads and / or transactions.
 *
 * @see DSL
 * @see Configuration
 * @author Lukas Eder
...
public interface DSLContext extends Scope {
   ...
   *
   * <p>This will map a schema onto another one, depending on configured schema mapping in this
   * <code>DSLContext</code>. If no applicable schema mapping can be found, the schema itself is
   * returned.
   *
   ...
   *
   * <p>This will map a table onto another one, depending on configured table mapping in this <code>
   * DSLContext</code>. If no applicable table mapping can be found, the table itself is returned.
   *
   * @param table A table
  ...
  /**
   * A JDBC connection that runs each statement through the {@link #parser()} first, prior to
   * re-generating and running the SQL.
   *
   * <p>Static statements are translated eagerly upon execution, e.g. of {@link
   * java.sql.Statement#executeQuery(String)}. Prepared statements are prepared lazily once all bind
   * variables are available, because the specific bind value and type may influence the generated
   * SQL. As such, a {@link PreparedStatement} created from a parsing connection does not yet
   * allocate any server side resources until it is executed for the first time.
   *
   * <p>The {@link Configuration#cacheProvider()} is called for {@link
   * CacheType#CACHE_PARSING_CONNECTION} to provide a translation cache to avoid the overhead of
   * re-parsing and re-generating the same SQL string all the time. By default, this is an LRU
   * cache.
   *
   * <p>The resulting {@link Connection} wraps an underlying JDBC connection that has been obtained
   * from {@link ConnectionProvider#acquire()} and must be released by calling {@link
   * Connection#close()}, which calls {@link ConnectionProvider#release(Connection)}.
   */
  @NotNull
  Connection parsingConnection();

  /**
   * A JDBC data source that runs each statement through the {@link #parser()} first, prior to
   * re-generating and running the SQL.
   *
   * <p>This simply wraps the {@link #parsingConnection()} in a {@link DataSource}.
   */
  @NotNull
  DataSource parsingDataSource();

  /**
   * An R2DBC {@link ConnectionFactory} that runs each statement through the {@link #parser()}
  ...
  /**
   * A JDBC connection that proxies the underlying connection to run the jOOQ Diagnostics Pack on
   * executed queries.
   *
   * <p><strong>This is experimental functionality.</strong>
   */
  @NotNull
   ...
   * executed queries.
   *
   * <p>This simply wraps the {@link #diagnosticsConnection()} in a {@link DataSource}.
   */
  @NotNull
   ...
   *
   * <p>This method returns meta information provided by {@link Configuration#metaProvider()}, which
   * defaults to a wrapper type that gives access to your JDBC connection's {@link DatabaseMetaData}
   * as obtained from your {@link ConnectionProvider}.
   *
  ...
  /** Access the database meta data from an explicit JDBC {@link DatabaseMetaData}. */
  @NotNull
   ...
   *
   * <p>This will not connect to your database to get live meta information, unlike {@link #meta()}
   * and {@link #meta(DatabaseMetaData)}.
   */
  @NotNull
   ...
   *
   * <p>This is convenience for wrapping all argument {@link String}s in {@link Source}. The same
   * set of content types are supported as in {@link #meta(Source...)}.
   *
   * @throws DataDefinitionException if the sources do not produce consistent meta data.
   ...
   * Create meta data from a set of sources.
   *
   * <p>This method creates a {@link Meta} representation from a set of source content, which can be
   * any of:
   *
   * <ul>
   *   <li>A set of DDL scripts, which will be parsed using {@link #parser()}.
   *   <li>A set of XML files, which will be unmarshalled into {@link InformationSchema} objects.
   * </ul>
   *
   * <p>This will not connect to your database to get live meta information, unlike {@link #meta()}
   * and {@link #meta(DatabaseMetaData)}.
   ...
   *
   * <p>This works the same way as {@link #meta(Source...)}, without the need of parsing the DDL
   * scripts.
   *
   * @throws DataDefinitionException if the sources do not produce consistent meta data.
   */
   ...
   * transactional</code>'s outcome.
   *
   * <p>The argument transactional code should not capture any scope but derive its {@link
   * Configuration} from the {@link TransactionalCallable#run(Configuration)} argument in order to
   * create new statements.
   *
   * @param transactional The transactional code
   * @return The transactional outcome
   * @throws RuntimeException any runtime exception thrown by the <code>transactional</code> logic,
   *     indicating that a rollback has occurred.
   * @throws DataAccessException any database problem that may have arised when executing the <code>
   *     transactional</code> logic, or a wrapper for any checked exception thrown by the <code>
   *     transactional</code> logic, indicating that a rollback has occurred.
   */
  <T> T transactionResult(TransactionalCallable<T> transactional);

  /**
   * Run a {@link ContextTransactionalRunnable} in the context of this <code>DSLContext</code>'s
   * underlying {@link #configuration()}'s {@link Configuration#transactionProvider()}, and return
   * the <code>transactional</code>'s outcome.
   *
   * <p>The argument transactional code may capture scope to derive its {@link Configuration} from
   * the "context" in order to create new statements. This context can be provided, for instance, by
   * {@link ThreadLocalTransactionProvider} automatically.
   *
   * @param transactional The transactional code
   * @return The transactional outcome
   * @throws ConfigurationException if the underlying {@link Configuration#transactionProvider()} is
   *     not able to provide context (i.e. currently, it is not a {@link
  ...
  /**
   * Run a {@link TransactionalRunnable} in the context of this <code>DSLContext</code>'s underlying
   * {@link #configuration()}'s {@link Configuration#transactionProvider()}.
   *
   * <p>The argument transactional code should not capture any scope but derive its {@link
   * Configuration} from the {@link TransactionalCallable#run(Configuration)} argument in order to
   * create new statements.
  ...
  /**
   * Run a {@link ContextTransactionalRunnable} in the context of this <code>DSLContext</code>'s
   * underlying {@link #configuration()}'s {@link Configuration#transactionProvider()}.
   *
   * <p>The argument transactional code may capture scope to derive its {@link Configuration} from
   * the "context" in order to create new statements. This context can be provided, for instance, by
   * {@link ThreadLocalTransactionProvider} automatically.
   ...
   *
   * <p>The <code>TransactionCallable</code> is run in the context of this <code>DSLContext</code>'s
   * underlying {@link #configuration()}'s {@link Configuration#transactionProvider()}, and returns
   * the <code>transactional</code>'s outcome in a new {@link CompletionStage} that is
   * asynchronously completed by a task run by an {@link Executor} provided by the underlying {@link
   * #configuration()}'s {@link Configuration#executorProvider()}.
   *
   * @param transactional The transactional code
   * @return The transactional outcome
   ...
   *
   * <p>The <code>TransactionRunnable</code> is run in the context of this <code>DSLContext</code>'s
   * underlying {@link #configuration()}'s {@link Configuration#transactionProvider()}, and returns
   * the <code>transactional</code>'s outcome in a new {@link CompletionStage} that is
   * asynchronously completed by a task run by an {@link Executor} provided by the underlying {@link
   * #configuration()}'s {@link Configuration#executorProvider()}.
   ...
   * underlying {@link #configuration()}'s {@link Configuration#transactionProvider()}, and returns
   * the <code>transactional</code>'s outcome in a new {@link CompletionStage} that is
   * asynchronously completed by a task run by a given {@link Executor}.
   *
   * @param transactional The transactional code
   * @return The transactional outcome
   * @throws ConfigurationException If this is run with a {@link ThreadLocalTransactionProvider}.
  ...
  <T> CompletionStage<T> transactionResultAsync(
      Executor executor, TransactionalCallable<T> transactional) throws ConfigurationException;
  ...
  /**
   * Run a {@link ConnectionCallable} in the context of this <code>DSLContext</code>'s underlying
   * {@link #configuration()}'s {@link Configuration#connectionProvider()}.
   *
   * @param callable The code running statements against the <code>connection</code>.
   * @return The outcome of the callable
   */
  <T> T connectionResult(ConnectionCallable<T> callable);

  /**
   * Run a {@link ConnectionRunnable} in the context of this <code>DSLContext</code>'s underlying
   * {@link #configuration()}'s {@link Configuration#connectionProvider()}.
   *
   * @param runnable The code running statements against the <code>connection</code>.
   */
  void connection(ConnectionRunnable runnable);

  /**
   * Run a {@link MockRunnable} in the context of this <code>DSLContext</code> 's underlying {@link
   * #configuration()}'s, and of a {@link MockDataProvider} and return the <code>mockable</code>'s
   * outcome.
   */
  ...
  /**
   * Run a {@link MockRunnable} in the context of this <code>DSLContext</code> 's underlying {@link
   * #configuration()}'s, and of a {@link MockDataProvider}.
   */
   ...
   *
   * <p>This will return an initialised render context as such:
   *
   * <ul>
   *   <li><code>
   *       {@link RenderContext#castMode()} == {@link org.jooq.RenderContext.CastMode#DEFAULT DEFAULT}
   *       </code>
   *   <li><code>{@link RenderContext#declareFields()} == false</code>
   *   <li><code>{@link RenderContext#declareTables()} == false</code>
   *   <li><code>{@link RenderContext#format()} == false</code>
   *   <li><code>{@link RenderContext#paramType()} == {@link ParamType#INDEXED}</code>
   *   <li><code>{@link RenderContext#qualify()} == true</code>
   *   <li><code>{@link RenderContext#subquery()} == false</code>
   * </ul>
   *
   ...
   *
   * <p>This is the same as calling <code>renderContext().render(part)</code>
   *
  ...
  /**
   * Render a QueryPart in the context of this <code>DSLContext</code>, rendering bind variables as
   * named parameters.
   *
   * <p>This is the same as calling <code>renderContext().paramType(NAMED).render(part)</code>
   *
   * @param part The {@link QueryPart} to be rendered
   * @return The rendered SQL
   */
  ...
  /**
   * Render a QueryPart in the context of this <code>DSLContext</code>, rendering bind variables as
   * named parameters, or inlined parameters if they have no name.
   *
   * <p>This is the same as calling <code>renderContext().paramType(NAMED_OR_INLINED).render(part)
   * </code>
   *
   * @param part The {@link QueryPart} to be rendered
   * @return The rendered SQL
  ...
  /**
   * Render a QueryPart in the context of this <code>DSLContext</code>, inlining all bind variables.
   *
   * <p>This is the same as calling <code>renderContext().inline(true).render(part)</code>
   *
   * @param part The {@link QueryPart} to be rendered
   * @return The rendered SQL
   */
  ...
  /**
   * Retrieve the bind values that will be bound by a given <code>QueryPart</code>.
   *
   * <p>The returned <code>List</code> is immutable. To modify bind values, use {@link
   * #extractParams(QueryPart)} instead.
   *
   * <p>Unlike {@link #extractParams(QueryPart)}, which returns also inlined parameters, this
   * returns only actual bind values that will render an actual bind value as a question mark <code>
   * "?"</code>
   */
  @NotNull
   ...
   *
   * <p>The <code>Map</code> itself is immutable, but the {@link Param} elements allow for modifying
   * bind values on an existing {@link Query} (or any other {@link QueryPart}).
   *
   * <p>Bind values created with {@link DSL#val(Object)} will have their bind index as name.
   *
   * @see Param
   ...
   *
   * <p>This will return an initialised bind context as such:
   *
   * <ul>
   *   <li><code>{@link RenderContext#declareFields()} == false</code>
   *   <li><code>{@link RenderContext#declareTables()} == false</code>
   * </ul>
   *
   * <p>BindContext for JOOQ INTERNAL USE only. Avoid referencing it directly
   *
   * @deprecated - [#6280] - 3.10 - Do not reuse this method. It will be completely internal with
   *     jOOQ 4.0
   */
   ...
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   *
   * @param sql The SQL
  ...
  /**
   * Create a new query holding plain SQL. There must be as many bind variables contained in the
   * SQL, as passed in the bindings parameter.
   *
   * <p>Example:
   *
   * <p><code><pre>
   ...
   *
   * <p>Unlike {@link #query(String, Object...)}, the SQL passed to this method should not contain
   * any bind variables. Instead, you can pass {@link QueryPart} objects to the method which will be
   * rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses! One way to escape
   * literals is to use {@link DSL#name(String...)} and similar methods
   *
   * @param sql The SQL clause, containing {numbered placeholders} where query parts can be injected
   * @param parts The {@link QueryPart} objects that are rendered at the {numbered placeholder}
   *     locations
   * @return A query wrapping the plain SQL
   * @see SQL
   ...
   *
   * <p><code><pre>
   * String sql = "FETCH ALL IN \"&lt;unnamed cursor 1&gt;\"";</pre></code> Example (SQLite):
   *
   * <p><code><pre>
   ...
   * Execute a new query holding plain SQL.
   *
   * <p>There must be as many bind variables contained in the SQL, as passed in the bindings
   * parameter
   *
   * <p>Example (Postgres):
   *
   ...
   * Execute a new query holding plain SQL.
   *
   * <p>Unlike {@link #fetch(String, Object...)}, the SQL passed to this method should not contain
   * any bind variables. Instead, you can pass {@link QueryPart} objects to the method which will be
   * rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   *
   * <p>The returned {@link Cursor} holds a reference to the executed {@link PreparedStatement} and
   * the associated {@link ResultSet}. Data can be fetched (or iterated over) lazily, fetching
   * records from the {@link ResultSet} one by one.
   *
   * <p>Example (Postgres):
   *
   * <p><code><pre>
   ...
   * records from the {@link ResultSet} one by one.
   *
   * <p>Unlike {@link #fetchLazy(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   * the {@link #configuration()}'s {@link Configuration#executorProvider()}.
   *
   * <p>Example (Postgres):
   *
   * <p><code><pre>
   ...
   *
   * @param sql The SQL
   * @return The completion stage. The completed result will never be <code>null</code>.
   * @see SQL
   */
  @NotNull
  @Support
  ...
  /**
   * Fetch results in a new {@link CompletionStage} that is asynchronously completed by a task
   ...
   * Execute a new query holding plain SQL and "lazily" return the generated result.
   *
   * <p>The returned {@link Stream} holds a reference to the executed {@link PreparedStatement} and
   * the associated {@link ResultSet}. Data can be fetched (or iterated over) lazily, fetching
   * records from the {@link ResultSet} one by one.
   *
   * <p>Example (Postgres):
   ...
   *
   * @param sql The SQL
   * @return The results from the executed query. This is never <code>null</code>, even if the
   *     database returns no {@link ResultSet}
   * @throws DataAccessException if something went wrong executing the query
   * @see SQL
   */
  @NotNull
   ...
   * records from the {@link ResultSet} one by one.
   *
   * <p>Unlike {@link #fetchStream(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   * fetchLazy("select {0}, {1} from {2}", val(1), inline("test"), name("DUAL"));
   ...
   * Execute a new query holding plain SQL, possibly returning several result sets.
   *
   * <p>Unlike {@link #fetchMany(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   * Execute a new query holding plain SQL.
   *
   * <p>Unlike {@link #fetchOne(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
  ...
  @Support
  @PlainSQL
  Record fetchSingle(String sql, Object... bindings)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @Support
  @PlainSQL
  Record fetchSingle(String sql, QueryPart... parts)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @Support
  @PlainSQL
  Optional<Record> fetchOptional(String sql, Object... bindings)
  ...
  @Support
  @PlainSQL
  Optional<Record> fetchOptional(String sql, QueryPart... parts)
      throws DataAccessException, TooManyRowsException;
   ...
   * @throws DataAccessException if something went wrong executing the query
   * @throws TooManyRowsException if the query returned more than one record
   * @throws InvalidResultException if the query returned a record with more than one value
   * @see SQL
   */
  @Nullable
  @Support
  @PlainSQL
  Object fetchValue(SQL sql)
  ...
  @Support
  @PlainSQL
  Object fetchValue(String sql, Object... bindings)
      throws DataAccessException, TooManyRowsException, InvalidResultException;

  /**
   * Execute a new query holding plain SQL.
   *
   * <p>Unlike {@link #fetchValue(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   * fetchOne("select {0}, {1} from {2}", val(1), inline("test"), name("DUAL"));
  ...
  @Support
  @PlainSQL
  Object fetchValue(String sql, QueryPart... parts)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @Support
  @PlainSQL
  Optional<?> fetchOptionalValue(SQL sql)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @Support
  @PlainSQL
  Optional<?> fetchOptionalValue(String sql, Object... bindings)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @Support
  @PlainSQL
  Optional<?> fetchOptionalValue(String sql, QueryPart... parts)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @Support
  @PlainSQL
  List<?> fetchValues(String sql, Object... bindings)
  ...
  @Support
  @PlainSQL
  List<?> fetchValues(String sql, QueryPart... parts)
      throws DataAccessException, InvalidResultException;
   ...
   * Execute a new query holding plain SQL.
   *
   * <p>Unlike {@link #execute(String, Object...)}, the SQL passed to this method should not contain
   * any bind variables. Instead, you can pass {@link QueryPart} objects to the method which will be
   * rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   * Create a new query holding plain SQL.
   *
   * <p>There must not be any bind variables contained in the SQL
   *
   * <p>Use this method, when you want to take advantage of the many ways to fetch results in jOOQ,
   * using {@link ResultQuery}. Some examples:
   *
   * <p>
   *
   * <table border="1">
   ...
   * Create a new query holding plain SQL.
   *
   * <p>Unlike {@link #resultQuery(String, Object...)}, the SQL passed to this method should not
   * contain any bind variables. Instead, you can pass {@link QueryPart} objects to the method which
   * will be rendered at indexed locations of your SQL string as such: <code><pre>
   * // The following query
   ...
   *
   * <p>After fetching all data, the JDBC ResultSet will be closed.
   *
   * <p>Use {@link #fetchLazy(ResultSet)}, to fetch one <code>Record</code> at a time, instead of
   * load the entire <code>ResultSet</code> into a jOOQ <code>Result</code> at once.
   *
   ...
   * load the entire <code>ResultSet</code> into a jOOQ <code>Result</code> at once.
   *
   * <p>The additional <code>fields</code> argument is used by jOOQ to coerce field names and data
   ...
   * load the entire <code>ResultSet</code> into a jOOQ <code>Result</code> at once.
   *
   * <p>The additional <code>types</code> argument is used by jOOQ to coerce data types to the
   * desired output
   *
   * @param rs The JDBC ResultSet to fetch data from
  ...
  /**
   * Fetch a record from a JDBC {@link ResultSet} and transform it to a jOOQ {@link Record}.
   *
   * <p>This will internally fetch all records and throw an exception if there was more than one
   * resulting record.
   *
   * @param rs The JDBC ResultSet to fetch data from
   * @return The record or <code>null</code>, if no record was found.
  ...
  @Nullable
  @Support
  Record fetchOne(ResultSet rs, Field<?>... fields)
      throws DataAccessException, TooManyRowsException;
  ...
  @Nullable
  @Support
  Record fetchOne(ResultSet rs, DataType<?>... types)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Record fetchSingle(ResultSet rs, Field<?>... fields)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Record fetchSingle(ResultSet rs, DataType<?>... types)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Record fetchSingle(ResultSet rs, Class<?>... types)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Optional<Record> fetchOptional(ResultSet rs, Field<?>... fields)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Optional<Record> fetchOptional(ResultSet rs, DataType<?>... types)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  Optional<Record> fetchOptional(ResultSet rs, Class<?>... types)
      throws DataAccessException, TooManyRowsException;

  /**
   * Fetch a record from a JDBC {@link ResultSet} and return the only contained value.
   *
   * <p>This will internally fetch all records and throw an exception if there was more than one
   * resulting record.
   *
  ...
  @Nullable
  @Support
  Object fetchValue(ResultSet rs)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
   ...
   * resulting record.
   *
   * <p>The additional <code>field</code> argument is used by jOOQ to coerce field names and data
   * types to the desired output
   *
   * @param rs The JDBC ResultSet to fetch data from
  ...
  @Nullable
  @Support
  <T> T fetchValue(ResultSet rs, Field<T> field)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
   ...
   * resulting record.
   *
   * <p>The additional <code>type</code> argument is used by jOOQ to coerce data types to the
   * desired output
   *
   * @param rs The JDBC ResultSet to fetch data from
  ...
  @Nullable
  @Support
  <T> T fetchValue(ResultSet rs, DataType<T> type)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @Nullable
  @Support
  <T> T fetchValue(ResultSet rs, Class<T> type)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @NotNull
  @Support
  Optional<?> fetchOptionalValue(ResultSet rs)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> Optional<T> fetchOptionalValue(ResultSet rs, Field<T> field)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> Optional<T> fetchOptionalValue(ResultSet rs, DataType<T> type)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> Optional<T> fetchOptionalValue(ResultSet rs, Class<T> type)
      throws DataAccessException, TooManyRowsException, InvalidResultException;

  /**
   * Fetch a result from a JDBC {@link ResultSet} and return the only contained column's values.
   *
   * @param rs The JDBC ResultSet to fetch data from
   * @return The values. This will never be <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
  ...
  @NotNull
  @Support
  <T> List<T> fetchValues(ResultSet rs, Field<T> field)
      throws DataAccessException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> List<T> fetchValues(ResultSet rs, DataType<T> type)
      throws DataAccessException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> List<T> fetchValues(ResultSet rs, Class<T> type)
      throws DataAccessException, InvalidResultException;

  /**
   * Wrap a JDBC {@link ResultSet} into a jOOQ {@link Cursor}.
   *
   * <p>Use {@link #fetch(ResultSet)}, to load the entire <code>ResultSet</code> into a jOOQ <code>
   * Result</code> at once.
   *
   * @param rs The JDBC ResultSet to fetch data from
   * @return The cursor. This will never be <code>null</code>.
   ...
   *
   * <p>The supplied string is supposed to be formatted in a human-readable way. This is the same as
   * calling <code>fetchFromTXT(string, "{null}")</code>
   *
   ...
   * Fetch all data from a formatted string.
   *
   * <p>This method supports parsing results from two types of human-readable formats:
   *
   * <h3>The jOOQ {@link Result#format()}</h3>
   *
   * <p>This format is recognised by the fact that the first line starts with a "plus" sign: <code>
   * <pre>
   * +-----+-----+--------------------------+
   ...
   * |val2 | 2   | more text                |
   * +-----+-----+--------------------------+
   * </pre></code> This method will decode the above formatted string according to the following
   * rules:
   *
   * <ul>
   *   <li>The number of columns is defined by the number of dash groups in the first line. Groups
   *       are separated by exactly one "plus" sign
   *   <li>The column types are <code>VARCHAR(N)</code> where <code>
   *       N = number of dashes per dash group</code>
   *   <li>The column names are defined by the trimmed text contained in the second row
   *   <li>The data is defined by the trimmed text contained in the subsequent rows
   * </ul>
   *
   * <h3>The H2 database test data format</h3>
   *
   * <p>The supplied string is supposed to be formatted in the following, human-readable way: <code>
   * <pre>
   * COL1  COL2   COL3 containing whitespace
   * ----- ----   --------------------------
   * val1  1      some text
   ...
   *
   * <ul>
   *   <li>The number of columns is defined by the number of dash groups in the second line. Groups
   *       are separated by space(s)
   *   <li>The column types are <code>VARCHAR(N)</code> where <code>
   *       N = number of dashes per dash group</code>
   *   <li>The column names are defined by the trimmed text contained in the first row
   *   <li>The data is defined by the trimmed text contained in the subsequent rows
   * </ul>
   *
   * <h3>Both parsing methods</h3>
   *
   * <p>Both parsing methods make no assumption about the resulting data types. Instead, all data is
   * string-based.
   *
   * @param string The formatted string
   * @param nullLiteral The string literal to be used as <code>null</code> value.
   * @return The transformed result. This will never be <code>null</code>.
   * @throws DataAccessException If the supplied string does not adhere to the above format rules.
   */
  @NotNull
   ...
   *
   * <p>This is the inverse operation of {@link Result#formatHTML()}. It works according to the
   * following parsing rules:
   *
   * <ul>
   *   <li>The input is expected to be well-formed XML. XHTML conformance is not required - i.e.
   *       unknown elements / attributes, or elements / attributes not specified here, such as
   *       <code>&lt;caption&gt;</code>, <code>&lt;thead&gt;</code>, <code>&lt;tbody&gt;</code> are
   *       simply ignored.
   *   <li>The surrounding <code>&lt;table&gt;</code> element is optional, but it may appear only
   *       once
   *   <li>A single row containing table headings <code>&lt;th&gt;</code> is allowed. Further rows
   *       containing table headings are ignored. Table headings define field names. In the absence
   *       of table headings, field names are generated.
   *   <li>The first row <code>&lt;tr&gt;</code> specifies the number of columns in the table
   *       (regardless if it contains table headings or not). Subsequent rows containing less
   *       columns will be padded. Subsequent rows containing more columns will be truncated.
   *   <li>Comments are ignored
   *   <li>Nested tables are not supported
   * </ul>
   *
   * <p>Ideal input looks like this: <code><pre>
   * &lt;table&gt;
   ...
   *
   * <p>This is the same as calling <code>fetchFromCSV(string, ',')</code> and the inverse of
   * calling {@link Result#formatCSV()}. The first row of the CSV data is required to hold field
   * name information. Subsequent rows may contain data, which is interpreted as {@link String}. Use
   * the various conversion methods to retrieve other data types from the <code>Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   *   <li>{@link Result#getValues(int, Class)}
   *   <li>{@link Result#getValues(String, Class)}
   *   <li>{@link Result#getValues(Field, Converter)}
   *   <li>{@link Result#getValues(int, Converter)}
   *   <li>{@link Result#getValues(String, Converter)}
   * </ul>
   *
   * <p>Missing values result in <code>null</code>. Empty values result in empty <code>Strings
   * </code>
   *
   ...
   * Fetch all data from a CSV string.
   *
   * <p>This is inverse of calling {@link Result#formatCSV(char)}. The first row of the CSV data is
   * required to hold field name information. Subsequent rows may contain data, which is interpreted
   * as {@link String}. Use the various conversion methods to retrieve other data types from the
   * <code>Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   ...
   *
   * <p>This is the same as calling <code>fetchFromCSV(string, ',')</code> and the inverse of
   * calling {@link Result#formatCSV(boolean)}. Rows may contain data, which is interpreted as
   * {@link String}. Use the various conversion methods to retrieve other data types from the <code>
   * Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   ...
   * Fetch all data from a CSV string.
   *
   * <p>This is inverse of calling {@link Result#formatCSV(boolean, char)}. Rows may contain data,
   * which are interpreted as {@link String}. Use the various conversion methods to retrieve other
   * data types from the <code>Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   *   <li>{@link Result#getValues(int, Class)}
  ...
  @NotNull
  @Support
  Result<Record> fetchFromCSV(String string, boolean header, char delimiter)
      throws DataAccessException;

  /**
   * Fetch all data from a JSON string.
   *
   * <p>This is the inverse of calling {@link Result#formatJSON()}. Use the various conversion
   * methods to retrieve other data types from the <code>Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   *   <li>{@link Result#getValues(int, Class)}
   ...
   *
   * <p>This is the inverse of calling {@link Result#formatXML()}. Use the various conversion
   * methods to retrieve other data types from the <code>Result</code>:
   *
   * <ul>
   *   <li>{@link Result#getValues(Field, Class)}
   ...
   *
   * <ul>
   *   <li>{@link #fetchFromCSV(String)}
   *   <li>{@link #fetchFromTXT(String)}
   * </ul>
   *
   * The first element of the argument list should contain column names. Subsequent elements contain
   * actual data. The degree of all arrays contained in the argument should be the same, although
   * this is not a requirement. jOOQ will ignore excess data, and fill missing data with <code>null
   * </code>.
   *
   ...
   * </ul>
   *
   * The degree of all arrays contained in the argument should be the same, although this is not a
   * requirement. jOOQ will ignore excess data, and fill missing data with <code>null</code>.
   *
   * @param data The data to be transformed into a <code>Result</code>
   * @param header Whether to interpret the first line as a set of column names.
   * @return The transformed result. This will never be <code>null</code>.
   */
  @NotNull
  @Support
   ...
   *
   * <p>The <code>RECURSIVE</code> keyword may be optional or unsupported in some databases, in case
   * of which it will not be rendered. For optimal database interoperability and readability,
   * however, it is suggested that you use {@link #with(String)} for strictly non-recursive CTE and
   * {@link #withRecursive(String)} for strictly recursive CTE.
   */
  @NotNull
  @CheckReturnValue
   ...
   * <p>The <code>RECURSIVE</code> keyword may be optional or unsupported in some databases, in case
   * of which it will not be rendered. For optimal database interoperability and readability,
   * however, it is suggested that you use {@link #with(String, String...)} for strictly
   * non-recursive CTE and {@link #withRecursive(String, String...)} for strictly recursive CTE.
   */
  @NotNull
  @CheckReturnValue
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
   ...
   * <p>The <code>RECURSIVE</code> keyword may be optional or unsupported in some databases, in case
   * of which it will not be rendered. For optimal database interoperability and readability,
   * however, it is suggested that you use {@link #with(Name)} for strictly non-recursive CTE and
   * {@link #withRecursive(Name)} for strictly recursive CTE.
   */
  @NotNull
  @CheckReturnValue
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
   ...
   * <p>The <code>RECURSIVE</code> keyword may be optional or unsupported in some databases, in case
   * of which it will not be rendered. For optimal database interoperability and readability,
   * however, it is suggested that you use {@link #with(Name, Name...)} for strictly non-recursive
   * CTE and {@link #withRecursive(Name, Name...)} for strictly recursive CTE.
   */
  @NotNull
  @CheckReturnValue
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
   ...
   * non-recursive CTE and {@link #withRecursive(String, String...)} for strictly recursive CTE.
   *
   * <p>This works in a similar way as {@link #with(String, String...)}, except that all column
   * names are produced by a function that receives the CTE's {@link Select} columns as input.
   *
   ...
   *
   * <p>This works in a similar way as {@link #with(String, String...)}, except that all column
   * names are produced by a function that receives the CTE's {@link Select} columns and their
   * column indexes as input.
   *
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
   *     functionality that would not be possible otherwise - yet they add complexity in jOOQ's
  ...
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
  WithAsStep4 with(
      String alias, String fieldAlias1, String fieldAlias2, String fieldAlias3, String fieldAlias4);
  ...
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
  WithAsStep4 with(
      Name alias, Name fieldAlias1, Name fieldAlias2, Name fieldAlias3, Name fieldAlias4);
   ...
   * CommonTableExpression}s.
   *
   * <p>Reusable {@link CommonTableExpression} types can be constructed through
   *
   * <ul>
   *   <li>{@link DSL#name(String...)}
   *   <li>{@link Name#fields(String...)}
   *   <li>{@link DerivedColumnList#as(Select)}
   * </ul>
   *
   * <p>The <code>RECURSIVE</code> keyword may be optional or unsupported in some databases, in case
   * of which it will not be rendered. For optimal database interoperability and readability,
   * however, it is suggested that you use {@link #with(CommonTableExpression...)} for strictly
   * non-recursive CTE and {@link #withRecursive(CommonTableExpression...)} for strictly recursive
   * CTE.
   */
  @NotNull
  @CheckReturnValue
  ...
  @CheckReturnValue
  @Support({FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
  WithAsStep withRecursive(
      String alias, Function<? super Field<?>, ? extends String> fieldNameFunction);
  ...
  WithAsStep3 withRecursive(
      String alias, String fieldAlias1, String fieldAlias2, String fieldAlias3);
   ...
   *
   * <p>This will project the known columns from the argument table querying {@link Table#fields()}.
   * If no known columns are available (e.g. because the table has been created using {@link
   * DSL#table(String)}), then <code>SELECT *</code> is projected.
   *
   * <p>Example:
   *
   * <p><code><pre>
   ...
   *
   * <p>Without knowing any columns from the argument table (see {@link #selectFrom(Table)}), this
   * will project <code>SELECT *</code>.
   *
   * <p>Example:
   *
   * <p><code><pre>
   ...
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   * </pre></code>
   *
   * <p>Note that passing an empty collection conveniently produces <code>SELECT *</code> semantics,
   * i.e. it:
   *
   * <ul>
   *   <li>Renders <code>SELECT tab1.col1, tab1.col2, ..., tabN.colN</code> if all columns are known
   *   <li>Renders <code>SELECT *</code> if not all columns are known, e.g. when using plain SQL
   * </ul>
   *
   ...
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectFieldOrAsterisk...)} instead.
   *
   * <p>Example: <code><pre>
   * DSLContext create = DSL.using(configuration);
   *
   ...
   * Create a new DSL select statement.
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Field#in(Select)}, {@link
   * Field#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row2#in(Select)}, {@link
   * Row2#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row3#in(Select)}, {@link
   * Row3#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3> SelectSelectStep<Record3<T1, T2, T3>> select(
      SelectField<T1> field1, SelectField<T2> field2, SelectField<T3> field3);

  /**
   * Create a new DSL select statement.
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row4#in(Select)}, {@link
   * Row4#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4> SelectSelectStep<Record4<T1, T2, T3, T4>> select(
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row5#in(Select)}, {@link
   * Row5#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5> SelectSelectStep<Record5<T1, T2, T3, T4, T5>> select(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row6#in(Select)}, {@link
   * Row6#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5, T6> SelectSelectStep<Record6<T1, T2, T3, T4, T5, T6>> select(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row7#in(Select)}, {@link
   * Row7#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5, T6, T7> SelectSelectStep<Record7<T1, T2, T3, T4, T5, T6, T7>> select(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row8#in(Select)}, {@link
   * Row8#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectField, SelectField, SelectField, SelectField, SelectField, SelectField,
   * SelectField, SelectField)} instead.
   *
   * <p>Example: <code><pre>
   * using(configuration)
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5, T6, T7, T8> SelectSelectStep<Record8<T1, T2, T3, T4, T5, T6, T7, T8>> select(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row9#in(Select)}, {@link
   * Row9#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectField, SelectField, SelectField, SelectField, SelectField, SelectField,
   * SelectField, SelectField, SelectField)} instead.
   *
   * <p>Example: <code><pre>
   * using(configuration)
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row10#in(Select)}, {@link
   * Row10#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectField, SelectField, SelectField, SelectField, SelectField, SelectField,
   * SelectField, SelectField, SelectField, SelectField)} instead.
   *
   * <p>Example: <code><pre>
   * using(configuration)
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row11#in(Select)}, {@link
   * Row11#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectField, SelectField, SelectField, SelectField, SelectField, SelectField,
   * SelectField, SelectField, SelectField, SelectField, SelectField)} instead.
   *
   * <p>Example: <code><pre>
   * using(configuration)
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row12#in(Select)}, {@link
   * Row12#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#select(SelectField, SelectField, SelectField, SelectField, SelectField, SelectField,
   * SelectField, SelectField, SelectField, SelectField, SelectField, SelectField)} instead.
   *
   * <p>Example: <code><pre>
   * using(configuration)
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row13#in(Select)}, {@link
   * Row13#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row14#in(Select)}, {@link
   * Row14#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row15#in(Select)}, {@link
   * Row15#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row16#in(Select)}, {@link
   * Row16#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row17#in(Select)}, {@link
   * Row17#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row18#in(Select)}, {@link
   * Row18#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row19#in(Select)}, {@link
   * Row19#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row20#in(Select)}, {@link
   * Row20#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row21#in(Select)}, {@link
   * Row21#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #select(SelectFieldOrAsterisk...)}, except that it declares
   * additional record-level typesafety, which is needed by {@link Row22#in(Select)}, {@link
   * Row22#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   * </pre></code>
   *
   * <p>Note that passing an empty collection conveniently produces <code>SELECT DISTINCT *</code>
   * semantics, i.e. it:
   *
   * <ul>
   *   <li>Renders <code>SELECT DISTINCT tab1.col1, tab1.col2, ..., tabN.colN</code> if all columns
   *       are known
   *   <li>Renders <code>SELECT DISTINCT *</code> if not all columns are known, e.g. when using
   *       plain SQL
   * </ul>
   *
   ...
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   * DSL#selectDistinct(SelectFieldOrAsterisk...)} instead.
   *
   * <p>Example: <code><pre>
   * DSLContext create = DSL.using(configuration);
   *
   ...
   * Create a new DSL select statement.
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Field#in(Select)},
   * {@link Field#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row2#in(Select)}, {@link
   * Row2#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2> SelectSelectStep<Record2<T1, T2>> selectDistinct(
      SelectField<T1> field1, SelectField<T2> field2);

  /**
   * Create a new DSL select statement.
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row3#in(Select)}, {@link
   * Row3#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3> SelectSelectStep<Record3<T1, T2, T3>> selectDistinct(
      SelectField<T1> field1, SelectField<T2> field2, SelectField<T3> field3);
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row4#in(Select)}, {@link
   * Row4#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4> SelectSelectStep<Record4<T1, T2, T3, T4>> selectDistinct(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4);
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row5#in(Select)}, {@link
   * Row5#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5> SelectSelectStep<Record5<T1, T2, T3, T4, T5>> selectDistinct(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row6#in(Select)}, {@link
   * Row6#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5, T6> SelectSelectStep<Record6<T1, T2, T3, T4, T5, T6>> selectDistinct(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row7#in(Select)}, {@link
   * Row7#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
  ...
  @CheckReturnValue
  @Support
  <T1, T2, T3, T4, T5, T6, T7> SelectSelectStep<Record7<T1, T2, T3, T4, T5, T6, T7>> selectDistinct(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row8#in(Select)}, {@link
   * Row8#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row9#in(Select)}, {@link
   * Row9#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row10#in(Select)},
   * {@link Row10#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row11#in(Select)},
   * {@link Row11#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row12#in(Select)},
   * {@link Row12#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row13#in(Select)},
   * {@link Row13#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row14#in(Select)},
   * {@link Row14#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row15#in(Select)},
   * {@link Row15#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row16#in(Select)},
   * {@link Row16#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row17#in(Select)},
   * {@link Row17#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row18#in(Select)},
   * {@link Row18#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row19#in(Select)},
   * {@link Row19#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row20#in(Select)},
   * {@link Row20#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row21#in(Select)},
   * {@link Row21#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This is the same as {@link #selectDistinct(SelectFieldOrAsterisk...)}, except that it
   * declares additional record-level typesafety, which is needed by {@link Row22#in(Select)},
   * {@link Row22#equal(Select)} and other predicate building methods taking subselect arguments.
   *
   * <p>This creates an attached, renderable and executable <code>SELECT</code> statement from this
   * {@link DSLContext}. If you don't need to render or execute this <code>SELECT</code> statement
   * (e.g. because you want to create a subselect), consider using the static {@link
   ...
   *
   * <p>This type of insert may feel more convenient to some users, as it uses the <code>UPDATE
   * </code> statement's <code>SET a = b</code> syntax.
   *
   * <p>Example: <code><pre>
   * DSLContext create = DSL.using(configuration);
   *
  ...
  @CheckReturnValue
  @Support
  <R extends Record, T1, T2> InsertValuesStep2<R, T1, T2> insertInto(
  ...
  @CheckReturnValue
  @Support
  <R extends Record, T1, T2, T3> InsertValuesStep3<R, T1, T2, T3> insertInto(
  ...
  @CheckReturnValue
  @Support
  <R extends Record, T1, T2, T3, T4> InsertValuesStep4<R, T1, T2, T3, T4> insertInto(
  ...
  @CheckReturnValue
  @Support
  <R extends Record, T1, T2, T3, T4, T5> InsertValuesStep5<R, T1, T2, T3, T4, T5> insertInto(
      Table<R> into,
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
  ...
  @CheckReturnValue
  @Support
  <R extends Record> InsertValuesStepN<R> insertInto(
      Table<R> into, Collection<? extends Field<?>> fields);
   ...
   * </pre></code>
   *
   * <p>Note that some databases support table expressions more complex than simple table
   * references. In MySQL, for instance, you can write <code><pre>
   ...
   *
   * <p>This statement is available from DSL syntax only. It is known to be supported in some way by
   ...
   * </pre></code>
   *
   * <p>Note: Using this method, you can also create an H2-specific MERGE statement without field
   * specification. See also {@link #mergeInto(Table, Field...)}
   */
  @NotNull
  @CheckReturnValue
  ...
  /**
   * Create a new DSL UPSERT statement ({@link SQLDialect#H2} <code>MERGE</code>) or {@link
   * SQLDialect#HANA} <code>UPSERT</code>).
   *
  ...
  @CheckReturnValue
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record, T1, T2> MergeKeyStep2<R, T1, T2> mergeInto(
  ...
  @CheckReturnValue
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record, T1, T2, T3> MergeKeyStep3<R, T1, T2, T3> mergeInto(
  ...
  @CheckReturnValue
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record, T1, T2, T3, T4> MergeKeyStep4<R, T1, T2, T3, T4> mergeInto(
  ...
  @CheckReturnValue
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record, T1, T2, T3, T4, T5> MergeKeyStep5<R, T1, T2, T3, T4, T5> mergeInto(
      Table<R> table,
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
  ...
  @CheckReturnValue
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record, T1, T2, T3, T4, T5, T6> MergeKeyStep6<R, T1, T2, T3, T4, T5, T6> mergeInto(
      Table<R> table,
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
  ...
  @CheckReturnValue
  @Support({CUBRID, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  <R extends Record> MergeKeyStepN<R> mergeInto(
      Table<R> table, Collection<? extends Field<?>> fields);
   ...
   * </pre></code>
   *
   * <p>Some but not all databases support aliased tables in delete statements.
   *
   * <p>Note that some databases support table expressions more complex than simple table
   * references. In MySQL, for instance, you can write this to form a multi table <code>DELETE
   * </code> statement:
   *
   * <p><code><pre>
   ...
   * </pre></code>
   *
   * <p>For single table delete statements that depend on multiple tables, use the {@link
   * DeleteUsingStep#using(TableLike)} clause, instead:
   *
   * <p><code><pre>
   * create.delete(t1)
   *       .using(t2)
   ...
   * Create a new DSL delete statement.
   *
   * <p>This is an alias for {@link #deleteFrom(Table)}
   */
  @NotNull
  @CheckReturnValue
  @Support
   ...
   *
   * <p>This essentially runs the following logic: <code><pre>
   ...
   * Create a batch statement to execute a set of queries in batch mode (without bind values).
   *
   * <p>This is a convenience method for calling <code>
   * <pre>batch(query(queries[0]), query(queries[1]), ...)</pre></code>.
   *
   * @see #query(String)
   * @see #batch(Query...)
   * @see java.sql.Statement#executeBatch()
   ...
   * </pre></code>
   *
   * <p>Note: bind values will be inlined to a static batch query as in {@link #batch(Query...)}, if
   * you choose to execute queries with <code>
   * {@link Settings#getStatementType()} == {@link StatementType#STATIC_STATEMENT}</code>
   *
   * @see java.sql.Statement#executeBatch()
   */
  @NotNull
   ...
   * Create a batch statement to execute a set of queries in batch mode (with bind values).
   *
   * <p>This is a convenience method for calling <code><pre>batch(query(sql))</pre></code>.
   *
   * @see #query(String)
   * @see #batch(Query)
   * @see java.sql.Statement#executeBatch()
   ...
   * Create a batch statement to execute a set of queries in batch mode (with bind values).
   *
   * <p>This is a convenience method for calling {@link #batch(Query)} and then binding values one
   * by one using {@link BatchBindStep#bind(Object...)}
   *
   * <p>Note: bind values will be inlined to a static batch query as in {@link #batch(Query...)}, if
   * you choose to execute queries with <code>
   * {@link Settings#getStatementType()} == {@link StatementType#STATIC_STATEMENT}</code>
   ...
   * Create a batch statement to execute a set of queries in batch mode (with bind values).
   *
   * <p>This is a convenience method for calling <code><pre>batch(query(sql), bindings)</pre></code>
   * .
   *
   * @see #query(String)
  ...
  /**
   * Create a batch statement to execute a set of <code>INSERT</code> and <code>UPDATE</code>
   * queries in batch mode (with bind values) according to {@link UpdatableRecord#store()}
   * semantics.
   *
   * <p>This batch operation can be executed in two modes:
   *
   * <p>
   *
   * <h5>With <code>{@link Settings#getStatementType()} == {@link StatementType#PREPARED_STATEMENT}
   * </code> (the default)</h5>
   *
   * <p>In this mode, record order is preserved as much as possible, as long as two subsequent
   * records generate the same SQL (with bind variables). The number of executed batch operations
   * corresponds to <code>[number of distinct rendered SQL statements]</code>. In the worst case,
   * this corresponds to the number of total records.
   *
   * <p>The record type order is preserved in the way they are passed to this method. This is an
   * example of how statements will be ordered: <code><pre>
   ...
   *                      .execute();
   * </pre></code> The above results in <code>result.length == 8</code> and the following 4 separate
   * batch statements:
   *
   * <ol>
   *   <li>INSERT a1, a3, a5
   ...
   * <p>
   *
   * <h5>With <code>{@link Settings#getStatementType()} == {@link StatementType#STATIC_STATEMENT}
   * </code> </h5>
   *
   * <p>This mode may be better for large and complex batch store operations, as the order of
   * records is preserved entirely, and jOOQ can guarantee that only a single batch statement is
   * serialised to the database.
   *
   * <p>
   *
   * <h5>A note on MERGE / UPSERT semantics</h5>
   *
   * <p>This method (just like {@link UpdatableRecord#store()}) does not implement the semantics of
   * an actual <code>UPSERT</code> or <code>MERGE</code> statement, which delegates the decision of
   * whether to <code>INSERT</code> or <code>UPDATE</code> a record to the database. The decision is
   * made by the client (jOOQ) depending on whether each individual record has been fetched from the
   * database prior to storing it.
   *
  ...
  /**
   * Create a batch statement to execute a set of <code>INSERT</code> queries in batch mode (with
   * bind values) according to {@link TableRecord#insert()} semantics.
   *
   * @see #batchStore(UpdatableRecord...)
  ...
  /**
   * Create a batch statement to execute a set of <code>UPDATE</code> queries in batch mode (with
   * bind values) according to {@link UpdatableRecord#update()} semantics.
   *
   * @see #batchStore(UpdatableRecord...)
  ...
  /**
   * Create a batch statement to execute a set of <code>MERGE</code> queries in batch mode (with
   * bind values) according to {@link UpdatableRecord#merge()} semantics.
   *
  ...
  /**
   * Create a batch statement to execute a set of <code>DELETE</code> queries in batch mode (with
   * bind values) according to {@link UpdatableRecord#delete()} sematics.
   *
   * <p>This batch operation can be executed in two modes:
   *
   * <p>
   ...
   *                      .execute();
   * </pre></code> The above results in <code>result.length == 8</code> and the following 5 separate
   * batch statements:
   *
   * <ol>
   *   <li>DELETE a1, a2, a3
   ...
   * </code> </h5>
   *
   * <p>This mode may be better for large and complex batch delete operations, as the order of
   * records is preserved entirely, and jOOQ can guarantee that only a single batch statement is
   * serialised to the database.
   *
   ...
   *
   * <p>Set a vendor specific session configuration to a new value.
   *
   ...
   *
   * <p>Set a vendor specific transaction-local configuration to a new value.
   *
   ...
   *
   * <p>Set the current catalog to a new value.
   *
   ...
   *
   * <p>Set the current schema to a new value.
   *
   ...
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createView(String, String...)} except that the view's field names
   * are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
  ...
  CreateViewAsStep<Record> createView(
      String view, Function<? super Field<?>, ? extends String> fieldNameFunction);
   ...
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createView(Name, Name...)} except that the view's field names are
   * derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
  ...
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
  CreateViewAsStep<Record> createView(
      Name view, Function<? super Field<?>, ? extends Name> fieldNameFunction);
  ...
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
  CreateViewAsStep<Record> createView(
      Name view, BiFunction<? super Field<?>, ? super Integer, ? extends Name> fieldNameFunction);

  /**
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createView(Table, Field...)} except that the view's field names are
   * derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
  ...
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
  CreateViewAsStep<Record> createView(
      Table<?> view, Function<? super Field<?>, ? extends Field<?>> fieldNameFunction);
   ...
   * Create a new DSL <code>CREATE OR REPLACE VIEW</code> statement.
   *
   * <p>This works like {@link #createOrReplaceView(String, String...)} except that the view's field
   * names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createOrReplaceView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
  ...
  @CheckReturnValue
  @Support({FIREBIRD, H2, MARIADB, MYSQL, POSTGRES})
  CreateViewAsStep<Record> createOrReplaceView(
      String view, Function<? super Field<?>, ? extends String> fieldNameFunction);
   ...
   * Create a new DSL <code>CREATE OR REPLACE VIEW</code> statement.
   *
   * <p>This works like {@link #createOrReplaceView(Name, Name...)} except that the view's field
   * names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createOrReplaceView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   ...
   * Create a new DSL <code>CREATE OR REPLACE VIEW</code> statement.
   *
   * <p>This works like {@link #createOrReplaceView(Table, Field...)} except that the view's field
   * names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createOrReplaceView(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   ...
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createViewIfNotExists(String, String...)} except that the view's
   * field names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createViewIfNotExists(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
  ...
  @CheckReturnValue
  @Support({FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
  CreateViewAsStep<Record> createViewIfNotExists(
      String view, Function<? super Field<?>, ? extends String> fieldNameFunction);
   ...
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createViewIfNotExists(Name, Name...)} except that the view's field
   * names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createViewIfNotExists(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   ...
   * Create a new DSL <code>CREATE VIEW</code> statement.
   *
   * <p>This works like {@link #createViewIfNotExists(Table, Field...)} except that the view's field
   * names are derived from the view's {@link Select} statement using a function.
   *
   * @see DSL#createViewIfNotExists(String, String...)
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   ...
   *
   * <p>This is implemented for the following dialects:
   *
   * <ul>
   *   <li>{@link SQLDialect#ACCESS}: Using <code>@@identity</code>
   *   <li>{@link SQLDialect#ASE}: Using <code>@@identity</code>
   *   <li>{@link SQLDialect#DERBY}: Using <code>identity_val_local()</code>
   *   <li>{@link SQLDialect#H2}: Using <code>identity()</code>
   *   <li>{@link SQLDialect#HSQLDB}: Using <code>identity()</code>
   *   <li>{@link SQLDialect#INFORMIX}: Using <code>dbinfo('sqlca.sqlerrd1')</code>
   *   <li>{@link SQLDialect#INGRES}: Using <code>last_identity()</code>
   *   <li>{@link SQLDialect#MARIADB}: Using <code>last_insert_id()</code>
   *   <li>{@link SQLDialect#MYSQL}: Using <code>last_insert_id()</code>
   *   <li>{@link SQLDialect#POSTGRES}: Using <code>lastval()</code>
   *   <li>{@link SQLDialect#SQLITE}: Using <code>last_insert_rowid()</code>
   *   <li>{@link SQLDialect#SQLSERVER}: Using <code>@@identity</code>
   *   <li>{@link SQLDialect#SYBASE}: Using <code>@@identity</code>
   *   <li>{@link SQLDialect#VERTICA}: Using <code>last_insert_id()</code>
   * </ul>
   *
   * @return The last inserted ID. This may be <code>null</code> in some dialects, if no such number
   *     is available.
   * @throws DataAccessException if something went wrong executing the query
   */
  @NotNull
  ...
  /**
   * Convenience method to fetch the NEXTVAL for a sequence directly from this {@link DSLContext}'s
   * underlying JDBC {@link Connection}.
   *
   * @throws DataAccessException if something went wrong executing the query
   */
  @NotNull
  ...
  /**
   * Convenience method to fetch several NEXTVAL for a sequence directly from this {@link
   * DSLContext}'s underlying JDBC {@link Connection}.
   *
  ...
  /**
   * Convenience method to fetch the CURRVAL for a sequence directly from this {@link DSLContext}'s
   * underlying JDBC {@link Connection}.
   *
   * @throws DataAccessException if something went wrong executing the query
   */
   ...
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
  ...
  /**
   * Create a new {@link Record} that can be inserted into the corresponding table.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
  ...
  /**
   * Create a new pre-filled {@link Record} that can be inserted into the corresponding table.
   *
   * <p>This performs roughly the inverse operation of {@link Record#into(Class)}
   *
   * <p>The resulting record will have its internal "changed" flags set to true for all values. This
   * means that {@link UpdatableRecord#store()} will perform an <code>INSERT</code> statement. If
   * you wish to store the record using an <code>UPDATE</code> statement, use {@link
   ...
   */
  @NotNull
  <T1, T2, T3, T4> Record4<T1, T2, T3, T4> newRecord(
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5> Record5<T1, T2, T3, T4, T5> newRecord(
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6> Record6<T1, T2, T3, T4, T5, T6> newRecord(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7> Record7<T1, T2, T3, T4, T5, T6, T7> newRecord(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8> Record8<T1, T2, T3, T4, T5, T6, T7, T8> newRecord(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8, T9> Record9<T1, T2, T3, T4, T5, T6, T7, T8, T9> newRecord(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   *
   * <p>The result is attached to this {@link Configuration} by default. This result can be used as
   ...
   * Create a new empty {@link Result}.
   *
   * <p>The resulting result is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * @return The new result
   */
   ...
   */
  @NotNull
  <T1, T2, T3> Result<Record3<T1, T2, T3>> newResult(
   ...
   */
  @NotNull
  <T1, T2, T3, T4> Result<Record4<T1, T2, T3, T4>> newResult(
      Field<T1> field1, Field<T2> field2, Field<T3> field3, Field<T4> field4);
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5> Result<Record5<T1, T2, T3, T4, T5>> newResult(
      Field<T1> field1, Field<T2> field2, Field<T3> field3, Field<T4> field4, Field<T5> field5);
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6> Result<Record6<T1, T2, T3, T4, T5, T6>> newResult(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7> Result<Record7<T1, T2, T3, T4, T5, T6, T7>> newResult(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
   ...
   */
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8> Result<Record8<T1, T2, T3, T4, T5, T6, T7, T8>> newResult(
      Field<T1> field1,
      Field<T2> field2,
      Field<T3> field3,
      Field<T4> field4,
  ...
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8, T9>
      Result<Record9<T1, T2, T3, T4, T5, T6, T7, T8, T9>> newResult(
          Field<T1> field1,
          Field<T2> field2,
          Field<T3> field3,
          Field<T4> field4,
  ...
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>
      Result<Record10<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>> newResult(
          Field<T1> field1,
          Field<T2> field2,
          Field<T3> field3,
          Field<T4> field4,
  ...
  @NotNull
  <T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11>
      Result<Record11<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11>> newResult(
          Field<T1> field1,
          Field<T2> field2,
          Field<T3> field3,
          Field<T4> field4,
  ...
  /**
   * Execute a {@link ResultQuery} in the context of this <code>DSLContext</code> and return a
   * cursor.
   *
   * @param query The query to execute
   * @return The cursor. This will never be <code>null</code>.
  ...
  @NotNull
  @Support
  <R extends Record> Optional<R> fetchOptional(ResultQuery<R> query)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  <T, R extends Record1<T>> Optional<T> fetchOptionalValue(ResultQuery<R> query)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  @NotNull
  @Support
  <T> Optional<T> fetchOptionalValue(TableField<?, T> field)
      throws DataAccessException, TooManyRowsException, InvalidResultException;
  ...
  /**
   * Execute a {@link ResultQuery} in the context of this <code>DSLContext</code> and return all
   * values for the only column.
   *
   * @param query The query to execute
   * @return The values. This will never be <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
  ...
  /**
   * Execute a {@link Select} query in the context of this <code>DSLContext</code> and return a
   * <code>COUNT(*)</code> value.
   *
   * <p>This wraps a pre-existing <code>SELECT</code> query in another one to calculate the <code>
   * COUNT(*)</code> value, without modifying the original <code>SELECT</code>. An example: <code>
   * <pre>
   * -- Original query:
   ...
   *   SELECT id, title FROM book WHERE title LIKE '%a%'
   * )
   * </pre></code> This is particularly useful for those databases that do not support the <code>
   * COUNT(*) OVER()</code> window function to calculate total results in paged queries.
   *
   ...
   *
   * <p>This executes <code><pre>SELECT COUNT(*) FROM table</pre></code>
   *
   ...
   *
   * <p>This executes <code><pre>SELECT COUNT(*) FROM table WHERE condition</pre></code>
   *
   * @param table The table whose records to count
   ...
   * <p>This executes <code><pre>SELECT COUNT(*) FROM table WHERE condition</pre></code>
   *
   * <p>Convenience API for calling {@link #fetchCount(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @param table The table whose records to count
   ...
   */
  @Support
  int fetchCount(Table<?> table, Collection<? extends Condition> conditions)
      throws DataAccessException;

  /**
   * Check if a {@link Select} would return any records, if it were executed.
   *
   * <p>This wraps a pre-existing <code>SELECT</code> query in another one to check for result
   * existence, without modifying the original <code>SELECT</code>. An example: <code><pre>
   * -- Original query:
   * SELECT id, title FROM book WHERE title LIKE '%a%'
   *
   * -- Wrapped query:
   ...
   *
   * <p>This executes <code><pre>SELECT EXISTS(SELECT * FROM table)</pre></code>
   *
   * @param table The table whose records to count
   ...
   *
   * <p>This executes <code><pre>SELECT EXISTS(SELECT * FROM table WHERE condition)</pre></code>
   *
   * @param table The table whose records to count
   ...
   * <p>This executes <code><pre>SELECT EXISTS(SELECT * FROM table WHERE condition)</pre></code>
   *
   * <p>Convenience API for calling {@link #fetchExists(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @param table The table whose records to count
   * @return Whether the table contains any records that satisfy a condition
   ...
   */
  @Support
  boolean fetchExists(Table<?> table, Collection<? extends Condition> conditions)
      throws DataAccessException;
   ...
   * </code>.
   *
   * <p>The result and its contained records are attached to this {@link Configuration} by default.
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * @return The results from the executed query. This will never be <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
   */
  ...
  /**
   * Execute and return all records for <code>
   * <pre>SELECT table.col1, table.col2 FROM table WHERE condition</pre></code>.
   *
   * <p>The result and its contained records are attached to this {@link Configuration} by default.
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   ...
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetch(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The results from the executed query. This will never be <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
  ...
  @NotNull
  @Support
  <R extends Record> Result<R> fetch(Table<R> table, Condition... conditions)
      throws DataAccessException;
  ...
  @NotNull
  @Support
  <R extends Record> Result<R> fetch(Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException;

  /**
   * Execute and return zero or one record for <code>
   * <pre>SELECT table.col1, table.col2 FROM table</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
  ...
  @Nullable
  @Support
  <R extends Record> R fetchOne(Table<R> table, Condition condition)
      throws DataAccessException, TooManyRowsException;
   ...
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchOne(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The record or <code>null</code>, if no record was found.
   * @throws DataAccessException if something went wrong executing the query
  ...
  @Nullable
  @Support
  <R extends Record> R fetchOne(Table<R> table, Condition... conditions)
      throws DataAccessException, TooManyRowsException;
  ...
  @Nullable
  @Support
  <R extends Record> R fetchOne(Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException, TooManyRowsException;

  /**
   * Execute and return exactly one record for <code>
   * <pre>SELECT table.col1, table.col2 FROM table</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
  ...
  @NotNull
  @Support
  <R extends Record> R fetchSingle(Table<R> table, Condition condition)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
   ...
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchSingle(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The record. This is never <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
  ...
  @NotNull
  @Support
  <R extends Record> R fetchSingle(Table<R> table, Condition... conditions)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;
  ...
  @NotNull
  @Support
  <R extends Record> R fetchSingle(Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException, NoDataFoundException, TooManyRowsException;

  /**
   * Execute and return exactly one record for <code><pre>SELECT F1, F2, ..., FN</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchSingle(ResultQuery)} with {@link
  ...
  @NotNull
  @Support
  <T1, T2> Record2<T1, T2> fetchSingle(SelectField<T1> field1, SelectField<T2> field2)
      throws DataAccessException;
  ...
  @NotNull
  @Support
  <T1, T2, T3> Record3<T1, T2, T3> fetchSingle(
      SelectField<T1> field1, SelectField<T2> field2, SelectField<T3> field3)
      throws DataAccessException;
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4> Record4<T1, T2, T3, T4> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4)
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4, T5> Record5<T1, T2, T3, T4, T5> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4, T5, T6> Record6<T1, T2, T3, T4, T5, T6> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4, T5, T6, T7> Record7<T1, T2, T3, T4, T5, T6, T7> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4, T5, T6, T7, T8> Record8<T1, T2, T3, T4, T5, T6, T7, T8> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
  ...
  @NotNull
  @Support
  <T1, T2, T3, T4, T5, T6, T7, T8, T9> Record9<T1, T2, T3, T4, T5, T6, T7, T8, T9> fetchSingle(
      SelectField<T1> field1,
      SelectField<T2> field2,
      SelectField<T3> field3,
      SelectField<T4> field4,
  ...
  @NotNull
  @Support
  <R extends Record> Optional<R> fetchOptional(Table<R> table)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  <R extends Record> Optional<R> fetchOptional(Table<R> table, Condition condition)
      throws DataAccessException, TooManyRowsException;
   ...
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchOptional(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The record
   * @throws DataAccessException if something went wrong executing the query
  ...
  @NotNull
  @Support
  <R extends Record> Optional<R> fetchOptional(Table<R> table, Condition... conditions)
      throws DataAccessException, TooManyRowsException;
  ...
  @NotNull
  @Support
  <R extends Record> Optional<R> fetchOptional(
      Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException, TooManyRowsException;

  /**
   * Execute and return zero or one record for <code>
   * <pre>SELECT table.col1, table.col2 FROM table LIMIT 1</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
  ...
  /**
   * Execute and return zero or one record for <code>
   * <pre>SELECT table.col1, table.col2 FROM table WHERE condition LIMIT 1</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
  ...
  /**
   * Execute and return zero or on e record for <code>
   * <pre>SELECT table.col1, table.col2 FROM table WHERE condition LIMIT 1</pre></code>.
   *
   * <p>The resulting record is attached to this {@link Configuration} by default. Use {@link
   * Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchAny(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The record or <code>null</code> if no record was returned
   * @throws DataAccessException if something went wrong executing the query
  ...
  @Nullable
  @Support
  <R extends Record> R fetchAny(Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException;

  /**
   * Execute and return all records lazily for <code>
   * <pre>SELECT table.col1, table.col2 FROM table</pre></code>.
   *
   * <p>The result and its contained records are attached to this {@link Configuration} by default.
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
  ...
  @NotNull
  @Support
  <R extends Record> Cursor<R> fetchLazy(Table<R> table, Condition condition)
      throws DataAccessException;
   ...
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchLazy(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The cursor. This will never be <code>null</code>.
   * @throws DataAccessException if something went wrong executing the query
  ...
  @NotNull
  @Support
  <R extends Record> Cursor<R> fetchLazy(Table<R> table, Condition... conditions)
      throws DataAccessException;
  ...
  @NotNull
  @Support
  <R extends Record> Cursor<R> fetchLazy(Table<R> table, Collection<? extends Condition> conditions)
      throws DataAccessException;

  /**
   * Execute and return all records asynchronously for <code>
   * <pre>SELECT table.col1, table.col2 FROM table</pre></code>.
   *
   * <p>The result and its contained records are attached to this {@link Configuration} by default.
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   ...
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchAsync(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The completion stage. The completed result will never be <code>null</code>.
   */
  ...
  @NotNull
  @Support
  <R extends Record> CompletionStage<Result<R>> fetchAsync(
      Table<R> table, Collection<? extends Condition> condition);
   ...
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchAsync(Executor, Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @return The completion stage. The completed result will never be <code>null</code>.
   */
  ...
  @Support
  <R extends Record> CompletionStage<Result<R>> fetchAsync(
      Executor executor, Table<R> table, Condition... conditions);
  ...
  @Support
  <R extends Record> CompletionStage<Result<R>> fetchAsync(
      Executor executor, Table<R> table, Collection<? extends Condition> conditions);
  ...
  @NotNull
  @Support
  <R extends Record> Stream<R> fetchStream(Table<R> table, Condition condition)
      throws DataAccessException;
   ...
   * Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * <p>Convenience API for calling {@link #fetchStream(Table, Condition)} with {@link
   * DSL#and(Condition...)}.
   *
   * @throws DataAccessException if something went wrong executing the query
   */
  @NotNull
  @Support
  <R extends Record> Stream<R> fetchStream(Table<R> table, Condition... conditions)
      throws DataAccessException;
  ...
  @NotNull
  @Support
  <R extends Record> Stream<R> fetchStream(
      Table<R> table, Collection<? extends Condition> conditions) throws DataAccessException;

  /**
   * Insert one record.
   *
   * <p>This executes something like the following statement: <code>
   * <pre>INSERT INTO [table] ... VALUES [record] </pre></code>
   *
   * <p>Unlike {@link UpdatableRecord#store()}, this does not change any of the argument <code>
   * record</code>'s internal "changed" flags, such that a subsequent call to {@link
   * UpdatableRecord#store()} might lead to another <code>INSERT</code> statement being executed.
   *
  ...
  /**
   * Update a table. <code>
   * <pre>UPDATE [table] SET [modified values in record] WHERE [record is supplied record] </pre>
   * </code>
   *
  ...
  /**
   * Update a table. <code>
   * <pre>UPDATE [table] SET [modified values in record] WHERE [condition]</pre></code>
   *
   * @return The number of updated records
   * @throws DataAccessException if something went wrong executing the query
   */
  ...
  /**
   * Delete a record from a table. <code>
   * <pre>DELETE FROM [table] WHERE [record is supplied record]</pre></code>
   *
  ...
  /**
   * Delete a record from a table. <code><pre>DELETE FROM [table] WHERE [condition]</pre></code>
   *
   * @return The number of deleted records
   * @throws DataAccessException if something went wrong executing the query
   */
