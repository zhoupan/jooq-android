 ...
 *
 * <p>This type is the model API representation of a {@link Select} statement, which can be mutated
 * after creation. The advantage of this API compared to the DSL API is a more simple approach to
 * writing dynamic SQL.
 *
 * <p>Instances can be created using {@link DSLContext#selectQuery()} and overloads.
 *
 * @author Lukas Eder
 */
public interface SelectQuery<R extends Record> extends Select<R>, ConditionProvider {
   ...
   *
   * <p>This also sets the <code>distinct</code> flag to <code>true</code>
   */
  ...
  /**
   * Add a T-SQL style <code>INTO</code> clause to the <code>SELECT</code> statement to create a new
   * table from a <code>SELECT</code> statement.
   */
  ...
  /**
   * Joins the existing table product to a new table using a condition, connecting them with each
   ...
   * @throws DataAccessException If there is no non-ambiguous key definition known to jOOQ
   */
  @Support
   ...
   *
   * <p>Calling this with an empty argument list will result in an empty <code>GROUP BY ()</code>
  ...
  /**
   * Adds a new condition to the having clause of query, connecting it with each other with <code>
   * operator</code>.
   *
  ...
  /**
   * Adds new conditions to the having clause of query, connecting them with each other with <code>
   * operator</code>.
   *
   * @param operator The operator to use to add the conditions to the existing conditions
   * @param conditions The condition
  ...
  /**
   * Adds a new condition to the qualify clause of query, connecting it with each other with <code>
   * operator</code>.
   *
   * @param operator The operator to use to add the conditions to the existing conditions
   * @param condition The condition
  ...
  /**
   * Adds new conditions to the qualify clause of query, connecting them with each other with <code>
   * operator</code>.
   *
   * @param operator The operator to use to add the conditions to the existing conditions
   * @param conditions The condition
   ...
   * </pre></code>
   *
   * <p>You can also use this clause for any other database, that accepts hints or options at the
   * same syntactic location, e.g. for MySQL's <code>SQL_CALC_FOUND_ROWS</code> option: <code><pre>
   * create.select(field1, field2)
   ...
   * </pre></code>
   *
   * <p>The outcome of such a query is this: <code><pre>
   * SELECT [hint] field1, field2 FROM table1
   * </pre></code>
   *
   * <p>For SQL Server style table hints, see {@link Table#with(String)}
   *
   ...
   *
   * <p>You can also use this clause for any other database, that accepts hints or options at the
   * same syntactic location, e.g. for DB2's isolation clause: <code><pre>
   * create.select(field1, field2)
   *       .from(table1)
   ...
   * Adds ordering fields.
   *
   * <p>Indexes start at <code>1</code> in SQL!
   *
   * <p>Note, you can use <code>addOrderBy(DSL.val(1).desc())</code> or <code>
   * addOrderBy(DSL.literal(1).desc())</code> to apply descending ordering
   *
   ...
   *
   * <p>If there is no <code>LIMIT .. OFFSET</code> or <code>TOP</code> clause in your RDBMS, or if
   * your RDBMS does not natively support offsets, this is emulated with a <code>ROW_NUMBER()</code>
   ...
   *
   * <p>This is the same as calling {@link #addLimit(int, int)} with offset = 0
   *
   ...
   * </code> clauses!
   *
   * <p>If there is no <code>LIMIT</code> or <code>TOP</code> clause in your RDBMS, or if your RDBMS
   * does not natively support offsets, this is emulated with a <code>ROW_NUMBER()</code> window
   ...
   *
   * <p>If there is no <code>LIMIT</code> or <code>TOP</code> clause in your RDBMS, or the <code>
   * LIMIT</code> or <code>TOP</code> clause does not support bind values, or if your RDBMS does not
   * natively support offsets, this may be emulated with a <code>ROW_NUMBER()</code> window function
   ...
   *
   * <p>This has been observed to be supported by any of these dialects:
   *
   * <ul>
   *   <li><a href=
   *       "http://publib.boulder.ibm.com/infocenter/db2luw/v9r7/index.jsp?topic=/com.ibm.db2.luw.sql.ref.doc/doc/r0000879.html"
   *       >DB2 FOR UPDATE and similar clauses</a>
   *   <li><a href= "http://db.apache.org/derby/docs/10.7/ref/rrefsqlj31783.html"> Derby's FOR
   *       UPDATE clause</a>
   *   <li><a href="http://www.h2database.com/html/grammar.html#select">H2's FOR UPDATE clause</a>
   *   <li><a href= "http://www.hsqldb.org/doc/2.0/guide/dataaccess-chapt.html#N11DA9" >HSQLDB's FOR
   *       UPDATE clause</a>
   *   <li><a href= "http://dev.mysql.com/doc/refman/5.5/en/innodb-locking-reads.html" >MySQL's
   *       InnoDB locking reads</a>
   *   <li><a href= "http://www.techonthenet.com/oracle/cursors/for_update.php">Oracle's PL/SQL FOR
   *       UPDATE clause</a>
   *   <li><a href= "http://www.postgresql.org/docs/9.0/static/sql-select.html#SQL-FOR-UPDATE-SHARE"
   *       >Postgres FOR UPDATE / FOR SHARE</a>
   * </ul>
   *
   * <p>
   *
   * <h5>Emulation</h5>
   *
   * <p>{@link SQLDialect#SQLSERVER}: jOOQ will try to lock the database record using <code>
   * WITH (ROWLOCK, UPDLOCK)</code> hints.
   *
   * <h5>Not supported</h5>
   *
   * <p>These dialects are known not to support the <code>FOR UPDATE</code> clause in regular SQL:
   *
   * <ul>
   *   <li>{@link SQLDialect#SQLITE}
   * </ul>
   *
   * <p>If your dialect does not support this clause, jOOQ will still render it, if you apply it to
   * your query. This might then cause syntax errors reported either by your database or your JDBC
   * driver.
   *
   * <p>You shouldn't combine this with {@link #setForShare(boolean)}
   *
  ...
  /**
   * Some RDBMS allow for specifying the fields that should be locked by the <code>FOR UPDATE</code>
   * clause, instead of the full row.
   *
   * <p>This automatically sets the {@link #setForUpdate(boolean)} flag, and unsets the {@link
   * #setForShare(boolean)} flag, if it was previously set.
   *
   * <p>This has been observed to be natively supported by any of these dialects:
   *
   * <ul>
   *   <li>DB2
   *   <li>Derby
   ...
   * </ul>
   *
   * <p>Note, that {@link SQLDialect#DB2} has some stricter requirements regarding the updatability
  ...
  /**
   * Some RDBMS allow for specifying the tables that should be locked by the <code>FOR UPDATE</code>
   * clause, instead of the full row.
   *
   * <p>This automatically sets the {@link #setForUpdate(boolean)} flag, and unsets the {@link
   * #setForShare(boolean)} flag, if it was previously set.
   ...
   * </ul>
   *
   * <p>jOOQ emulates this by locking all known fields of [<code>tables</code>] for any of these
   * dialects:
   *
   * <ul>
   *   <li>DB2
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR UPDATE</code>
   * clause. In this case, the session will wait for some <code>seconds</code>, before aborting the
   * lock acquirement if the lock is not available.
   *
   * <p>This automatically sets the {@link #setForUpdate(boolean)} flag, and unsets the {@link
   * #setForShare(boolean)} flag, if it was previously set.
   *
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR UPDATE</code>
   * clause. In this case, the session will not wait before aborting the lock acquirement if the
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR UPDATE</code>
   * clause. In this case, the session will skip all locked rows from the select statement, whose
   * lock is not available.
   *
   * <p>This automatically sets the {@link #setForUpdate(boolean)} flag, and unsets the {@link
   * #setForShare(boolean)} flag, if it was previously set.
   ...
   * driver.
   *
   * <p>You shouldn't combine this with {@link #setForUpdate(boolean)}
   *
  ...
  /**
   * Some RDBMS allow for specifying the fields that should be locked by the <code>
   * FOR &lt;lock_mode&gt;</code> clause, instead of the full row.
   *
   * <p>In case no lock mode has been set yet, it will implicitly be set to <code>UPDATE</code>
   * (i.e. {@link #setForUpdate(boolean)}).
   *
   * <p>Depending on the dialect and lock mode this flag may or may not be supported.
   *
   * <p>Note, that {@link SQLDialect#DB2} has some stricter requirements regarding the updatability
   * of fields. Refer to the DB2 documentation for further details
   *
  ...
  /**
   * Some RDBMS allow for specifying the tables that should be locked by the <code>
   * FOR &lt;lock_mode&gt;</code> clause, instead of the full row.
   *
   * <p>In case no lock mode has been set yet, it will implicitly be set to <code>UPDATE</code>
   * (i.e. {@link #setForUpdate(boolean)}).
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR &lt;lock_mode&gt;
   * </code> clause. In this case, the session will wait for some <code>seconds</code>, before
   * aborting the lock acquirement if the lock is not available.
   *
   * <p>In case no lock mode has been set yet, it will implicitly be set to <code>UPDATE</code>
   * (i.e. {@link #setForUpdate(boolean)}).
   *
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR &lt;lock_mode&gt;
   * </code> clause. In this case, the session will not wait before aborting the lock acquirement if
  ...
  /**
   * Some RDBMS allow for specifying the locking mode for the applied <code>FOR &lt;lock_mode&gt;
   * </code> clause. In this case, the session will skip all locked rows from the select statement,
   * whose lock is not available.
   *
   * <p>In case no lock mode has been set yet, it will implicitly be set to <code>UPDATE</code>
   * (i.e. {@link #setForUpdate(boolean)}).
   *
