 ...
 * An identifier.
 *
 * <p>A <code>Name</code> or identifier is a {@link QueryPart} that renders a SQL identifier
 * according to the settings specified in {@link Settings#getRenderQuotedNames()} and {@link
 ...
 * </pre></code>
 *
 * <p>Instances can be created using {@link DSL#name(String)} and overloads.
 *
 * @author Lukas Eder
 */
public interface Name extends QueryPart, Comparable<Name> {
   ...
   *
   * <p>Quoting of names can be overridden by {@link Settings#getRenderQuotedNames()}.
   *
   * @author Lukas Eder
   */
  enum Quoted {
     ...
     *
     * <p>The behaviour of this name's quoting is governed by {@link
     * RenderQuotedNames#EXPLICIT_DEFAULT_QUOTED} and {@link
  ...
  /**
   * Get the last segment of the qualified name (usually a {@link Table}, {@link Field}, or {@link
   * Parameter} name).
   */
  @Nullable
  String last();
   ...
   *
   * <p>An unnamed object will have an <code>""</code> empty string as its name. If a table has no
   * explicit schema, it will be located in the default schema whose name is <code>""</code>.
   */
  boolean empty();
   ...
   *
   * <p>This is <code>true</code> as soon as {@link #getName()} has a length of more than <code>1
   * </code> and {@link #qualifier()} is not null.
   */
  boolean qualified();

  /**
   * Whether this is a qualified name and the {@link #qualifier()} is also a qualified name.
   *
   * <p>This is <code>true</code> as soon as {@link #getName()} has a length of more than <code>2
   * </code> and {@link #qualifier()} is not null and its {@link #qualified()} property is also
   * true.
   */
   ...
   *
   * <p>This creates a window definition that can be
   *
   * <ul>
  ...
  /**
   * Specify a subselect to refer to by the <code>Name</code> to form a common table expression.
   *
   * <p>Column names are implicitly inherited from the <code>SELECT</code> statement.
   */
  @NotNull
  ...
  /**
   * Specify a materialized subselect to refer to by the <code>Name</code> to form a common table
   * expression.
   *
  ...
  /**
   * Specify a non-materialized subselect to refer to by the <code>Name</code> to form a common
   * table expression.
   *
  ...
  /**
   * Add a list of fields to this name to make this name a {@link DerivedColumnList}.
   *
   * <p>The <code>DerivedColumnList</code> can then be used along with a subselect to form a {@link
   * CommonTableExpression} to be used with <code>WITH</code> clauses.
   */
  @NotNull
  @Support({FIREBIRD, H2, HSQLDB, IGNITE, MARIADB, MYSQL, POSTGRES, SQLITE})
   ...
   * CommonTableExpression} to be used with <code>WITH</code> clauses.
   *
   * <p>This works in a similar way as {@link #fields(String...)}, except that all column names are
   * produced by a function that receives the CTE's {@link Select} columns and their column indexes
   * as input.
   *
   ...
   *
   * <p>This works in a similar way as {@link #fields(String...)}, except that all column names are
   * produced by a function that receives the CTE's {@link Select} columns as input.
   *
   * @deprecated - 3.14.0 - [#10156] - These methods will be removed without replacement from a
   *     future jOOQ. They offer convenience that is unidiomatic for jOOQ's DSL, without offering
   *     functionality that would not be possible otherwise - yet they add complexity in jOOQ's
