 ...
 *
 * <p>The step in constructing the {@link Loader} object where you can set the optional CSV loader
 * options.
 *
 * <p>
 *
...
public interface LoaderCSVOptionsStep<R extends Record> extends LoaderListenerStep<R> {
  ...
  /**
   * Specify that a certain number of rows should be ignored from the CSV file. This is useful for
   * skipping processing information
   *
   * <p>By default, this is set to <code>1</code>, as CSV files are expected to hold a header row.
   *
   ...
   *
   * <p>By default, this is set to <code>null</code>, which means that all empty strings are loaded
   * into the database as such. In some databases (e.g. {@link SQLDialect#ORACLE}), this is
   * effectively the same as loading <code>NULL</code>.
   *
   * <p>In order to treat empty strings as <code>null</code>, you can set the <code>nullString
   * </code> to <code>""</code>. If the null string is overridden with something like <code>{null}
   * </code>, for instance, then empty strings will also be loaded as such by jOOQ.
   */
  @NotNull
  @CheckReturnValue
  @Support
