...
public final class SettingsTools {
    ...
    if (settings == null) {
      ...
      if (in != null) {
        try (InputStream i = in) {
          log.warn(
              "DEPRECATION",
              "Loading system wide default settings via the classpath /jooq-settings.xml resource has been deprecated. Please use explicit Settings in your Configuration references, instead.");
          settings = MiniJAXB.unmarshal(i, Settings.class);
        } catch (IOException e) {
          log.error("Error while reading settings: " + e);
        }
   ...
   *
   * <p>The {@link ParamType} can be overridden by the {@link StatementType}. If the latter is set
   * to {@link StatementType#STATIC_STATEMENT}, then the former defaults to {@link
   * ParamType#INLINED}.
   */
  ...
  /**
   * The interpreter locale that is applicable, or the default locale if no such locale is
   * configured.
   */
  ...
  /**
   * Backwards compatible access to {@link RenderKeywordCase} and/or {@link RenderKeywordStyle} (the
   * latter being deprecated).
   */
  ...
  /**
   * Backwards compatible access to {@link RenderNameCase} and/or {@link RenderNameStyle} (the
   * latter being deprecated).
   */
  ...
  /**
   * Backwards compatible access to {@link RenderQuotedNames} and/or {@link RenderNameStyle} (the
   * latter being deprecated).
   */
  ...
  public static final TransformUnneededArithmeticExpressions
      getTransformUnneededArithmeticExpressions(Settings settings) {
   ...
   *
   * <ul>
   *   <li>If the JVM flag <code>-Dorg.jooq.settings</code> points to a valid settings file on the
   *       classpath, this will be loaded
   *   <li>If the JVM flag <code>-Dorg.jooq.settings</code> points to a valid settings file on the
   *       file system, this will be loaded
   *   <li>If a valid settings file is found on the classpath at <code>/jooq-settings.xml</code>,
   *       this will be loaded
   *   <li>Otherwise, a new <code>Settings</code> object is created with its defaults
   * </ul>
   */
  ...
  /**
   * Return <code>timeout</code> if it is not <code>0</code>, or the specified {@link
   * Settings#getQueryTimeout()}.
   */
  public static final int getQueryTimeout(int timeout, Settings settings) {
    return timeout != 0
        ? timeout
        : settings.getQueryTimeout() != null ? settings.getQueryTimeout() : 0;
  }

  /**
   * Return <code>poolable</code> if it is not <code>null</code>, or the specified {@link
  ...
  public static final QueryPoolable getQueryPoolable(QueryPoolable poolable, Settings settings) {
    return poolable != null && poolable != QueryPoolable.DEFAULT
        ? poolable
        : settings.getQueryPoolable() != null ? settings.getQueryPoolable() : QueryPoolable.DEFAULT;
  }

  /**
   * Return <code>maxRows</code> if it is not <code>0</code>, or the specified {@link
   * Settings#getMaxRows()}.
   */
  ...
  public static final boolean fetchIntermediateResult(Configuration configuration) {
    switch (defaultIfNull(
        configuration.settings().getFetchIntermediateResult(), WHEN_RESULT_REQUESTED)) {
      case ALWAYS:
        return true;
      ...
      default:
        throw new IllegalStateException(
            "Unhandled FetchIntermediateResult: "
                + configuration.settings().getFetchIntermediateResult());
    }
  }

  /**
   * Return <code>fetchSize</code> if it is not <code>0</code>, or the specified {@link
   * Settings#getFetchSize()}.
   */
  public static final int getFetchSize(int fetchSize, Settings settings) {
    return fetchSize != 0
        ? fetchSize
        : settings.getFetchSize() != null ? settings.getFetchSize() : 0;
  }
  ...
  /**
   * Return <code>fetchServerOutputSize</code> if it is not <code>0</code>, or the specified {@link
  ...
  public static final int getFetchServerOutputSize(int fetchServerOutputSize, Settings settings) {
    return fetchServerOutputSize != 0
        ...
        : settings.getFetchServerOutputSize() != null ? settings.getFetchServerOutputSize() : 0;
  }
}
