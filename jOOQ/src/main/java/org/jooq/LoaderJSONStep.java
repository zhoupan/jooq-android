 ...
 *
 * <p>The step in constructing the {@link org.jooq.Loader} object where you can set the mandatory
 * JSON loader options.
 *
 * @author Lukas Eder
...
public interface LoaderJSONStep<R extends Record> {
   ...
   *
   * <p>The JSON column at index <code>i</code> is inserted into the table field at index <code>i
   * </code>. If <code>fields[i] == null</code> or <code>fields.length &lt;= i</code>, then the JSON
   * column is skipped.
   */
  @NotNull
  @CheckReturnValue
   ...
   * new ArrayList(fields).size() &lt;= i</code>, then the JSON column is skipped.
   */
  @NotNull
  @CheckReturnValue
  @Support
   ...
   *
   * <p>The input field obtained from {@link LoaderFieldContext#field()} wraps the JSON column name
   * if any, or an unspecified field enumeration is used. The {@link LoaderFieldContext#index()}
