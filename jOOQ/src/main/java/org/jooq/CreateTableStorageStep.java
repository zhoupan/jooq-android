...
public interface CreateTableStorageStep extends CreateTableFinalStep {
   ...
   *
   * <p>Storage clauses will always be appended to the <em>end</em> of everything else that jOOQ
   * renders, including possibly other storage clauses, such as {@link
   * CreateTableOnCommitStep#onCommitDeleteRows()} or similar clauses. If custom storage clauses
   * should be mixed with jOOQ-provided storage clauses, it is recommended not to use the jOOQ API
   * and use the custom clause API for all storage clauses instead.
   *
   * <p>Storage clauses will be separated from previous elements by a separator (whitespace or
   * newline) to ensure syntactic integrity.
   *
   * <p>Example usage:
   *
   * <p><code><pre>
