...
public interface InsertOnDuplicateStep<R extends Record> extends InsertReturningStep<R> {
   ...
   *
   * <p>Only {@link SQLDialect#POSTGRES} and {@link SQLDialect#SQLITE} have native support for this
   * clause. The other dialects can emulate it using <code>MERGE</code>, if table meta data is
   * available.
   */
  @NotNull
  @CheckReturnValue
   ...
   *
   * <p>This will try to <code>INSERT</code> a record. If there is a primary key or unique key in
   * this <code>INSERT</code> statement's affected table that matches the value being inserted, then
   * the <code>UPDATE</code> clause is executed instead.
   *
   * <p>MySQL natively implements this type of clause. jOOQ can emulate this clause using a <code>
   * MERGE</code> statement on some other databases. The conditions for a RDBMS to emulate this
   * clause are:
   *
   * <ul>
   *   <li>The <code>INSERT</code> statement's table is a {@link Table} with a {@link
   *       Table#getPrimaryKey()}
   *   <li>The RDBMS supports the <code>MERGE</code> clause (see {@link
   *       DSLContext#mergeInto(Table)}).
   * </ul>
   *
   * <p>H2 supports this clause in MySQL mode.
   */
  @NotNull
  @CheckReturnValue
   ...
   * <p>This will try to <code>INSERT</code> a record. If there is a primary key or unique key in
   * this <code>INSERT</code> statement's affected table that matches the value being inserted, then
   * the <code>INSERT</code> statement is ignored.
   *
