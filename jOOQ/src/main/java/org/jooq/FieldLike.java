...
public interface FieldLike {
   ...
   *
   * <p>This method is useful for things like <code>SELECT y.*, (SELECT a FROM x) FROM y</code>
   *
   ...
   * The underlying field representation of this object.
   *
   * <p>This method is useful for things like <code>SELECT y.*, (SELECT a FROM x) [alias] FROM y
   * </code>
   *
   ...
   * in many SQL dialects!
   *
   * <p>This works like {@link #asField(String)}, except that field aliases are provided by a
   * function. This is useful, for instance, to prefix all columns with a common prefix (on {@link
