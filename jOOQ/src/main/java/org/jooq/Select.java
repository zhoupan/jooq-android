 ...
 * </pre></code>
 *
 * <p>Instances can be created using {@link DSL#select(SelectFieldOrAsterisk...)}, or {@link
 * DSLContext#selectQuery()} and overloads.
 *
...
public interface Select<R extends Record> extends ResultQuery<R>, TableLike<R>, FieldLike {
   ...
   *
   * @throws IllegalArgumentException If the argument select has the same identity as this select.
   *     The jOOQ 3.x API is mutable, which means that calls to the DSL API mutate this instance.
   *     Adding this instance as an set operation argument would lead to a {@link
   *     StackOverflowError} when generating the SQL.
   */
  @NotNull
  @CheckReturnValue
  @Support
