...
/**
 * An implementation for the SQL standard <code>INTERVAL DAY TO SECOND</code> data type.
 *
 * <p><code>DayToSecond</code> is a {@link Number} whose {@link Number#intValue()} represents the
 * (truncated) number of milliseconds of the interval, {@link Number#doubleValue()} represents the
 * approximative number of milliseconds (including hours, minutes, seconds, nanoseconds) of the
 * interval.
 *
 * <p>Note: only a few databases actually support this data type on its own. You can still use it
 * for date time arithmetic in other databases, though, through {@link Field#add(Field)} and {@link
 * Field#sub(Field)} Databases that have been observed to natively support <code>INTERVAL</code>
 * data types are:
 *
 * <ul>
 *   <li>{@link SQLDialect#HSQLDB}
 *   <li>{@link SQLDialect#INGRES}
 *   <li>{@link SQLDialect#ORACLE}
 *   <li>{@link SQLDialect#POSTGRES}
 * </ul>
 *
 * <p>These dialects have been observed to partially support <code>INTERVAL</code> data types in
 * date time arithmetic functions, such as <code>TIMESTAMPADD</code>, and <code>TIMESTAMPDIFF</code>
 * :
 *
 * <ul>
 *   <li>{@link SQLDialect#MARIADB}
...
public final class DayToSecond extends Number implements Interval, Comparable<DayToSecond> {
   ...
   * @return The parsed <code>INTERVAL DAY TO SECOND</code> object, or <code>null</code> if the
  ...
  /**
   * Load a {@link Double} representation of a <code>INTERVAL DAY TO SECOND</code> by assuming
  ...
  /**
   * Transform a {@link Duration} into a {@link DayToSecond} interval by taking its number of
   * milliseconds.
   */
  ...
  public final double getTotalSeconds() {
    return getSign()
        * (nano / 1000000000.0 + seconds + 60.0 * minutes + 3600.0 * hours + 3600.0 * 24.0 * days);
  }
  ...
  public final int compareTo(DayToSecond that) {
    ...
    else if (days > that.days) return 1;
    else if (hours < that.hours) return -1;
    else if (hours > that.hours) return 1;
    else if (minutes < that.minutes) return -1;
    else if (minutes > that.minutes) return 1;
    else if (seconds < that.seconds) return -1;
    else if (seconds > that.seconds) return 1;
    else return Integer.compare(nano, that.nano);
  }
  ...
  public final int hashCode() {
    final int prime = 31;
    int result = 0;
    if (days != 0) result = prime * result + days;
    if (hours != 0) result = prime * result + hours;
    if (minutes != 0) result = prime * result + minutes;
    if (nano != 0) result = prime * result + nano;
    if (seconds != 0) result = prime * result + seconds;
    return result;
  }
  ...
  public final boolean equals(Object obj) {
    ...
    if (getClass() == obj.getClass()) {
      ...
      if (negative != other.negative && doubleValue() != 0.0) return false;
      return true;
    } else if (obj instanceof YearToSecond) return obj.equals(this);
    else return false;
  }
