 ...
 *
 * <p>This connection buffers consecutive identical prepared statements for <code>DELETE</code>,
 * <code>INSERT</code>, <code>MERGE</code>, <code>UPDATE</code> statements, instead of allowing for
 * them to be executed directly.
 *
 * <p>Calls to {@link PreparedStatement#executeUpdate()} or {@link PreparedStatement#execute()} are
 * replaced by {@link PreparedStatement#addBatch()}. The update count of such operations is always
 * zero, and cannot be retrieved later on. When any of the following events happen, the batch is
 * executed using {@link Statement#executeBatch()}:
 *
 * <ul>
 *   <li>The {@link BatchedConnection#close} method is called (the call is not delegated to the
 *       wrapped connection).
 *   <li>A {@link Connection#prepareStatement(String)} call is made with a different SQL string.
 *   <li>Any other type of statement is created or other API is called, such as {@link #commit()}.
 * </ul>
 *
 * @author Lukas Eder
 */
