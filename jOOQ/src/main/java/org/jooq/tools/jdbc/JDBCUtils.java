...
public class JDBCUtils {
   ...
   *
   * <p>This method tries to guess the <code>SQLDialect</code> of a connection from the its
   * connection URL as obtained by {@link DatabaseMetaData#getURL()}. If the dialect cannot be
   * guessed from the URL (e.g. when using an JDBC-ODBC bridge), further actions may be implemented
   * in the future.
   *
   * @return The appropriate {@link SQLDialect} or {@link SQLDialect#DEFAULT} if no dialect could be
   *     derived from the connection. Never <code>null</code>.
   * @see #dialect(String)
   */
  @NotNull
   ...
   *
   * <p>This method tries to guess the <code>SQLDialect</code> of a connection from the its {@link
   * ConnectionFactoryMetadata} as obtained by {@link ConnectionFactory#getMetadata()}. If the
   * dialect cannot be guessed, further actions may be implemented in the future.
   *
   * @return The appropriate {@link SQLDialect} or {@link SQLDialect#DEFAULT} if no dialect could be
   *     derived from the connection. Never <code>null</code>.
   * @see #dialect(String)
  ...
  /**
   * "Guess" the {@link SQLDialect} from an R2DBC {@link io.r2dbc.spi.Connection} instance.
   *
   * <p>This method tries to guess the <code>SQLDialect</code> of a connection from the its {@link
   * ConnectionMetadata} as obtained by {@link io.r2dbc.spi.Connection#getMetadata()}. If the
   * dialect cannot be, further actions may be implemented in the future.
   *
   * @return The appropriate {@link SQLDialect} or {@link SQLDialect#DEFAULT} if no dialect could be
   *     derived from the connection. Never <code>null</code>.
   * @see #dialect(String)
  ...
  private static SQLDialect dialectFromProductName(String product) {
    ...
    else if (p.contains("postgres")) return POSTGRES;
    else return DEFAULT;
  }
  ...
  public static final SQLDialect dialect(String url) {
    ...
    else if (url.contains(":hsqldb:")) return HSQLDB;
    else if (url.contains(":ignite:")) return IGNITE;
    else if (url.contains(":mariadb:")) return MARIADB;
   ...
   *
   * @return The appropriate JDBC driver class or <code>"java.sql.Driver"</code> if no driver class
   *     could be derived from the URL. Never <code>null</code>.
   */
  @NotNull
   ...
   *
   * <p>This method will silently ignore if <code>connection</code> is <code>null</code>, or if
   * {@link Connection#close()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>statement</code> is <code>null</code>, or if
   * {@link Statement#close()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>resultSet</code> is <code>null</code>, or if
   * {@link ResultSet#close()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>resultSet</code> or <code>statement</code> is
   * <code>null</code>, or if {@link ResultSet#close()} or {@link Statement#close()} throws an
   * exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>closeable</code> is <code>null</code>, or if
   * {@link Closeable#close()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>closeable</code> is <code>null</code>, or if
   * {@link AutoCloseable#close()} throws an exception.
   */
   ...
   * Safely free a blob.
   *
   * <p>This method will silently ignore if <code>blob</code> is <code>null</code>, or if {@link
   * Blob#free()} throws an exception.
   */
   ...
   * Safely free a clob.
   *
   * <p>This method will silently ignore if <code>clob</code> is <code>null</code>, or if {@link
   * Clob#free()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>xml</code> is <code>null</code>, or if {@link
   * SQLXML#free()} throws an exception.
   */
   ...
   *
   * <p>This method will silently ignore if <code>array</code> is <code>null</code>, or if {@link
   * Array#free()} throws an exception.
   */
   ...
   *
   * <p>This is useful to check if primitive types obtained from the JDBC API were actually SQL NULL
   * values.
   *
   * @param stream The data source from which a value was read
   * @param value The value that was read
   * @return The <code>value</code> or <code>null</code> if the {@link SQLInput#wasNull()} is <code>
   *     true</code>
   */
   ...
   * @param value The value that was read
   * @return The <code>value</code> or <code>null</code> if the {@link ResultSet#wasNull()} is
   *     <code>true</code>
   */
   ...
   * @param value The value that was read
   * @return The <code>value</code> or <code>null</code> if the {@link CallableStatement#wasNull()}
   *     is <code>true</code>
   */
