...
public class MockResultSet extends JDBC41ResultSet implements ResultSet, Serializable {
  ...
  public <T> T unwrap(Class<T> iface) throws SQLException {
    ...
    else throw new SQLException("MockResultSet does not implement " + iface);
  }
  ...
  void checkNotClosed() throws SQLException {
    if (result == null) throw new SQLException("ResultSet is already closed");
  }
  ...
  private Converter<?, ?> converter(int columnIndex) throws SQLException {
    if (columnIndex > 0 && columnIndex <= converters.length) return converters[columnIndex - 1];
    else throw new SQLException("Unknown column index : " + columnIndex);
  }

  private long getMillis(
      Calendar cal, int year, int month, int day, int hour, int minute, int second, int millis) {
