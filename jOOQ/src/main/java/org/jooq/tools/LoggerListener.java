...
/**
 * A default {@link ExecuteListener} that just logs events to java.util.logging, log4j, or slf4j
 * using the {@link JooqLogger}
 *
 * @author Lukas Eder
 */
public class LoggerListener extends DefaultExecuteListener {
  ...
  public void recordEnd(ExecuteContext ctx) {
    ...
    if (log.isDebugEnabled() && ctx.record() != null && !FALSE.equals(ctx.data(DO_BUFFER))) {
      ...
      if (buffer == null) ctx.data(BUFFER, buffer = ctx.dsl().newResult(ctx.record().fields()));
      if (buffer.size() < maxRows()) buffer.add(ctx.record());
    }
  }
  ...
  private void log(Result<?> result) {
    logMultiline(
        "Fetched result",
        result.format(TXTFormat.DEFAULT.maxRows(maxRows()).maxColWidth(maxColWidth())),
        Level.FINE);
  }

  private int maxRows() {
    if (log.isTraceEnabled()) return 500;
    else if (log.isDebugEnabled()) return 5;
    else return 0;
  }

  private int maxColWidth() {
    if (log.isTraceEnabled()) return 500;
    else if (log.isDebugEnabled()) return 50;
    else return 0;
  }

  @Override
  public void executeEnd(ExecuteContext ctx) {
    if (ctx.rows() >= 0) if (log.isDebugEnabled()) log.debug("Affected row(s)", ctx.rows());
  }
    ...
    if (ctx.routine() != null)
      if (log.isDebugEnabled())
        logMultiline(
            ...
            "" + StringUtils.defaultIfNull(record(ctx.configuration(), ctx.routine()), "N/A"),
            Level.FINE);
  }

  @Override
  public void exception(ExecuteContext ctx) {
    if (log.isDebugEnabled()) log.debug("Exception", ctx.exception());
  }
