...
public class Reflect {
   ...
   * @throws ReflectException if anything went wrong compiling the class.
   */
  public static Reflect compile(String name, String content, CompileOptions options)
   ...
   *
   * <p>This is the same as calling <code>on(Class.forName(name, classLoader))</code>
   *
   * @param name A fully qualified class name.
   ...
   * Wrap a class name.
   *
   * <p>This is the same as calling <code>onClass(Class.forName(name))</code>
   *
   * @param name A fully qualified class name
   * @return A wrapped class object, to be used for further reflection.
   * @throws ReflectException If any reflection exception occurred.
   ...
   * Wrap a class name, loading it via a given class loader.
   *
   * <p>This is the same as calling <code>onClass(Class.forName(name, classLoader))</code>
   *
   * @param name A fully qualified class name.
   * @param classLoader The class loader in whose context the class should be loaded.
   * @return A wrapped class object, to be used for further reflection.
   ...
   *
   * <ul>
   *   <li><code>null</code> for reference types (including wrapper types)
   *   <li><code>0</code> for numeric primitive types (including <code>char</code>)
   ...
   *
   * <p>This method is also capable of setting the value of (static) final fields. This may be
   * convenient in situations where no {@link SecurityManager} is expected to prevent this, but do
   * note that (especially static) final fields may already have been inlined by the javac and/or
   ...
   *
   * <p>For restrictions of usage regarding setting values on final fields check: <a href=
   ...
   * that rule, the first one encountered is called. i.e. when calling <code><pre>
   ...
   *
   * @param proxyType The interface type that is implemented by the proxy
   * @param additionalInterfaces Additional interfaces that are implemented by the proxy
   * @return A proxy for the wrapped object
   */
  @SuppressWarnings("unchecked")
          ...
          public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            // [#14] Emulate POJO behaviour on wrapped map objects
              ...
              if (method.isDefault()) {
                ...
                // Java 8 version
                else proxyLookup = CACHED_LOOKUP_CONSTRUCTOR.newInstance(proxyType);
  ...
  public static <T> Class<T> wrapper(Class<T> type) {
    if (type == null) return null;
      ...
      else if (int.class == type) return (Class<T>) Integer.class;
      else if (long.class == type) return (Class<T>) Long.class;
      else if (short.class == type) return (Class<T>) Short.class;
      else if (byte.class == type) return (Class<T>) Byte.class;
      else if (double.class == type) return (Class<T>) Double.class;
      else if (float.class == type) return (Class<T>) Float.class;
      else if (char.class == type) return (Class<T>) Character.class;
      else if (void.class == type) return (Class<T>) Void.class;
    }
