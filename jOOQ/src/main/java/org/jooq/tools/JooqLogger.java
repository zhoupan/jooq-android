 ...
 *
 * <p>This logger provides abstraction over the three logger APIs supported optionally by jOOQ.
 ...
 * </ul>
 *
 * <code>JooqLogger</code> tries to instantiate any of the above loggers, catching potential {@link
 * NoClassDefFoundError}'s in case any logger API cannot be found on the classpath.
 *
 * @author Lukas Eder
 */
public final class JooqLogger implements Log {
   ...
   *
   * @param message The log message
   * @param throwable An exception whose stacktrace is logged along with the message
   */
  @Override
  ...
  public boolean isWarnEnabled() {
    if (!globalThreshold.supports(Log.Level.WARN)) return false;
    if (!supportsWarn) return false;
    else if (slf4j != null) return slf4j.isWarnEnabled();
    else return util.isLoggable(java.util.logging.Level.WARNING);
  }
  ...
  /** [#12405] The common {@link SimpleFormatter} format to be set in all of jOOQ's CLIs. */
