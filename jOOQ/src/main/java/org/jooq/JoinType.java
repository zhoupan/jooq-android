...
public enum JoinType {
  ...
  NATURAL_LEFT_OUTER_JOIN(
      "natural left outer join", "natural left outer join", "natural left join", false),
  /** <code>NATURAL RIGHT OUTER JOIN</code> two tables. */
  @NotNull
  @Support({CUBRID, DERBY, FIREBIRD, H2, HSQLDB, MARIADB, MYSQL, POSTGRES})
  NATURAL_RIGHT_OUTER_JOIN(
      "natural right outer join", "natural right outer join", "natural right join", false),
  /** <code>NATURAL FULL OUTER JOIN</code> two tables. */
  @NotNull
  @Support({FIREBIRD, HSQLDB, POSTGRES})
  NATURAL_FULL_OUTER_JOIN(
      "natural full outer join", "natural full outer join", "natural full join", false),
  ...
  /**
   * Whether a <code>JOIN</code> operation of this type must be qualified with <code>ON</code> or
   * <code>USING</code>.
   */
