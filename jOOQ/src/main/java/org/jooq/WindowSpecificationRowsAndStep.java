...
public interface WindowSpecificationRowsAndStep {
  ...
  /** Add a <code>... AND UNBOUNDED PRECEDING</code> frame clause to the window specification. */
  @NotNull
  @Support({H2, MARIADB, MYSQL, POSTGRES, SQLITE})
  WindowSpecificationExcludeStep andUnboundedPreceding();

  /** Add a <code>... AND [number] PRECEDING</code> frame clause to the window specification. */
  @NotNull
  @Support({H2, MARIADB, MYSQL, POSTGRES, SQLITE})
  WindowSpecificationExcludeStep andPreceding(int number);

  /** Add a <code>... AND CURRENT ROW</code> frame clause to the window specification. */
  @NotNull
  @Support({H2, MARIADB, MYSQL, POSTGRES, SQLITE})
  WindowSpecificationExcludeStep andCurrentRow();

  /** Add a <code>... AND UNBOUNDED FOLLOWING</code> frame clause to the window specification. */
  @NotNull
  @Support({H2, MARIADB, MYSQL, POSTGRES, SQLITE})
  WindowSpecificationExcludeStep andUnboundedFollowing();

  /** Add a <code>... AND [number] FOLLOWING</code> frame clause to the window specification. */
  @NotNull
  @Support({H2, MARIADB, MYSQL, POSTGRES, SQLITE})
