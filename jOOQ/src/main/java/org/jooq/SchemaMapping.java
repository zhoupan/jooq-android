 ...
 *
 * <p>There are several use cases, when the run-time schema configuration may be different from the
 * compile-time (or code-generation-time) schema configuration. Say, you develop a schema called
 * <code>DEV</code>. It contains a table <code>DEV.T</code>. When you install your database on a
 * productive system, you might have two schemata:
 *
 * <ul>
 *   <li><code>PROD</code>: The productive schema. It contains the table <code>PROD.T</code>
 *   <li><code>BACKUP</code>: The productive backup schema. This schema might be shared with other
 *       applications, so you might have table name collisions. Therefore, you'd want to map your
 *       table <code>DEV.T</code> onto <code>BACKUP.MY_T</code>
 * </ul>
 *
 * <p>This can be achieved with the <code>SchemaMapping</code>, where you can map schemata and
 * tables, for them to render different names at run-time, than at compile-time.
 *
 * @author Lukas Eder
 * @see <a
...
public class SchemaMapping implements Serializable {
  ...
  private transient volatile Map<String, Catalog> catalogs;
  ...
  private static void logDeprecation() {
    if (!loggedDeprecation) {
      ...
      log.warn(
          "DEPRECATION",
          "org.jooq.SchemaMapping is deprecated as of jOOQ 2.0.5. Consider using jOOQ's runtime configuration org.jooq.conf.Settings instead");
    }
  }

  /**
   * Set a schema as the default schema. This results in the supplied schema being omitted in
   * generated SQL.
   *
   * <p>If the supplied mapping has already been added using {@link #add(Schema, Schema)}, then
   * <code>use()</code> has no effect.
   *
  ...
  public Catalog map(Catalog catalog) {
    ...
    if (catalog instanceof RenamedCatalog) return catalog;
    Catalog result = catalog;
    if (result == null) result = DSL.catalog(name(""));
                ...
                if (!isBlank(c.getOutput()))
                  if (c.getInput() != null && !c.getOutput().equals(catalogName))
                    ...
                    result =
                        new RenamedCatalog(
                            result,
                            c.getInputExpression().matcher(catalogName).replaceAll(c.getOutput()));
                break;
              }
            }
  ...
  public Schema map(Schema schema) {
    ...
    if (schema instanceof RenamedSchema) return schema;
    Schema result = schema;
    if (result == null) result = schema(name(""));
    Catalog catalog = result.getCatalog();
    if (catalog == null) catalog = DSL.catalog(name(""));
                      ...
                      if (s.getInput() != null && !s.getOutput().equals(schemaName))
                        ...
                        result =
                            new RenamedSchema(
                                map(catalog),
                                result,
                                s.getInputExpression()
                                    .matcher(schemaName)
                                    .replaceAll(s.getOutput()));
                    break catalogLoop;
                  }
                }
                // [#7498] Even without schema mapping configuration, we may still need to map the
                // catalog
                    ...
                    if (s.getInput() != null && !s.getOutput().equals(schemaName))
                      ...
                      result =
                          new RenamedSchema(
                              catalog,
                              result,
                              s.getInputExpression().matcher(schemaName).replaceAll(s.getOutput()));
                  break;
                }
              }
  ...
  public <R extends Record> Table<R> map(Table<R> table) {
    ...
    if (result != null
        && (!mapping().getSchemata().isEmpty() || !mapping().getCatalogs().isEmpty())) {
      Catalog catalog = result.getCatalog();
      ...
      if (catalog == null) catalog = DSL.catalog(name(""));
      if (schema == null) schema = schema(name(""));
      String catalogName = catalog.getName();
                          ...
                          if (t.getInput() != null && !t.getOutput().equals(tableName))
                            ...
                            result =
                                new RenamedTable<>(
                                    map(schema),
                                    result,
                                    t.getInputExpression()
                                        .matcher(tableName)
                                        .replaceAll(t.getOutput()));
                        break catalogLoop;
                      }
                    }
                  }
                }
                // [#7498] Even without table mapping configuration, we may still need to map the
                // schema
  ...
  private final boolean matches(MappedCatalog c, String catalogName) {
    return (c.getInput() != null && catalogName.equals(c.getInput()))
        || (c.getInputExpression() != null
            && c.getInputExpression().matcher(catalogName).matches());
  }
  ...
  /** Synonym for {@link #use(String)}. Added for better interoperability with Spring */
