...
/**
 * Cursors allow for lazy, sequential access to an underlying JDBC {@link ResultSet}. Unlike {@link
 * Result}, data can only be accessed sequentially, using an {@link Iterator}, or the cursor's
 * {@link #hasNext()} and {@link #fetch()} methods.
 *
 * <p>Client code must close this {@link Cursor} in order to close the underlying {@link
 * PreparedStatement} and {@link ResultSet}
 *
 * <p>The cursor can be consumed in two ways:
 *
 * <ul>
 *   <li>Record by record: Such methods can be recognised by the term "Next" in the method name,
 *       e.g. {@link #fetchNext(int)}.
 *   <li>Completely in one go: Such methods do not have the term "Next" in their method names, e.g.
 *       {@link #fetch()}.
 * </ul>
 *
 * <p>Note: Unlike usual implementations of {@link Iterable}, a <code>Cursor</code> can only provide
 * one {@link Iterator}!
 *
...
public interface Cursor<R extends Record> extends Fields, Iterable<R>, Formattable, AutoCloseable {
   ...
   *
   * <p>This will conveniently close the <code>Cursor</code>, after the last <code>Record</code> was
   * fetched.
   *
   ...
   * default. Use {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * @param number The number of records to fetch. If this is <code>0</code> or negative an empty
   *     list is returned, the cursor is untouched. If this is greater than the number of remaining
   ...
   *
   * <p>This is the same as calling <code>fetch().into(type)</code>. See {@link Record#into(Class)}
   * for more details
   *
   ...
   *
   * <p>This is the same as calling <code>fetch().into(table)</code>. See {@link Record#into(Class)}
   * for more details
   *
   * <p>The result and its contained records are attached to the original {@link Configuration} by
   * default. Use {@link Settings#isAttachRecords()} to override this behaviour.
   ...
   * {@link Settings#isAttachRecords()} to override this behaviour.
   *
   * @return The next record from the cursor, or <code>null</code> if there is no next record.
   * @throws DataAccessException if something went wrong executing the query
   */
  @Nullable
   ...
   *
   * <p>This is the same as calling <code>fetchOne().into(type)</code>. See {@link
   ...
   *
   * <p>This is the same as calling <code>fetchOne().into(table)</code>. See {@link
   * Record#into(Class)} for more details
   *
   * <p>The resulting record is attached to the original {@link Configuration} by default. Use
   * {@link Settings#isAttachRecords()} to override this behaviour.
   ...
   */
  @NotNull
  <E> Optional<E> fetchNextOptionalInto(Class<? extends E> type)
      throws DataAccessException, MappingException;
   ...
   */
  @NotNull
  <Z extends Record> Optional<Z> fetchNextOptionalInto(Table<Z> table)
      throws DataAccessException, MappingException;
   ...
   *
   * <p>If you fetch all records from the underlying {@link ResultSet}, jOOQ <code>Cursor</code>
   * implementations will close themselves for you. Calling <code>close()</code> again will have no
   * effect.
   *
   * @throws DataAccessException if something went wrong executing the query
   */
  ...
  /**
   * Check whether this <code>Cursor</code> has been explicitly or "conveniently" closed.
   *
   * <p>Explicit closing can be achieved by calling {@link #close()} from client code. "Convenient"
   * closing is done by any of the other methods, when the last record was fetched.
   */
  boolean isClosed();
   ...
   *
   * <p>This will return a {@link ResultSet} wrapping the JDBC driver's <code>ResultSet</code>.
   * Closing this <code>ResultSet</code> may close the producing {@link Statement} or {@link
   * PreparedStatement}, depending on your setting for {@link ResultQuery#keepStatement(boolean)}.
   *
   * <p>Modifying this <code>ResultSet</code> will affect this <code>Cursor</code>.
   *
   * @return The underlying <code>ResultSet</code>. May be <code>null</code>, for instance when the
   *     <code>Cursor</code> is closed.
   */
  @Nullable
  ResultSet resultSet();
}
