...
/**
 * Wrapper around a {@link StringBuilder} which can be used to serialize a JAXB-annotated Java
 * object graph to XML. The JAXB objects must however also implement the {@link XMLAppendable}
 * interface for this to work.
 *
 * <p>Use {@link #formatting()} to create an instance producing formatted XML output and {@link
 * #nonFormatting()} to produce XML without any formatting whitespace (i.e. everything on one line).
 *
...
public final class XMLBuilder {
  ...
  public XMLBuilder append(XMLAppendable appendable) {
    if (appendable != null) appendable.appendTo(this);
    return this;
  }
  ...
  public XMLBuilder append(String wrappingElementName, String elementName, List<?> list) {
    if (list != null) {
      ...
      for (Object o : list) {
        if (o instanceof XMLAppendable) append(elementName, (XMLAppendable) o);
        else append(elementName, o);
      }
  ...
  private XMLBuilder openTag(String elementName) {
    if (format && onNewLine) for (int i = 0; i < indentLevel; i++) builder.append("    ");
