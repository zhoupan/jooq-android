 ...
 *
 * <p>The step in constructing the {@link Loader} object where you can specify the load type and
 * data source.
 *
 * @author Lukas Eder
...
public interface LoaderSourceStep<R extends Record> {
   ...
   * Load in-memory data.
   *
   * <p>Feed a set of array representations of records to the loader API. Each array's elements are
   * matched with the subsequent {@link LoaderRowsStep#fields(Field...)} specification, by index.
   * The values in each array are converted to the matching field's {@link DataType} via {@link
   * DataType#convert(Object)}. The matching is similar to that of {@link Record#fromArray(Object[],
   * Field...)}.
   */
  @NotNull
  @CheckReturnValue
   ...
   * Load in-memory data.
   *
   * <p>Like {@link #loadArrays(Object[][])}, providing the possibility of lazy materialisation of
   * the input arrays.
   *
