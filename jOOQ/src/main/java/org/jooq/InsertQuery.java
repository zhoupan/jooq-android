 ...
 *
 * <p>This type is the model API representation of a {@link Insert} statement, which can be mutated
 * after creation. The advantage of this API compared to the DSL API is a more simple approach to
 * writing dynamic SQL.
 *
 * <p>Instances can be created using {@link DSLContext#insertQuery(Table)} and overloads.
 *
...
public interface InsertQuery<R extends Record> extends StoreQuery<R>, Insert<R>, ConditionProvider {
   ...
   *
   * <p>Calling this method will cause subsequent calls to {@link #addValue(Field, Object)} (and
   * similar) to fill the next record.
   *
   * <p>If this call is not followed by {@link #addValue(Field, Object)} calls, then this call has
   * no effect.
   *
   * <p>If this call is done on a fresh insert statement (without any values yet), then this call
  ...
  /**
   * Whether a <code>ON CONFLICT</code> clause should be added to this <code>INSERT</code>
   * statement.
   *
   * <p>When setting this flag to <code>true</code>, be sure to also add values "for update" using
   * the {@link #addValueForUpdate(Field, Field)} methods.
   *
  ...
  /**
   * Whether use a <code>ON CONFLICT</code> or <code>ON CONFLICT ON CONSTRAINT</code> clause in this
  ...
  /**
   * Whether a <code>ON DUPLICATE KEY UPDATE</code> clause should be added to this <code>INSERT
   * </code> statement.
   *
   * <p>When setting this flag to <code>true</code>, be sure to also add values "for update" using
   * the {@link #addValueForUpdate(Field, Field)} methods.
   *
   * <p>The <code>ON DUPLICATE KEY UPDATE</code> flag is mutually exclusive with the <code>
   * ON DUPLICATE KEY IGNORE</code> flag (see {@link #onDuplicateKeyIgnore(boolean)}. Setting one
  ...
  /**
   * Whether an <code>ON DUPLICATE KEY IGNORE</code> clause should be added to this <code>INSERT
   * </code> statement.
   *
   * <p>This clause is not actually supported in this form by any database, but can be emulated as
   * such:
   *
   * <table border="1">
   * <tr>
  ...
  /**
   * Add a value to the <code>ON DUPLICATE KEY UPDATE</code> clause of this <code>INSERT</code>
  ...
  /**
   * Add multiple values to the <code>ON DUPLICATE KEY UPDATE</code> clause of this <code>INSERT
   * </code> statement, where this is supported.
   *
   * <p>Please assure that key/value pairs have matching <code>&lt;T&gt;</code> types. Values can
   * either be of type <code>&lt;T&gt;</code> or <code>Field&lt;T&gt;</code>
   *
   * @see InsertOnDuplicateStep#onDuplicateKeyUpdate()
   */
  @Support({CUBRID, DERBY, H2, HSQLDB, MARIADB, MYSQL, POSTGRES, SQLITE})
   ...
   *
   * <p>This is for use with {@link SQLDialect#POSTGRES}'s {@link #onConflict(Field...)} clause.
   *
  ...
  /**
   * Adds a new condition to the query, connecting it to existing conditions with {@link
   * Operator#AND}.
   *
   * <p>This is for use with {@link SQLDialect#POSTGRES}'s {@link #onConflict(Field...)} clause.
   *
  ...
  /**
   * Adds a new condition to the query, connecting it to existing conditions with the provided
   * operator.
   *
   * <p>This is for use with {@link SQLDialect#POSTGRES}'s {@link #onConflict(Field...)} clause.
   *
   ...
   * {@inheritDoc}
   *
   * <p>This feature works with <code>INSERT</code> statements for all SQL dialects
   */
  @Override
  @Support
  void setReturning();
