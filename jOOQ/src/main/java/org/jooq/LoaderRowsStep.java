 ...
 *
 * <p>The step in constructing the {@link Loader} object where you can set the mandatory row loader
 * options.
 *
 * <p>
 *
...
public interface LoaderRowsStep<R extends Record> {
   ...
   *
   * <p>The record column at index <code>i</code> is inserted into the table field at index <code>i
   * </code>. If <code>fields[i] == null</code> or <code>fields.length &lt;= i</code>, then the
   * record column is skipped.
   */
  @NotNull
  @CheckReturnValue
  @Support
   ...
   *
   * <p>The record column at index <code>i</code> is inserted into the table field at index <code>i
   * </code>. If <code>new ArrayList(fields).get(i) == null</code> or <code>
   * new ArrayList(fields).size() &lt;= i</code>, then the record column is skipped.
   */
  @NotNull
  @CheckReturnValue
  @Support
   ...
   *
   * <p>The input field obtained from {@link LoaderFieldContext#field()} corresponds to the source
   * record field, if any, or an unspecified field enumeration is used. The {@link
   * LoaderFieldContext#index()} property corresponds to the source column index.
   */
  @NotNull
  @CheckReturnValue
  @Support
  ...
  /**
   * Indicate that all input fields which have a corresponding field in the target table (with the
   * same name) should be loaded.
   *
