...
public interface MergeUsingStep<R extends Record> extends MergeKeyStepN<R> {
  ...
  /**
   * Add the <code>USING</code> clause to the SQL standard <code>MERGE</code> statement.
   *
   * <p>Note that {@link SQLDialect#DERBY} supports this clause only with ordinary table references,
   * not with derived tables.
   */
  @NotNull
  @CheckReturnValue
  ...
  /**
   * Add a dummy <code>USING</code> clause to the SQL standard <code>MERGE</code> statement
   *
   * <p>This results in <code>USING(SELECT 1 FROM DUAL)</code> for most RDBMS, or in <code>
   * USING(SELECT 1) AS [dummy_table(dummy_field)]</code> in SQL Server, where derived tables need
   * to be aliased.
   */
  @NotNull
  @CheckReturnValue
