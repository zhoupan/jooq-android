...
/**
 * A wrapping object for {@link DatabaseMetaData} or for other sources of database meta information
 * (e.g. {@link InformationSchema})
 *
 * <p>This object can be obtained through {@link DSLContext#meta()} in order to provide convenient
 * access to your database meta data. This abstraction has two purposes:
 *
 * <p>
 *
 * <ol>
 *   <li>To increase API convenience, as no checked {@link SQLException} is thrown, only the
 *       unchecked {@link DataAccessException}
 *   <li>To increase API convenience, as the returned objects are always jOOQ objects, not JDBC
 *       {@link ResultSet} objects with hard-to-remember API constraints
 * </ol>
 *
 * @author Lukas Eder
 */
public interface Meta extends Scope {
   ...
   *
   * <p>For those databases that don't really support JDBC meta data catalogs, a single empty
   * catalog (named <code>""</code>) will be returned. In other words, there is always at least one
  ...
  /**
   * Get a catalog object by name from the underlying meta data source, or <code>null</code> if no
   * such object exists.
   *
   * @throws DataAccessException If something went wrong fetching the meta objects
   */
  ...
  /**
   * Eager-create an in-memory copy of this {@link Meta} instance without any connection to the
   ...
   *
   * <p>See {@link #migrateTo(Meta, MigrationConfiguration)} for more details.
   *
   * @throws DataAccessException If something went wrong fetching the meta objects
   ...
   * Generate a migration script to get from this meta data to another one.
   *
   * <p>To some extent, some database migrations can be generated automatically by comparing two
   * versions of a schema. This is what <code>migrateTo()</code> does. It supports:
   *
   * <p>
   *
   * <ul>
   *   <li>Schema additions / removals
   *   <li>Table additions / removals
   *   <li>Column additions / removals
   *   <li>Column data type changes
   *   <li>Constraint additions / removals
   *   <li>Index additions / removals
   *   <li>Sequence additions / removals
   *   <li>Comment additions / removals
   * </ul>
   *
   * <p>More complex, structural changes, such as moving some columns from one table to another, or
   * turning a to-one relationship into a to-many relationship, as well as data migrations, can
   * currently not be detected automatically.
   *
   * @throws DataAccessException If something went wrong fetching the meta objects
   */
  @NotNull
   ...
   *
   * <p>This allows for serialising schema meta information as XML using JAXB. See also {@link
   * Constants#XSD_META} for details.
   */
  @NotNull
