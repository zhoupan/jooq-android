...
public final class Source {
  ...
  /** Create a source from an input stream using a specific character set. */
  public static final Source of(
      InputStream inputStream, int length, CharsetDecoder charsetDecoder) {
   ...
   *
   * @throws IOException When something goes wrong creating a reader from this source.
   */
  public final Reader reader() throws IOException {
    try {
      if (string != null) return new StringReader(string);
      else if (bytes != null)
        if (length > -1) return inputStreamReader(new ByteArrayInputStream(bytes, 0, length));
        else return inputStreamReader(new ByteArrayInputStream(bytes));
      else if (reader != null)
        if (length > -1) return new LengthLimitedReader(reader, length);
        else return reader;
      else if (inputStream != null)
        if (length > -1)
          return inputStreamReader(new LengthLimitedInputStream(inputStream, length));
        else return inputStreamReader(inputStream);
      else if (file != null)
        return new BufferedReader(inputStreamReader(new FileInputStream(file)));
      else throw new IllegalStateException("Could not produce a reader from this source");
  ...
  public final String readString() throws IOException {
    ...
    try {
      ...
      int read;
      while ((read = r.read(buffer, 0, 8192)) >= 0) w.write(buffer, 0, read);
    } catch (java.io.IOException e) {
  ...
  private final Reader inputStreamReader(InputStream is) throws UnsupportedEncodingException {
    if (charsetName != null) return new InputStreamReader(is, charsetName);
    else if (charset != null) return new InputStreamReader(is, charset);
    else if (charsetDecoder != null) return new InputStreamReader(is, charsetDecoder);
  ...
  public String toString() {
    ...
    else if (bytes != null) return readString();
    else if (reader != null) return "Source (Reader)";
    else if (inputStream != null) return "Source (InputStream)";
    else if (file != null) return "Source (" + file + ")";
    else return "Source (other)";
  }
}
