 ...
 *
 * <p>Comparison operators listed here can be used to compare {@link Field} or {@link Row} values
 * with other {@link Field} or {@link Row} values, or with {@link Select} or {@link
 * QuantifiedSelect} values.
 *
 * <p>The following flags indicate whether the comparator can be used as:
 *
 * <ul>
 *   <li>a quantified comparison operator: {@link #supportsQuantifier()}. Example: <code>
 *       X = ANY (A, B, C)</code>
 *   <li>a subselect comparison operator: {@link #supportsSubselect()}. Example: <code>
 *       X = (SELECT A)</code>
 * </ul>
 *
 * @author Lukas Eder
...
public enum Comparator {
  ...
  private Comparator(
      String sql, boolean supportsQuantifier, boolean supportsSubselect, boolean supportsNulls) {
    this.sql = sql;
  ...
  /**
   * Get the inverse comparator such that <code>A [op] B</code> and <code>NOT(A [inverse op] B)
   * </code>.
   */
  ...
  /**
   * Get the mirrored comparator such that <code>A [op] B</code> and <code>B [mirrored op] A</code>,
   * or <code>null</code> if the comparator cannot be mirrored.
   */
