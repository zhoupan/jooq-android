...
public interface AlterDomainStep<T> {
  ...
  /**
   * Add the <code>DROP CONSTRAINT IF EXISTS</code> clause to the <code>ALTER DOMAIN</code>
   * statement.
   */
  ...
  /**
   * Add the <code>RENAME CONSTRAINT IF EXISTS</code> clause to the <code>ALTER DOMAIN</code>
   * statement.
   */
  @Support({POSTGRES})
  @NotNull
