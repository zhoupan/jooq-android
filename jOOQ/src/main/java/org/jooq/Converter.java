 ...
 *
 * <p>A general data type conversion interface that can be provided to jOOQ at various places in
 * order to perform custom data type conversion. Conversion is directed, this means that the <code>
 * Converter</code> is used
 *
 * <ul>
 *   <li>to load database types converting them to user types "FROM" the database. Hence, {@link
 *       #fromType()} is the type as defined in the database. Think of "FROM" = "reading".
 *   <li>to store user types converting them to database types "TO" the database. Think of "TO" =
 *       "writing". Hence, {@link #toType()} is the user-defined type
 * </ul>
 *
 * <p>Note: In order to avoid unwanted side-effects, it is highly recommended (yet not required) for
 * {@link #from(Object)} and {@link #to(Object)} to be <strong>reciprocal</strong>. The two methods
 * are reciprocal, if for all <code>X and Y</code>, it can be said that
 *
 * <ul>
 *   <li>if <code>Y.equals(converter.from(X))</code>, then <code>X.equals(converter.to(Y))</code>.
 *   <li><code>X.equals(converter.from(converter.to(X)))</code>
 *   <li><code>X.equals(converter.to(converter.from(X)))</code>
 * </ul>
 *
 * <p>Furthermore, it is recommended (yet not required) that
 *
 * <ul>
 *   <li><code>converter.from(null) == null</code>
 *   <li><code>converter.to(null) == null</code>
 * </ul>
 *
 * <p>
 *
...
public interface Converter<T, U> extends Serializable {
   ...
   * @param from A function converting from T to U when reading from the database.
  ...
  static <T, U> Converter<T, U> from(
      Class<T> fromType, Class<U> toType, Function<? super T, ? extends U> from) {
  ...
  static <T, U> Converter<T, U> to(
      Class<T> fromType, Class<U> toType, Function<? super U, ? extends T> to) {
   ...
   * Construct a new converter from functions.
   *
   * <p>This works like {@link Converter#of(Class, Class, Function, Function)}, except that both
   * conversion {@link Function}s are decorated with a function that always returns <code>null
   * </code> for <code>null</code> inputs.
   *
   * <p>Example:
   *
   * <p><code><pre>
   ...
   * Construct a new read-only converter from a function.
   *
   * <p>This works like {@link Converter#from(Class, Class, Function)}, except that the conversion
   * {@link Function} is decorated with a function that always returns <code>null</code> for <code>
   * null</code> inputs.
   *
   * <p>Example:
   *
   ...
   * Construct a new write-only converter from a function.
   *
   * <p>This works like {@link Converter#to(Class, Class, Function)}, except that the conversion
   * {@link Function} is decorated with a function that always returns <code>null</code> for <code>
   * null</code> inputs.
   *
   * <p>Example:
