 ...
 * variables.
 *
 * <p>This SPI is used by jOOQ users to implement support for custom data types that would otherwise
 * not be supported by jOOQ and/or JDBC. All of jOOQ's internal support for bind variable types is
 * implemented in {@link DefaultBinding}.
 *
 * <p>
 *
 ...
 * types programmatically by calling {@link DataType#asConvertedDataType(Converter)} or {@link
 * DataType#asConvertedDataType(Binding)}, for example. Custom data types can also be defined on
 * generated code using the <code>&lt;forcedType/&gt;</code> configuration, see <a href=
...
public interface Binding<T, U> extends Serializable {
   ...
   *
   * <p>Implementations should generate SQL code onto {@link BindingSQLContext#render()}, given the
   * context's bind variable located at {@link BindingSQLContext#value()}. Examples of such SQL code
   * are:
   *
   * <ul>
   *   <li><code>"?"</code>: Default implementations can simply generate a question mark.<br>
   *       <br>
   *   <li><code>"123"</code>: Implementations may choose to inline bind variables to influence
   *       execution plan generation.<br>
   *       <br>
   *       {@link RenderContext#paramType()} contains information whether inlined bind variables are
   *       expected in the current context.<br>
   *       <br>
   *   <li><code>"CAST(? AS DATE)"</code>: Cast a database to a more specific type. This can be
   *       useful in databases like Oracle, which map both <code>DATE</code> and <code>TIMESTAMP
   *       </code> SQL types to {@link Timestamp}.<br>
   *       <br>
   *       {@link RenderContext#castMode()} may contain some hints about whether casting is
   *       suggested in the current context.<br>
   *       <br>
   *   <li><code>"?::json"</code>: Vendor-specific bind variables can be supported, e.g. {@link
   *       SQLDialect#POSTGRES}'s JSON data type.
   * </ul>
   *
   * <p>Implementations must provide consistent behaviour between {@link #sql(BindingSQLContext)}
   * and {@link #set(BindingSetStatementContext)}, i.e. when bind variables are inlined, then they
   * must not be bound to the {@link PreparedStatement} in {@link #set(BindingSetStatementContext)}
   *
   * @param ctx The context object containing all argument objects.
   * @throws SQLException Implementations are allowed to pass on all {@link SQLException}s to the
   *     caller to be wrapped in {@link DataAccessException}s.
   */
   ...
   *
   * <p>Implementations are expected to produce a value by calling {@link
   ...
   *
   * <p>Implementations are expected to produce a value by calling {@link
   * BindingGetStatementContext#value(Object)}, passing the resulting value to the method.
   *
   * @param ctx The context object containing all argument objects.
   * @throws SQLException Implementations are allowed to pass on all {@link SQLException}s to the
   *     caller to be wrapped in {@link DataAccessException}s.
   ...
   *
   * <p>Implementations are expected to produce a value by calling {@link
   * BindingGetSQLInputContext#value(Object)}, passing the resulting value to the method.
   *
   * @param ctx The context object containing all argument objects.
   * @throws SQLException Implementations are allowed to pass on all {@link SQLException}s to the
   *     caller to be wrapped in {@link DataAccessException}s.
