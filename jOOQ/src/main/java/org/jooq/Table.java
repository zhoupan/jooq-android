 ...
 * A table.
 *
 * <p>Like {@link Field}, a {@link Table} is a basic building block of any {@link Query}, as they
 * all operate on at least one table. There are many different types of tables, including:
 *
 * <p>
 *
 * <ul>
 *   <li>Generated table references
 *   <li>Plain SQL tables created with {@link DSL#table(String)}
 *   <li>Table references created with {@link DSL#table(Name)}
 *   <li>Derived tables created with {@link DSL#table(Select)}
 *   <li>Join expressions created e.g. with {@link Table#join(TableLike)}
 *   <li>Common table expressions ({@link CommonTableExpression})
 *   <li>Unnested arrays referenced through {@link DSL#unnest(Field)} and overloads
 *   <li>Table valued functions as provided by the code generator
 *   <li>Etc.
 * </ul>
 *
 ...
 * </pre></code>
 *
 * <p>Instances can be created using {@link DSL#table(Name)} and overloads.
 *
...
public interface Table<R extends Record> extends TableLike<R>, RecordQualifier<R> {
   ...
   *
   * <p>With SQL:2003, the concept of <code>IDENTITY</code> columns was introduced in most RDBMS.
   * These are special kinds of columns that have auto-increment functionality when <code>INSERT
   * </code> statements are performed.
   *
   * <p>An <code>IDENTITY</code> column is usually part of the <code>PRIMARY KEY</code> or of a
   * <code>UNIQUE KEY</code> in the table, although in some RDBMS, this is not required. There can
   * only be at most one <code>IDENTITY</code> column.
   *
   * <p>Note: Unfortunately, this is not supported in the Oracle dialect, where identities emulated
   * by triggers cannot be formally detected.
   *
   * @return The table's <code>IDENTITY</code> information, or <code>null</code>, if no such
   ...
   *
   * @return The primary key. This is never <code>null</code> for an updatable table.
   */
  @Nullable
   ...
   *
   * <p>jOOQ supports optimistic locking in {@link UpdatableRecord#store()} and {@link
   * UpdatableRecord#delete()} if {@link Settings#isExecuteWithOptimisticLocking()} is enabled.
   * Optimistic locking is performed in a single <code>UPDATE</code> or <code>DELETE</code>
   * statement if tables provide a "version" or "timestamp" field, or in two steps using an
   * additional <code>SELECT .. FOR UPDATE</code> statement otherwise.
   *
   * <p>This method is overridden in generated subclasses if their corresponding tables have been
   * configured accordingly. A table may have both a "version" and a "timestamp" field.
   *
   * @return The "version" field, or <code>null</code>, if this table has no "version" field.
   ...
   * configured accordingly. A table may have both a "version" and a "timestamp" field.
   *
   * @return The "timestamp" field, or <code>null</code>, if this table has no "timestamp" field.
   ...
   *
   * @return All keys. This is never <code>null</code>. This is never empty for a {@link Table} with
   *     a {@link Table#getPrimaryKey()}.
   */
  @NotNull
  ...
  /**
   * Get a list of <code>FOREIGN KEY</code>'s of a specific table, referencing a this table.
   *
   * <p>This will recurse into joined tables.
   *
   * @param <O> The other table's record type
   * @param other The other table of the foreign key relationship
   * @return Some other table's <code>FOREIGN KEY</code>'s towards an this table. This is never
   *     <code>null</code>.
   */
  @NotNull
   ...
   *
   * @return This table's <code>FOREIGN KEY</code>'s. This is never <code>null</code>.
   */
  @NotNull
  List<ForeignKey<R, ?>> getReferences();

  /**
   * Get a list of <code>FOREIGN KEY</code>'s of this table, referencing a specific table.
   *
   * <p>This will recurse into joined tables.
   *
   * @param <O> The other table's record type
   * @param other The other table of the foreign key relationship
   * @return This table's <code>FOREIGN KEY</code>'s towards an other table. This is never <code>
   *     null</code>.
   */
  @NotNull
   ...
   * <code>SELECT</code>.
   *
   * <p>When using an asterisk, jOOQ will let the database server define the order of columns, as
   * well as which columns are included in the result set. If using jOOQ with generated code, this
   * may conflict with the column set and its ordering as defined at code generation time, meaning
   * columns may be in a different order, there may be fewer or more columns than expected. It is
   * usually better to list columns explicitly.
   *
   * @see DSL#asterisk()
   */
  @NotNull
   ...
   *
   * <p>A rowid value describes the physical location of a row on the disk, which can be used as a
   * replacement for a primary key in some situations - especially within a query, e.g. to self-join
   * a table:
   *
   * <p><code><pre>
   ...
   * </pre></code>
   *
   * <p>It is <em>not</em> recommended to use <code>rowid</code> values in client applications as
   * actual row identifiers as the database system may move a row to a different physical location
   * at any time, thus changing the rowid value. In general, use primary keys, instead.
   */
  @NotNull
   ...
   *
   * <p>Note that the case-sensitivity of the returned table depends on {@link
   * Settings#getRenderQuotedNames()}. By default, table aliases are quoted, and thus case-sensitive
   ...
   *
   * <p>Note that the case-sensitivity of the returned table and columns depends on {@link
   * Settings#getRenderQuotedNames()}. By default, table aliases are quoted, and thus case-sensitive
   * in many SQL dialects!
   *
   * <p>
   ...
   *
   * <p>Note, not all databases support derived column lists for their table aliases. On the other
   * hand, some databases do support derived column lists, but only for derived tables. jOOQ will
   * try to turn table references into derived tables to make this syntax work. In other words, the
   * following statements are equivalent: <code><pre>
   ...
   *
   * <p>Other databases may not support derived column lists at all, but they do support common
   * table expressions. The following statements are equivalent: <code><pre>
   * -- Using derived column lists to rename columns (e.g. Postgres)
   * SELECT t.a, t.b
   * FROM (
   *   SELECT 1, 2
   ...
   *
   * @param alias The alias name
   * @param fieldAliases The field aliases. Excess aliases are ignored, missing aliases will be
   ...
   * Create an alias for this table and its fields.
   *
   * <p>This works like {@link #as(String, String...)}, except that field aliases are provided by a
   ...
   *
   * <p>Note that the case-sensitivity of the returned table depends on {@link
   * Settings#getRenderQuotedNames()} and the {@link Name}. By default, table aliases are quoted,
   * and thus case-sensitive in many SQL dialects - use {@link DSL#unquotedName(String...)} for
   * case-insensitive aliases.
   *
   * <p>If the argument {@link Name#getName()} is qualified, then the {@link Name#last()} part will
   * be used.
   *
   * @param alias The alias name
   * @return The table alias
   ...
   * Create an alias for this table and its fields.
   *
   * <p>This works like {@link #as(Name, Name...)}, except that field aliases are provided by a
   * function. This is useful, for instance, to prefix all columns with a common prefix:
   *
   * <p><code><pre>
   * MY_TABLE.as("t1", f -&gt;"prefix_" + f.getName());
  ...
  @Support
  Table<R> as(
      Name alias, BiFunction<? super Field<?>, ? super Integer, ? extends Name> aliasFunction);
   ...
   * Create an alias for this table and its fields.
   *
   * <p>This works like {@link #as(Table, Field...)}, except that field aliases are provided by a
   * function. This is useful, for instance, to prefix all columns with a common prefix:
   *
   * <p><code><pre>
   ...
   * Operator#AND}.
   *
   * <p>The resulting table acts like a derived table that projects all of this table's columns and
   * filters by the argument {@link Condition}. If syntactically reasonable, the derived table may
   * be inlined to the query that selects from the resulting table.
   */
  @NotNull
  @Support
   ...
   *
   * <p>Depending on the <code>JoinType</code>, a subsequent {@link TableOnStep#on(Condition)} or
   * {@link TableOnStep#using(Field...)} clause is required. If it is required but omitted, a {@link
   * DSL#trueCondition()}, i.e. <code>1 = 1</code> condition will be rendered
   */
  @NotNull
  @Support
   ...
   *
   * <p>A synonym for {@link #innerJoin(TableLike)}.
   *
   ...
   * <code>INNER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #innerJoin(String)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>INNER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #innerJoin(String, Object...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>INNER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #innerJoin(String, QueryPart...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>INNER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #innerJoin(Name)}.
   *
   * @see DSL#table(Name)
   * @see #innerJoin(Name)
   */
   ...
   *
   * <p>A synonym for {@link #leftOuterJoin(TableLike)}.
   *
   ...
   * <code>LEFT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #leftOuterJoin(String)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>LEFT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #leftOuterJoin(String, Object...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>LEFT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #leftOuterJoin(String, QueryPart...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>LEFT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #leftOuterJoin(Name)}.
   *
   * @see DSL#table(Name)
   ...
   *
   * <p>A synonym for {@link #rightOuterJoin(TableLike)}.
   *
   * <p>This is only possible where the underlying RDBMS supports it.
   *
   ...
   * <code>RIGHT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #rightOuterJoin(String)}.
   *
   * <p>This is only possible where the underlying RDBMS supports it.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   ...
   * <code>RIGHT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #rightOuterJoin(String, Object...)}.
   *
   * <p>This is only possible where the underlying RDBMS supports it.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   ...
   * <code>RIGHT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #rightOuterJoin(String, QueryPart...)}.
   *
   * <p>This is only possible where the underlying RDBMS supports it
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   ...
   * <code>RIGHT OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #rightOuterJoin(Name)}.
   *
   * <p>This is only possible where the underlying RDBMS supports it
   *
   * @see DSL#table(Name)
   ...
   *
   * <p>A synonym for {@link #fullOuterJoin(TableLike)}.
   */
  @NotNull
   ...
   * <code>FULL OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #fullOuterJoin(SQL)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>FULL OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #fullOuterJoin(String)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>FULL OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #fullOuterJoin(String, Object...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>FULL OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #fullOuterJoin(String, QueryPart...)}.
   *
   * <p><b>NOTE</b>: When inserting plain SQL into jOOQ objects, you must guarantee syntax
   * integrity. You may also create the possibility of malicious SQL injection. Be sure to properly
   * use bind variables and/or escape literals when concatenated into SQL clauses!
   ...
   * <code>FULL OUTER JOIN</code> a table to this table.
   *
   * <p>A synonym for {@link #fullOuterJoin(Name)}.
   */
  @NotNull
  @Support({FIREBIRD, HSQLDB, POSTGRES})
   ...
   *
   * <p>If this syntax is unavailable, it is emulated with a regular <code>INNER JOIN</code>. The
   * following two constructs are equivalent: <code><pre>
   * A cross join B
   * A join B on 1 = 1
   * </pre></code>
   */
   ...
   *
   * <p>If this is not supported by your RDBMS, then jOOQ will try to emulate this behaviour using
   * the information provided in this query.
   */
  @NotNull
  @Support
   ...
   *
   * <p>This is a convenience method for self-joins, comparing complete records between tables.
   *
  ...
  /**
   * Create a predicate comparing records from self-non-equi-joined tables. This is a convenience
   * method for self-joins, comparing complete records between tables.
   *
   * <p>For example: <code><pre>
   * MyTable a = MY_TABLE.as("a");
   * MyTable b = MY_TABLE.as("b");
  ...
  /**
   * Create a new <code>TABLE</code> reference from this table, applying relational division.
   *
   * <p>Relational division is the inverse of a cross join operation. The following is an
   * approximate definition of a relational division: <code><pre>
   ...
   * </pre></code>
   *
   * <p>With jOOQ, you can simplify using relational divisions by using the following syntax: <code>
   * <pre>
   * C.divideBy(B).on(C.ID.equal(B.C_ID)).returning(C.TEXT)
   * </pre></code>
   *
   * <p>The above roughly translates to <code><pre>
   ...
   * </pre></code>
   *
   * <p>Or in plain text: Find those TEXT values in C whose ID's correspond to all ID's in B. Note
   * that from the above SQL statement, it is immediately clear that proper indexing is of the
   * essence. Be sure to have indexes on all columns referenced from the <code>on(...)</code> and
   * <code>returning(...)</code> clauses.
   *
   * <p>For more information about relational division and some nice, real-life examples, see
   *
   * <ul>
   *   <li><a href="http://en.wikipedia.org/wiki/Relational_algebra#Division">http
   *       ://en.wikipedia.org/wiki/Relational_algebra#Division</a>
   *   <li><a href=
   ...
   * </ul>
   *
   * <p>This has been observed to work with all dialects
   */
  @NotNull
   ...
   * </code> predicate.
   *
   * <p>The following two SQL snippets are semantically equivalent: <code><pre>
   ...
   * </pre></code>
   *
   * <p>Notice that according to <a
   * href="https://en.wikipedia.org/wiki/Relational_algebra">Relational algebra's</a> understanding
   * of left semi join, the right hand side of the left semi join operator is not projected, i.e. it
   * cannot be accessed from <code>WHERE</code> or <code>SELECT</code> or any other clause than
   * <code>ON</code>.
   */
  @NotNull
  @Support
  ...
  /**
   * A synthetic <code>LEFT ANTI JOIN</code> clause that translates to an equivalent <code>
   * NOT EXISTS</code> predicate.
   *
   * <p>The following two SQL snippets are semantically equivalent: <code><pre>
   ...
   * <p>Notice that according to <a
   * href="https://en.wikipedia.org/wiki/Relational_algebra">Relational algebra's</a> understanding
   * of left anti join, the right hand side of the left anti join operator is not projected, i.e. it
   * cannot be accessed from <code>WHERE</code> or <code>SELECT</code> or any other clause than
   * <code>ON</code>.
   */
  @NotNull
