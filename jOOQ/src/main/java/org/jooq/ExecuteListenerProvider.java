 ...
 *
 * <p>In order to facilitate the lifecycle management of <code>ExecuteListener</code> instances that
 * are provided to a jOOQ {@link Configuration}, clients can implement this API. To jOOQ, it is thus
 * irrelevant, if execute listeners are stateful or stateless, local to an execution, or global to
 * an application.
 *
 * @author Lukas Eder
 * @see ExecuteListener
...
public interface ExecuteListenerProvider {
   ...
   *
   * <p>An <code>ExecuteListener</code> shall be provided exactly once per query execution
   * lifecycle, i.e. per <code>ExecuteContext</code>.
   *
