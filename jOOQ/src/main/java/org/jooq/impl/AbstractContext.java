...
abstract class AbstractContext<C extends Context<C>> extends AbstractScope implements Context<C> {
  ...
  AbstractContext(Configuration configuration, PreparedStatement stmt) {
    ...
    if (visitListeners != null) {
      ...
      this.visitListenersEnd =
          configuration.settings().getVisitListenerEndInvocationOrder() != REVERSE
              ? visitListeners
              : Tools.reverse(visitListeners.clone());
    } else {
    ...
    this.castMode =
        m == ParamCastMode.ALWAYS
            ? CastMode.ALWAYS
            : m == ParamCastMode.NEVER ? CastMode.NEVER : CastMode.DEFAULT;
   ...
   *
   * <p>This method returns a clause to emit as a surrounding event before / after visiting a query
   * part. This is needed for all reusable query parts, whose clause type is ambiguous at the
   * container site. An example:
   *
   * <p><code><pre>SELECT * FROM [A CROSS JOIN B]</pre></code>
   *
   * <p>The type of the above <code>JoinTable</code> modelling <code>A CROSS JOIN B</code> is not
   * known to the surrounding <code>SELECT</code> statement, which only knows {@link Table} types.
   * The {@link Clause#TABLE_JOIN} event that is required to be emitted around the {@link
   * Context#visit(QueryPart)} event has to be issued here in <code>AbstractContext</code>.
   */
  ...
  /** A {@link VisitContext} is always in the scope of the current {@link RenderContext}. */
