...
abstract class AbstractField<T> extends AbstractTypedNamed<T> implements Field<T>, ScopeMappable {
  ...
  public final SortField<Integer> sortAsc(Collection<T> sortList) {
    return Tools.isEmpty(sortList)
        ? sortConstant()
        : DSL.field(this, fieldsArray(sortList.toArray(), getDataType())).asc();
  }
  ...
  public final SortField<Integer> sortDesc(Collection<T> sortList) {
    return Tools.isEmpty(sortList)
        ? sortConstant()
        : DSL.field(this, fieldsArray(sortList.toArray(), getDataType())).desc();
  }
  ...
  public final Field<T> mul(Field<? extends Number> value) {
    return new Expression<>(
        ...
        false,
        this,
        getDataType().isTemporal() ? nullSafe(value) : nullSafe(value, getDataType()));
  }
  ...
  public final Condition isTrue() {
    ...
    else return castIfNeeded(this, String.class).in(BooleanValues.TRUE_VALUES);
  }
  ...
  public final Condition isFalse() {
    ...
    else return castIfNeeded(this, String.class).in(BooleanValues.FALSE_VALUES);
  }
  ...
  public final Condition contains(T value) {
    if (value instanceof Field) return contains((Field) value);
    else return new Contains<>(this, value, true, true);
  }
  ...
  public final Condition betweenSymmetric(Field<T> minValue, Field<T> maxValue) {
    return betweenSymmetric(nullSafe(minValue, getDataType()))
        .and(nullSafe(maxValue, getDataType()));
  }
  ...
  public final Condition notBetweenSymmetric(Field<T> minValue, Field<T> maxValue) {
    return notBetweenSymmetric(nullSafe(minValue, getDataType()))
        .and(nullSafe(maxValue, getDataType()));
  }
  ...
  public final Condition compare(
      Comparator comparator, QuantifiedSelect<? extends Record1<T>> query) {
  ...
  private final <Z extends Number> Field<Z> numeric() {
    ...
    else return (Field<Z>) cast(BigDecimal.class);
  }

  @SuppressWarnings("unchecked")
  private final Field<String> varchar() {
    if (getDataType().isString()) return (Field<String>) this;
    else return cast(String.class);
  }

  @SuppressWarnings("unchecked")
  private final <Z extends java.util.Date> Field<Z> date() {
    if (getDataType().isTemporal()) return (Field<Z>) this;
    else return (Field<Z>) cast(Timestamp.class);
  }
  ...
  public final Field<T> coalesce(T option, T... options) {
    return DSL.coalesce(
        this,
        Tools.combine(Tools.field(option, this), Tools.fields(options, this).toArray(EMPTY_FIELD)));
  }
