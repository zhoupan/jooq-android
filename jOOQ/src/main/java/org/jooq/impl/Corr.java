...
final class Corr extends DefaultAggregateFunction<BigDecimal> {

  Corr(Field<? extends Number> y, Field<? extends Number> x) {
    super(false, N_CORR, NUMERIC, nullSafeNotNull(y, INTEGER), nullSafeNotNull(x, INTEGER));
  }
  ...
  private final void acceptEmulation(Context<?> ctx) {
    ...
    ctx.visit(
        fo(covarPop(x, y)).div(fon(DSL.stddevPop(x(x, y))).times(fo(DSL.stddevPop(y(x, y))))));
  }
}
