 ...
 *
 * <p>This also acts as a static data type registry for jOOQ internally.
 *
...
public class DefaultDataType<T> extends AbstractDataTypeX<T> {
  ...
  private static final Pattern NORMALISE_PATTERN =
      Pattern.compile("\"|\\.|\\s|\\(\\w+(\\s*,\\s*\\w+)*\\)|(NOT\\s*NULL)?");

  /** A pattern to be used to replace all precision, scale, and length information. */
  ...
  /**
   * The Java class corresponding to this data type's <code>&lt;U&gt;</code> type, i.e. the user
   * type in case a {@link Binding} applies.
   */
  private final Class<T> uType;

  /**
   * The Java class corresponding to this data type's <code>&lt;T&gt;</code> type, i.e. the database
   * type in case a {@link Binding} applies.
   */
  ...
  /** The type name prefix (prior to length, precision, scale) used for casting to this type. */
  private final String castTypePrefix;

  /** The type name suffix (after length, precision, scale) used for casting to this type. */
  ...
  private static final Integer integerPrecision(Class<?> type, Integer precision) {
    if (precision == null)
      if (type == Long.class || type == ULong.class) precision = LONG_PRECISION;
      ...
      if (result == null) {
        ...
        try {
          ...
          if (QualifiedRecord.class.isAssignableFrom(type))
            return (DataType<T>)
                ((QualifiedRecord<?>) type.getDeclaredConstructor().newInstance())
                    .getQualifier()
                    .getDataType();
  ...
  public static final DataType<?> getDataType(SQLDialect dialect, String t, int p, int s)
  ...
  public static final DataType<?> getDataType(
      SQLDialect dialect, String t, int p, int s, boolean forceIntegerTypesOnZeroScaleDecimals)
      throws SQLDialectNotSupportedException {
  ...
  /** Convert a type name (using precision and scale) into a Java class */
  public static final Class<?> getType(SQLDialect dialect, String t, int p, int s)
      throws SQLDialectNotSupportedException {
