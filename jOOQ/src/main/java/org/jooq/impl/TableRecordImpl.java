...
public class TableRecordImpl<R extends TableRecord<R>> extends AbstractQualifiedRecord<R>
  ...
  /**
   * Set a generated version and timestamp value onto this record after successfully storing the
   * record.
   */
  ...
  private static final Timestamp truncate(Timestamp ts, DataType<?> type) {
    if (type.isDate()) return new Timestamp(ts.getYear(), ts.getMonth(), ts.getDate(), 0, 0, 0, 0);
    else if (!type.precisionDefined() || type.precision() >= 3) return ts;
    else
      return new Timestamp(
          (ts.getTime() / TRUNCATE[type.precision()]) * TRUNCATE[type.precision()]);
  }
