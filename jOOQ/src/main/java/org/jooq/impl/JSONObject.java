...
final class JSONObject<T> extends AbstractField<T>
      ...
      case MARIADB:
        {
          ...
          if (entries.size() > 1) {
            ctx.visit(
                JSONEntryImpl.jsonMerge(
                    ctx, "{}", Tools.map(entries, e -> jsonObject(e), Field[]::new)));
          } else if (!entries.isEmpty()
              && isJSONArray((first = entries.iterator().next()).value())) {
            ctx.visit(
                jsonObject(
  ...
  private static final boolean isJSONArray(Field<?> field) {
    return field instanceof JSONArray
        ...
        || field instanceof ScalarSubquery
            && isJSONArray(((ScalarSubquery<?>) field).query.getSelect().get(0));
  }
  ...
  private final void acceptStandard(Context<?> ctx) {
    ...
    ctx.visit(N_JSON_OBJECT)
        .sql('(')
        .visit(
            QueryPartListView.wrap(QueryPartCollectionView.wrap(entries), jsonNull, jsonReturning)
                .separator(""))
        .sql(')');
  }
  ...
  public boolean equals(Object that) {
    if (that instanceof JSONObject) {
      return StringUtils.equals(type, ((JSONObject) that).type)
