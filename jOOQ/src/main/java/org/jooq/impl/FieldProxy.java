...
/**
 * A {@link Field} that acts as another field, allowing for the proxied field to be replaced.
 *
 * @author Lukas Eder
 */
...
final class FieldProxy<T> extends AbstractField<T> implements TableField<Record, T> {
  ...
  /** The position in the parsed SQL string where this field proxy was encountered. */
  ...
  final void scopeOwner(Query query) {
    if (!resolved && scopeOwner == null) scopeOwner = query;
  }
