...
/**
 * {@link MetaProvider} implementation which can {@link MetaProvider#provide() provide} a {@link
 * Meta} implementation based on a set of DDL scripts as the input.
 *
...
final class TranslatingMetaProvider implements MetaProvider {
      ...
      return new Snapshot(
          new DefaultMetaProvider(
                  configuration
                      ...
                      .set(configuration.settings().getInterpreterDialect()))
              .provide());
    }
  }
  ...
  final class DDLDatabaseInitializer implements AutoCloseable {
    ...
    /**
     * Parses and executes the script represented by {@code reader} against the H2 database. If the
     * script references a schema which doesn't exist, it will be automatically created first.
     *
     * <p>Any parser errors will be thrown. It is however possible to delimit sections which cannot
     * be parsed using special comments.
     *
