...
/** @author Lukas Eder */
abstract class AbstractQuery<R extends Record> extends AbstractAttachableQueryPart
    implements Query {
  ...
  /**
   * Default implementation to indicate whether this query should close the {@link ResultSet} after
   * execution. Subclasses may override this method.
   */
  ...
  protected void prepare(ExecuteContext ctx) throws SQLException {
    if (ctx.statement() == null) ctx.statement(ctx.connection().prepareStatement(ctx.sql()));
  }
  ...
  /** Default implementation for executable check. Subclasses may override this method. */
  @Override
  ...
  private final Rendered getSQL0(ExecuteContext ctx) {
    ...
    if (ctx.type() == DDL) {
      ...
      try {
        ...
        result =
            new Rendered(
                render.visit(this).render(), render.bindValues(), render.skipUpdateCounts());
