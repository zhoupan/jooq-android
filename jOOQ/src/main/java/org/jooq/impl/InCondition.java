...
final class InCondition<T> extends AbstractCondition {
  ...
  public final void accept(Context<?> ctx) {
    if (field.getDataType().isEmbeddable())
      if (comparator == IN) ctx.visit(row(embeddedFields(field)).in(rows()));
      else ctx.visit(row(embeddedFields(field)).notIn(rows()));
  ...
  private final void accept0(Context<?> ctx) {
            ...
            for (int i = 0; i < values.size(); i += IN_LIMIT) {
              if (i > 0) {
                ...
                if (comparator == Comparator.IN) {
                  ctx.formatSeparator().visit(K_OR).sql(' ');
                } else {
    ...
    } else toSQLSubValues(ctx, padded(ctx, values));
  }
  ...
  static <T> List<T> padded(Context<?> ctx, List<T> list) {
    return ctx.paramType() == INDEXED && TRUE.equals(ctx.settings().isInListPadding())
        ? new PaddedList<>(
            list,
            REQUIRES_IN_LIMIT.contains(ctx.dialect()) ? IN_LIMIT : Integer.MAX_VALUE,
