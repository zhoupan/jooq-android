...
final class LoaderImpl<R extends Record>
  ...
  public final LoaderRowsStep<R> loadRecords(Iterator<? extends Record> records) {
    return loadArrays(
        new MappingIterator<Record, Object[]>(
            records,
            value -> {
  ...
  private final void checkFlags() {
    if (bulk != BULK_NONE && onDuplicate != ON_DUPLICATE_KEY_ERROR)
      throw new LoaderConfigurationException(
          "Cannot apply bulk loading with onDuplicateKey flags. Turn off either flag.");
  }
  ...
  private static final class CachedPSListener extends DefaultExecuteListener
    ...
    public void close() throws SQLException {
      for (CachedPS ps : map.values()) safeClose(ps.getDelegate());
    }
  }
              ...
              if (FALSE.equals(c.settings().isCachePreparedStatementInLoader())) {
                ...
                try (CachedPSListener cache = new CachedPSListener()) {
                  executeSQL(
                      ...
                      c.derive(
                              combine(
                                  new DefaultExecuteListenerProvider(cache),
                                  c.executeListenerProviders()))
                          .dsl());
                }
              }
            });
    ...
    execution:
    {
      ...
      while (iterator.hasNext() && ((row = iterator.next()) != null)) {
        try {
          ...
          for (int i = 0; i < row.length; i++)
            if (StringUtils.equals(nullString, row[i])) row[i] = null;
