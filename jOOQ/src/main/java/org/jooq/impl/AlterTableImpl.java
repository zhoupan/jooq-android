...
final class AlterTableImpl extends AbstractDDLQuery
  ...
  public final AlterTableImpl to(Name newName) {
    if (renameColumn != null) return to(field(newName));
    else if (renameConstraint != null) return to(constraint(newName));
  ...
  public final void accept(Context<?> ctx) {
    if ((ifExists && !supportsIfExists(ctx))
        || ((ifExistsColumn || ifExistsConstraint || ifNotExistsColumn)
            && !supportsIfExistsColumn(ctx)))
      tryCatch(
          ctx,
  ...
  private final void acceptFirstBeforeAfter(Context<?> ctx) {
    if (addFirst && ctx.family() != FIREBIRD) ctx.sql(' ').visit(K_FIRST);
