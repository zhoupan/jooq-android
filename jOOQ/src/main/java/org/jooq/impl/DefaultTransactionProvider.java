 ...
 *
 * <p>This implementation is entirely based on JDBC transactions and is intended to work with {@link
 * DefaultConnectionProvider} (which is implicitly created when using {@link
 * DSL#using(Connection)}).
 *
 * <p>
 *
 * <h3>Nesting of transactions</h3>
 *
 * By default, nested transactions are supported by modeling them implicitly with JDBC {@link
 * Savepoint}s, if supported by the underlying JDBC driver, and if {@link #nested()} is <code>true
 * </code>. To deactivate nested transactions, use {@link
...
public class DefaultTransactionProvider implements TransactionProvider {
  ...
  /**
   * This {@link Savepoint} serves as a marker for top level transactions if {@link #nested()}
   * transactions are deactivated.
   */
  ...
  /** @param nested Whether nested transactions via {@link Savepoint}s are supported. */
  ...
  private final Savepoint setSavepoint(Configuration configuration, boolean topLevel) {
    if (topLevel || !nested()) return IGNORED_SAVEPOINT;
    else return connection(configuration).setSavepoint();
  }
  ...
  /**
   * Ensure an <code>autoCommit</code> value on the connection, if it was set to <code>true</code>,
   * originally.
   */
