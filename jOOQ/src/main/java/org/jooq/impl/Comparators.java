...
final class Comparators {
  ...
  private static final class ForeignKeyComparator implements Comparator<ForeignKey<?, ?>> {
    ...
    public int compare(ForeignKey<?, ?> o1, ForeignKey<?, ?> o2) {
      ...
      if (c != 0) return c;
      else return KEY_COMP.compare(o1.getKey(), o2.getKey());
    }
  }
  ...
  private static final class IndexComparator implements Comparator<Index> {
    ...
    public int compare(Index o1, Index o2) {
      ...
      if (c != 0) return c;
      c =
          defaultIfNull(o1.getWhere(), noCondition())
              .toString()
              .compareTo(defaultIfNull(o2.getWhere(), noCondition()).toString());
      if (c != 0) return c;
      ...
      for (int i = 0; i < f1.size(); i++) {
        ...
        if (d1 == DEFAULT) d1 = ASC;
        SortOrder d2 = s2.getOrder();
        if (d2 == DEFAULT) d2 = ASC;
        c = d1.compareTo(d2);
        if (c != 0) return c;
      }
      return 0;
