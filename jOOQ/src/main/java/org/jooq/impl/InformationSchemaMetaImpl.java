...
final class InformationSchemaMetaImpl extends AbstractMeta {
  ...
  private final void init(InformationSchema meta) {
    ...
    for (org.jooq.util.xml.jaxb.Catalog xc : meta.getCatalogs()) {
      InformationSchemaCatalog ic =
          new InformationSchemaCatalog(xc.getCatalogName(), xc.getComment());
      catalogs.add(ic);
      ...
      if (tableType == TableType.VIEW) {
        ...
        for (org.jooq.util.xml.jaxb.View vt : meta.getViews()) {
          if (StringUtils.equals(
                  defaultIfNull(xt.getTableCatalog(), ""), defaultIfNull(vt.getTableCatalog(), ""))
              && StringUtils.equals(
            ...
            for (CheckConstraint cc : meta.getCheckConstraints()) {
              if (constraintName.equals(
                  name(
                      ...
                      cc.getConstraintSchema(),
                      cc.getConstraintName()))) {
                table.checks.add(
                    new CheckImpl<>(
                        table, constraintName, DSL.condition(cc.getCheckClause()), true));
                continue tableConstraintLoop;
              }
            }
  ...
  private static final class InformationSchemaTable extends TableImpl<Record> {
    ...
    InformationSchemaTable(
        String name, Schema schema, String comment, TableType tableType, String source) {
      super(
          DSL.name(name),
          schema,
          null,
          null,
          DSL.comment(comment),
          tableType == TableType.VIEW ? TableOptions.view(source) : TableOptions.of(tableType));
    }
