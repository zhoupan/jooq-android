...
final class GenerateSeries extends AbstractTable<Record1<Integer>>
      ...
      visitSubquery(
          ...
          withRecursive(name, name)
              .as(
                  select(from)
                      .unionAll(
                          select(iadd(f, step == null ? inline(1) : step))
                              .from(name)
                              .where(f.lt(to))))
              .select(f)
              .from(name));
  ...
  public final Table<Record1<Integer>> autoAlias(Context<?> ctx) {
    if (EMULATE_WITH_RECURSIVE.contains(ctx.dialect())) return as(name);
    else if (EMULATE_SYSTEM_RANGE.contains(ctx.dialect())) return as(name, name);
    else return null;
  }
}
