...
/**
 * A class wrapping a {@link Select} query's <code>FOR UPDATE</code> and other locking clauses.
 *
 * @author Lukas Eder
 */
final class ForLock extends AbstractQueryPart {
  ...
  public final void accept(Context<?> ctx) {
    ...
    if (Tools.isNotEmpty(forLockOf)) {
      ...
      ctx.qualify(
          !NO_SUPPORT_FOR_UPDATE_QUALIFIED.contains(ctx.dialect()) && ctx.qualify(),
    ...
    if (forLockWaitMode != null) {
      ...
      if (forLockWaitMode == ForLockWaitMode.WAIT
        ...
        prependSQL(
            ctx.skipUpdateCount(), ctx.dsl().setLocal(N_LOCK_TIMEOUT, inline(forLockWait * 1000)));
      } else if (forLockWaitMode == ForLockWaitMode.WAIT
