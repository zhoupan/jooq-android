...
/** @author Lukas Eder */
final class DeleteQueryImpl<R extends Record> extends AbstractDMLQuery<R>
  ...
  public final void addUsing(Collection<? extends TableLike<?>> f) {
    for (TableLike<?> provider : f) using.add(provider.asTable());
  }
  ...
  final void accept0(Context<?> ctx) {
    ...
    if (!using.isEmpty() || multiTableJoin || specialDeleteAsSyntax && Tools.alias(t) != null) {
      ...
      if (REQUIRE_REPEAT_FROM_IN_USING.contains(ctx.dialect())
        ...
        u = new TableList(t);
        u.addAll(using);
      } else if (NO_SUPPORT_REPEAT_FROM_IN_USING.contains(ctx.dialect())
          && containsDeclaredTable(using, t)) {
