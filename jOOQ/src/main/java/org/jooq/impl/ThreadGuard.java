...
final class ThreadGuard {
  ...
  static interface GuardedOperation<V> {
    ...
    /**
     * This callback is executed if {@link #unguarded()} has already been executed on the current
     * stack.
     */
    V guarded();
  }
  ...
  static final void run(
      ThreadGuard.Guard guard, Runnable unguardedOperation, Runnable guardedOperation) {
    run(
        guard,
        () -> {
