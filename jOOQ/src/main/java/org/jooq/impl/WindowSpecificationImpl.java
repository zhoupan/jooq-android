...
final class WindowSpecificationImpl extends AbstractQueryPart
  ...
  private static final Set<SQLDialect> OMIT_PARTITION_BY_ONE =
      SQLDialect.supportedBy(CUBRID, MYSQL, SQLITE);
    ...
    if (o.isEmpty()) {
      boolean ordered =
          w instanceof Ntile && REQUIRES_ORDER_BY_IN_NTILE.contains(ctx.dialect())
              || w instanceof PositionalWindowFunction
                  && ((PositionalWindowFunction<?>) w).isLeadOrLag()
                  && REQUIRES_ORDER_BY_IN_LEAD_LAG.contains(ctx.dialect())
              || w instanceof RankingFunction
                  && ((RankingFunction<?>) w).isRankOrDenseRank()
                  && REQUIRES_ORDER_BY_IN_RANK_DENSE_RANK.contains(ctx.dialect())
              || w instanceof RankingFunction
                  && !((RankingFunction<?>) w).isRankOrDenseRank()
  ...
  private final void toSQLRows(Context<?> ctx, Integer rows) {
    if (rows == Integer.MIN_VALUE) ctx.visit(K_UNBOUNDED_PRECEDING);
    else if (rows == Integer.MAX_VALUE) ctx.visit(K_UNBOUNDED_FOLLOWING);
    else if (rows < 0) ctx.sql(-rows).sql(' ').visit(K_PRECEDING);
    else if (rows > 0) ctx.sql(rows).sql(' ').visit(K_FOLLOWING);
    else ctx.visit(K_CURRENT_ROW);
  }
  ...
  @Override
  public final WindowSpecificationPartitionByStep partitionBy(
