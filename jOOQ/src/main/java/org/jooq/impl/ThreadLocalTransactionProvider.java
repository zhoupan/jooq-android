...
/**
 * A {@link TransactionProvider} that implements thread-bound transaction semantics.
 *
 * <p>Use this <code>TransactionProvider</code> if your transactions are thread-bound, meaning that
 * a transaction and its underlying {@link Connection} will never leave the thread that started the
 * transaction.
 *
 * <p>When this <code>TransactionProvider</code> is used, users must pass their custom {@link
 * ConnectionProvider} implementations to this <code>TransactionProvider</code>, instead of passing
 * it to the {@link Configuration}.
 *
 * @author Lukas Eder
 */
public class ThreadLocalTransactionProvider implements TransactionProvider {
  ...
  public ThreadLocalTransactionProvider(ConnectionProvider connectionProvider, boolean nested) {
    ...
    this.delegateTransactionProvider =
        new DefaultTransactionProvider(localConnectionProvider, nested);
  ...
  public void begin(TransactionContext ctx) {
    ...
    if (delegateTransactionProvider.nestingLevel(ctx.configuration()) == 1)
      localTxConnection.set(
          ((DefaultConnectionProvider)
                  ctx.configuration().data(DATA_DEFAULT_TRANSACTION_PROVIDER_CONNECTION))
              .connection);
  }
