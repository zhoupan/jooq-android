...
/** @author Lukas Eder */
abstract class AbstractAggregateFunction<T> extends AbstractWindowFunction<T>
    implements AggregateFunction<T>, OrderedAggregateFunction<T>, ArrayAggOrderByStep<T> {
  ...
  final void acceptArguments3(
      Context<?> ctx,
      QueryPartCollectionView<Field<?>> args,
      Function<? super Field<?>, ? extends Field<?>> fun) {
    if (filter == null || SUPPORT_FILTER.contains(ctx.dialect())) ctx.visit(wrap(args).map(fun));
    else ctx.visit(wrap(args).map(arg -> DSL.when(filter, arg == ASTERISK ? one() : arg)).map(fun));
  }
      ...
      switch (ctx.family()) {
        default:
          if (SUPPORT_FILTER.contains(ctx.dialect()))
            ctx.sql(' ').visit(K_FILTER).sql(" (").visit(K_WHERE).sql(' ').visit(filter).sql(')');
          break;
      }
    }
  }
  ...
  @Override
  public /* non-final */ AbstractAggregateFunction<T> orderBy(
  ...
  /**
   * Apply this aggregate function's <code>ORDER BY</code>, <code>FILTER</code> and <code>OVER
   * </code> clauses to an argument aggregate function.
   */
  ...
  /**
   * Apply this aggregate function's <code>FILTER</code> and <code>OVER</code> clauses to an
   * argument aggregate function.
   */
  ...
  final <U> Field<U> o(WindowBeforeOverStep<U> function) {
    if (windowSpecification != null) return function.over(windowSpecification);
    else if (windowDefinition != null) return function.over(windowDefinition);
    else if (windowName != null) return function.over(windowName);
    else return function;
  }

  /** Type safe <code>NVL2(y, x, null)</code> for statistical function emulations. */
  final <U extends Number> Field<U> x(Field<U> x, Field<? extends Number> y) {
    return DSL.nvl2(y, x, DSL.NULL(x.getDataType()));
  }

  /** Type safe <code>NVL2(x, y, null)</code> for statistical function emulations. */
