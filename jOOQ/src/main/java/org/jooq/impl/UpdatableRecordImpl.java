...
public class UpdatableRecordImpl<R extends UpdatableRecord<R>> extends TableRecordImpl<R>
  ...
  private final int storeMerge0(Field<?>[] storeFields, TableField<R, ?>[] keys) {
    ...
    if (lockingActive()) {
      ...
      else return storeInsert0(storeFields);
    } else {
  ...
  private final boolean lockingActive() {
    return isExecuteWithOptimisticLocking()
        && (isTimestampOrVersionAvailable() || isExecuteWithOptimisticLockingIncludeUnversioned());
  }
  ...
  private final boolean lockValuePresent() {
    ...
    return getRecordVersion() != null
        ...
        || getTable().getRecordVersion() == null
            && getTable().getRecordTimestamp() == null
            && fetched;
  }
  ...
  public final void refresh(final Field<?>... refreshFields) {
    ...
    if (select.execute() == 1) {
      ...
      throw new NoDataFoundException(
          "Exactly one row expected for refresh. Record does not exist in database.");
    }
  }
  ...
  private final boolean isExecuteWithOptimisticLockingIncludeUnversioned() {
    ...
    return configuration == null
        || !TRUE.equals(
            configuration.settings().isExecuteWithOptimisticLockingExcludeUnversioned());
  }
  ...
  /**
   * Perform an additional SELECT .. FOR UPDATE to check if the underlying database record has been
   * changed compared to this record.
   */
