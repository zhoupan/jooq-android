...
final class AlterDomainImpl<T> extends AbstractDDLQuery
  ...
  private final void accept0(Context<?> ctx) {
    ...
    if (addConstraint != null) {
      ...
      ctx.visit(ctx.family() == FIREBIRD ? K_TO : K_RENAME_TO)
          .sql(' ')
          .data(DATA_CONSTRAINT_REFERENCE, true, c -> c.visit(renameTo));
    } else if (renameConstraint != null) {
      ctx.visit(K_RENAME_CONSTRAINT)
          .sql(' ')
          .data(
