...
final class QuantifiedComparisonCondition extends AbstractCondition implements LikeEscapeStep {
  ...
  private final void accept0(Context<?> ctx) {
    ...
    switch (comparator) {
      ...
      case NOT_SIMILAR_TO:
        emulateOperator =
            escape != null || NO_SUPPORT_QUANTIFIED_SIMILAR_TO.contains(ctx.dialect());
        break;
      default:
    ...
    if (quantifiedArray && SUPPORTS_QUANTIFIED_ARRAYS.contains(ctx.dialect()) && !emulateOperator) {
      ...
      ctx.visit(
          CombinedCondition.of(
              query.quantifier == Quantifier.ALL ? Operator.AND : Operator.OR,
              query.values != null
                  ? map(query.values, v -> comparisonCondition(comparator, (Field<String>) v))
                  : map(
                      ((Param<? extends Object[]>) query.array).getValue(),
                      v ->
                          v instanceof Field
                              ? comparisonCondition(comparator, (Field<String>) v)
                              : comparisonCondition(comparator, v))));
