...
/** @author Lukas Eder */
final class ReferenceImpl<R extends Record, O extends Record> extends AbstractKey<R>
  ...
  public final Result<O> fetchParents(Collection<? extends R> records) {
    ...
    else return extractDSLContext(records).selectFrom(parents(records)).fetch();
  }
  ...
  public final Result<R> fetchChildren(Collection<? extends O> records) {
    ...
    else return extractDSLContext(records).selectFrom(children(records)).fetch();
  }
  ...
  private static <R extends Record> TableField<R, ?>[] truncate(
      TableField<R, ?>[] fields1, TableField<?, ?>[] fields2) {
    if (fields1.length <= fields2.length) return fields1;
    else return Arrays.copyOf(fields1, fields2.length);
  }
