...
final class XMLHandler<R extends Record> extends DefaultHandler {
  ...
  public final void startElement(String uri, String localName, String qName, Attributes attributes)
    ...
    if (!s.inResult && "result".equalsIgnoreCase(qName)) {
      ...
      if (f.getDataType().isMultiset()) {
        ...
        s =
            new State<>(
                (AbstractRow<R>) f.getDataType().getRow(),
                (Class<R>) f.getDataType().getRecordType());
        s.inResult = true;
      } else throw new UnsupportedOperationException("Nested result sets not supported yet");
      ...
      if (s.inColumn) {
        ...
        if (f.getDataType().isRecord()) {
          ...
          s.inResult = true;
        } else throw new UnsupportedOperationException("Nested records not supported yet");
      }
    } else {
      if (s.result == null) {
        String fieldName;
  ...
  private static boolean onlyValueFields(List<Field<?>> fields) {
    ...
    else return !anyMatch(fields, f -> !"value".equalsIgnoreCase(f.getName()));
  }
