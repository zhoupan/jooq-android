...
final class CursorImpl<R extends Record> extends AbstractCursor<R> {
  ...
  CursorImpl(
      ...
      Field<?>[] fields,
      int[] internIndexes,
      boolean keepStatement,
      boolean keepResultSet) {
    this(
        ctx,
        listener,
        fields,
        ...
        keepStatement,
        keepResultSet,
        (Class<? extends R>) RecordImplN.class,
        0,
        true);
  }
  ...
  CursorImpl(
    ...
    this.rsContext =
        new DefaultBindingGetResultSetContext<>(ctx.configuration(), ctx.data(), rs, 0);
    this.maxRows = maxRows;
    this.autoclosing = autoclosing;
    if (internIndexes != null) {
      this.intern = new boolean[fields.length];
      for (int i : internIndexes) intern[i] = true;
    } else this.intern = null;
  }
  ...
  public final Result<R> fetchNext(int number) {
    ...
    iterator();
    ResultImpl<R> result =
        new ResultImpl<>(((DefaultExecuteContext) ctx).originalConfiguration(), fields);
    ctx.result(result);
    listener.resultStart(ctx);
    for (int i = 0; i < number && iterator().hasNext(); i++) result.addRecord(iterator().next());
    ctx.result(result);
  ...
  final class CursorResultSet extends JDBC41ResultSet implements ResultSet {
    ...
    public final void close() throws SQLException {
      ...
      } finally // [#1868] If this Result / Cursor was "kept" through a lazy
    ...
    private final void logUpdate(int columnIndex, Object x) throws SQLException {
      if (log.isDebugEnabled()) {
        log.debug(
            "Updating Result",
            "Updating Result position " + getRow() + ":" + columnIndex + " with value " + x);
      }
    }
    ...
    private final void logUpdate(String columnLabel, Object x) throws SQLException {
      if (log.isDebugEnabled()) {
        log.debug(
            "Updating Result",
            "Updating Result position " + getRow() + ":" + columnLabel + " with value " + x);
      }
    }
  ...
  final class CursorIterator implements Iterator<R> {
     ...
     *
     * <ul>
     *   <li>null: it's not known whether there is a next record
     *   <li>true: there is a next record, and it has been pre-fetched
     *   <li>false: there aren't any next records
     * </ul>
     */
    ...
    private final RecordDelegate<AbstractRecord> recordDelegate =
        Tools.newRecord(
            true,
            (Supplier<AbstractRecord>) factory,
    ...
    private class CursorRecordInitialiser
        implements ThrowingFunction<AbstractRecord, AbstractRecord, SQLException> {
      ...
      public AbstractRecord apply(AbstractRecord record) throws SQLException {
        ...
        for (int i = 0; i < size; i++) setValue(record, initialiserFields.field(i), i);
        if (intern != null)
          for (int i = 0; i < intern.length; i++) if (intern[i]) record.intern0(i);
        ctx.record(record);
      ...
      private final <T> void setValue(AbstractRecord record, Field<T> field, int index)
          ...
          if (nested != null) {
            ...
            value =
                (T)
                    Tools.newRecord(
                            true,
                            (Class<AbstractRecord>) recordType,
                            (AbstractRow<AbstractRecord>) nested,
                            ((DefaultExecuteContext) ctx).originalConfiguration())
                        .operate(operation);
            // [#7100] TODO: Is there a more elegant way to do this?
            if (f != field) value = ((Converter<Object, T>) field.getConverter()).from(value);
        ...
        } // [#5901] Improved error logging, mostly useful when there are some data type conversion
        // errors
        catch (Exception e) {
          throw new SQLException(
              "Error while reading field: " + field + ", at JDBC index: " + (offset + index + 1),
              e);
        }
      }
    }
