...
final class CreateDomainImpl<T> extends AbstractDDLQuery
    implements CreateDomainAsStep,
  ...
  CreateDomainImpl(Configuration configuration, Domain<?> domain, boolean createDomainIfNotExists) {
    this(configuration, domain, createDomainIfNotExists, null, null, null);
  }
  ...
  final Field<T> $default_() {
    return default_;
  }

  final Collection<? extends Constraint> $constraints() {
    return constraints;
  }
  ...
  private final void accept0(Context<?> ctx) {
    ...
    if (constraints != null)
      if (ctx.family() == FIREBIRD)
        ctx.formatSeparator()
            .visit(DSL.check(DSL.and(Tools.map(constraints, c -> ((ConstraintImpl) c).$check()))));
      else for (Constraint constraint : constraints) ctx.formatSeparator().visit(constraint);
  }
}
