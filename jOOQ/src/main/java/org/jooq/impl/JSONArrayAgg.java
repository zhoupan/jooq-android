 ...
 * @author Lukas Eder
 */
final class JSONArrayAgg<J> extends AbstractAggregateFunction<J>
  ...
  private final Field<?> groupConcatEmulation(Context<?> ctx) {
    ...
    return DSL.concat(
        ...
        CustomField.of(
            ...
            c1 -> {
              c1.visit(
                  groupConcatEmulationWithoutArrayWrappers(
                      CustomField.of(
                          Names.N_FIELD,
                          VARCHAR,
                          c2 -> acceptArguments2(c2, QueryPartListView.wrap(arg2))),
                      withinGroupOrderBy));
  ...
  static final Field<?> groupConcatEmulationWithoutArrayWrappers(
      Field<?> field, SortFieldList orderBy) {
    return Tools.isEmpty(orderBy) ? groupConcat(field) : groupConcat(field).orderBy(orderBy);
  }
