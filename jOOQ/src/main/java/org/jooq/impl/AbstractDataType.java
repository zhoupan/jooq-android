...
abstract class AbstractDataType<T> extends AbstractNamed implements DataType<T> {
  ...
  public final int getSQLType(Configuration configuration) {
    ...
    else if (tType == BigDecimal.class) return Types.DECIMAL;
    else if (tType == Byte.class) return Types.TINYINT;
    else if (tType == byte[].class) return Types.BLOB;
    else if (tType == Clob.class) return Types.CLOB;
    else if (Tools.isDate(tType)) {
      switch (configuration.family()) {
        default:
          return Types.DATE;
      }
    } else if (tType == Double.class) return Types.DOUBLE;
    else if (tType == Float.class) return Types.FLOAT;
    else if (tType == Integer.class) return Types.INTEGER;
    else if (tType == Long.class) return Types.BIGINT;
    else if (tType == Short.class) return Types.SMALLINT;
  ...
  private String getCastTypeName0(SQLDialect dialect) {
    ...
    if (lengthDefined() && length() > 0) {
      ...
      else return castTypePrefix0() + "(" + precision() + ")" + castTypeSuffix0();
    } else return castTypeName0();
  }

  @Override
  public /* non-final */ String getCastTypeName(Configuration configuration) {
    return ((AbstractDataType<T>) getDataType(configuration))
        .getCastTypeName0(configuration.dialect());
  }
  ...
  public /* final */ T convert(Object object) {
    ...
    else return Convert.convert(object, getType());
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof AbstractDataType)) return false;
    AbstractDataType<?> other = (AbstractDataType<?>) obj;
    if (getDialect() != other.getDialect()) return false;
    if (!eq(length0(), other.length0())) return false;
    if (!eq(precision0(), other.precision0())) return false;
    if (!eq(scale0(), other.scale0())) return false;
    if (getType() == null) {
      if (other.getType() != null) return false;
    } else if (!getType().equals(other.getType())) return false;
    if (tType0() == null) {
      if (other.tType0() != null) return false;
    } else if (!tType0().equals(other.tType0())) return false;
    if (typeName0() == null) {
      if (other.typeName0() != null) return false;
    } else if (!typeName0().equals(other.typeName0())) return false;
    return true;
  }
}
