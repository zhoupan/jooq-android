...
final class RowOverlapsCondition<T1, T2> extends AbstractCondition {
  ...
  private static final Set<SQLDialect> EMULATE_NON_STANDARD_OVERLAPS =
      SQLDialect.supportedUntil(CUBRID, DERBY, FIREBIRD, H2, MARIADB, MYSQL, SQLITE);
  ...
  public final void accept(Context<?> ctx) {
    ...
    if (!standardOverlaps || EMULATE_NON_STANDARD_OVERLAPS.contains(ctx.dialect())) {
      ...
      if (intervalOverlaps)
        ...
        ctx.visit(
            right1.le(castIfNeeded(left2, right1)).and(left1.le(castIfNeeded(right2, left1))));
    } else // These dialects seem to have trouble with INTERVAL OVERLAPS predicates
    if (intervalOverlaps && EMULATE_INTERVAL_OVERLAPS.contains(ctx.dialect()))
      ctx.visit(right1.le(iadd(left1, left2)).and(left1.le(iadd(right1, right2))));
    else
      // Everyone else can handle OVERLAPS
      ctx.sql('(').visit(left).sql(' ').visit(K_OVERLAPS).sql(' ').visit(right).sql(')');
  }
