...
final class RowSubqueryCondition extends AbstractCondition {
        ...
        switch (comparator) {
          ...
          default:
            return emulationUsingExists(
                ...
                left,
                q.query,
                q.quantifier == ALL ? comparator.inverse() : comparator,
                q.quantifier == ALL);
        }
  ...
  private static final QueryPartInternal emulationUsingExists(
      Context<?> ctx, Row row, Select<?> select, Comparator comparator, boolean notExists) {
  ...
  private static final SelectOrderByStep<Record> emulatedSubselect(
      Context<?> ctx, Row row, Select<?> s, Comparator c) {
