...
/** @author Lukas Eder */
abstract class AbstractTable<R extends Record> extends AbstractNamed
  ...
  @Override
  public /* non-final for covariant overriding */ Table<R> as(
      String alias, String... fieldAliases) {
    return as(DSL.name(alias), Tools.names(fieldAliases));
  }

  @Override
  public final Table<R> as(
      String alias, Function<? super Field<?>, ? extends String> aliasFunction) {
    ...
    if (identity == null) {
      for (Field<?> f : fields())
        if (f instanceof TableField && f.getDataType().identity())
          if (identity == null) identity = new IdentityImpl(this, (TableField) f);
          else
            log.info(
                "Multiple identities",
   ...
   * {@inheritDoc}
   *
   * <p>Subclasses should no longer override this method, which may be made final in the future.
   */
  @Override
  ...
  /**
   * Subclasses may call this method to create {@link TableField} objects that are linked to this
   * table.
   *
   ...
   */
  protected static final <R extends Record, T> TableField<R, T> createField(
      Name name, DataType<T> type, Table<R> table) {
   ...
   */
  protected static final <R extends Record, T> TableField<R, T> createField(
      Name name, DataType<T> type, Table<R> table, String comment) {
   ...
   */
  protected static final <R extends Record, T, U> TableField<R, U> createField(
      Name name, DataType<T> type, Table<R> table, String comment, Converter<T, U> converter) {
   ...
   */
  protected static final <R extends Record, T, U> TableField<R, U> createField(
      Name name, DataType<T> type, Table<R> table, String comment, Binding<T, U> binding) {
   ...
   */
  protected final <T, U> TableField<R, U> createField(
      Name name, DataType<T> type, String comment, Converter<T, U> converter) {
   ...
   */
  protected final <T, U> TableField<R, U> createField(
      Name name, DataType<T> type, String comment, Binding<T, U> binding) {
  ...
  public final Table<R> as(Table<?> otherTable, Field<?>... otherFields) {
    return as(
        otherTable.getUnqualifiedName(),
        Tools.map(otherFields, Field::getUnqualifiedName, Name[]::new));
  }

  @Override
  public final Table<R> as(
      Table<?> otherTable, Function<? super Field<?>, ? extends Field<?>> aliasFunction) {
  ...
  public final Table<R> as(
    ...
    return as(
        otherTable.getUnqualifiedName(), (f, i) -> aliasFunction.apply(f, i).getUnqualifiedName());
  }
