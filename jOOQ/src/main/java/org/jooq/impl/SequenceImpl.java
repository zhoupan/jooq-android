    ...
    public final void accept(Context<?> ctx) {
      ...
      switch (family) {
        ...
        case MARIADB:
          ...
          else if (family == H2) ctx.visit(sequence).sql('.').visit(method.keyword);
          else if (family == HSQLDB) ctx.visit(K_CURRENT_VALUE_FOR).sql(' ').visit(sequence);
          else if (family == MARIADB) ctx.visit(K_PREVIOUS_VALUE_FOR).sql(' ').visit(sequence);
          else if (family == FIREBIRD) ctx.visit(N_GEN_ID).sql('(').visit(sequence).sql(", 0)");
          else {
