...
/**
 * An array collector that delays the instantiation of the array type until it is known.
 *
 * <p>{@link Records#intoArray(Object[])} and similar collectors require an eager specification of
 * the resulting array type. In some cases, this array type cannot be known until a query is
 * executed (e.g in the case of a plain SQL {@link ResultQuery}). In that case, we rely {@link
 * ResultQuery#collect(Collector)} to inject a {@link Fields} implementation prior to running a
 * collection.
 *
 * @author Lukas Eder
 */
