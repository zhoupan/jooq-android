...
final class DefaultUnwrapperProvider implements UnwrapperProvider {
  ...
  static final class DefaultUnwrapper implements Unwrapper {
    ...
    public <T> T unwrap(Wrapper wrapper, Class<T> iface) {
      if (wrapper instanceof Connection) return unwrap((Connection) wrapper, iface);
      else if (wrapper instanceof Statement) return unwrap((Statement) wrapper, iface);
      else throw new IllegalArgumentException("Cannot unwrap: " + wrapper);
    }
