...
class DefaultExecuteContext implements ExecuteContext {
   ...
   *
   * <p>[#1326] This is necessary in those dialects that have long-lived temporary lob objects,
   * which can cause memory leaks in certain contexts, where the lobs' underlying session /
   * connection is long-lived as well. Specifically, Oracle and ojdbc have some trouble when
   * streaming temporary lobs to UDTs:
   *
   * <ol>
   *   <li>The lob cannot have a call-scoped life time with UDTs
   *   <li>Freeing the lob after binding will cause an ORA-22275
   *   <li>Not freeing the lob after execution will cause an {@link OutOfMemoryError}
   * </ol>
   *
   * <p>
   *
   ...
   *
   * <p>[#1544] There exist some corner-cases regarding the {@link SQLOutput} API, used for UDT
   * serialisation / deserialisation, which have no elegant solutions of obtaining a {@link
   * Configuration} and thus a JDBC {@link Connection} object short of:
   *
   * <ul>
   *   <li>Making assumptions about the JDBC driver and using proprietary API, e.g. that of ojdbc
   *   <li>Dealing with this problem globally by using such a local configuration
   * </ul>
   *
   * @see <a
   ...
   *
   * <p>It can be safely assumed that such a configuration is available once the {@link
   * ExecuteContext} has been established, until the statement is closed.
   */
   ...
   *
   * <p>It can be safely assumed that such a connection is available once the {@link ExecuteContext}
   * has been established, until the statement is closed.
   */
  ...
  /**
   * Get the registered connection's "target connection" through {@link
   * Configuration#unwrapperProvider()} if applicable.
   *
   * <p>It can be safely assumed that such a connection is available once the {@link ExecuteContext}
   * has been established, until the statement is closed.
   */
  ...
  public final Query[] batchQueries() {
    return batch ? batchQueries : query != null ? new Query[] {query} : EMPTY_QUERY;
  }
  ...
  public final String[] batchSQL() {
    return batch ? batchSQL : routine != null || query != null ? new String[] {sql} : EMPTY_STRING;
  }
  ...
  /**
   * Initialise this {@link DefaultExecuteContext} with a pre-existing {@link Connection}.
   *
   * <p>[#3191] This is needed, e.g. when using {@link Query#keepStatement(boolean)}.
   */
  ...
  private final SettingsEnabledConnection wrapConnection(
      ConnectionProvider provider, Connection c) {
    return new SettingsEnabledConnection(
        new ProviderEnabledConnection(provider, c), derivedConfiguration.settings(), this);
  }
  ...
  public final int[] batchRows() {
    return batch ? batchRows : routine != null || query != null ? new int[] {rows} : EMPTY_INT;
  }
      ...
      if (oldStack != null) {
        ...
        newStack[0] =
            new StackTraceElement(
                "org.jooq_" + Constants.VERSION + "." + dialect(), "debug", null, -1);
