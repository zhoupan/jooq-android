...
final class InformationSchemaExport {
  ...
  static final InformationSchema exportTables(Configuration configuration, List<Table<?>> tables) {
    ...
    for (Table<?> t : tables) if (t.getSchema() != null) includedSchemas.add(t.getSchema());

    for (Schema s : includedSchemas)
      if (s.getCatalog() != null) includedCatalogs.add(s.getCatalog());
  ...
  static final InformationSchema exportCatalogs(
      Configuration configuration, List<Catalog> catalogs) {
    InformationSchema result = new InformationSchema();
  ...
  private static final void exportDomain0(
      Configuration configuration, InformationSchema result, Domain<?> d) {
    ...
    if (!isBlank(catalogName)) id.setDomainCatalog(catalogName);

    if (!isBlank(schemaName)) id.setDomainSchema(schemaName);

    id.setDomainName(domainName);
    id.setDataType(d.getDataType().getTypeName(configuration));

    if (d.getDataType().lengthDefined()) id.setCharacterMaximumLength(d.getDataType().length());

    if (d.getDataType().precisionDefined()) id.setNumericPrecision(d.getDataType().precision());

    if (d.getDataType().scaleDefined()) id.setNumericScale(d.getDataType().scale());
  ...
  private static final void exportSequence0(
      Configuration configuration, InformationSchema result, Sequence<?> q) {
    ...
    if (!isBlank(catalogName)) iq.setSequenceCatalog(catalogName);

    if (!isBlank(schemaName)) iq.setSequenceSchema(schemaName);

    iq.setSequenceName(q.getName());
    iq.setDataType(q.getDataType().getTypeName(configuration));

    if (q.getDataType().lengthDefined()) iq.setCharacterMaximumLength(q.getDataType().length());

    if (q.getDataType().precisionDefined()) iq.setNumericPrecision(q.getDataType().precision());

    if (q.getDataType().scaleDefined()) iq.setNumericScale(q.getDataType().scale());

    if (q.getStartWith() != null)
      iq.setStartValue(
          Convert.convert(
              q.getStartWith() instanceof Param
                  ? ((Param<?>) q.getStartWith()).getValue()
                  : q.getStartWith().toString(),
              BigInteger.class));
    if (q.getIncrementBy() != null)
      iq.setIncrement(
          Convert.convert(
              q.getIncrementBy() instanceof Param
                  ? ((Param<?>) q.getIncrementBy()).getValue()
                  : q.getIncrementBy().toString(),
              BigInteger.class));
    if (q.getMinvalue() != null)
      iq.setMinimumValue(
          Convert.convert(
              q.getMinvalue() instanceof Param
                  ? ((Param<?>) q.getMinvalue()).getValue()
                  : q.getMinvalue().toString(),
              BigInteger.class));
    if (q.getMaxvalue() != null)
      iq.setMaximumValue(
          Convert.convert(
              q.getMaxvalue() instanceof Param
                  ? ((Param<?>) q.getMaxvalue()).getValue()
                  : q.getMaxvalue().toString(),
              BigInteger.class));
    iq.setCycleOption(q.getCycle());
    if (q.getCache() != null)
      iq.setCache(
          Convert.convert(
              q.getCache() instanceof Param
                  ? ((Param<?>) q.getCache()).getValue()
                  : q.getCache().toString(),
              BigInteger.class));
  ...
  private static final void exportSchema0(InformationSchema result, Schema s) {
    ...
    if (!isBlank(catalogName)) is.setCatalogName(catalogName);
  ...
  private static final void exportTable0(
      Configuration configuration,
      InformationSchema result,
      Table<?> t,
      Set<Table<?>> includedTables) {
    ...
    if (!isBlank(catalogName)) it.setTableCatalog(catalogName);

    if (!isBlank(schemaName)) it.setTableSchema(schemaName);
    ...
    if (t.getOptions().type() == org.jooq.TableOptions.TableType.VIEW) {
      ...
      if (!isBlank(catalogName)) iv.setTableCatalog(catalogName);

      if (!isBlank(schemaName)) iv.setTableSchema(schemaName);
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (!isBlank(catalogName)) ic.setTableCatalog(catalogName);

      if (!isBlank(schemaName)) ic.setTableSchema(schemaName);
      ...
      if (f.getDataType().lengthDefined()) ic.setCharacterMaximumLength(f.getDataType().length());

      if (f.getDataType().precisionDefined()) ic.setNumericPrecision(f.getDataType().precision());

      if (f.getDataType().scaleDefined()) ic.setNumericScale(f.getDataType().scale());
    ...
    for (Check<?> chk : t.getChecks())
      if (includedTables.contains(chk.getTable())) exportCheck0(configuration, result, t, chk);

    for (Index index : t.getIndexes()) exportIndex0(result, t, index);
  }

  private static final void exportCheck0(
      Configuration configuration, InformationSchema result, Table<?> t, Check<?> chk) {
    ...
    if (!isBlank(catalogName)) c.setConstraintCatalog(catalogName);

    if (!isBlank(schemaName)) c.setConstraintSchema(schemaName);
  ...
  private static final void exportIndex0(InformationSchema result, Table<?> t, Index index) {
    ...
    if (!isBlank(catalogName)) i.withIndexCatalog(catalogName).withTableCatalog(catalogName);

    if (!isBlank(schemaName)) i.withIndexSchema(schemaName).withTableSchema(schemaName);
    ...
    for (SortField<?> sortField : index.getFields()) {
      ...
      if (!isBlank(catalogName)) ic.withIndexCatalog(catalogName).withTableCatalog(catalogName);

      if (!isBlank(schemaName)) ic.withIndexSchema(schemaName).withTableSchema(schemaName);
  ...
  private static final void exportKey0(
      InformationSchema result, Table<?> t, Key<?> key, TableConstraintType constraintType) {
    ...
    if (constraintType == FOREIGN_KEY) {
      ...
      if (!isBlank(catalogName)) rc.setConstraintCatalog(catalogName);

      if (!isBlank(ukCatalogName)) rc.setUniqueConstraintCatalog(ukCatalogName);

      if (!isBlank(schemaName)) rc.setConstraintSchema(schemaName);

      if (!isBlank(ukSchemaName)) rc.setUniqueConstraintSchema(ukSchemaName);
  ...
  private static final void exportTableConstraint(
    ...
    if (!isBlank(catalogName)) tc.withConstraintCatalog(catalogName).withTableCatalog(catalogName);

    if (!isBlank(schemaName)) tc.withConstraintSchema(schemaName).withTableSchema(schemaName);
