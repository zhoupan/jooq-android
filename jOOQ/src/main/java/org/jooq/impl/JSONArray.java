...
final class JSONArray<T> extends AbstractField<T>
    ...
    switch (ctx.family()) {
      case POSTGRES:
        if (onNull == JSONOnNull.ABSENT_ON_NULL) {
          ...
          ctx.visit(getDataType() == JSON ? N_JSON_BUILD_ARRAY : N_JSONB_BUILD_ARRAY)
              .sql('(')
              .visit(mapped)
              .sql(')');
        break;
  ...
  public boolean equals(Object that) {
    if (that instanceof JSONArray) {
      return StringUtils.equals(type, ((JSONArray) that).type)
