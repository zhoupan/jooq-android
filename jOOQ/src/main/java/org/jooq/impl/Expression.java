...
final class Expression<T> extends AbstractTransformable<T> {
  ...
  private static class DateExpression<T> extends AbstractField<T> {
    ...
    public final void accept(Context<?> ctx) {
      ...
      else if (rhs.getDataType().isInterval()) acceptIntervalExpression(ctx);
      else acceptNumberExpression(ctx);
    }
        ...
        case MYSQL:
          {
            ...
            if (rhs.getType() == YearToMonth.class)
              ctx.visit(N_DATE_ADD)
                  ...
                  .visit(K_INTERVAL)
                  .sql(' ')
                  .visit(Tools.field(interval, SQLDataType.VARCHAR))
                  .sql(' ')
                  .visit(K_YEAR_MONTH)
                  .sql(')');
            ...
            //         https://jira.mariadb.org/browse/MDEV-14452
              ...
              ctx.visit(N_DATE_ADD)
                  ...
                  .visit(
                      Tools.field(
                          TRUNC_TO_MICROS.matcher("" + interval).replaceAll("$1"),
                          SQLDataType.VARCHAR))
                  .sql(' ')
        ...
        case DERBY:
          {
            ...
            if (rhs.getType() == YearToMonth.class)
              ctx.sql("{fn ")
                  ...
                  .sql('(')
                  .visit(N_SQL_TSI_MONTH)
                  .sql(", ")
                  .visit(p(sign * rhsAsYTM().intValue()))
                  .sql(", ")
                  .visit(lhs)
                  .sql(") }");
            else
              ctx.sql("{fn ")
                  .visit(N_TIMESTAMPADD)
                  .sql('(')
                  .visit(N_SQL_TSI_SECOND)
                  .sql(", ")
                  .visit(p(sign * (long) rhsAsDTS().getTotalSeconds()))
                  .sql(", {fn ")
                  .visit(N_TIMESTAMPADD)
                  .sql('(')
                  .visit(ctx.family() == DERBY ? N_SQL_TSI_FRAC_SECOND : N_SQL_TSI_MILLI_SECOND)
                  .sql(", ")
                  .visit(
                      p(
                          sign
                              * (long) rhsAsDTS().getMilli()
                              * (ctx.family() == DERBY ? 1000000L : 1L)))
                  .sql(", ")
                  .visit(lhs)
                  .sql(") }) }");
        ...
        case FIREBIRD:
          {
            ...
            // [#10448] Firebird only supports adding integers
              ...
              ctx.visit(N_DATEADD)
                  ...
                  .visit(K_MILLISECOND)
                  .sql(", ")
                  .visit(p(sign * (long) rhsAsDTS().getMilli()))
                  .sql(", ")
                  .visit(N_DATEADD)
                  .sql('(')
                  .visit(K_SECOND)
        ...
        case SQLITE:
          if (operator == ADD)
            ctx.visit(N_STRFTIME)
                ...
                .visit(lhs)
                .sql(", ")
                .visit(rhsAsNumber().concat(inline(" day")))
                .sql(')');
          else
            ctx.visit(N_STRFTIME)
                .sql("('%Y-%m-%d %H:%M:%f', ")
    ...
    private final YearToSecond rhsAsYTS() {
      try {
        ...
        throw new DataTypeException(
            "Cannot perform datetime arithmetic with a non-numeric, non-interval data type on the right hand side of the expression: "
                + rhs,
            e);
      }
    }
