...
final class RowField<ROW extends Row, REC extends Record> extends AbstractField<REC> {
  ...
  static final Set<SQLDialect> NO_NATIVE_SUPPORT =
  ...
  AbstractRow<REC> emulatedFields(Configuration configuration) {
    return (AbstractRow<REC>)
        row0(
            map(
                row.fields(),
                x ->
                    x.as(
                        getUnqualifiedName().unquotedName()
                            + configuration.settings().getNamePathSeparator()
                            + x.getName()),
                Field[]::new));
  }
