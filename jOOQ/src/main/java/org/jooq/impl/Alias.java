...
final class Alias<Q extends QueryPart> extends AbstractQueryPart {
  ...
  Alias(
      ...
      Name alias,
      Name[] fieldAliases,
      Predicate<Context<?>> wrapInParentheses) {
  ...
  final void toSQLAs(Context<?> ctx) {
    ...
    if (TRUE.equals(ctx.data(DATA_AS_REQUIRED))) {
      ...
      if (ctx.settings().getRenderOptionalAsKeywordForFieldAliases()
              == RenderOptionalKeyword.DEFAULT
          && SUPPORT_AS_REQUIRED.contains(ctx.dialect())) ctx.sql(' ').visit(K_AS);
      else if (ctx.settings().getRenderOptionalAsKeywordForFieldAliases()
          == RenderOptionalKeyword.ON) ctx.sql(' ').visit(K_AS);
    } else {
      if (ctx.settings().getRenderOptionalAsKeywordForTableAliases()
              == RenderOptionalKeyword.DEFAULT
          && SUPPORT_AS_REQUIRED.contains(ctx.dialect())) ctx.sql(' ').visit(K_AS);
      else if (ctx.settings().getRenderOptionalAsKeywordForTableAliases()
          == RenderOptionalKeyword.ON) ctx.sql(' ').visit(K_AS);
    }
  }
