 ...
 *
 * <p>The commercial jOOQ distributions support a variety of dialects, which are grouped into
 * dialect families. For instance, the PostgreSQL dialect family {@link #POSTGRES} is specialised by
 * its dialects
 *
 * <ul>
 *   <li>{@link #POSTGRES_9_3}
 *   <li>{@link #POSTGRES_9_4}
 *   <li>{@link #POSTGRES_9_5}
 *   <li>{@link #POSTGRES_10}
 *   <li>{@link #POSTGRES_11}
 *   <li>{@link #POSTGRES_12}
 * </ul>
 *
 * <p>The open source jOOQ distributions only support the dialect family, which corresponds to the
 * latest supported dialect version of the commercial distribution. A full list of mappings between
 * dialect families and latest supported version can be seen here <a href=
 * "https://www.jooq.org/download/support-matrix">https://www.jooq.org/download/support-matrix</a>.
 *
 * <p>If a dialect is documented but does not seem to be available in your jOOQ Edition, you may be
 * using the wrong edition, e.g. because of a transitive dependency introduced by Spring Boot. See
 * this article about how to exclude such transitive dependencies from your classpath <a href=
...
public enum SQLDialect {
   ...
   *
   * <p>This dialect is chosen in the absence of a more explicit dialect. It is not intended to be
   ...
   *
   * <p>This family behaves like the versioned dialect {@link #POSTGRES_13}.
   *
   * <p>While this family (and its dialects) have been observed to work to some extent on Amazon
   ...
   *
   * <p>The resulting set of dialects contain all the families and dialect versions that precede the
   * argument dialects.
   */
  @NotNull
   ...
   *
   * <p>The resulting set of dialects contain all the families and dialect versions that support the
   * argument dialect, i.e. that succeed it.
   */
  @NotNull
   ...
   *
   * <p>The resulting set of dialects contain all the families and dialect versions that support the
   * argument dialects, i.e. that succeed them.
   */
  @NotNull
  ...
  private static final void addSupportedBy(SQLDialect dialect, EnumSet<SQLDialect> supported) {
    ...
    if (dialect.isFamily()) supported.addAll(dialect.predecessors());
      ...
      for (SQLDialect candidate = dialect.family();
          candidate != dialect;
          candidate = candidate.predecessor()) supported.add(candidate);
  }
   ...
   *
   * <p>Unsupported, non-output dialects include:
   *
   * <ul>
   *   <li>{@link #DEFAULT}: A hypothetical dialect used for {@link QueryPart#toString()} calls of
   *       unattached query parts.
   *   <li>{@link #SQL99}: A legacy version of {@link #DEFAULT}.
   *   <li>{@link #POSTGRESPLUS}: A not yet supported dialect.
   * </ul>
   */
   ...
   * The dialect family.
   *
   * <p>This returns the dialect itself, if it has no "parent family". E.g. <code><pre>
   ...
   *
   * <p>If this is a dialect version (e.g. {@link #POSTGRES_9_4}) within a family (e.g. {@link
   * #POSTGRES}), then the predecessor will point to the historically previous dialect version (e.g.
   * {@link #POSTGRES_9_3}) within the same family, or to the dialect itself if there was no
   * predecessor explicitly supported by jOOQ.
   */
  @NotNull
   ...
   *
   * <p>Recursively calls {@link #predecessor()} and finds all the preceding dialects to this one,
   * including this one.
   */
  @NotNull
   ...
   *
   * <ul>
   *   <li><code>true</code> if this dialect is the same as the other dialect
   *   <li><code>true</code> if this dialect precedes the other dialect via any number of calls to
   *       {@link #predecessor()}
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li><code>false</code> if the two dialects do not belong to the same family
   * </ul>
   *
   * <p>This is useful to see if some feature is supported by <em>"at least"</em> a given dialect
   * version. Example: <code><pre>
   ...
   *
   * <ul>
   *   <li><code>false</code> if dialects don't belong to the same family
   *   <li><code>true</code> if either dialect {@link #isFamily()}
   *   <li><code>true</code> if <code>other</code> dialect precedes this dialect
   * </ul>
   *
   * <p>The <code>other</code> argument dialect is typically referenced from a {@link Support}
  ...
  public final class ThirdParty {
     ...
     *
     * @return The appropriate JDBC driver class or <code>"java.sql.Driver"</code> if no driver
     *     class could be derived from the URL. Never <code>null</code>.
     */
    @NotNull
    ...
    /**
     * The Spring DB name or <code>null</code>, if the db name is not supported by Spring.
     *
     * <p>The name returned by this method corresponds to the DB id as referenced in <code>
     * org/springframework/jdbc/support/sql-error-codes.xml</code>
     */
    @Nullable
    ...
    /**
     * The Hibernate dialect name or <code>null</code>, if the dialect is not supported by
     * Hibernate.
     *
     * <p>
     *
