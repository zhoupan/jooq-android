...
final class Tools {
  static final <T> T checkSQLDialect(
    ...
    try {
      ...
      if (support != null) {
        ...
        EnumSet<SQLDialect> supported =
            EnumSet.copyOf(
                support.value().length > 0 ? asList(support.value()) : asList(SQLDialect.values()));
        ...
        if (allowedFail)
          return error.apply(
              "The allowed dialects in scope "
                  + allowed
                  + " do not include any of the supported dialects: "
                  + supported);
        ...
        if (requiredFail)
          return error.apply(
              "Not all of the required dialects "
                  + required
                  + " from the current scope are supported "
                  + supported);
      }
  ...
  static Element enclosing(TreePath path) {
    ...
    if (enclosingMethod != null) return elementFromDeclaration(enclosingMethod);
