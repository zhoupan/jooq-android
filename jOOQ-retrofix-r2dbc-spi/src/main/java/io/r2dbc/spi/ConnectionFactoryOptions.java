 ...
 *
 * <p>A {@link ConnectionFactoryOptions} represents a configuration state that consists of one or
 * more {@link Option} objects. Each configuration option can be specified at most once to associate
 * a value with its key.
 *
 * <p>New objects are constructed through {@link #builder()} or by {@link #parse(CharSequence)
 * parsing} a R2DBC Connection URL. {@link ConnectionFactoryOptions} can be augmented to allow
 * staged construction of the configuration state using a {@link #mutate() initialized builder} to
 * create a new {@link ConnectionFactoryOptions} objects with mutations applied. Values of this
 * class are immutable once created. Builder initializers are left unchanged.
 *
 * <p>Example usage:
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * <p>Note that Connection URL Parsing cannot access {@link Option} type information {@code T} due
 * to Java's type erasure. Options configured by URL parsing are represented as {@link String}
 * values.
 *
...
public final class ConnectionFactoryOptions {
  ...
  /**
   * Driver protocol name. Typically represented as {@code tcp} or a database vendor-specific
   * protocol string.
   */
  ...
  /**
   * Parses a R2DBC Connection URL and returns the parsed {@link ConnectionFactoryOptions}. URL
   * format: {@code r2dbc:driver[:protocol]}://[user:password@]host[:port][/path][?option=value]}.
   *
   ...
   * @throws NoSuchOptionException if {@code option} is not configured or there is no value for
   *     {@code option}
   */
  ...
  public static final class Builder {
    ...
    /**
     * Populates the builder with the existing values from a configured {@link
    ...
    /**
     * Populates the builder with the existing values from a configured {@link
     * ConnectionFactoryOptions} allowing to {@link Predicate filter} which values to include.
     *
     * @param connectionFactoryOptions a configured {@link ConnectionFactoryOptions}
     * @param filter a {@link Predicate filter function} to include/exclude existing {@link
     *     Option}s.
     * @return this {@link Builder}
     * @throws IllegalArgumentException if {@code connectionFactoryOptions} or {@code filter} is
     *     {@code null}
     */
    public Builder from(
        ConnectionFactoryOptions connectionFactoryOptions, Predicate<Option<?>> filter) {
      Assert.requireNonNull(connectionFactoryOptions, "connectionFactoryOptions must not be null");
      Assert.requireNonNull(filter, "filter must not be null");
      connectionFactoryOptions.options.forEach(
          (k, v) -> {
            if (filter.test(k)) {
