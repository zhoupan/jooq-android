...
/**
 * Specification of properties to be used when starting a transaction. This interface is typically
 * implemented by code that calls {@link Connection#beginTransaction(TransactionDefinition)}.
 *
...
public interface TransactionDefinition {
  ...
  /**
   * Retrieve a transaction attribute by its {@link Option} identifier. This low-level interface
   * allows querying transaction attributes supported by the {@link Connection} that should be
   * applied when starting a new transaction.
   *
   * @param option the option to retrieve the value for
   * @param <T> requested value type
   * @return the value of the transaction attribute. Can be {@code null} to indicate absence of the
   *     attribute.
