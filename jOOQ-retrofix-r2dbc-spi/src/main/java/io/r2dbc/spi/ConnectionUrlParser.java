...
abstract class ConnectionUrlParser {
  ...
  private static final Set<String> PROHIBITED_QUERY_OPTIONS =
      Stream.of(
  ...
  static void validate(String url) {
    ...
    if (!url.startsWith(R2DBC_SCHEME + ":") && !url.startsWith(R2DBC_SSL_SCHEME + ":")) {
      throw new IllegalArgumentException(
          String.format("URL %s does not start with the %s scheme", url, R2DBC_SCHEME));
    }
  ...
  /**
   * Parse a {@link CharSequence query string} and decode percent encoding according to RFC3986,
   * section 2.4. Percent-encoded octets are decoded using UTF-8.
   *
   * @param s input text
  ...
  /**
   * Simplified fork of {@link URLDecoder}. The supplied encoding is used to determine what
   * characters are represented by any consecutive sequences of the form {@code %xy}.
   *
