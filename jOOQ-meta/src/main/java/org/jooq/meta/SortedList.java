 ...
 *
 * <p>Since the sort behaviour is stable, {@link List#add(Object)} and other single item methods can
 * sort the list in O(N). However, large additions of M elements should still be done using {@link
 * List#addAll(Collection)} to sort the list in O(N log N), instead of O(M*N), which is quadradic in
 * the worst case. Removals don't require sorting.
 */
