...
/**
 * A model for all known Referenced Key / Referencing Key relations in the {@link Database}'s
 * schema.
 *
 * @author Lukas Eder
 */
public interface Relations {
  ...
  /**
   * Get the the primary for a given column, or <code>null</code> if that column is not part of the
   * primary key.
   */
  UniqueKeyDefinition getPrimaryKey(ColumnDefinition column);

  /**
   * Get a list of unique keys (not including the primary key) for a given table, that the column
   * participates in.
   */
  ...
  /**
   * Get a list of unique keys (including the primary key) for a given table, that the column
   * participates in.
   */
  ...
  /**
   * Get a list of foreign keys for a given table, that the column participates in. Returns an empty
   * list if the given column is not part of any foreign key.
   */
  List<ForeignKeyDefinition> getForeignKeys(ColumnDefinition column);

  /**
   * Get a list of foreign keys for a given table. Returns an empty list if the given table has no
   * foreign keys.
   */
  List<ForeignKeyDefinition> getForeignKeys(TableDefinition table);

  /**
   * Get a list of <code>CHECK</code> constraints for a given table. Returns an empty list if the
   * given table has no <code>CHECK</code> constraints.
   */
