  ...
  protected void loadForeignKeys(DefaultRelations relations) throws SQLException {
    ...
    for (Record record :
        create()
            ...
            .join("sys.sysconglomerates fg")
            .on("fg.conglomerateid = f.conglomerateid")
            .join("sys.systables        ft")
            .on("ft.tableid = fg.tableid")
            .join("sys.sysschemas       fs")
            .on("ft.schemaid = fs.schemaid")
            .join("sys.sysconstraints   pc")
            .on("pc.constraintid = f.keyconstraintid")
            .join("sys.systables        pt")
              ...
              for (int i = 0; i < columnIndexes.size(); i++) {
                indexColumns.add(
                    new DefaultIndexColumnDefinition(
                        this, table.getColumn(columnIndexes.get(i)), SortOrder.ASC, i + 1));
              }
            }
  ...
  public ResultQuery<
    ...
    return create()
        .select(
            ...
            nullif(
                    SYSSEQUENCES.MINIMUMVALUE,
                    case_(cast(SYSSEQUENCES.SEQUENCEDATATYPE, VARCHAR))
