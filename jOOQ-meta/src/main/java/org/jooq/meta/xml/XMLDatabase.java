...
public class XMLDatabase extends AbstractDatabase {
    ...
    if (info == null) {
      ...
      final String xml =
          getProperties()
              .getProperty(
                  "xmlFiles",
                  getProperties().getProperty("xmlFile", getProperties().getProperty("xml-file")));
      final String xsl =
                    ...
                    if (StringUtils.isBlank(xsl)) {
                      ...
                      try {
                        ...
                        throw new RuntimeException(
                            "Error while transforming XML file " + xml + " with XSL file " + xsl,
                            e4);
                      } finally {
                    ...
                    if (column != null)
                      indexColumns.add(
                          new DefaultIndexColumnDefinition(
                              this,
                              column,
                              Boolean.TRUE.equals(ic.isIsDescending())
                                  ? SortOrder.DESC
                                  : SortOrder.ASC,
                      ...
                      log.error(
                          String.format(
                              "Column %s not found in table %s.", ic.getColumnName(), table));
                  }
                else log.error(String.format("No columns found for index %s.", name));
              }
    ...
    for (KeyColumnUsage usage : keyColumnUsage(PRIMARY_KEY)) {
      ...
      if (table != null)
        relations.addPrimaryKey(
            key, table, table.getColumn(columnName), tc != null && !FALSE.equals(tc.isEnforced()));
    }
  }
      ...
      if (constraintType == constraint.getConstraintType()
        ...
        for (KeyColumnUsage usage : info().getKeyColumnUsages())
          if (StringUtils.equals(
                  defaultIfNull(constraint.getConstraintCatalog(), ""),
                  defaultIfNull(usage.getConstraintCatalog(), ""))
              && StringUtils.equals(
                  defaultIfNull(constraint.getConstraintSchema(), ""),
                  defaultIfNull(usage.getConstraintSchema(), ""))
              && StringUtils.equals(
  ...
  protected void loadForeignKeys(DefaultRelations relations) {
    for (ReferentialConstraint fk : info().getReferentialConstraints()) {
      if (getInputSchemata().contains(fk.getConstraintSchema())) {
        for (KeyColumnUsage usage : info().getKeyColumnUsages()) {
          if (StringUtils.equals(
                  defaultIfNull(fk.getConstraintCatalog(), ""),
                  defaultIfNull(usage.getConstraintCatalog(), ""))
              && StringUtils.equals(
                  defaultIfNull(fk.getConstraintSchema(), ""),
                  defaultIfNull(usage.getConstraintSchema(), ""))
              && StringUtils.equals(
            ...
            TableConstraint fktc =
                tableConstraint(
                    fk.getConstraintCatalog(), fk.getConstraintSchema(), fk.getConstraintName());
            TableConstraint uktc =
                tableConstraint(
  ...
  private TableConstraint tableConstraint(
    ...
    for (TableConstraint uk : info().getTableConstraints())
      if (StringUtils.equals(
              defaultIfNull(constraintCatalog, ""), defaultIfNull(uk.getConstraintCatalog(), ""))
          && StringUtils.equals(
              defaultIfNull(constraintSchema, ""), defaultIfNull(uk.getConstraintSchema(), ""))
          && StringUtils.equals(
        ...
        if (tableType == TableType.VIEW) {
          ...
          for (View view : info().getViews()) {
            if (StringUtils.equals(
                    ...
                    defaultIfNull(view.getTableCatalog(), ""))
                && StringUtils.equals(
                    defaultIfNull(table.getTableSchema(), ""),
                    defaultIfNull(view.getTableSchema(), ""))
                && StringUtils.equals(
