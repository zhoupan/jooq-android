...
public class MySQLRoutineDefinition extends AbstractRoutineDefinition {
  ...
  @Deprecated
  public MySQLRoutineDefinition(
      SchemaDefinition schema, String name, String comment, String params, String returns) {
  ...
  private void init55() {
    ...
    for (Record record :
        create()
            ...
            .where(
                PARAMETERS.SPECIFIC_SCHEMA.in(
                    getSchema().getInputName(), getSchema().getInputName()))
      ...
      if (inOut == null) {
        addParameter(
            InOutDefinition.RETURN, new DefaultParameterDefinition(this, "RETURN_VALUE", -1, type));
      } else {
  ...
  private ParameterDefinition createParameter(
      Matcher matcher, int group, int columnIndex, String paramName) {
