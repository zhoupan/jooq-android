...
public class H2Database extends AbstractDatabase implements ResultQueryDatabase {
  ...
  protected List<IndexDefinition> getIndexes0() throws SQLException {
    ...
    Map<Record, Result<Record>> indexes =
        create()
            ...
            .and(getIncludeSystemIndexes() ? noCondition() : not(condition(INDEXES.IS_GENERATED)))
            .orderBy(
                INDEXES.TABLE_SCHEMA,
                INDEXES.TABLE_NAME,
                INDEXES.INDEX_NAME,
            ...
            .fetchGroups(
                new Field[] {
                  INDEXES.TABLE_SCHEMA, INDEXES.TABLE_NAME, INDEXES.INDEX_NAME, INDEXES.NON_UNIQUE
                },
                new Field[] {INDEXES.COLUMN_NAME, INDEXES.ORDINAL_POSITION});
    indexLoop:
    for (Entry<Record, Result<Record>> entry : indexes.entrySet()) {
      ...
      for (Record column : columns)
        if (table.getColumn(column.get(INDEXES.COLUMN_NAME)) == null) continue indexLoop;
      result.add(
  ...
  protected void loadPrimaryKeys(DefaultRelations relations) throws SQLException {
    for (Record record : primaryKeys(getInputSchemata())) {
      ...
      if (schema != null) {
        ...
        if (table != null) relations.addPrimaryKey(primaryKey, table, table.getColumn(columnName));
      }
    }
  }
  ...
  protected void loadUniqueKeys(DefaultRelations relations) throws SQLException {
    for (Record record : uniqueKeys(getInputSchemata())) {
      ...
      if (schema != null) {
        ...
        String columnName = record.get(INDEXES.COLUMN_NAME);
        TableDefinition table = getTable(schema, tableName);
        if (table != null) relations.addUniqueKey(primaryKey, table, table.getColumn(columnName));
      }
    }
  }

  @Override
  public ResultQuery<Record6<String, String, String, String, String, Integer>> primaryKeys(
      List<String> schemas) {
    // Workaround for https://github.com/h2database/h2database/issues/1000
    return keys(
        schemas, Arrays.<Field<String>>asList(inline("PRIMARY KEY"), inline("PRIMARY_KEY")));
  }

  @Override
  public ResultQuery<Record6<String, String, String, String, String, Integer>> uniqueKeys(
      List<String> schemas) {
  ...
  private ResultQuery<Record6<String, String, String, String, String, Integer>> keys(
      List<String> schemas, List<Field<String>> constraintTypes) {
    return create()
        .select(
        ...
        .orderBy(CONSTRAINTS.TABLE_SCHEMA, CONSTRAINTS.CONSTRAINT_NAME, INDEXES.ORDINAL_POSITION);
  }
  ...
  protected void loadForeignKeys(DefaultRelations relations) throws SQLException {
    for (Record record :
        create()
            ...
            and(
                CROSS_REFERENCES
                    ...
                    .CONSTRAINT_TYPE
                    .in("PRIMARY KEY", "PRIMARY_KEY", "UNIQUE"))
            .orderBy(
      ...
      SchemaDefinition uniqueKeySchema =
          getSchema(record.get(CROSS_REFERENCES.referencedConstraint().CONSTRAINT_SCHEMA));
  ...
  protected void loadCheckConstraints(DefaultRelations relations) throws SQLException {
    ...
    Select<Record4<String, String, String, String>> inlineChecks =
        is1_4_201()
  ...
  protected List<SchemaDefinition> getSchemata0() throws SQLException {
    return create()
        .select(SCHEMATA.SCHEMA_NAME, SCHEMATA.REMARKS)
        .from(SCHEMATA)
  ...
  public ResultQuery<
    ...
    return create()
        .select(
            ...
            nullif(SEQUENCES.MAX_VALUE, inline(DEFAULT_SEQUENCE_MAXVALUE))
                .coerce(NUMERIC)
                .as(SEQUENCES.MAX_VALUE),
            SEQUENCES.IS_CYCLE,
        ...
        .and(
            !getIncludeSystemSequences()
                ? upper(SEQUENCES.SEQUENCE_NAME).notLike(inline("SYSTEM!_SEQUENCE!_%"), '!')
                : noCondition())
        .orderBy(SEQUENCES.SEQUENCE_SCHEMA, SEQUENCES.SEQUENCE_NAME);
  }
    ...
    for (Record record : sequences(getInputSchemata())) {
      ...
      if (schema != null) {
        ...
        DefaultDataTypeDefinition type =
            new DefaultDataTypeDefinition(this, schema, H2DataType.BIGINT.getTypeName());
        result.add(
  ...
  static final class /* record */ TableRecord {
    ...
    public TableRecord(String schema, String table, TableType type, String comment, String source) {
      this.schema = schema;
      this.table = table;
      this.type = type;
    ...
    public boolean equals(Object o) {
      ...
      TableRecord other = (TableRecord) o;
      if (!java.util.Objects.equals(this.schema, other.schema)) return false;
      if (!java.util.Objects.equals(this.table, other.table)) return false;
      if (!java.util.Objects.equals(this.type, other.type)) return false;
      if (!java.util.Objects.equals(this.comment, other.comment)) return false;
      if (!java.util.Objects.equals(this.source, other.source)) return false;
      return true;
    }

    @Override
    public int hashCode() {
      return java.util.Objects.hash(this.schema, this.table, this.type, this.comment, this.source);
    }

    @Override
    public String toString() {
      return new StringBuilder("TableRecord[")
          .append("schema=")
          .append(this.schema)
          .append(", table=")
          .append(this.table)
        ...
        create()
            .select(
                ...
                when(TABLES.TABLE_TYPE.eq(inline("VIEW")), inline(TableType.VIEW.name()))
                    .when(
                        TABLES.STORAGE_TYPE.like(inline("%TEMPORARY%")),
                        inline(TableType.TEMPORARY.name()))
                    .else_(inline(TableType.TABLE.name()))
                    .convertFrom(TableType::valueOf)
                    .as("table_type"),
                TABLES.REMARKS,
                    ...
                    field(
                        DSL.exists(
                            select(one())
                                ...
                                .where(
                                    field(name("a", FUNCTION_ALIASES.ALIAS_SCHEMA.getName()))
                                        .eq(FUNCTION_ALIASES.ALIAS_SCHEMA))
                                .and(
                                    field(name("a", FUNCTION_ALIASES.ALIAS_NAME.getName()))
                                        .eq(FUNCTION_ALIASES.ALIAS_NAME))
                                .and(
                                    field(name("a", FUNCTION_ALIASES.COLUMN_COUNT.getName()))
                                        .ne(FUNCTION_ALIASES.COLUMN_COUNT))))))
            .as("overloaded");
    for (Record record :
        create()
            .select(
            ...
            .orderBy(FUNCTION_ALIASES.ALIAS_NAME)
            .fetch()) {
      ...
      if (schema != null) {
        ...
        String overload =
            record.get(overloaded) ? record.get(FUNCTION_ALIASES.COLUMN_COUNT, String.class) : null;
        result.add(
            new H2RoutineDefinition(schema, name, comment, typeName, precision, scale, overload));
      }
    }
    return result;
  }
  ...
  protected List<EnumDefinition> getEnums0() throws SQLException {
    ...
    if (!is1_4_197()) return result;
    getInlineEnums(result);
    getDomainEnums(result);
    return result;
  }

  static final class /* record */ EnumRecord {
    ...
    @Override
    public boolean equals(Object o) {
      if (!(o instanceof EnumRecord)) return false;
      EnumRecord other = (EnumRecord) o;
      if (!java.util.Objects.equals(this.schema, other.schema)) return false;
      if (!java.util.Objects.equals(this.table, other.table)) return false;
      if (!java.util.Objects.equals(this.column, other.column)) return false;
      if (!java.util.Objects.equals(this.type, other.type)) return false;
      return true;
    }
    ...
    @Override
    public int hashCode() {
      return java.util.Objects.hash(this.schema, this.table, this.column, this.type);
    }

    @Override
    public String toString() {
      return new StringBuilder("EnumRecord[")
          .append("schema=")
          .append(this.schema)
          .append(", table=")
          .append(this.table)
  ...
  private void getInlineEnums(List<EnumDefinition> result) {
    ...
    for (EnumRecord r :
        create()
            .select(
                COLUMNS.TABLE_SCHEMA, COLUMNS.TABLE_NAME, COLUMNS.COLUMN_NAME, COLUMNS.COLUMN_TYPE)
            .from(COLUMNS)
            .where(
                COLUMNS
                    .COLUMN_TYPE
      ...
      SchemaDefinition schema = getSchema(r.schema);
      if (schema == null) continue enumLoop;
  ...
  static final class /* record */ DomainRecord {
    ...
    public boolean equals(Object o) {
      ...
      DomainRecord other = (DomainRecord) o;
      if (!java.util.Objects.equals(this.schema, other.schema)) return false;
      if (!java.util.Objects.equals(this.name, other.name)) return false;
      if (!java.util.Objects.equals(this.sql, other.sql)) return false;
      return true;
    }

    @Override
    public int hashCode() {
      return java.util.Objects.hash(this.schema, this.name, this.sql);
    }

    @Override
    public String toString() {
      return new StringBuilder("DomainRecord[")
          .append("schema=")
          .append(this.schema)
          .append(", name=")
          .append(this.name)
  ...
  private void getDomainEnums(List<EnumDefinition> result) {
    ...
    for (DomainRecord r :
        create()
            .select(DOMAINS.DOMAIN_SCHEMA, DOMAINS.DOMAIN_NAME, DOMAINS.SQL)
            .from(DOMAINS)
      ...
      CSVReader reader =
          new CSVReader(
              new StringReader(
                  r.sql.replaceAll("(?i:(^.*as enum\\()|(\\)[^)]*$))", "")), // Separator
              ',', // Quote character
              '\'', // Strict quotes
              true);
      for (String string : reader.next()) definition.addLiteral(string);
  ...
  protected List<DomainDefinition> getDomains0() throws SQLException {
    ...
    for (Record record :
      ...
      boolean isTimestamp =
          record.get(DOMAINS.TYPE_NAME).trim().toLowerCase().startsWith("timestamp");
      ...
      DataTypeDefinition baseType =
          new DefaultDataTypeDefinition(
              ...
              isTimestamp ? record.get(DOMAINS.SCALE) : record.get(DOMAINS.PRECISION),
              isTimestamp ? 0 : record.get(DOMAINS.SCALE),
              !record.get(DOMAINS.IS_NULLABLE, boolean.class),
              record.get(DOMAINS.COLUMN_DEFAULT));
      DefaultDomainDefinition domain =
          new DefaultDomainDefinition(schema, record.get(DOMAINS.DOMAIN_NAME), baseType);
  ...
  boolean is1_4_197() {
    ...
    if (is1_4_197 == null) is1_4_197 = exists(COLUMNS.COLUMN_TYPE);
    return is1_4_197;
  }

  boolean is1_4_198() {
    // [#5874] The COLUMNS.IS_VISIBLE column was introduced in H2 1.4.198
    if (is1_4_198 == null) is1_4_198 = exists(COLUMNS.IS_VISIBLE);
    return is1_4_198;
  }
  ...
  boolean is1_4_201() {
    ...
    if (is1_4_201 == null) is1_4_201 = !exists(COLUMNS.CHECK_CONSTRAINT);
    return is1_4_201;
  }
}
