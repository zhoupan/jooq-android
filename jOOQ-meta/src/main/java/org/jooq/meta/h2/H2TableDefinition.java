    ...
    for (Record record :
        create()
            .select(
                            ...
                            .when(
                                COLUMNS.INTERVAL_TYPE.like(
                                    any(
                                        ...
                                        inline("%MINUTE%"),
                                        inline("%SECOND%"))),
                                inline("INTERVAL DAY TO SECOND"))
                            .else_(COLUMNS.TYPE_NAME))
                        : COLUMNS.TYPE_NAME)
                    .as(COLUMNS.TYPE_NAME),
                (((H2Database) getDatabase()).is1_4_197() ? COLUMNS.COLUMN_TYPE : COLUMNS.TYPE_NAME)
                    .as(COLUMNS.COLUMN_TYPE),
                choose()
                    .when(
                        COLUMNS.NUMERIC_PRECISION.eq(maxP).and(COLUMNS.NUMERIC_SCALE.eq(maxS)),
                        zero())
                    .otherwise(COLUMNS.CHARACTER_MAXIMUM_LENGTH)
                    .as(COLUMNS.CHARACTER_MAXIMUM_LENGTH),
                COLUMNS
                    .NUMERIC_PRECISION
                    .decode(maxP, zero(), COLUMNS.NUMERIC_PRECISION)
                ...
                ((H2Database) getDatabase()).is1_4_198()
                    ...
                    : inline("").as(COLUMNS.DOMAIN_SCHEMA),
                ((H2Database) getDatabase()).is1_4_198()
                    ? COLUMNS.DOMAIN_NAME
                    : inline("").as(COLUMNS.DOMAIN_NAME))
            .from(COLUMNS)
            .where(COLUMNS.TABLE_SCHEMA.equal(getSchema().getName()))
            .and(COLUMNS.TABLE_NAME.equal(getName()))
            .and(
                !getDatabase().getIncludeInvisibleColumns()
                    ? ((H2Database) getDatabase()).is1_4_198()
      ...
      DataTypeDefinition type =
          new DefaultDataTypeDefinition(
              ...
              isTimestamp ? 0 : record.get(COLUMNS.NUMERIC_SCALE),
