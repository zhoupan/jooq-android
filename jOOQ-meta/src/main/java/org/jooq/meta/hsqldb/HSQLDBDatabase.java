...
public class HSQLDBDatabase extends AbstractDatabase implements ResultQueryDatabase {
              ...
              for (Record column : cols) {
                indexColumns.add(
                    new DefaultIndexColumnDefinition(
                        ...
                        "D".equals(column.get(SYSTEM_INDEXINFO.ASC_OR_DESC))
                            ? SortOrder.DESC
                            : SortOrder.ASC,
  ...
  private ResultQuery<Record6<String, String, String, String, String, Integer>> keys(
      List<String> schemas, String constraintType) {
    return create()
        .select(
  ...
  protected void loadForeignKeys(DefaultRelations relations) throws SQLException {
    ...
    Result<?> result =
        create()
            ...
            .on(
                TABLE_CONSTRAINTS.CONSTRAINT_SCHEMA.eq(
                    REFERENTIAL_CONSTRAINTS.UNIQUE_CONSTRAINT_SCHEMA))
            .and(
                TABLE_CONSTRAINTS.CONSTRAINT_NAME.eq(
                    REFERENTIAL_CONSTRAINTS.UNIQUE_CONSTRAINT_NAME))
            .join(pkKcu)
      ...
      if (table != null) {
        relations.addCheckConstraint(
            ...
            new DefaultCheckConstraintDefinition(
                schema, table, record.get(constraintName), record.get(cc.CHECK_CLAUSE)));
      }
    }
  }
  ...
  public ResultQuery<
    ...
    return create()
        .select(
            ...
            decode(SEQUENCES.CYCLE_OPTION, inline("YES"), inline(true), inline(false))
                .as(SEQUENCES.CYCLE_OPTION),
        ...
        create()
            .select(
                ...
                when(VIEWS.VIEW_DEFINITION.lower().like(inline("create%")), VIEWS.VIEW_DEFINITION)
                    .else_(
                        inline("create view \"")
                            .concat(SYSTEM_TABLES.TABLE_NAME)
                            .concat("\" as ")
    ...
    for (Record record :
        create()
            .select(
                ...
                field(
                        ROUTINES.ROUTINE_DEFINITION.likeRegex(
                            ".*(?i:(\\w+\\s+)+aggregate\\s+function).*"))
                    .as("aggregate"))
            .from(ROUTINES)
