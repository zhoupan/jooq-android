...
/**
 * Postgres uses the ANSI default INFORMATION_SCHEMA, but unfortunately ships with a non-capitalised
 * version of it: <code>information_schema</code>. Hence the {@link HSQLDBDatabase} is not used
 * here.
 *
 * @author Lukas Eder
 */
public class PostgresDatabase extends AbstractDatabase implements ResultQueryDatabase {
        ...
        create()
            .select(
                ...
                array(
                        select(
                                field(
                                    "pg_get_indexdef({0}, k + 1, true)",
                                    String.class, i.INDEXRELID))
  ...
  protected void loadForeignKeys(DefaultRelations relations) throws SQLException {
    ...
    // The JDBC driver implements this correctly through the pg_catalog, although the sorting and
    // column name casing is wrong, too.
    ...
    for (Record record :
        create()
            .select(
                ...
                replace(
                        field("pg_get_constraintdef({0}.oid)", VARCHAR, pc),
                        inline("CHECK "),
                        inline(""))
                    .as(cc.CHECK_CLAUSE))
            .from(pc)
            ...
            .unionAll(
                getIncludeSystemCheckConstraints()
                    ...
                    : select(inline(""), inline(""), inline(""), inline(""))
      ...
      if (table != null) {
        relations.addCheckConstraint(
            ...
            new DefaultCheckConstraintDefinition(
                schema, table, record.get(cc.CONSTRAINT_NAME), record.get(cc.CHECK_CLAUSE)));
      }
    }
  }
  ...
  protected List<TableDefinition> getTables0() throws SQLException {
    ...
    for (Record record :
                ...
                select(
                    ...
                    .unionAll(
                        select(
                                field(
                                    "{0}::varchar",
                                    PG_NAMESPACE.NSPNAME.getDataType(), PG_NAMESPACE.NSPNAME),
                                field(
                                    "{0}::varchar",
                                    PG_CLASS.RELNAME.getDataType(), PG_CLASS.RELNAME),
                                field(
                                    "{0}::varchar",
                                    PG_CLASS.RELNAME.getDataType(), PG_CLASS.RELNAME),
                                PG_DESCRIPTION.DESCRIPTION,
                    ...
                    .unionAll(
                        tableValuedFunctions()
                            ? select(
                                ...
                                .from(ROUTINES)
                                .join(PG_NAMESPACE)
                                .on(ROUTINES.SPECIFIC_SCHEMA.eq(PG_NAMESPACE.NSPNAME))
                                .join(PG_PROC)
      ...
      switch (tableType) {
        case FUNCTION:
          result.add(
              new PostgresTableValuedFunction(
                  schema, name, record.get(ROUTINES.SPECIFIC_NAME), comment));
          break;
        case MATERIALIZED_VIEW:
          result.add(new PostgresMaterializedViewDefinition(schema, name, comment));
          break;
        default:
          PostgresTableDefinition t =
              new PostgresTableDefinition(schema, name, comment, tableType, source);
          result.add(t);
    ...
    if (is84()) {
      for (Record5<String, String, String, String, Integer> inheritance :
          create()
              .select(
                  cn.NSPNAME,
                  ct.RELNAME,
              ...
              .from(ct)
              .join(cn)
              .on(ct.RELNAMESPACE.eq(oid(cn)))
              .join(i)
              .on(i.INHRELID.eq(oid(ct)))
              .join(pt)
              .on(i.INHPARENT.eq(oid(pt)))
              .join(pn)
              .on(pt.RELNAMESPACE.eq(oid(pn)))
  ...
  public ResultQuery<
    ...
    return create()
        ...
        .select(
            ...
            nullif(SEQUENCES.MINIMUM_VALUE.cast(BIGINT), inline(1L))
                .coerce(NUMERIC)
                .as(SEQUENCES.MINIMUM_VALUE),
            nullif(
                    SEQUENCES.MAXIMUM_VALUE.cast(DECIMAL_INTEGER),
                    power(
                            cast(inline(2L), DECIMAL_INTEGER),
                            cast(SEQUENCES.NUMERIC_PRECISION.minus(inline(1L)), DECIMAL_INTEGER))
                        .minus(inline(1L)))
                .coerce(NUMERIC)
                .as(SEQUENCES.MAXIMUM_VALUE),
        ...
        .and(
            !getIncludeSystemSequences()
                ? row(SEQUENCES.SEQUENCE_SCHEMA, SEQUENCES.SEQUENCE_NAME)
                    .notIn(
                        select(
                                ...
                                COLUMNS
                                    .TABLE_NAME
                                    .concat(inline("_"))
                                    .concat(COLUMNS.COLUMN_NAME)
                                    .concat(inline("_seq")))
                            .from(COLUMNS)
                            .where(
                                COLUMNS.COLUMN_DEFAULT.eq(
                                    inline("nextval('")
                                        .concat(
                                            COLUMNS
                                                .TABLE_NAME
                ...
                : noCondition())
        .unionAll(
            is10() && getIncludeSystemSequences()
                ? select(
                        inline(null, VARCHAR),
                        ...
                        field(
                            ...
                            INTEGER,
                            PG_SEQUENCE.pgType().TYPBASETYPE,
                            PG_SEQUENCE.pgType().TYPTYPMOD),
                        inline(0),
  ...
  static final /* record */ class Identifier {
    ...
    @Override
    public int hashCode() {
      return java.util.Objects.hash(this.schema, this.name);
    }
  ...
  protected List<EnumDefinition> getEnums0() throws SQLException {
    ...
    if (exists(PG_ENUM)) {
      ...
      for (Identifier type :
          create()
              .select(PG_TYPE.pgNamespace().NSPNAME, PG_TYPE.TYPNAME)
              .from(PG_TYPE)
          ...
          create()
              ...
              .select(
                  ...
                  field(
                          ...
                          INTEGER, d.TYPBASETYPE, d.TYPTYPMOD)
                      .as(DOMAINS.CHARACTER_MAXIMUM_LENGTH),
                  field(
                          "information_schema._pg_numeric_precision({0}, {1})",
                          INTEGER, d.TYPBASETYPE, d.TYPTYPMOD)
                      .as(DOMAINS.NUMERIC_PRECISION),
                  field(
                          "information_schema._pg_numeric_scale({0}, {1})",
                          INTEGER, d.TYPBASETYPE, d.TYPTYPMOD)
                      .as(DOMAINS.NUMERIC_SCALE),
                  src)
              .from(d)
              .join(name("domains"))
  ...
  protected List<UDTDefinition> getUDTs0() throws SQLException {
    ...
    if (exists(ATTRIBUTES)) {
      for (Identifier udt :
          create()
              .selectDistinct(ATTRIBUTES.UDT_SCHEMA, ATTRIBUTES.UDT_NAME)
              .from(ATTRIBUTES)
    ...
    return create()
        ...
        .and(
            is12()
                ? condition(
                    "nameconcatoid({0}, {1}) = {2}",
                    PG_PROC.PRONAME, oid(PG_PROC), r1.SPECIFIC_NAME)
                : PG_PROC.PRONAME.concat("_").concat(oid(PG_PROC)).eq(r1.SPECIFIC_NAME))
        .where(r1.ROUTINE_SCHEMA.in(getInputSchemata()))
        .and(tableValuedFunctions() ? condition(not(PG_PROC.PRORETSET)) : noCondition())
        .and(!getIncludeTriggerRoutines() ? r1.DATA_TYPE.ne(inline("trigger")) : noCondition())
        .orderBy(r1.ROUTINE_SCHEMA.asc(), r1.ROUTINE_NAME.asc(), field(name("overload")).asc())
        .collect(
            mapping(
                r -> new PostgresRoutineDefinition(this, r),
