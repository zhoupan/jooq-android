...
public class PostgresTableDefinition extends AbstractTableDefinition {
  ...
  public PostgresTableDefinition(
  ...
  public List<ColumnDefinition> getElements0() throws SQLException {
    ...
    Field<String> dataType =
        when(
                COLUMNS.INTERVAL_TYPE.like(any(inline("%YEAR%"), inline("%MONTH%"))),
                inline("INTERVAL YEAR TO MONTH"))
            .when(
                COLUMNS.INTERVAL_TYPE.like(
                    any(inline("%DAY%"), inline("%HOUR%"), inline("%MINUTE%"), inline("%SECOND%"))),
        ...
        create()
            .select(
                ...
                nvl(
                        ...
                        when(
                            COLUMNS.UDT_NAME.in(
                                inline("_varchar"), inline("_bpchar"), inline("_char")),
                ...
                COLUMNS.IS_NULLABLE,
                (when(isIdentity, inline(null, String.class)).else_(COLUMNS.COLUMN_DEFAULT))
                    .as(COLUMNS.COLUMN_DEFAULT),
