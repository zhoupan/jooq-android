 ...
 * The DDL database.
 *
 * <p>This meta data source parses a set of SQL scripts, translates them to the H2 dialect and runs
 * them on an in-memory H2 database before reverse engineering the outcome.
 *
 * <p>The SQL scripts are located in the <code>scripts</code> scripts property available from {@link
 * #getProperties()}.
 *
 * @author Lukas Eder
 */
public class DDLDatabase extends AbstractInterpretingDatabase {
  ...
  protected void export() throws Exception {
    ...
    final String defaultNameCase =
        getProperties().getProperty("defaultNameCase", "as_is").toUpperCase();
    boolean parseIgnoreComments =
        !"false".equalsIgnoreCase(getProperties().getProperty("parseIgnoreComments"));
    String parseIgnoreCommentStart =
        getProperties()
            .getProperty("parseIgnoreCommentStart", defaultSettings.getParseIgnoreCommentStart());
    String parseIgnoreCommentStop =
        getProperties()
            .getProperty("parseIgnoreCommentStop", defaultSettings.getParseIgnoreCommentStop());
    logExecutedQueries =
        !"false".equalsIgnoreCase(getProperties().getProperty("logExecutedQueries"));
    logExecutionResults =
        !"false".equalsIgnoreCase(getProperties().getProperty("logExecutionResults"));
                ...
                new DefaultVisitListener() {
                  ...
                  public void visitStart(VisitContext vc) {
                    if (vc.queryPart() instanceof Name) {
                      ...
                      if (changed) vc.queryPart(DSL.name(parts));
                    }
                  }
                });
      }
  ...
  private void load(DSLContext ctx, Source source) {
    ...
    try {
      ...
      for (Query query : queries) {
        ...
        for (; ; ) {
          try {
            if (logExecutedQueries) log.info(query);
