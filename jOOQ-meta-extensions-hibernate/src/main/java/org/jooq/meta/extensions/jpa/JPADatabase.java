 ...
 * The JPA database.
 *
 * <p>This jOOQ-meta schema source works on an undisclosed in-memory {@link H2Database}, which is
 * constructed from a set of JPA-annotated entities using Spring and Hibernate:
 *
 * <p>
 *
 * <ul>
 *   <li>Spring discovers all the JPA-annotated entities in the comma-separated list of <code>
 *       packages</code> (configured in the code generator)
 *   <li>Those entities are passed to Hibernate's {@link SchemaExport} to generate an empty database
 *       schema in the in-memory H2 database
 *   <li>A jOOQ {@link H2Database} is used to reverse-engineer this schema again
 * </ul>
 *
 * @author Lukas Eder
 */
public class JPADatabase extends AbstractInterpretingDatabase {
  ...
  protected void export() throws Exception {
    ...
    boolean useAttributeConverters =
        Boolean.parseBoolean(
            getProperties()
                .getProperty(
                    "useAttributeConverters",
    ...
    // [#5845] Use the correct ClassLoader to load the jpa entity classes defined in the user
    // project
    ...
    if (count > 0) log.info("Entities added", "Number of entities added: " + count);
    else
      log.warn(
          "No entities added",
          (""
    ...
    if (useAttributeConverters) loadAttributeConverters(metadata.getAnnotatedClasses());
  }
  ...
  private final void loadAttributeConverters(Collection<? extends Class<?>> classes) {
    try {
      ...
      for (Entry<Name, AttributeConverter<?, ?>> entry : extractor.extract().entrySet()) {
        ...
        if (convertToEntityAttribute == null) {
          log.info(
              "AttributeConverter",
              "Cannot use AttributeConverter: " + entry.getValue().getClass().getName());
        ...
        ForcedType forcedType =
            new ForcedType()
                ...
                .withConverter(
                    String.format(
                        "new %s(%s.class)",
                        JPAConverter.class.getName(), entry.getValue().getClass().getName()));
    ...
    // AttributeConverter is part of JPA 2.1. Older JPA providers may not have this type, yet
      ...
      log.warn(
          "AttributeConverter",
          "Cannot load AttributeConverters due to missing class: " + e.getMessage());
    }
  }
}
