 ...
 * a stream containing only the red widgets, and then transform it into a stream of {@code int}
 * values representing the weight of each red widget. Then this stream is summed to produce a total
 * weight.
 *
 * <p>In addition to {@code Stream}, which is a stream of object references, there are primitive
 * specializations for {@link IntStream}, {@link LongStream}, and {@link DoubleStream}, all of which
 * are referred to as "streams" and conform to the characteristics and restrictions described here.
 *
 * <p>To perform a computation, stream <a href="package-summary.html#StreamOps">operations</a> are
 * composed into a <em>stream pipeline</em>. A stream pipeline consists of a source (which might be
 * an array, a collection, a generator function, an I/O channel, etc), zero or more <em>intermediate
 * operations</em> (which transform a stream into another stream, such as {@link
 * Stream#filter(Predicate)}), and a <em>terminal operation</em> (which produces a result or
 * side-effect, such as {@link Stream#count()} or {@link Stream#forEach(Consumer)}). Streams are
 * lazy; computation on the source data is only performed when the terminal operation is initiated,
 * and source elements are consumed only as needed.
 *
 * <p>A stream implementation is permitted significant latitude in optimizing the computation of the
 * result. For example, a stream implementation is free to elide operations (or entire stages) from
 * a stream pipeline -- and therefore elide invocation of behavioral parameters -- if it can prove
 * that it would not affect the result of the computation. This means that side-effects of
 * behavioral parameters may not always be executed and should not be relied upon, unless otherwise
 * specified (such as by the terminal operations {@code forEach} and {@code forEachOrdered}). (For a
 * specific example of such an optimization, see the API note documented on the {@link #count}
 * operation. For more detail, see the <a href="package-summary.html#SideEffects">side-effects</a>
 * section of the stream package documentation.)
 *
 * <p>Collections and streams, while bearing some superficial similarities, have different goals.
 ...
 * computational operations which will be performed in aggregate on that source. However, if the
 * provided stream operations do not offer the desired functionality, the {@link #iterator()} and
 * {@link #spliterator()} operations can be used to perform a controlled traversal.
 *
 * <p>A stream pipeline, like the "widgets" example above, can be viewed as a <em>query</em> on the
 * stream source. Unless the source was explicitly designed for concurrent modification (such as a
 * {@link java.util.concurrent.ConcurrentHashMap}), unpredictable or erroneous behavior may result
 * from modifying the stream source while it is being queried.
 *
 * <p>Most stream operations accept parameters that describe user-specified behavior, such as the
 * lambda expression {@code w -> w.getWeight()} passed to {@code mapToInt} in the example above. To
 * preserve correct behavior, these <em>behavioral parameters</em>:
 *
 * <ul>
 *   <li>must be <a href="package-summary.html#NonInterference">non-interfering</a> (they do not
 *       modify the stream source); and
 ...
 * org.java.util.function.Function}, and are often lambda expressions or method references. Unless
 * otherwise specified these parameters must be <em>non-null</em>.
 *
 * <p>A stream should be operated on (invoking an intermediate or terminal stream operation) only
 * once. This rules out, for example, "forked" streams, where the same source feeds two or more
 * pipelines, or multiple traversals of the same stream. A stream implementation may throw {@link
 * IllegalStateException} if it detects that the stream is being reused. However, since some stream
 * operations may return their receiver rather than a new stream object, it may not be possible to
 * detect reuse in all cases.
 *
 * <p>Streams have a {@link #close()} method and implement {@link AutoCloseable}, but nearly all
 * stream instances do not actually need to be closed after use. Generally, only streams whose
 * source is an IO channel (such as those returned by {@link
 * java.nio.file.Files#lines(java.nio.file.Path, java.nio.charset.Charset)}) will require closing.
 * Most streams are backed by collections, arrays, or generating functions, which require no special
 * resource management. (If a stream does require closing, it can be declared as a resource in a
 * {@code try}-with-resources statement.)
 *
 * <p>Stream pipelines may execute either sequentially or in <a
 * href="package-summary.html#Parallelism">parallel</a>. This execution mode is a property of the
 * stream. Streams are created with an initial choice of sequential or parallel execution. (For
 * example, {@link java.util.Collection#stream() Collection.stream()} creates a sequential stream,
 * and {@link java.util.Collection#parallelStream() Collection.parallelStream()} creates a parallel
 * one.) This choice of execution mode may be modified by the {@link #sequential()} or {@link
 * #parallel()} methods, and may be queried with the {@link #isParallel()} method.
 *
...
public interface Stream<T> extends BaseStream<T, Stream<T>> {
   ...
   *
   * <p><b>API Note:</b><br>
   * The {@code flatMap()} operation has the effect of applying a one-to-many transformation to the
  ...
  /**
   * Returns an {@code IntStream} consisting of the results of replacing each element of this stream
   * with the contents of a mapped stream produced by applying the provided mapping function to each
   * element. Each mapped stream is {@link org.java.util.stream.BaseStream#close() closed} after its
  ...
  /**
   * Returns an {@code LongStream} consisting of the results of replacing each element of this
   * stream with the contents of a mapped stream produced by applying the provided mapping function
   * to each element. Each mapped stream is {@link org.java.util.stream.BaseStream#close() closed}
   * after its contents have been placed into this stream. (If a mapped stream is {@code null} an
  ...
  /**
   * Returns an {@code DoubleStream} consisting of the results of replacing each element of this
   * stream with the contents of a mapped stream produced by applying the provided mapping function
   * to each element. Each mapped stream is {@link org.java.util.stream.BaseStream#close() closed}
   * after its contents have placed been into this stream. (If a mapped stream is {@code null} an
   * empty stream is used, instead.)
   *
   * <p>This is an <a href="package-summary.html#StreamOps">intermediate operation</a>.
   *
   ...
   * provided mapping function to each element in conjunction with a {@linkplain Consumer consumer}
   * argument that accepts replacement elements. The mapping function calls the consumer zero or
   * more times to provide the replacement elements.
   *
   * <p>This is an <a href="package-summary.html#StreamOps">intermediate operation</a>.
   *
   * <p>If the {@linkplain Consumer consumer} argument is used outside the scope of its application
   * to the mapping function, the results are undefined.
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation invokes {@link #flatMap flatMap} on this stream, passing a function
   * that behaves as follows. First, it calls the mapper function with a {@code Consumer} that
   * accumulates replacement elements into a newly created internal buffer. When the mapper function
   * returns, it creates a stream from the internal buffer. Finally, it returns this stream to
   * {@code flatMap}.
   *
   * <p><b>API Note:</b><br>
   * This method is similar to {@link #flatMap flatMap} in that it applies a one-to-many
   * transformation to the elements of the stream and flattens the result elements into a new
   * stream. This method is preferable to {@code flatMap} in the following circumstances:
   *
   * <ul>
   *   <li>When replacing each stream element with a small (possibly zero) number of elements. Using
   *       this method avoids the overhead of creating a new Stream instance for every group of
   *       result elements, as required by {@code flatMap}.
   *   <li>When it is easier to use an imperative approach for generating result elements than it is
   *       to return them in the form of a Stream.
   * </ul>
   *
   * <p>If a lambda expression is provided as the mapper function argument, additional type
   * information may be necessary for proper inference of the element type {@code <R>} of the
   * returned stream. This can be provided in the form of explicit type declarations for the lambda
   * parameters or as an explicit type argument to the {@code mapMulti} call.
   *
   * <p><b>Examples</b>
   *
   * <p>Given a stream of {@code Number} objects, the following produces a list containing only the
   ...
   * }</pre>
   *
   * <p>If we have an {@code Iterable<Object>} and need to recursively expand its elements that are
   * themselves of type {@code Iterable}, we can use {@code mapMulti} as follows:
   *
   * <pre>{@code
   * class C {
  ...
  /**
   * Returns an {@code IntStream} consisting of the results of replacing each element of this stream
   * with multiple elements, specifically zero or more elements. Replacement is performed by
   * applying the provided mapping function to each element in conjunction with a {@linkplain
   * IntConsumer consumer} argument that accepts replacement elements. The mapping function calls
   ...
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation invokes {@link #flatMapToInt flatMapToInt} on this stream, passing a
   * function that behaves as follows. First, it calls the mapper function with an {@code
   * IntConsumer} that accumulates replacement elements into a newly created internal buffer. When
   * the mapper function returns, it creates an {@code IntStream} from the internal buffer. Finally,
   * it returns this stream to {@code flatMapToInt}.
   *
   * @param mapper a <a href="package-summary.html#NonInterference">non-interfering</a>, <a
   *     href="package-summary.html#Statelessness">stateless</a> function that generates replacement
   *     elements
  ...
  /**
   * Returns a {@code LongStream} consisting of the results of replacing each element of this stream
   * with multiple elements, specifically zero or more elements. Replacement is performed by
   * applying the provided mapping function to each element in conjunction with a {@linkplain
   * LongConsumer consumer} argument that accepts replacement elements. The mapping function calls
   * the consumer zero or more times to provide the replacement elements.
   *
   * <p>This is an <a href="package-summary.html#StreamOps">intermediate operation</a>.
   *
   * <p>If the {@linkplain LongConsumer consumer} argument is used outside the scope of its
   * application to the mapping function, the results are undefined.
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation invokes {@link #flatMapToLong flatMapToLong} on this stream, passing
   * a function that behaves as follows. First, it calls the mapper function with a {@code
   * LongConsumer} that accumulates replacement elements into a newly created internal buffer. When
   * the mapper function returns, it creates a {@code LongStream} from the internal buffer. Finally,
   * it returns this stream to {@code flatMapToLong}.
   *
   * @param mapper a <a href="package-summary.html#NonInterference">non-interfering</a>, <a
   *     href="package-summary.html#Statelessness">stateless</a> function that generates replacement
   *     elements
  ...
  /**
   * Returns a {@code DoubleStream} consisting of the results of replacing each element of this
   * stream with multiple elements, specifically zero or more elements. Replacement is performed by
   * applying the provided mapping function to each element in conjunction with a {@linkplain
   * DoubleConsumer consumer} argument that accepts replacement elements. The mapping function calls
   * the consumer zero or more times to provide the replacement elements.
   *
   * <p>This is an <a href="package-summary.html#StreamOps">intermediate operation</a>.
   *
   * <p>If the {@linkplain DoubleConsumer consumer} argument is used outside the scope of its
   * application to the mapping function, the results are undefined.
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation invokes {@link #flatMapToDouble flatMapToDouble} on this stream,
   * passing a function that behaves as follows. First, it calls the mapper function with an {@code
   * DoubleConsumer} that accumulates replacement elements into a newly created internal buffer.
  ...
  /**
   * Returns a stream consisting of the distinct elements (according to {@link
   * Object#equals(Object)}) of this stream.
   *
   * <p>For ordered streams, the selection of distinct elements is stable (for duplicated elements,
   ...
   *
   * <p><b>API Note:</b><br>
   * Preserving stability for {@code distinct()} in parallel pipelines is relatively expensive
   * (requires that the operation act as a full barrier, with substantial buffering overhead), and
   * stability is often not needed. Using an unordered stream source (such as {@link
   * RefStreams#generate(Supplier)}) or removing the ordering constraint with {@link #unordered()}
   * may result in significantly more efficient execution for {@code distinct()} in parallel
   * pipelines, if the semantics of your situation permit. If consistency with encounter order is
   * required, and you are experiencing poor performance or memory utilization with {@code
   * distinct()} in parallel pipelines, switching to sequential execution with {@link #sequential()}
   ...
   * If the elements of this stream are not {@code Comparable}, a {@code
   * java.lang.ClassCastException} may be thrown when the terminal operation is executed.
   *
   * <p>For ordered streams, the sort is stable. For unordered streams, no stability guarantees are
   * made.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">stateful intermediate operation</a>.
   *
   ...
   * {@link RefStreams#generate(Supplier)}) or removing the ordering constraint with {@link
   ...
   * unordered stream source (such as {@link RefStreams#generate(Supplier)}) or removing the
   ...
   *
   * <p>One use-case for {@code takeWhile} is executing a stream pipeline for a certain duration.
   ...
   *
   * <p>The behavior of this operation is explicitly nondeterministic. For parallel stream
   * pipelines, this operation does <em>not</em> guarantee to respect the encounter order of the
   * stream, as doing so would sacrifice the benefit of parallelism. For any given element, the
   * action may be performed at whatever time and in whatever thread the library chooses. If the
   * action accesses shared state, it is responsible for providing the required synchronization.
   *
   * @param action a <a href="package-summary.html#NonInterference">non-interfering</a> action to
   ...
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   * <p>This operation processes the elements one at a time, in encounter order if one exists.
   * Performing the action for one element <a
   * href="../concurrent/package-summary.html#MemoryVisibility"><i>happens-before</i></a> performing
   * the action for subsequent elements, but for any given element, the action may be performed in
  ...
  /**
   * Returns an array containing the elements of this stream, using the provided {@code generator}
   * function to allocate the returned array, as well as any additional arrays that might be
   * required for a partitioned execution or for resizing.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   * <p><b>API Note:</b><br>
   * The generator function takes an integer, which is the size of the desired array, and produces
   ...
   * for all {@code t}, {@code accumulator.apply(identity, t)} is equal to {@code t}. The {@code
   ...
   * but is not constrained to execute sequentially.
   *
   * <p>The {@code identity} value must be an identity for the combiner function. This means that
   * for all {@code u}, {@code combiner(identity, u)} is equal to {@code u}. Additionally, the
   ...
   * {@code map} and {@code reduce} operations. The {@code accumulator} function acts as a fused
   * mapper and accumulator, which can sometimes be more efficient than separate mapping and
   ...
   * }</pre>
   *
   * <p>Like {@link #reduce(Object, BinaryOperator)}, {@code collect} operations can be parallelized
   * without requiring additional synchronization.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   * <p><b>API Note:</b><br>
   * There are many existing classes in the JDK whose signatures are well-suited for use with method
   ...
   *
   * <p>If the stream is parallel, and the {@code Collector} is {@link
   * Collector.Characteristics#CONCURRENT concurrent}, and either the stream is unordered or the
   * collector is {@link Collector.Characteristics#UNORDERED unordered}, then a concurrent reduction
   * will be performed (see {@link Collector} for details on concurrent reduction.)
   *
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   * <p>When executed in parallel, multiple intermediate results may be instantiated, populated, and
   ...
   * href="../../lang/package-summary.html#Value-based-Classes">value-based</a>. Callers should make
   * no assumptions about the identity of the returned instances. Identity-sensitive operations on
   * these instances (reference equality ({@code ==}), identity hash code, and synchronization) are
   * unreliable and should be avoided.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   * <p><b>API Note:</b><br>
   * If more control over the returned object is required, use {@link
   ...
   * Most instances of Stream will override this method and provide an implementation that is highly
   * optimized compared to the default implementation.
   *
   ...
   *
   * @param comparator a <a href="package-summary.html#NonInterference">non-interfering</a>, <a
   *     href="package-summary.html#Statelessness">stateless</a> {@code Comparator} to compare
   *     elements of this stream
   ...
   * (as may be the case for {@code flatMap} or {@code filter} operations). Thus the count is the
   * size of the {@code List} and there is no need to execute the pipeline and, as a side-effect,
  ...
  /**
   * Returns an {@link Optional} describing the first element of this stream, or an empty {@code
   * Optional} if the stream is empty. If the stream has no encounter order, then any element may be
   * returned.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">short-circuiting terminal operation</a>.
   *
  ...
  /**
   * A mutable builder for a {@code Stream}. This allows the creation of a {@code Stream} by
   * generating elements individually and adding them to the {@code Builder} (without the copying
   ...
   * built phase begins when the {@link #build()} method is called, which creates an ordered {@code
   * Stream} whose elements are the elements that were added to the stream builder, in the order
   * they were added.
   *
