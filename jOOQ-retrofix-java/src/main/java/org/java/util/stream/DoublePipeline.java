...
/**
 * Abstract base class for an intermediate pipeline stage or pipeline source stage implementing
 * whose elements are of type {@code double}.
 *
...
abstract class DoublePipeline<E_IN> extends AbstractPipeline<E_IN, Double, DoubleStream>
   ...
   */
  DoublePipeline(
      Supplier<? extends Spliterator<Double>> source, int sourceFlags, boolean parallel) {
   ...
   * The implementation attempts to cast to a Spliterator.OfDouble, and throws an exception if this
  ...
  @Override
  final <P_IN> Node<Double> evaluateToNode(
  ...
  @Override
  final <P_IN> Spliterator<Double> wrap(
      PipelineHelper<Double> ph, Supplier<Spliterator<P_IN>> supplier, boolean isParallel) {
  ...
  @Override
  final Node.Builder<Double> makeNodeBuilder(
      long exactSizeIfKnown, IntFunction<Double[]> generator) {
  ...
  public final DoubleStream map(DoubleUnaryOperator mapper) {
    ...
    return new StatelessOp<Double>(
        this, StreamShape.DOUBLE_VALUE, StreamOpFlag.NOT_SORTED | StreamOpFlag.NOT_DISTINCT) {
  ...
  public final IntStream mapToInt(DoubleToIntFunction mapper) {
    Objects.requireNonNull(mapper);
    return new IntPipeline.StatelessOp<Double>(
        this, StreamShape.DOUBLE_VALUE, StreamOpFlag.NOT_SORTED | StreamOpFlag.NOT_DISTINCT) {
  ...
  public final LongStream mapToLong(DoubleToLongFunction mapper) {
    Objects.requireNonNull(mapper);
    return new LongPipeline.StatelessOp<Double>(
        this, StreamShape.DOUBLE_VALUE, StreamOpFlag.NOT_SORTED | StreamOpFlag.NOT_DISTINCT) {
              ...
              if (result != null) {
                if (!cancellationRequested) {
                  ...
                  do {} while (!downstream.cancellationRequested()
                      && s.tryAdvance(downstreamAsDouble));
                }
              }
            } finally {
              if (result != null) {
  ...
  abstract static class StatelessOp<E_IN> extends DoublePipeline<E_IN> {
    ...
    /**
     * Construct a new DoubleStream by appending a stateless intermediate operation to an existing
     * stream.
     *
  ...
  abstract static class StatefulOp<E_IN> extends DoublePipeline<E_IN> {
    ...
    /**
     * Construct a new DoubleStream by appending a stateful intermediate operation to an existing
     * stream.
     *
     * @param upstream the upstream pipeline stage
     * @param inputShape the stream shape for the upstream pipeline stage
