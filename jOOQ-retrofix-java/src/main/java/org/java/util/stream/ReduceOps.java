...
import org.java.util.OptionalLong;
...
import org.java.util.function.ObjLongConsumer;
import org.java.util.function.Supplier;

/**
 * Factory for creating instances of {@code TerminalOp} that implement reductions.
 *
 * @since 1.8
 */
final class ReduceOps {
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a functional reduce on reference values.
   *
   ...
   */
  public static <T, U> TerminalOp<T, U> makeRef(
      U seed, BiFunction<U, ? super T, U> reducer, BinaryOperator<U> combiner) {
  ...
  public static <T, R> TerminalOp<T, R> makeRef(
      Supplier<R> seedFactory, BiConsumer<R, ? super T> accumulator, BiConsumer<R, R> reducer) {
  ...
  /**
   * Constructs a {@code TerminalOp} that counts the number of stream elements. If the size of the
   * pipeline is known then count is the size and there is no need to evaluate the pipeline. If the
   * size of the pipeline is non known then count is produced, via reduction, using a {@link
   * CountingSink}.
   *
   * @param <T> the type of the input elements
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a functional reduce on {@code int} values.
   *
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a mutable reduce on {@code int} values.
   *
   * @param <R> The type of the result
   ...
   * @return A {@code ReduceOp} implementing the reduction
   */
  public static <R> TerminalOp<Integer, R> makeInt(
      Supplier<R> supplier, ObjIntConsumer<R> accumulator, BinaryOperator<R> combiner) {
  ...
  public static TerminalOp<Integer, Long> makeIntCounting() {
    return new ReduceOp<Integer, Long, CountingSink<Integer>>(StreamShape.INT_VALUE) {
      ...
      @Override
      public <P_IN> Long evaluateParallel(
          PipelineHelper<Integer> helper, Spliterator<P_IN> spliterator) {
        long size = helper.exactOutputSizeIfKnown(spliterator);
        if (size != -1L) return size;
        return super.evaluateParallel(helper, spliterator);
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a functional reduce on {@code long} values.
   *
   * @param identity the identity for the combining function
   * @param operator the combining function
   * @return a {@code TerminalOp} implementing the reduction
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a functional reduce on {@code long} values,
   * producing an optional long result.
   *
   * @param operator the combining function
   * @return a {@code TerminalOp} implementing the reduction
   */
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a mutable reduce on {@code long} values.
   *
   * @param <R> the type of the result
   * @param supplier a factory to produce a new accumulator of the result type
   * @param accumulator a function to incorporate an int into an accumulator
   ...
   * @return a {@code TerminalOp} implementing the reduction
   */
  public static <R> TerminalOp<Long, R> makeLong(
      Supplier<R> supplier, ObjLongConsumer<R> accumulator, BinaryOperator<R> combiner) {
    Objects.requireNonNull(supplier);
    Objects.requireNonNull(accumulator);
    Objects.requireNonNull(combiner);
    class ReducingSink extends Box<R>
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a functional reduce on {@code double} values.
   *
   * @param identity the identity for the combining function
   * @param operator the combining function
   * @return a {@code TerminalOp} implementing the reduction
   */
  public static TerminalOp<Double, Double> makeDouble(
  ...
  /**
   * Constructs a {@code TerminalOp} that implements a mutable reduce on {@code double} values.
   *
   * @param <R> the type of the result
   * @param supplier a factory to produce a new accumulator of the result type
   * @param accumulator a function to incorporate an int into an accumulator
   ...
   * @return a {@code TerminalOp} implementing the reduction
   */
  public static <R> TerminalOp<Double, R> makeDouble(
      Supplier<R> supplier, ObjDoubleConsumer<R> accumulator, BinaryOperator<R> combiner) {
    Objects.requireNonNull(supplier);
    Objects.requireNonNull(accumulator);
    Objects.requireNonNull(combiner);
    class ReducingSink extends Box<R>
  ...
  abstract static class CountingSink<T> extends Box<Long>
  ...
  /**
   * A type of {@code TerminalSink} that implements an associative reducing operation on elements of
   * type {@code T} and producing a result of type {@code R}.
   *
  ...
  /**
   * State box for a single state element, used as a base class for {@code AccumulatingSink}
   * instances
   *
  ...
  /**
   * A {@code TerminalOp} that evaluates a stream pipeline and sends the output into an {@code
   * AccumulatingSink}, which performs a reduce operation. The {@code AccumulatingSink} must
   * represent an associative reducing operation.
   *
  ...
  private abstract static class ReduceOp<T, R, S extends AccumulatingSink<T, R, S>>
    ...
    /**
     * Create a {@code ReduceOp} of the specified stream shape which uses the specified {@code
     * Supplier} to create accumulating sinks.
     *
  ...
  private static final class ReduceTask<P_IN, P_OUT, R, S extends AccumulatingSink<P_OUT, R, S>>
    ...
    ReduceTask(
        ReduceOp<P_OUT, R, S> op, PipelineHelper<P_OUT> helper, Spliterator<P_IN> spliterator) {
