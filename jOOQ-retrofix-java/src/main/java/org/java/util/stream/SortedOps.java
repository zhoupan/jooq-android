...
import org.java.lang.Iterables;
import org.java.util.Comparators;
...
final class SortedOps {
   ...
   */
  static <T> Stream<T> makeRef(
      AbstractPipeline<?, T, ?> upstream, Comparator<? super T> comparator) {
  ...
  private static final class OfRef<T> extends ReferencePipeline.StatefulOp<T, T> {
    ...
    /** Sort using natural order of {@literal <T>} which must be {@code Comparable}. */
    ...
    public Sink<T> opWrapSink(int flags, Sink<T> sink) {
      ...
      if (StreamOpFlag.SORTED.isKnown(flags) && isNaturalSort) return sink;
    ...
    @Override
    public <P_IN> Node<T> opEvaluateParallel(
      ...
      if (StreamOpFlag.SORTED.isKnown(helper.getStreamAndOpFlags()) && isNaturalSort) {
        ...
        org.java.util.J8Arrays.parallelSort(flattenedData, comparator);
  ...
  private static final class OfInt extends IntPipeline.StatefulOp<Integer> {
    ...
    public Sink<Integer> opWrapSink(int flags, Sink<Integer> sink) {
      Objects.requireNonNull(sink);
      if (StreamOpFlag.SORTED.isKnown(flags)) return sink;
      else if (StreamOpFlag.SIZED.isKnown(flags)) return new SizedIntSortingSink(sink);
      else return new IntSortingSink(sink);
    }
    ...
    public <P_IN> Node<Integer> opEvaluateParallel(
      ...
      if (StreamOpFlag.SORTED.isKnown(helper.getStreamAndOpFlags())) {
        ...
        org.java.util.J8Arrays.parallelSort(content);
  ...
  private static final class OfLong extends LongPipeline.StatefulOp<Long> {
    ...
    public Sink<Long> opWrapSink(int flags, Sink<Long> sink) {
      Objects.requireNonNull(sink);
      if (StreamOpFlag.SORTED.isKnown(flags)) return sink;
      else if (StreamOpFlag.SIZED.isKnown(flags)) return new SizedLongSortingSink(sink);
      else return new LongSortingSink(sink);
    }

    @Override
    public <P_IN> Node<Long> opEvaluateParallel(
        PipelineHelper<Long> helper, Spliterator<P_IN> spliterator, IntFunction<Long[]> generator) {
      if (StreamOpFlag.SORTED.isKnown(helper.getStreamAndOpFlags())) {
        return helper.evaluate(spliterator, false, generator);
      } else {
  ...
  private static final class OfDouble extends DoublePipeline.StatefulOp<Double> {
    ...
    public Sink<Double> opWrapSink(int flags, Sink<Double> sink) {
      Objects.requireNonNull(sink);
      if (StreamOpFlag.SORTED.isKnown(flags)) return sink;
      else if (StreamOpFlag.SIZED.isKnown(flags)) return new SizedDoubleSortingSink(sink);
      else return new DoubleSortingSink(sink);
    }
   ...
   *
   * <p>Note: documentation below applies to reference and all primitive sinks.
   *
   * <p>Sorting sinks first accept all elements, buffering then into an array or a re-sizable data
   * structure, if the size of the pipeline is known or unknown respectively. At the end of the sink
   * protocol those elements are sorted and then pushed downstream. This class records if {@link
   * #cancellationRequested} is called. If so it can be inferred that the source pushing source
   * elements into the pipeline knows that the pipeline is short-circuiting. In such cases
   * sub-classes pushing elements downstream will preserve the short-circuiting protocol by calling
   * {@code downstream.cancellationRequested()} and checking the result is {@code false} before an
   * element is pushed.
   *
   * <p>Note that the above behaviour is an optimization for sorting with sequential streams. It is
   * not an error that more elements, than strictly required to produce a result, may flow through
   * the pipeline. This can occur, in general (not restricted to just sorting), for short-circuiting
   * parallel pipelines.
   */
  private abstract static class AbstractRefSortingSink<T> extends Sink.ChainedReference<T, T> {
    ...
    /**
     * Records is cancellation is requested so short-circuiting behavior can be preserved when the
     * sorted elements are pushed downstream.
     *
