...
import org.java.util.function.LongFunction;

/**
 * Factory methods for constructing implementations of {@link Node} and {@link Node.Builder} and
 * their primitive specializations. Fork/Join tasks for collecting output from a {@link
 * PipelineHelper} to a {@link Node} and flattening {@link Node}s.
 *
 * @since 1.8
 */
final class Nodes {
  ...
  static final class OfPrimitive {
    ...
    static <
            ...
            T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>>
        T[] asArray(
            Node.OfPrimitive<T, T_CONS, T_ARR, T_SPLITR, T_NODE> this_,
  ...
  static final class OfDouble {
    ...
    static Node.OfDouble truncate(
        Node.OfDouble this_, long from, long to, IntFunction<Double[]> generator) {
      if (from == 0 && to == this_.count()) return this_;
      long size = to - from;
     ...
     *
     * <p><b>Implementation Requirements:</b><br>
     * the default implementation invokes {@link Node.OfDouble#asPrimitiveArray()} to obtain a
     * double[] array then and copies the elements from that double[] array into the boxed Double[]
     * array. This is not efficient and it is recommended to invoke {@link #copyInto(Object, int)}.
     */
  ...
  static final class OfLong {
    ...
    static Node.OfLong truncate(
        Node.OfLong this_, long from, long to, IntFunction<Long[]> generator) {
      if (from == 0 && to == this_.count()) return this_;
      long size = to - from;
     ...
     *
     * <p><b>Implementation Requirements:</b><br>
     * the default implementation invokes {@link Node.OfLong#asPrimitiveArray()} to obtain a long[]
  ...
  static final class OfInt {
    ...
    static Node.OfInt truncate(
        Node.OfInt this_, long from, long to, IntFunction<Integer[]> generator) {
      if (from == 0 && to == this_.count()) return this_;
      long size = to - from;
     ...
     *
     * <p><b>Implementation Requirements:</b><br>
     * the default implementation invokes {@link Node.OfInt#asPrimitiveArray()} to obtain an int[]
     * array then and copies the elements from that int[] array into the boxed Integer[] array. This
     * is not efficient and it is recommended to invoke {@link #copyInto(Object, int)}.
     */
   ...
   *
   * <p>The count of the concatenated node is equal to the sum of the count of each child. Traversal
   * of the concatenated node traverses the content of each child in encounter order of the list of
   * children. Splitting a spliterator obtained from the concatenated node preserves the encounter
   * order of the list of children.
   *
   * <p>The result may be a concatenated node, the input sole node if the size of the list is 1, or
   * an empty node.
   *
   ...
   *
   * <p>The node will hold a reference to the collection and will not make a copy.
   *
   * @param <T> the type of elements held by the node
   ...
   *     capacity desired. A fixed capacity builder will fail if the wrong number of elements are
  ...
  /**
   * Collect, in parallel, elements output from a pipeline and describe those elements with a {@link
   * Node}.
   *
   * <p><b>Implementation Requirements:</b><br>
   ...
   * output elements can be placed into the array concurrently by leaf tasks at the correct offsets.
   * If the exact size is not known, output elements are collected into a conc-node whose shape
   * mirrors that of the computation. This conc-node can then be flattened in parallel to produce a
   * flat {@code Node} if desired.
   *
   ...
   * If the exact size is not known, output elements are collected into a conc-node whose shape
   * mirrors that of the computation. This conc-node can then be flattened in parallel to produce a
   * flat {@code Node.OfInt} if desired.
   *
   ...
   */
  public static <P_IN> Node.OfInt collectInt(
      PipelineHelper<Integer> helper, Spliterator<P_IN> spliterator, boolean flattenTree) {
    long size = helper.exactOutputSizeIfKnown(spliterator);
    if (size >= 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
      if (size >= MAX_ARRAY_SIZE) throw new IllegalArgumentException(BAD_SIZE);
   ...
   * If the exact size is not known, output elements are collected into a conc-node whose shape
   * mirrors that of the computation. This conc-node can then be flattened in parallel to produce a
   * flat {@code Node.OfLong} if desired.
   *
   * @param <P_IN> the type of elements from the source Spliterator
   * @param helper the pipeline helper describing the pipeline
   * @param flattenTree whether a conc node should be flattened into a node describing an array
   ...
   */
  public static <P_IN> Node.OfLong collectLong(
      PipelineHelper<Long> helper, Spliterator<P_IN> spliterator, boolean flattenTree) {
    long size = helper.exactOutputSizeIfKnown(spliterator);
    if (size >= 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
      if (size >= MAX_ARRAY_SIZE) throw new IllegalArgumentException(BAD_SIZE);
   ...
   * If the exact size is not known, output elements are collected into a conc-node whose shape
   * mirrors that of the computation. This conc-node can then be flattened in parallel to produce a
   * flat {@code Node.OfDouble} if desired.
   *
   * @param <P_IN> the type of elements from the source Spliterator
   * @param helper the pipeline helper describing the pipeline
   * @param flattenTree whether a conc node should be flattened into a node describing an array
  ...
  public static <P_IN> Node.OfDouble collectDouble(
      PipelineHelper<Double> helper, Spliterator<P_IN> spliterator, boolean flattenTree) {
    long size = helper.exactOutputSizeIfKnown(spliterator);
    if (size >= 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
      if (size >= MAX_ARRAY_SIZE) throw new IllegalArgumentException(BAD_SIZE);
  ...
  /**
   * Flatten, in parallel, a {@link Node}. A flattened node is one that has no children. If the node
   * is already flat, it is simply returned.
   *
   * <p><b>Implementation Requirements:</b><br>
   * If a new node is to be created, the generator is used to create an array whose length is {@link
  ...
  /**
   * Flatten, in parallel, a {@link Node.OfInt}. A flattened node is one that has no children. If
   * the node is already flat, it is simply returned.
   *
   * <p><b>Implementation Requirements:</b><br>
   * If a new node is to be created, a new int[] array is created whose length is {@link
   * Node#count()}. Then the node tree is traversed and leaf node elements are placed in the array
   * concurrently by leaf tasks at the correct offsets.
   *
   * @param node the node to flatten
  ...
  /**
   * Flatten, in parallel, a {@link Node.OfLong}. A flattened node is one that has no children. If
   * the node is already flat, it is simply returned.
   *
   * <p><b>Implementation Requirements:</b><br>
   * If a new node is to be created, a new long[] array is created whose length is {@link
   * Node#count()}. Then the node tree is traversed and leaf node elements are placed in the array
   * concurrently by leaf tasks at the correct offsets.
   *
   * @param node the node to flatten
  ...
  /**
   * Flatten, in parallel, a {@link Node.OfDouble}. A flattened node is one that has no children. If
   * the node is already flat, it is simply returned.
   *
   * <p><b>Implementation Requirements:</b><br>
   * If a new node is to be created, a new double[] array is created whose length is {@link
   * Node#count()}. Then the node tree is traversed and leaf node elements are placed in the array
   * concurrently by leaf tasks at the correct offsets.
   *
   * @param node the node to flatten
  ...
  private static class ArrayNode<T> implements Node<T> {
    ...
    @Override
    public Spliterator<T> spliterator() {
      return org.java.util.J8Arrays.spliterator(array, 0, curSize);
    }
  ...
  static final class ConcNode<T> extends AbstractConcNode<T, Node<T>> implements Node<T> {
    ...
    public Node<T> truncate(long from, long to, IntFunction<T[]> generator) {
      ...
      if (from >= leftCount) return right.truncate(from - leftCount, to - leftCount, generator);
      else if (to <= leftCount) return left.truncate(from, to, generator);
      else {
        return // "lgtm[java/subtle-inherited-call]"
        Nodes. // "lgtm[java/subtle-inherited-call]"
            conc( // "lgtm[java/subtle-inherited-call]"
            getShape(), // "lgtm[java/subtle-inherited-call]"
            left.truncate(from, leftCount, generator),
    ...
    private abstract static class OfPrimitive<
            E,
            T_CONS,
            T_ARR,
      ...
      @Override
      public String toString() {
        if (count() < 32) return String.format("%s[%s.%s]", this.getClass().getName(), left, right);
        else return String.format("%s[size=%d]", this.getClass().getName(), count());
      }
    }
    ...
    static final class OfLong
      ...
      @Override
      public org.java.util.stream.Node.OfLong truncate(
          long from, long to, IntFunction<Long[]> generator) {
        return Nodes.OfLong.truncate(this, from, to, generator);
      }
    ...
    static final class OfDouble
        extends ConcNode.OfPrimitive<
            Double, DoubleConsumer, double[], Spliterator.OfDouble, Node.OfDouble>
        implements Node.OfDouble {
  ...
  private abstract static class InternalNodeSpliterator<
          T, S extends Spliterator<T>, N extends Node<T>>
    ...
    /**
     * Initiate a stack containing, in left-to-right order, the child nodes covered by this
     * spliterator
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Depth first search, in left-to-right order, of the node tree, using an explicit stack, to
    ...
    protected final boolean initTryAdvance() {
      ...
      if (tryAdvanceSpliterator == null) {
        if (lastNodeSpliterator == null) {
          ...
          if (leaf != null) tryAdvanceSpliterator = (S) leaf.spliterator();
          else {
    ...
    public final S trySplit() {
      if (curNode == null || tryAdvanceSpliterator != null)
        ...
        return null;
      else if (lastNodeSpliterator != null) return (S) lastNodeSpliterator.trySplit();
    ...
    public final long estimateSize() {
      ...
      if (lastNodeSpliterator != null) return lastNodeSpliterator.estimateSize();
      else {
        long size = 0;
  ...
  private static final class FixedNodeBuilder<T> extends ArrayNode<T> implements Node.Builder<T> {
    ...
    public String toString() {
      return String.format(
          "FixedNodeBuilder[%d][%s]", array.length - curSize, Arrays.toString(array));
    }
  }
  ...
  private static class IntArrayNode implements Node.OfInt {
    ...
    @Override
    public OfInt truncate(long from, long to, IntFunction<Integer[]> generator) {
      return Nodes.OfInt.truncate(this, from, to, generator);
    }
  ...
  private static class LongArrayNode implements Node.OfLong {
    ...
    @Override
    public org.java.util.stream.Node.OfLong getChild(int i) {
      return Nodes.OfPrimitive.getChild();
    }
  ...
  private static class DoubleArrayNode implements Node.OfDouble {
    ...
    @Override
    public OfDouble truncate(long from, long to, IntFunction<Double[]> generator) {
      return Nodes.OfDouble.truncate(this, from, to, generator);
    }
    ...
    public String toString() {
      return String.format(
          "DoubleArrayNode[%d][%s]", array.length - curSize, Arrays.toString(array));
    }
  }
  ...
  private static final class IntFixedNodeBuilder extends IntArrayNode
    ...
    public String toString() {
      return String.format(
          "IntFixedNodeBuilder[%d][%s]", array.length - curSize, Arrays.toString(array));
    }
  }
  ...
  private static final class LongFixedNodeBuilder extends LongArrayNode
    ...
    public String toString() {
      return String.format(
          "LongFixedNodeBuilder[%d][%s]", array.length - curSize, Arrays.toString(array));
    }
  }
  ...
  private static final class DoubleFixedNodeBuilder extends DoubleArrayNode
    ...
    public String toString() {
      return String.format(
          "DoubleFixedNodeBuilder[%d][%s]", array.length - curSize, Arrays.toString(array));
    }
  }
  ...
  private static final class IntSpinedNodeBuilder extends SpinedBuffer.OfInt
    ...
    @Override
    public org.java.util.stream.Node.OfInt getChild(int i) {
      return Nodes.OfPrimitive.getChild();
    }
  }
  ...
  private static final class DoubleSpinedNodeBuilder extends SpinedBuffer.OfDouble
    ...
    @Override
    public org.java.util.stream.Node.OfDouble getChild(int i) {
      return Nodes.OfPrimitive.getChild();
    }
  ...
  private abstract static class SizedCollectorTask<
          P_IN,
          P_OUT,
          T_SINK extends Sink<P_OUT>,
    ...
    SizedCollectorTask(
        Spliterator<P_IN> spliterator, PipelineHelper<P_OUT> helper, int arrayLength) {
    ...
    SizedCollectorTask(
        K parent, Spliterator<P_IN> spliterator, long offset, long length, int arrayLength) {
      super(parent);
      this.spliterator = spliterator;
    ...
    static final class OfInt<P_IN>
        extends SizedCollectorTask<P_IN, Integer, Sink.OfInt, OfInt<P_IN>> implements Sink.OfInt {
      ...
      OfInt(
          SizedCollectorTask.OfInt<P_IN> parent,
          Spliterator<P_IN> spliterator,
          long offset,
          long length) {
        super(parent, spliterator, offset, length, parent.array.length);
    ...
    static final class OfLong<P_IN>
        extends SizedCollectorTask<P_IN, Long, Sink.OfLong, OfLong<P_IN>> implements Sink.OfLong {
      ...
      OfLong(
          SizedCollectorTask.OfLong<P_IN> parent,
          Spliterator<P_IN> spliterator,
          long offset,
          long length) {
        super(parent, spliterator, offset, length, parent.array.length);
    ...
    static final class OfDouble<P_IN>
      ...
      OfDouble(
          SizedCollectorTask.OfDouble<P_IN> parent,
          Spliterator<P_IN> spliterator,
          long offset,
          long length) {
        super(parent, spliterator, offset, length, parent.array.length);
  ...
  @SuppressWarnings("serial")
  abstract static class ToArrayTask<T, T_NODE extends Node<T>, K extends ToArrayTask<T, T_NODE, K>>
    ...
    private static class OfPrimitive<
            T,
            T_CONS,
            T_ARR,
            T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>,
      ...
      private OfPrimitive(
          OfPrimitive<T, T_CONS, T_ARR, T_SPLITR, T_NODE> parent, T_NODE node, int offset) {
        super(parent, node, offset);
        this.array = parent.array;
      }
  ...
  private static class CollectorTask<
          P_IN, P_OUT, T_NODE extends Node<P_OUT>, T_BUILDER extends Node.Builder<P_OUT>>
    ...
    CollectorTask(
        CollectorTask<P_IN, P_OUT, T_NODE, T_BUILDER> parent, Spliterator<P_IN> spliterator) {
