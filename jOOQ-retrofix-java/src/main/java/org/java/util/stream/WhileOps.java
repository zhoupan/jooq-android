...
/**
 * Factory for instances of a takeWhile and dropWhile operations that produce subsequences of their
 * input stream.
 *
 * @since 9
 */
final class WhileOps {
   ...
   */
  static <T> Stream<T> makeTakeWhileRef(
      AbstractPipeline<?, T, ?> upstream, Predicate<? super T> predicate) {
  ...
  static IntStream makeTakeWhileInt(
      AbstractPipeline<?, Integer, ?> upstream, IntPredicate predicate) {
    Objects.requireNonNull(predicate);
    return new IntPipeline.StatefulOp<Integer>(upstream, StreamShape.INT_VALUE, TAKE_FLAGS) {
      ...
      <P_IN> Spliterator<Integer> opEvaluateParallelLazy(
        ...
        if (StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
          return opEvaluateParallel(helper, spliterator, INT_ARR_GEN).spliterator();
        } else {
   ...
   * @param predicate the predicate that returns false to halt taking.
   */
  static LongStream makeTakeWhileLong(
      AbstractPipeline<?, Long, ?> upstream, LongPredicate predicate) {
    Objects.requireNonNull(predicate);
    return new LongPipeline.StatefulOp<Long>(upstream, StreamShape.LONG_VALUE, TAKE_FLAGS) {
      ...
      <P_IN> Spliterator<Long> opEvaluateParallelLazy(
        ...
        if (StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
          return opEvaluateParallel(helper, spliterator, LONG_ARR_GEN).spliterator();
        } else {
      ...
      @Override
      <P_IN> Node<Long> opEvaluateParallel(
   ...
   * @param predicate the predicate that returns false to halt taking.
   */
  static DoubleStream makeTakeWhileDouble(
      AbstractPipeline<?, Double, ?> upstream, DoublePredicate predicate) {
    Objects.requireNonNull(predicate);
    return new DoublePipeline.StatefulOp<Double>(upstream, StreamShape.DOUBLE_VALUE, TAKE_FLAGS) {
      ...
      <P_IN> Spliterator<Double> opEvaluateParallelLazy(
        ...
        if (StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
          return opEvaluateParallel(helper, spliterator, DOUBLE_ARR_GEN).spliterator();
        } else {
  ...
  /**
   * A specialization for the dropWhile operation that controls if elements to be dropped are
   * counted and passed downstream.
   *
   * <p>This specialization is utilized by the {@link TakeWhileTask} for pipelines that are ordered.
   * In such cases elements cannot be dropped until all elements have been collected.
   *
   * @param <T> the type of both input and output elements
   */
  interface DropWhileOp<T> {
    ...
    /**
     * Accepts a {@code Sink} which will receive the results of this dropWhile operation, and return
     ...
     * @param retainAndCountDroppedElements true if elements to be dropped are counted and passed to
     *     the sink, otherwise such elements are actually dropped and not passed to the sink.
  ...
  interface DropWhileSink<T> extends Sink<T> {
    ...
    /**
     * @return the could of elements that would have been dropped and instead were passed
     *     downstream.
     */
    long getDropCount();
  }
   ...
   */
  static <T> Stream<T> makeDropWhileRef(
      AbstractPipeline<?, T, ?> upstream, Predicate<? super T> predicate) {
    Objects.requireNonNull(predicate);
      ...
      public DropWhileSink<T> opWrapSink(Sink<T> sink, boolean retainAndCountDroppedElements) {
        class OpSink extends Sink.ChainedReference<T, T> implements DropWhileSink<T> {
          ...
          public void accept(T t) {
            ...
            if (retainAndCountDroppedElements && !takeElement) dropCount++;
            // If ordered need to process element, otherwise
            // skip if element is dropped
            if (retainAndCountDroppedElements || takeElement) downstream.accept(t);
          }
   ...
   * @param predicate the predicate that returns false to halt dropping.
   */
  static IntStream makeDropWhileInt(
      AbstractPipeline<?, Integer, ?> upstream, IntPredicate predicate) {
    Objects.requireNonNull(predicate);
    class Op extends IntPipeline.StatefulOp<Integer> implements DropWhileOp<Integer> {
      ...
      public DropWhileSink<Integer> opWrapSink(
          Sink<Integer> sink, boolean retainAndCountDroppedElements) {
   ...
   * @param predicate the predicate that returns false to halt dropping.
   */
  static LongStream makeDropWhileLong(
      AbstractPipeline<?, Long, ?> upstream, LongPredicate predicate) {
    Objects.requireNonNull(predicate);
    class Op extends LongPipeline.StatefulOp<Long> implements DropWhileOp<Long> {
      ...
      public DropWhileSink<Long> opWrapSink(
          Sink<Long> sink, boolean retainAndCountDroppedElements) {
   ...
   * @param predicate the predicate that returns false to halt dropping.
   */
  static DoubleStream makeDropWhileDouble(
      AbstractPipeline<?, Double, ?> upstream, DoublePredicate predicate) {
    Objects.requireNonNull(predicate);
    class Op extends DoublePipeline.StatefulOp<Double> implements DropWhileOp<Double> {
      ...
      public DropWhileSink<Double> opWrapSink(
          Sink<Double> sink, boolean retainAndCountDroppedElements) {
  ...
  //
  /**
   * A spliterator supporting takeWhile and dropWhile operations over an underlying spliterator
   * whose covered elements have no encounter order.
   *
   * <p>Concrete subclasses of this spliterator support reference and primitive types for takeWhile
   * and dropWhile.
   *
   * <p>For the takeWhile operation if during traversal taking completes then taking is cancelled
   * globally for the splitting and traversal of all related spliterators. Cancellation is governed
   * by a shared {@link AtomicBoolean} instance. A spliterator in the process of taking when
   * cancellation occurs will also be cancelled but not necessarily immediately. To reduce
   * contention on the {@link AtomicBoolean} instance, cancellation may be acted on after a small
   * number of additional elements have been traversed.
   *
   * <p>For the dropWhile operation if during traversal dropping completes for some, but not all
   * elements, then it is cancelled globally for the traversal of all related spliterators
   * (splitting is not cancelled). Cancellation is governed in the same manner as for the takeWhile
   * operation.
   *
   * @param <T> the type of elements returned by this spliterator
   * @param <T_SPLITR> the type of the spliterator
   */
  abstract static class UnorderedWhileSpliterator<T, T_SPLITR extends Spliterator<T>>
    ...
    abstract static class OfRef<T> extends UnorderedWhileSpliterator<T, Spliterator<T>>
      ...
      static final class Dropping<T> extends OfRef<T> {
        ...
        public boolean tryAdvance(Consumer<? super T> action) {
          if (takeOrDrop) {
            ...
            while ( // If advanced one element
            (adv = s.tryAdvance(this))
                && // and if not cancelled
                checkCancelOnCount()
                && p.test(t)) {
              // and test on element passes
    ...
    abstract static class OfInt extends UnorderedWhileSpliterator<Integer, Spliterator.OfInt>
        implements IntConsumer, Spliterator.OfInt {
    ...
    abstract static class OfLong extends UnorderedWhileSpliterator<Long, Spliterator.OfLong>
        implements LongConsumer, Spliterator.OfLong {
    ...
    abstract static class OfDouble extends UnorderedWhileSpliterator<Double, Spliterator.OfDouble>
        implements DoubleConsumer, Spliterator.OfDouble {
   ...
   *
   * <p>If the pipeline has encounter order then all tasks to the right of a task where traversal
   * was short-circuited are cancelled. The results of completed (and cancelled) tasks are
   * discarded. The result of merging a short-circuited left task and right task (which may or may
   * not be short-circuited) is that left task.
   *
   * <p>If the pipeline has no encounter order then all tasks to the right of a task where traversal
   * was short-circuited are cancelled. The results of completed (and possibly cancelled) tasks are
   * not discarded, as there is no need to throw away computed results. The result of merging does
   * not change if a left task was short-circuited. No attempt is made, once a leaf task stopped
   * taking, for it to cancel all other tasks, and further more, short-circuit the computation with
   * its result.
   *
  ...
  private static final class TakeWhileTask<P_IN, P_OUT>
    ...
    TakeWhileTask(
        AbstractPipeline<P_OUT, P_OUT, ?> op,
    ...
    Node<P_OUT> merge() {
      if (leftChild.thisNodeSize == 0) {
        ...
        return Nodes.conc(
            op.getOutputShape(), leftChild.getLocalResult(), rightChild.getLocalResult());
      }
    }
   ...
   *
   * <p>If the pipeline has encounter order then each leaf task will not drop elements but will
   * obtain a count of the elements that would have been otherwise dropped. That count is used as an
   * index to track elements to be dropped. Merging will update the index so it corresponds to the
   * index that is the end of the global prefix of elements to be dropped. The root is truncated
   * according to that index.
   *
   * <p>If the pipeline has no encounter order then each leaf task will drop elements. Leaf tasks
   * are ordinarily merged. No truncation of the root node is required. No attempt is made, once a
   * leaf task stopped dropping, for it to cancel all other tasks, and further more, short-circuit
   * the computation with its result.
   *
   * @param <P_IN> Input element type to the stream pipeline
   * @param <P_OUT> Output element type from the stream pipeline
   */
  ...
  private static final class DropWhileTask<P_IN, P_OUT>
    ...
    protected final Node<P_OUT> doLeaf() {
      ...
      long sizeIfKnown =
          isChild && isOrdered && StreamOpFlag.SIZED.isPreserved(op.sourceOrOpFlags)
    ...
    public final void onCompletion(CountedCompleter<?> caller) {
      if (!isLeaf()) {
        if (isOrdered) {
          ...
          if (index == leftChild.thisNodeSize) index += rightChild.index;
        }
        thisNodeSize = leftChild.thisNodeSize + rightChild.thisNodeSize;
    ...
    private Node<P_OUT> doTruncate(Node<P_OUT> input) {
      return isOrdered ? input.truncate(index, input.count(), generator) : input;
    }
  }
}
