...
/**
 * Factory for instances of a short-circuiting stateful intermediate operations that produce
 * subsequences of their input stream.
 *
 * @since 1.8
 */
final class SliceOps {
  ...
  /**
   * Calculates the sliced size given the current size, number of elements skip, and the number of
   * elements to limit.
   *
  ...
  /**
   * Creates a slice spliterator given a stream shape governing the spliterator type. Requires that
   * the underlying Spliterator be SUBSIZED.
   */
  @SuppressWarnings("unchecked")
  static <P_IN> Spliterator<P_IN> sliceSpliterator(
      StreamShape shape, Spliterator<P_IN> s, long skip, long limit) {
    long sliceFence = calcSliceFence(skip, limit);
    switch (shape) {
      case REFERENCE:
        return new StreamSpliterators.SliceSpliterator.OfRef<>(s, skip, sliceFence);
      case INT_VALUE:
        return (Spliterator<P_IN>)
            new StreamSpliterators.SliceSpliterator.OfInt((Spliterator.OfInt) s, skip, sliceFence);
      case LONG_VALUE:
        return (Spliterator<P_IN>)
            new StreamSpliterators.SliceSpliterator.OfLong(
                (Spliterator.OfLong) s, skip, sliceFence);
      case DOUBLE_VALUE:
        return (Spliterator<P_IN>)
            new StreamSpliterators.SliceSpliterator.OfDouble(
  ...
  /**
   * Appends a "slice" operation to the provided stream. The slice operation may be skip-only,
   * limit-only, or skip-and-limit.
   *
  ...
  public static <T> Stream<T> makeRef(AbstractPipeline<?, T, ?> upstream, long skip, long limit) {
    if (skip < 0L) throw new IllegalArgumentException("Skip must be non-negative: " + skip);
    ...
    return new ReferencePipeline.StatefulOp<T, T>(upstream, StreamShape.REFERENCE, flags(limit)) {
      ...
      Spliterator<T> unorderedSkipLimitSpliterator(
          Spliterator<T> s, long skip, long limit, long sizeIfKnown) {
        if (skip <= sizeIfKnown) {
      ...
      <P_IN> Node<T> opEvaluateParallel(
        ...
        if (size > 0L && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
          ...
          Spliterator<T> s =
              unorderedSkipLimitSpliterator(helper.wrapSpliterator(spliterator), skip, limit, size);
  ...
  /**
   * Appends a "slice" operation to the provided IntStream. The slice operation may be may be
   * skip-only, limit-only, or skip-and-limit.
   *
  ...
  /**
   * Appends a "slice" operation to the provided LongStream. The slice operation may be may be
   * skip-only, limit-only, or skip-and-limit.
   *
  ...
  /**
   * Appends a "slice" operation to the provided DoubleStream. The slice operation may be may be
   * skip-only, limit-only, or skip-and-limit.
   *
  ...
  private static final class SliceTask<P_IN, P_OUT>
    ...
    protected final Node<P_OUT> doLeaf() {
      if (isRoot()) {
        long sizeIfKnown =
            StreamOpFlag.SIZED.isPreserved(op.sourceOrOpFlags)
    ...
    public final void onCompletion(CountedCompleter<?> caller) {
      if (!isLeaf()) {
        ...
        } else if (thisNodeSize == 0L) result = getEmptyResult();
        else if (leftChild.thisNodeSize == 0L) result = rightChild.getLocalResult();
        else {
          result =
              Nodes.conc(
      ...
      if (targetSize >= 0L && !isRoot() && isLeftCompleted(targetOffset + targetSize))
        cancelLaterNodes();
    ...
    private Node<P_OUT> doTruncate(Node<P_OUT> input) {
      long to =
          targetSize >= 0L ? Math.min(input.count(), targetOffset + targetSize) : thisNodeSize;
      return input.truncate(targetOffset, to, generator);
    }

    /**
     * Determine if the number of completed elements in this node and nodes to the left of this node
     ...
     *
     * <p>Computation terminates if all nodes have been processed or the number of completed
     * elements is greater than or equal to the target size.
     *
     * @param target the target size
