...
/**
 * An ordered collection of elements. Elements can be added, but not removed. Goes through a
 * building phase, during which elements can be added, and a traversal phase, during which elements
 * can be traversed in order but no further modifications are possible.
 *
 * <p>One or more arrays are used to store elements. The use of a multiple arrays has better
 * performance characteristics than a single array used by {@link ArrayList}, as when the capacity
 * of the list needs to be increased no copying of elements is required. This is usually beneficial
 * in the case where the results will be traversed a small number of times.
 *
...
class SpinedBuffer<E> extends AbstractSpinedBuffer implements Consumer<E> {
  ...
  public E get(long index) {
    ...
    if (spineIndex == 0) {
      if (index < elementIndex) return curChunk[((int) index)];
      ...
      public Spliterator<E> trySplit() {
        if (splSpineIndex < lastSpineIndex) {
          ...
          if (t == 0) return null;
            ...
            Spliterator<E> ret =
                org.java.util.J8Arrays.spliterator(splChunk, splElementIndex, splElementIndex + t);
            splElementIndex += t;
            return ret;
          }
        } else {
  ...
  /**
   * An ordered collection of primitive values. Elements can be added, but not removed. Goes through
   * a building phase, during which elements can be added, and a traversal phase, during which
   * elements can be traversed in order but no further modifications are possible.
   *
   * <p>One or more arrays are used to store elements. The use of a multiple arrays has better
   * performance characteristics than a single array used by {@link ArrayList}, as when the capacity
   * of the list needs to be increased no copying of elements is required. This is usually
   * beneficial in the case where the results will be traversed a small number of times.
   *
  ...
  abstract static class OfPrimitive<E, T_ARR, T_CONS> extends AbstractSpinedBuffer {
    ...
    abstract class BaseSpliterator<T_SPLITR extends Spliterator.OfPrimitive<E, T_CONS, T_SPLITR>>
      ...
      abstract T_SPLITR newSpliterator(
          int firstSpineIndex,
          int lastSpineIndex,
          int firstSpineElementIndex,
          int lastSpineElementFence);
  ...
  static class OfInt extends SpinedBuffer.OfPrimitive<Integer, int[], IntConsumer>
    ...
    public Spliterator.OfInt spliterator() {
      class Splitr extends BaseSpliterator<Spliterator.OfInt> implements Spliterator.OfInt {
        ...
        @Override
        Splitr newSpliterator(
            int firstSpineIndex,
            int lastSpineIndex,
            int firstSpineElementIndex,
            int lastSpineElementFence) {
          return new Splitr(
              firstSpineIndex, lastSpineIndex, firstSpineElementIndex, lastSpineElementFence);
        }
        ...
        Spliterator.OfInt arraySpliterator(int[] array, int offset, int len) {
          return org.java.util.J8Arrays.spliterator(array, offset, offset + len);
        }
    ...
    public String toString() {
      ...
      if (array.length < 200) {
        return String.format(
            "%s[length=%d, chunks=%d]%s",
            getClass().getSimpleName(), array.length, spineIndex, Arrays.toString(array));
      } else {
        int[] array2 = Arrays.copyOf(array, 200);
        return String.format(
            "%s[length=%d, chunks=%d]%s...",
            getClass().getSimpleName(), array.length, spineIndex, Arrays.toString(array2));
      }
    }
  }
  ...
  static class OfLong extends SpinedBuffer.OfPrimitive<Long, long[], LongConsumer>
    ...
    public void forEach(Consumer<? super Long> consumer) {
      if (consumer instanceof LongConsumer) {
        ...
        ((Spliterator<Long>) spliterator()).forEachRemaining(consumer);
      }
    }
  ...
  static class OfDouble extends SpinedBuffer.OfPrimitive<Double, double[], DoubleConsumer>
    ...
    public void forEach(Consumer<? super Double> consumer) {
      if (consumer instanceof DoubleConsumer) {
        ...
        ((Spliterator<Double>) spliterator()).forEachRemaining(consumer);
      }
    }
