...
/**
 * Spliterator implementations for wrapping and delegating spliterators, used in the implementation
 * of the {@link Stream#spliterator()} method.
 *
 * @since 1.8
 */
class StreamSpliterators {
  ...
  /**
   * Abstract wrapping spliterator that binds to the spliterator of a pipeline helper on first
   * operation.
   *
   * <p>This spliterator is not late-binding and will bind to the source spliterator when first
   * operated on.
   *
   * <p>A wrapping spliterator produced from a sequential stream cannot be split if there are
   * stateful operations present.
   */
  private abstract static class AbstractWrappingSpliterator<
          P_IN, P_OUT, T_BUFFER extends AbstractSpinedBuffer>
    ...
    /** Supplier for the source spliterator. Client provides either a spliterator or a supplier. */
    private Supplier<Spliterator<P_IN>> spliteratorSupplier;

    /** Source spliterator. Either provided from client or obtained from supplier. */
    Spliterator<P_IN> spliterator;

    /**
     * Sink chain for the downstream stages of the pipeline, ultimately leading to the buffer. Used
     * during partial traversal.
     */
    Sink<P_IN> bufferSink;

    /**
     * A function that advances one element of the spliterator, pushing it to bufferSink. Returns
     * whether any elements were processed. Used during partial traversal.
     */
    ...
    /**
     * True if full traversal has occurred (with possible cancellation). If doing a partial
     * traversal, there may be still elements in buffer.
     */
    boolean finished;

    /** Construct an AbstractWrappingSpliterator from a {@code Supplier<Spliterator>}. */
    ...
    AbstractWrappingSpliterator(
        PipelineHelper<P_OUT> ph, Spliterator<P_IN> spliterator, boolean parallel) {
      this.ph = ph;
    ...
    /**
     * Get an element from the source, pushing it into the sink chain, setting up the buffer if
     * needed
     *
    ...
    /**
     * If the buffer is empty, push elements into the sink chain until the source is empty or
  ...
  static final class WrappingSpliterator<P_IN, P_OUT>
    ...
    WrappingSpliterator(
        PipelineHelper<P_OUT> ph, Supplier<Spliterator<P_IN>> supplier, boolean parallel) {
  ...
  static final class IntWrappingSpliterator<P_IN>
    ...
    IntWrappingSpliterator(
        PipelineHelper<Integer> ph, Supplier<Spliterator<P_IN>> supplier, boolean parallel) {
      super(ph, supplier, parallel);
    }

    IntWrappingSpliterator(
        PipelineHelper<Integer> ph, Spliterator<P_IN> spliterator, boolean parallel) {
      super(ph, spliterator, parallel);
    }
  ...
  static final class LongWrappingSpliterator<P_IN>
    ...
    LongWrappingSpliterator(
        PipelineHelper<Long> ph, Supplier<Spliterator<P_IN>> supplier, boolean parallel) {
      super(ph, supplier, parallel);
    }

    LongWrappingSpliterator(
        PipelineHelper<Long> ph, Spliterator<P_IN> spliterator, boolean parallel) {
      super(ph, spliterator, parallel);
    }
  ...
  static final class DoubleWrappingSpliterator<P_IN>
    ...
    DoubleWrappingSpliterator(
        PipelineHelper<Double> ph, Supplier<Spliterator<P_IN>> supplier, boolean parallel) {
      super(ph, supplier, parallel);
    }

    DoubleWrappingSpliterator(
        PipelineHelper<Double> ph, Spliterator<P_IN> spliterator, boolean parallel) {
      super(ph, spliterator, parallel);
    }
  ...
  /**
   * Spliterator implementation that delegates to an underlying spliterator, acquiring the
   * spliterator from a {@code Supplier<Spliterator>} on the first call to any spliterator method.
   *
   * @param <T>
   */
  ...
  /** A slice Spliterator from a source Spliterator that reports {@code SUBSIZED}. */
  abstract static class SliceSpliterator<T, T_SPLITR extends Spliterator<T>> {
    ...
    protected abstract T_SPLITR makeSpliterator(
        T_SPLITR s, long sliceOrigin, long sliceFence, long origin, long fence);
    ...
    static final class OfRef<T> extends SliceSpliterator<T, Spliterator<T>>
      ...
      @Override
      protected Spliterator<T> makeSpliterator(
          Spliterator<T> s, long sliceOrigin, long sliceFence, long origin, long fence) {
    ...
    abstract static class OfPrimitive<
            T, T_SPLITR extends Spliterator.OfPrimitive<T, T_CONS, T_SPLITR>, T_CONS>
    ...
    static final class OfInt extends OfPrimitive<Integer, Spliterator.OfInt, IntConsumer>
      ...
      @Override
      protected Spliterator.OfInt makeSpliterator(
          Spliterator.OfInt s, long sliceOrigin, long sliceFence, long origin, long fence) {
    ...
    static final class OfLong extends OfPrimitive<Long, Spliterator.OfLong, LongConsumer>
      ...
      @Override
      protected Spliterator.OfLong makeSpliterator(
          Spliterator.OfLong s, long sliceOrigin, long sliceFence, long origin, long fence) {
    ...
    static final class OfDouble extends OfPrimitive<Double, Spliterator.OfDouble, DoubleConsumer>
      ...
      protected Spliterator.OfDouble makeSpliterator(
          Spliterator.OfDouble s, long sliceOrigin, long sliceFence, long origin, long fence) {
   ...
   *
   * <p>Note: The source spliterator may report {@code ORDERED} since that spliterator be the result
   * of a previous pipeline stage that was collected to a {@code Node}. It is the order of the
   * pipeline stage that governs whether this slice spliterator is to be used or not.
   */
  abstract static class UnorderedSliceSpliterator<T, T_SPLITR extends Spliterator<T>> {
    ...
    UnorderedSliceSpliterator(T_SPLITR s, long skip, long limit) {
      ...
      this.chunkSize =
          limit >= 0
              ? (int) Math.min(CHUNK_SIZE, ((skip + limit) / AbstractTask.getLeafTarget()) + 1)
              : CHUNK_SIZE;
    ...
    /**
     * Acquire permission to skip or process elements. The caller must first acquire the elements,
     * then consult this method for guidance as to what to do with the data.
     *
     * <p>We use an {@code AtomicLong} to atomically maintain a counter, which is initialized as
     * skip+limit if we are limiting, or skip only if we are not limiting. The user should consult
     * the method {@code checkPermits()} before acquiring data elements.
     *
    ...
    protected final long acquirePermits(long numElements) {
      ...
      if (unlimited) return Math.max(numElements - grabbing, 0);
    ...
    protected final PermitStatus permitStatus() {
      if (permits.get() > 0) return PermitStatus.MAYBE_MORE;
      else return unlimited ? PermitStatus.UNLIMITED : PermitStatus.NO_MORE;
    }
    ...
    static final class OfRef<T> extends UnorderedSliceSpliterator<T, Spliterator<T>>
      ...
      public boolean tryAdvance(Consumer<? super T> action) {
        ...
        while (permitStatus() != PermitStatus.NO_MORE) {
          if (!s.tryAdvance(this)) return false;
      ...
      public void forEachRemaining(Consumer<? super T> action) {
        ...
        while ((permitStatus = permitStatus()) != PermitStatus.NO_MORE) {
          if (permitStatus == PermitStatus.MAYBE_MORE) {
            ...
            if (sb == null) sb = new ArrayBuffer.OfRef<>(chunkSize);
            else sb.reset();
    ...
    abstract static class OfPrimitive<
      ...
      OfPrimitive(
          T_SPLITR s, UnorderedSliceSpliterator.OfPrimitive<T, T_CONS, T_BUFF, T_SPLITR> parent) {
        super(s, parent);
      }
      ...
      public boolean tryAdvance(T_CONS action) {
        ...
        while (permitStatus() != PermitStatus.NO_MORE) {
          if (!s.tryAdvance(consumer)) return false;
          else if (acquirePermits(1) == 1) {
      ...
      public void forEachRemaining(T_CONS action) {
        ...
        while ((permitStatus = permitStatus()) != PermitStatus.NO_MORE) {
          if (permitStatus == PermitStatus.MAYBE_MORE) {
            // Optimistically traverse elements up to a threshold of chunkSize
            if (sb == null) sb = bufferCreate(chunkSize);
            else sb.reset();
            @SuppressWarnings("unchecked")
  ...
  /**
   * A wrapping spliterator that only reports distinct elements of the underlying spliterator. Does
   * not preserve size and encounter order.
   */
   ...
   *
   * <p>Splitting divides the estimated size in two and stops when the estimate size is 0.
   *
   * <p>The {@code forEachRemaining} method if invoked will never terminate. The {@code tryAdvance}
