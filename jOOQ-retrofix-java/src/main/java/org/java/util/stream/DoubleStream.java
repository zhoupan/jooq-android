...
import org.java.util.function.DoubleFunction;
import org.java.util.function.DoublePredicate;
import org.java.util.function.DoubleSupplier;
import org.java.util.function.DoubleToIntFunction;
import org.java.util.function.DoubleToLongFunction;
import org.java.util.function.DoubleUnaryOperator;
import org.java.util.function.Function;
import org.java.util.function.ObjDoubleConsumer;
import org.java.util.function.Supplier;

/**
 * A sequence of primitive double-valued elements supporting sequential and parallel aggregate
 * operations. This is the {@code double} primitive specialization of {@link Stream}.
 *
 * <p>The following example illustrates an aggregate operation using {@link Stream} and {@link
 * DoubleStream}, computing the sum of the weights of the red widgets:
 *
 * <pre>{@code
...
public interface DoubleStream extends BaseStream<Double, DoubleStream> {
   ...
   * provided mapping function to each element in conjunction with a {@linkplain DoubleConsumer
   ...
   * that behaves as follows. First, it calls the mapper function with a {@code DoubleConsumer} that
   * accumulates replacement elements into a newly created internal buffer. When the mapper function
   * returns, it creates a {@code DoubleStream} from the internal buffer. Finally, it returns this
  ...
  /**
   * Returns a stream consisting of the distinct elements of this stream. The elements are compared
   * for equality according to {@link java.lang.Double#compare(double, double)}.
   *
  ...
  /**
   * Returns a stream consisting of the elements of this stream in sorted order. The elements are
   * compared for equality according to {@link java.lang.Double#compare(double, double)}.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">stateful intermediate operation</a>.
   *
   * @return the result stream
   ...
   * {@link DoubleStreams#generate(DoubleSupplier)}) or removing the ordering constraint with {@link
   ...
   * DoubleStreams#generate(DoubleSupplier)}) or removing the ordering constraint with {@link
   ...
   * unordered stream source (such as {@link DoubleStreams#generate(DoubleSupplier)}) or removing
   * the ordering constraint with {@link #unordered()} may result in significant speedups of {@code
   ...
   *
   * <p><b>API Note:</b><br>
   * Sum, min, max, and average are all special cases of reduction. Summing a stream of numbers can
   * be expressed as:
   *
   * <pre>{@code
   ...
   * function, and returns an {@code OptionalDouble} describing the reduced value, if any. This is
   * equivalent to:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Like {@link #reduce(double, DoubleBinaryOperator)}, {@code collect} operations can be
   ...
   *
   * <p>Summation is a special case of a <a href="package-summary.html#Reduction">reduction</a>. If
   * floating-point summation were exact, this method would be equivalent to:
   *
   * <pre>{@code
   * return reduce(0, Double::sum);
   * }</pre>
   *
   * However, since floating-point summation is not exact, the above code is not necessarily
   * equivalent to the summation computation done by this method.
   *
   ...
   * summation schemes, the output of this method may vary on the same input elements.
   *
   * <p>Various conditions can result in a non-finite sum being computed. This can occur even if the
   * all the elements being summed are finite. If any element is non-finite, the sum will be
   * non-finite:
   *
   * <ul>
   *   <li>If any element is a NaN, then the final sum will be NaN.
   *   <li>If the elements contain one or more infinities, the sum will be infinite or NaN.
   *       <ul>
   *         <li>If the elements contain infinities of opposite sign, the sum will be NaN.
   *         <li>If the elements contain infinities of one sign and an intermediate sum overflows to
   *             an infinity of the opposite sign, the sum may be NaN.
   *       </ul>
   * </ul>
   *
   * It is possible for intermediate sums of finite values to overflow into opposite-signed
   * infinities; if that occurs, the final sum will be NaN even if the elements are all finite.
   *
   * <p>If all the elements are zero, the sign of zero is <em>not</em> guaranteed to be preserved in
   * the final sum.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">terminal operation</a>.
   *
   ...
   * OptionalDouble if this stream is empty. The minimum element will be {@code Double.NaN} if any
   * stream element was NaN. Unlike the numerical comparison operators, this method considers
   * negative zero to be strictly smaller than positive zero. This is a special case of a <a
   ...
   * OptionalDouble if this stream is empty. The maximum element will be {@code Double.NaN} if any
   * stream element was NaN. Unlike the numerical comparison operators, this method considers
   * negative zero to be strictly smaller than positive zero. This is a special case of a <a
   * href="package-summary.html#Reduction">reduction</a> and is equivalent to:
   *
  ...
  /**
   * Returns a {@code DoubleSummaryStatistics} describing various summary data about the elements of
  ...
  /**
   * Returns an {@link OptionalDouble} describing the first element of this stream, or an empty
   * {@code OptionalDouble} if the stream is empty. If the stream has no encounter order, then any
  ...
  /**
   * Returns an {@link OptionalDouble} describing some element of the stream, or an empty {@code
  ...
  /**
   * Represents an operation that accepts a {@code double}-valued argument and a DoubleConsumer, and
   * returns no result. This functional interface is used by {@link
   * DoubleStream#mapMulti(DoubleStream.DoubleMapMultiConsumer) DoubleStreams.mapMulti} to replace a
   * double value with zero or more double values.
   *
   * <p>This is a <a href="../function/package-summary.html">functional interface</a> whose
   * functional method is {@link #accept(double, DoubleConsumer)}.
   *
