...
import org.java.util.function.LongToDoubleFunction;
import org.java.util.function.LongToIntFunction;
...
/**
 * A sequence of primitive long-valued elements supporting sequential and parallel aggregate
 * operations. This is the {@code long} primitive specialization of {@link Stream}.
 *
 * <p>The following example illustrates an aggregate operation using {@link Stream} and {@link
 * LongStream}, computing the sum of the weights of the red widgets:
 *
 * <pre>{@code
...
public interface LongStream extends BaseStream<Long, LongStream> {
   ...
   * provided mapping function to each element in conjunction with a {@linkplain LongConsumer
   ...
   * that behaves as follows. First, it calls the mapper function with a {@code LongConsumer} that
   * accumulates replacement elements into a newly created internal buffer. When the mapper function
   * returns, it creates a {@code LongStream} from the internal buffer. Finally, it returns this
   ...
   * {@link LongStreams#generate(LongSupplier)}) or removing the ordering constraint with {@link
   ...
   * LongStreams#generate(LongSupplier)}) or removing the ordering constraint with {@link
   ...
   * unordered stream source (such as {@link LongStreams#generate(LongSupplier)}) or removing the
   ...
   * function, and returns an {@code OptionalLong} describing the reduced value, if any. This is
   * equivalent to:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Like {@link #reduce(long, LongBinaryOperator)}, {@code collect} operations can be
  ...
  /**
   * Returns a {@code LongSummaryStatistics} describing various summary data about the elements of
  ...
  /**
   * Returns an {@link OptionalLong} describing the first element of this stream, or an empty {@code
   * OptionalLong} if the stream is empty. If the stream has no encounter order, then any element
   * may be returned.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">short-circuiting terminal operation</a>.
   *
  ...
  /**
   * Returns an {@link OptionalLong} describing some element of the stream, or an empty {@code
   * OptionalLong} if the stream is empty.
   *
   * <p>This is a <a href="package-summary.html#StreamOps">short-circuiting terminal operation</a>.
   *
  ...
  /**
   * Returns a {@code Stream} consisting of the elements of this stream, each boxed to a {@code
   * Long}.
   *
   * <p>This is an <a href="package-summary.html#StreamOps">intermediate operation</a>.
   *
   ...
   * built phase begins begins when the {@link #build()} method is called, which creates an ordered
   * stream whose elements are the elements that were added to the stream builder, in the order they
   * were added.
   *
  ...
  /**
   * Represents an operation that accepts a {@code long}-valued argument and a LongConsumer, and
   * returns no result. This functional interface is used by {@link
   * LongStream#mapMulti(LongStream.LongMapMultiConsumer) LongStream.mapMulti} to replace a long
   * value with zero or more long values.
   *
   * <p>This is a <a href="../function/package-summary.html">functional interface</a> whose
   * functional method is {@link #accept(long, LongConsumer)}.
   *
