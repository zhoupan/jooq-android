 ...
 * a stream. Supported variants include unordered traversal (elements are provided to the {@code
 * Consumer} as soon as they are available), and ordered traversal (elements are provided to the
 * {@code Consumer} in encounter order.)
 *
 * <p>Elements are provided to the {@code Consumer} on whatever thread and whatever order they
 * become available. For ordered traversals, it is guaranteed that processing an element
 * <em>happens-before</em> processing subsequent elements in the encounter order.
 *
 * <p>Exceptions occurring as a result of sending an element to the {@code Consumer} will be relayed
 * to the caller and traversal will be prematurely terminated.
 *
 * @since 1.8
 */
final class ForEachOps {
  ...
  /**
   * Constructs a {@code TerminalOp} that perform an action for every element of an {@code
   * IntStream}.
   *
  ...
  /**
   * Constructs a {@code TerminalOp} that perform an action for every element of a {@code
   * LongStream}.
   *
   ...
   *
   * <p>This terminal operation is stateless. For parallel evaluation, each leaf instance of a
  ...
  static final class ForEachOrderedTask<S, T> extends CountedCompleter<Void> {
    ...
    protected ForEachOrderedTask(
        PipelineHelper<T> helper, Spliterator<S> spliterator, Sink<T> action) {
      super(null);
      this.helper = helper;
      this.spliterator = spliterator;
      ...
      this.completionMap =
          new ConcurrentHashMap<>(
              Math.max(16, AbstractTask.getLeafTarget() << 1),
              0.75f,
    ...
    private static <S, T> void doCompute(ForEachOrderedTask<S, T> task) {
      ...
      if (task.getPendingCount() > 0) {
        ...
        Node.Builder<T> nb =
            task.helper.makeNodeBuilder(task.helper.exactOutputSizeIfKnown(rightSplit), generator);
