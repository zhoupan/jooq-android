...
import org.java.util.stream.Stream.Builder;
...
public final class RefStreams {
  ...
  /**
   * Returns a sequential {@link Stream} containing a single element, if non-null, otherwise returns
   * an empty {@code Stream}.
   *
   * @param t the single element
   * @param <T> the type of stream elements
   * @return a stream with a single element if the specified element is non-null, otherwise an empty
   *     stream
   * @since 9
   */
  ...
  /**
   * Returns a sequential ordered {@link Stream} whose elements are the specified values.
   *
   * @param <T> the type of stream elements
  ...
  /**
   * Returns an infinite sequential ordered {@link Stream} produced by iterative application of a
   ...
   *
   * <p>The first element (position {@code 0}) in the {@code Stream} will be the provided {@code
   ...
   * false.
   *
   * <p>{@code RefStreams.iterate} should produce the same sequence of elements as produced by the
   ...
   * @since 9
   */
  public static <T, S extends T> Stream<T> iterate(
      S seed, Predicate<? super S> hasNext, UnaryOperator<S> next) {
  ...
  /**
   * Returns an infinite sequential unordered {@link Stream} where each element is generated by the
   * provided {@code Supplier}. This is suitable for generating constant streams, streams of random
   * elements, etc.
   *
   * @param <T> the type of stream elements
  ...
  /**
   * Creates a lazily concatenated {@link Stream} whose elements are all the elements of the first
   * stream followed by all the elements of the second stream. The resulting stream is ordered if
  ...
  public static <T> Stream<T> concat(Stream<? extends T> a, Stream<? extends T> b) {
    ...
    Spliterator<T> split =
        new Streams.ConcatSpliterator.OfRef<>(
