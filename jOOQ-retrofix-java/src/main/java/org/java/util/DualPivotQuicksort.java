...
import org.java.util.concurrent.RecursiveTask;

/**
 * This class implements powerful and fully optimized versions, both sequential and parallel, of the
 * Dual-Pivot Quicksort algorithm by Vladimir Yaroslavskiy, Jon Bentley and Josh Bloch. This
 * algorithm offers O(n log(n)) performance on all data sets, and is typically faster than
 * traditional (one-pivot) Quicksort implementations.
 *
 * <p>There are also additional algorithms, invoked from the Dual-Pivot Quicksort, such as mixed
 * insertion sort, merging of runs and heap sort, counting sort and parallel merge sort.
 *
...
final class DualPivotQuicksort {
  ...
  /**
   * Calculates the double depth of parallel merging. Depth is negative, if tasks split before
   * sorting.
   *
   ...
   *
   * <p>To balance the faster splitting and parallelism of merge sort with the faster element
   * partitioning of Quicksort, ranges are subdivided in tiers such that, if there is enough
   * parallelism, the four-way parallel merge is started, still ensuring enough parallelism to
  ...
  /**
   * Sorts the specified array using the Dual-Pivot Quicksort and/or other sorts in special-cases,
   * possibly with parallel partitions.
   *
   * @param sorter parallel context
   * @param a the array to be sorted
   * @param bits the combination of recursion depth and bit flag, where the right bit "0" indicates
   *     that array is the leftmost part
   * @param low the index of the first element, inclusive, to be sorted
   * @param high the index of the last element, exclusive, to be sorted
   */
  static void sort(Sorter sorter, int[] a, int bits, int low, int high) {
    while (true) {
      ...
      // Pointers
      // The index of the last element of the left part
      int lower = low;
      // The index of the first element of the right part
      int upper = end;
      /*
   ...
   *
   * <p>Mixed insertion sort is combination of simple insertion sort, pin insertion sort and pair
   * insertion sort.
   *
   * <p>In the context of Dual-Pivot Quicksort, the pivot element from the left part plays the role
   * of sentinel, because it is less than any elements from the given part. Therefore, expensive
   * check of the left range can be skipped on each iteration unless it is the leftmost call.
   *
   * @param a the array to be sorted
   * @param low the index of the first element, inclusive, to be sorted
   ...
   */
  static int[] mergeRuns(
      int[] a, int[] b, int offset, int aim, boolean parallel, int[] run, int lo, int hi) {
    if (hi - lo == 1) {
      if (aim >= 0) {
        return a;
      }
  ...
  static void mergeParts(
      Merger merger, int[] dst, int k, int[] a1, int lo1, int hi1, int[] a2, int lo2, int hi2) {
   ...
   * @return the destination where runs are merged
   */
  static long[] mergeRuns(
      long[] a, long[] b, int offset, int aim, boolean parallel, int[] run, int lo, int hi) {
    if (hi - lo == 1) {
      if (aim >= 0) {
        return a;
      }
   ...
   */
  static void mergeParts(
      Merger merger, long[] dst, int k, long[] a1, int lo1, int hi1, long[] a2, int lo2, int hi2) {
    if (merger != null && a1 == a2) {
      while (true) {
        /*
         * The first part must be larger.
   ...
   * @return the destination where runs are merged
   */
  static float[] mergeRuns(
      float[] a, float[] b, int offset, int aim, boolean parallel, int[] run, int lo, int hi) {
    if (hi - lo == 1) {
      if (aim >= 0) {
        return a;
      }
   ...
   * @return the destination where runs are merged
   */
  static double[] mergeRuns(
      double[] a, double[] b, int offset, int aim, boolean parallel, int[] run, int lo, int hi) {
    if (hi - lo == 1) {
      if (aim >= 0) {
        return a;
      }
  ...
  private static final class Sorter extends CountedCompleter<Void> {
    ...
    Sorter(
        CountedCompleter<?> parent, Object a, Object b, int low, int size, int offset, int depth) {
      super(parent);
      this.a = a;
      this.b = b;
      this.low = low;
  ...
  private static final class Merger extends CountedCompleter<Void> {
    ...
    public final void compute() {
      if (dst instanceof int[]) {
        ...
        mergeParts(this, (float[]) dst, k, (float[]) a1, lo1, hi1, (float[]) a2, lo2, hi2);
      } else if (dst instanceof double[]) {
        mergeParts(this, (double[]) dst, k, (double[]) a1, lo1, hi1, (double[]) a2, lo2, hi2);
      } else {
        throw new IllegalArgumentException("Unknown type of array: " + dst.getClass().getName());
      }
      propagateCompletion();
    }
