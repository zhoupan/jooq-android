...
import org.java.util.stream.RefStreams;
 ...
 * such as {@link #orElse(Object) orElse()} (returns a default value if no value is present) and
 ...
 * code, or synchronization) on instances of {@code Optional} may have unpredictable results and
 * should be avoided.
 *
 * <p><b>API Note:</b><br>
 * {@code Optional} is primarily intended for use as a method return type where there is a clear
 * need to represent "no result", and where using {@code null} is likely to cause errors. A variable
 * whose type is {@code Optional} should never itself be {@code null}; it should always point to an
...
public final class Optional<T> {
   ...
   * {@code ==} or {@code !=} against instances returned by {@code Optional.empty()}. There is no
   ...
   *
   * @param value the value to describe; it's the caller's responsibility to ensure the value is
  ...
  /**
   * Returns an {@code Optional} describing the given value, if non-{@code null}, otherwise returns
   ...
   *
   * <p><b>API Note:</b><br>
   * This method supports post-processing on {@code Optional} values, without the need to explicitly
   * check for a return status. For example, the following code traverses a stream of URIs, selects
   * one that has not yet been processed, and creates a path from that URI, returning an {@code
   * Optional<Path>}:
   *
   * <pre>{@code
   * Optional<Path> p =
   ...
   * }</pre>
   *
   * Here, {@code findFirst} returns an {@code Optional<URI>}, and then {@code map} returns an
   * {@code Optional<Path>} for the desired URI if one exists.
   *
   ...
   *
   * <p><b>API Note:</b><br>
   * This method can be used to transform a {@code Stream} of optional elements to a {@code Stream}
   ...
   * @throws NullPointerException if value is not present and the supplying function is {@code null}
   */
   ...
   * and present {@code Optional}s must be unambiguously differentiable.
   *
