...
import org.java.util.stream.DoubleStreams;

/**
 * A container object which may or may not contain a {@code double} value. If a value is present,
 * {@code isPresent()} returns {@code true}. If no value is present, the object is considered
 ...
 * such as {@link #orElse(double) orElse()} (returns a default value if no value is present) and
 * {@link #ifPresent(DoubleConsumer) ifPresent()} (performs an action if a value is present).
 *
 ...
 * code, or synchronization) on instances of {@code OptionalDouble} may have unpredictable results
 * and should be avoided.
 *
 * <p><b>API Note:</b><br>
 * {@code OptionalDouble} is primarily intended for use as a method return type where there is a
 * clear need to represent "no result". A variable whose type is {@code OptionalDouble} should never
 * itself be {@code null}; it should always point to an {@code OptionalDouble} instance.
 *
 * @since 1.8
 */
public final class OptionalDouble {
   ...
   * generally only one empty instance, {@link OptionalDouble#EMPTY}, should exist per VM.
   */
  ...
  /**
   * Returns an empty {@code OptionalDouble} instance. No value is present for this {@code
   * OptionalDouble}.
   *
   * <p><b>API Note:</b><br>
   * Though it may be tempting to do so, avoid testing if an object is empty by comparing with
   * {@code ==} or {@code !=} against instances returned by {@code OptionalDouble.empty()}. There is
   * no guarantee that it is a singleton. Instead, use {@link #isEmpty()} or {@link #isPresent()}.
   *
  ...
  /**
   * If a value is present, returns a sequential {@link DoubleStream} containing only that value,
   * otherwise returns an empty {@code DoubleStream}.
   *
   * <p><b>API Note:</b><br>
   * This method can be used to transform a {@code Stream} of optional doubles to a {@code
   * DoubleStream} of present doubles:
   *
   * <pre>{@code
  ...
  /**
   * Indicates whether some other object is "equal to" this {@code OptionalDouble}. The other object
  ...
  /**
   * Returns a non-empty string representation of this {@code OptionalDouble} suitable for
   * debugging. The exact presentation format is unspecified and may vary between implementations
   * and versions.
   *
   * <p><b>Implementation Requirements:</b><br>
   * If a value is present the result must include its string representation in the result. Empty
   * and present {@code OptionalDouble}s must be unambiguously differentiable.
   *
