...
import org.java.util.stream.LongStreams;

/**
 * A container object which may or may not contain a {@code long} value. If a value is present,
 ...
 * such as {@link #orElse(long) orElse()} (returns a default value if no value is present) and
 * {@link #ifPresent(LongConsumer) ifPresent()} (performs an action if a value is present).
 *
 ...
 * code, or synchronization) on instances of {@code OptionalLong} may have unpredictable results and
 * should be avoided.
 *
 * <p><b>API Note:</b><br>
 * {@code OptionalLong} is primarily intended for use as a method return type where there is a clear
 * need to represent "no result". A variable whose type is {@code OptionalLong} should never itself
 * be {@code null}; it should always point to an {@code OptionalLong} instance.
 *
 * @since 1.8
 */
public final class OptionalLong {
   ...
   * generally only one empty instance, {@link OptionalLong#EMPTY}, should exist per VM.
   */
  ...
  /**
   * Returns an empty {@code OptionalLong} instance. No value is present for this {@code
   * OptionalLong}.
   *
   * <p><b>API Note:</b><br>
   * Though it may be tempting to do so, avoid testing if an object is empty by comparing with
   * {@code ==} or {@code !=} against instances returned by {@code OptionalLong.empty()}. There is
  ...
  /**
   * If a value is present, returns a sequential {@link LongStream} containing only that value,
   * otherwise returns an empty {@code LongStream}.
   *
   * <p><b>API Note:</b><br>
   * This method can be used to transform a {@code Stream} of optional longs to a {@code LongStream}
   * of present longs:
   *
   * <pre>{@code
  ...
  /**
   * Indicates whether some other object is "equal to" this {@code OptionalLong}. The other object
  ...
  /**
   * Returns a non-empty string representation of this {@code OptionalLong} suitable for debugging.
   ...
   * and present {@code OptionalLong}s must be unambiguously differentiable.
   *
