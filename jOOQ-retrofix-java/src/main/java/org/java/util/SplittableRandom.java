...
/**
 * A generator of uniform pseudorandom values applicable for use in (among other contexts) isolated
 * parallel computations that may generate subtasks. Class {@code SplittableRandom} supports methods
 * for producing pseudorandom numbers of type {@code int}, {@code long}, and {@code double} with
 * similar usages as for class {@link java.util.Random} but differs in the following ways:
 *
 * <ul>
 *   <li>Series of generated values pass the DieHarder suite testing independence and uniformity
 *       properties of random number generators. (Most recently validated with <a
 *       href="http://www.phy.duke.edu/~rgb/General/dieharder.php">version 3.31.1</a>.) These tests
 *       validate only the methods for certain types and ranges, but similar properties are expected
 *       to hold, at least approximately, for others as well. The <em>period</em> (length of any
 *       series of generated values before it repeats) is at least 2<sup>64</sup>.
 *   <li>Method {@link #split} constructs and returns a new SplittableRandom instance that shares no
 *       mutable state with the current instance. However, with very high probability, the values
 *       collectively generated by the two objects have the same statistical properties as if the
 *       same quantity of values were generated by a single thread using a single {@code
 *       SplittableRandom} object.
 *   <li>Instances of SplittableRandom are <em>not</em> thread-safe. They are designed to be split,
 *       not shared, across threads. For example, a {@link org.java.util.concurrent.ForkJoinTask
 *       fork/join-style} computation using random numbers might include a construction of the form
 *       {@code new Subtask(aSplittableRandom.split()).fork()}.
 *   <li>This class provides additional methods for generating random streams, that employ the above
 *       techniques when used in {@code stream.parallel()} mode.
 * </ul>
 *
 * <p>Instances of {@code SplittableRandom} are not cryptographically secure. Consider instead using
 * {@link java.security.SecureRandom} in security-sensitive applications. Additionally,
...
public final class SplittableRandom {
   ...
   */
  /**
   * The golden ratio scaled to 64bits, used as the initial gamma value for (unsplit)
   * SplittableRandoms.
   */
  ...
  /* ---------------- public methods ---------------- */
  /**
   * Creates a new SplittableRandom instance using the specified initial seed. SplittableRandom
   * instances created with the same seed in the same program generate identical sequences of
   * values.
   *
  ...
  /**
   * Creates a new SplittableRandom instance that is likely to generate sequences of values that are
   * statistically independent of those of any other instances in the current program; and may, and
   * typically does, vary across program invocations.
   */
  ...
  /**
   * Constructs and returns a new SplittableRandom instance that shares no mutable state with this
   * instance. However, with very high probability, the set of values collectively generated by the
   * two objects has the same statistical properties as if the same quantity of values were
   * generated by a single thread using a single SplittableRandom object. Either or both of the two
   * objects may be further split using the {@code split()} method, and the same expected
   * statistical properties apply to the entire set of generators constructed by such recursive
   * splitting.
   *
  ...
  public void nextBytes(byte[] bytes) {
    ...
    if (i < len) for (long rnd = nextLong(); i < len; rnd >>>= Byte.SIZE) bytes[i++] = (byte) rnd;
  }
  ...
  /**
   * Returns an effectively unlimited stream of pseudorandom {@code int} values from this generator
  ...
  public IntStream ints(long streamSize, int randomNumberOrigin, int randomNumberBound) {
    ...
    return StreamSupport.intStream(
        new RandomIntsSpliterator(this, 0L, streamSize, randomNumberOrigin, randomNumberBound),
        false);
  }

  /**
   * Returns an effectively unlimited stream of pseudorandom {@code int} values from this generator
   * and/or one split from it; each value conforms to the given origin (inclusive) and bound
   * (exclusive).
   *
   * <p><b>Implementation Note:</b><br>
  ...
  /**
   * Returns an effectively unlimited stream of pseudorandom {@code long} values from this generator
   * and/or one split from it.
   *
   * <p><b>Implementation Note:</b><br>
  ...
  public LongStream longs(long streamSize, long randomNumberOrigin, long randomNumberBound) {
    ...
    if (randomNumberOrigin >= randomNumberBound) throw new IllegalArgumentException(BAD_RANGE);
    return StreamSupport.longStream(
        new RandomLongsSpliterator(this, 0L, streamSize, randomNumberOrigin, randomNumberBound),
        false);
  }
  ...
  public LongStream longs(long randomNumberOrigin, long randomNumberBound) {
    if (randomNumberOrigin >= randomNumberBound) throw new IllegalArgumentException(BAD_RANGE);
    return StreamSupport.longStream(
        new RandomLongsSpliterator(this, 0L, Long.MAX_VALUE, randomNumberOrigin, randomNumberBound),
        false);
  }

  /**
   * Returns a stream producing the given {@code streamSize} number of pseudorandom {@code double}
   * values from this generator and/or one split from it; each value is between zero (inclusive) and
   * one (exclusive).
   *
   * @param streamSize the number of values to generate
   ...
   * generator and/or one split from it; each value is between zero (inclusive) and one (exclusive).
   *
   * <p><b>Implementation Note:</b><br>
  ...
  public DoubleStream doubles() {
    return StreamSupport.doubleStream(
        new RandomDoublesSpliterator(this, 0L, Long.MAX_VALUE, Double.MAX_VALUE, 0.0), false);
  }
  ...
  public DoubleStream doubles(
    ...
    if (!(randomNumberOrigin < randomNumberBound)) throw new IllegalArgumentException(BAD_RANGE);
    return StreamSupport.doubleStream(
        new RandomDoublesSpliterator(this, 0L, streamSize, randomNumberOrigin, randomNumberBound),
        false);
  }
  ...
  private static final class RandomDoublesSpliterator implements Spliterator.OfDouble {
    ...
    RandomDoublesSpliterator(
        SplittableRandom rng, long index, long fence, double origin, double bound) {
      this.rng = rng;
      this.index = index;
      this.fence = fence;
      this.origin = origin;
