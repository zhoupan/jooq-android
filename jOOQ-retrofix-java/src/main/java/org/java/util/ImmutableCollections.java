...
/**
 * Container class for unmodifiable collections. Not part of the public API. Mainly for namespace
 * management and shared infrastructure.
 *
 * <p>Serial warnings are suppressed throughout because all implementation classes use a serial
 * proxy and thus have no need to declare serialVersionUID.
 */
@SuppressWarnings("serial")
final class ImmutableCollections {

  /**
   * A "salt" value used for randomizing iteration order. This is initialized once and stays
   * constant for the lifetime of the JVM. It need not be truly random, but it needs to vary
   * sufficiently from one run to the next so that iteration order will vary between JVM runs.
   */
  static final long SALT32L;

  /**
   * For set and map iteration, we will iterate in "reverse" stochastically, decided at bootstrap
   * time.
   */
  ...
  static {
    ...
    // calculations.
    // slice of pi
    long color = 0x243F_6A88_85A3_08D3L;
  ...
  /**
   * The reciprocal of load factor. Given a number of elements to store, multiply by this factor to
   * get the table size.
   */
  ...
  /**
   * Copies a collection into a new List, unless the arg is already a safe, null-prohibiting
   * unmodifiable list, in which case the arg itself is returned. Null argument or null elements in
   * the argument will result in NPE.
   *
  ...
  static <E> List<E> listCopy(Collection<? extends E> coll) {
    if (coll instanceof List12 || (coll instanceof ListN && !((ListN<?>) coll).allowNulls)) {
      ...
      // implicit nullcheck of coll
      return (List<E>) Lists.of(coll.toArray());
    }
  }

  /**
   * Creates a new List from an untrusted array, creating a new array for internal storage, and
   * checking for and rejecting null elements.
   *
   * @param <E> the List's element type
  ...
  static <E> List<E> listFromArray(@SuppressWarnings("unchecked") E... input) {
    ...
    @SuppressWarnings("unchecked")
    E[] // implicit nullcheck of input
        tmp = (E[]) new Object[input.length];
   ...
   *
   * <p>A trusted array has no references retained by the caller. It can therefore be safely reused
   * as the List's internal storage, avoiding a defensive copy. The array's class must be
   * Object[].class. This method is declared with a parameter type of Object... instead of E... so
   * that a varargs call doesn't accidentally create an array of some class other than
   * Object[].class.
   *
   * @param <E> the List's element type
   * @param input the input array
  ...
  static <E> List<E> listFromTrustedArray(Object... input) {
    for (Object o : input) {
      // implicit null check of 'input' array
  ...
  static final class SubList<E> extends AbstractImmutableList<E> implements RandomAccess {
    ...
    /**
     * Constructs a sublist of an arbitrary AbstractImmutableList, which is not a SubList itself.
     */
  ...
  static final class Set12<E> extends AbstractImmutableSet<E> implements Serializable {
    ...
    @Override
    public boolean contains(Object o) {
      // implicit nullcheck of o
      return o.equals(e0) || e1.equals(o);
    }
  ...
  /**
   * An array-based Set implementation. The element array must be strictly larger than the size (the
   * number of contained elements) so that at least one null is always present.
   *
  ...
  /**
   * An array-based Map implementation. There is a single array "table" that contains keys and
   * values interleaved: table[0] is kA, table[1] is vA, table[2] is kB, table[3] is vB, etc. The
   * table size must be even. It must also be strictly larger than the size (the number of key-value
   * pairs contained in the map) so that at least one null key is always present.
   *
    ...
    class MapNIterator extends Iterators.ImmutableIt<Map.Entry<K, V>>
      ...
      public Map.Entry<K, V> next() {
        if (remaining > 0) {
          ...
          @SuppressWarnings("unchecked")
          Map.Entry<K, V> e = new KeyValueHolder<K, V>((K) table[idx], (V) table[idx + 1]);
          remaining--;
          return e;
        } else {
          throw new NoSuchElementException();
...
final class ColSer implements Serializable {
  ...
  /**
   * Indicates the type of collection that is serialized. The low order 8 bits have the value 1 for
   * an immutable {@code List}, 2 for an immutable {@code Set}, 3 for an immutable {@code Map}, and
   * 4 for an immutable {@code List} that allows null elements.
   *
   * <p>Any other value causes an {@link InvalidObjectException} to be thrown. The high order 24
   * bits are zero when an instance is serialized, and they are ignored when an instance is
   * deserialized. They can thus be used by future implementations without causing compatibility
   * issues.
   *
   * <p>The tag value also determines the interpretation of the transient {@code Object[] array}
   * field. For {@code List} and {@code Set}, the array's length is the size of the collection, and
   * the array contains the elements of the collection. Null elements are not allowed. For {@code
   * Set}, duplicate elements are not allowed.
   *
   * <p>For {@code Map}, the array's length is twice the number of mappings present in the map. The
   * array length is necessarily even. The array contains a succession of key and value pairs:
   * {@code k1, v1, k2, v2, ..., kN, vN.} Nulls are not allowed, and duplicate keys are not allowed.
   *
   * @serial
   * @since 9
   */
  ...
  /**
   * Reads objects from the stream and stores them in the transient {@code Object[] array} field.
   *
   * @serialData A nonnegative int, indicating the count of objects, followed by that many objects.
  ...
  /**
   * Writes objects to the stream from the transient {@code Object[] array} field.
   *
   * @serialData A nonnegative int, indicating the count of objects, followed by that many objects.
  ...
  /**
   * Creates and returns an unmodifiable collection from this proxy class. The instance returned is
   * created as if by calling one of the static factory methods for <a
   * href="Lists.html#unmodifiable">List</a>, <a href="Maps.html#unmodifiable">Map</a>, or <a
   * href="Sets.html#unmodifiable">Set</a>. This proxy class is the serial form for all unmodifiable
   * collection instances, regardless of implementation type. This is necessary to ensure that the
   * existence of any particular implementation type is kept out of the serialized form.
   *
   * @return a collection created from this proxy object
   * @throws InvalidObjectException if the tag value is illegal or if an exception is thrown during
