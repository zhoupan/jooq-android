...
/**
 * A container object which may or may not contain an {@code int} value. If a value is present,
 ...
 * such as {@link #orElse(int) orElse()} (returns a default value if no value is present) and {@link
 * #ifPresent(IntConsumer) ifPresent()} (performs an action if a value is present).
 *
 ...
 * code, or synchronization) on instances of {@code OptionalInt} may have unpredictable results and
 * should be avoided.
 *
 * <p><b>API Note:</b><br>
 * {@code OptionalInt} is primarily intended for use as a method return type where there is a clear
 * need to represent "no result". A variable whose type is {@code OptionalInt} should never itself
 * be {@code null}; it should always point to an {@code OptionalInt} instance.
 *
 * @since 1.8
 */
public final class OptionalInt {
   ...
   * Generally only one empty instance, {@link OptionalInt#EMPTY}, should exist per VM.
   */
  private OptionalInt() {
   ...
   * {@code ==} or {@code !=} against instances returned by {@code OptionalInt.empty()}. There is no
  ...
  /**
   * If a value is present, returns a sequential {@link IntStream} containing only that value,
   * otherwise returns an empty {@code IntStream}.
   *
   * <p><b>API Note:</b><br>
   * This method can be used to transform a {@code Stream} of optional integers to an {@code
   * IntStream} of present integers:
   *
   * <pre>{@code
  ...
  /**
   * Indicates whether some other object is "equal to" this {@code OptionalInt}. The other object is
   * considered equal if:
   *
   * <ul>
  ...
  /**
   * Returns a non-empty string representation of this {@code OptionalInt} suitable for debugging.
   ...
   * and present {@code OptionalInt}s must be unambiguously differentiable.
   *
