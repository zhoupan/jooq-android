 ...
 * interface methods in the {@link Map} interface and, in addition, the <a
 * href="http://openjdk.java.net/jeps/269">JEP 269</a> {@code "Unmodifiable Map Static Factory
 * Methods"} in the {@link Map} interface that were introduced in Java 9.
 *
 * <h2><a id="unmodifiable">Unmodifiable Maps</a></h2>
 *
 * <p>The {@link Maps#of() Maps.of}, {@link Maps#ofEntries(Map.Entry...) Maps.ofEntries}, and {@link
 * Maps#copyOf(Map) Maps.copyOf} static factory methods provide a convenient way to create
 * unmodifiable maps. The {@code Map} instances created by these methods have the following
 * characteristics:
 *
 * <ul>
 *   <li>They are <a href="./package-summary.html#unmodifiable"><i>unmodifiable</i></a>. Keys and
 *       values cannot be added, removed, or updated. Calling any mutator method on the Map will
 *       always cause {@code UnsupportedOperationException} to be thrown. However, if the contained
 *       keys or values are themselves mutable, this may cause the Map to behave inconsistently or
 *       its contents to appear to change.
 *   <li>They disallow {@code null} keys and values. Attempts to create them with {@code null} keys
 *       or values result in {@code NullPointerException}.
 *   <li>They are serializable if all keys and values are serializable.
 *   <li>They reject duplicate keys at creation time. Duplicate keys passed to a static factory
 *       method result in {@code IllegalArgumentException}.
 *   <li>The iteration order of mappings is unspecified and is subject to change.
 *   <li>They are <a href="../lang/package-summary.html#Value-based-Classes">value-based</a>.
 *       Callers should make no assumptions about the identity of the returned instances. Factories
 *       are free to create new instances or reuse existing ones. Therefore, identity-sensitive
...
public final class Maps {
   ...
   *     (optional)
   * @throws ClassCastException if the key or value is of an inappropriate type for the map
   *     (optional)
   * @throws NullPointerException if the specified key or value is null, and the map does not permit
   *     null keys or values (optional)
   * @throws IllegalArgumentException if some property of the specified key or value prevents it
   *     from being stored in the map (optional)
   * @since 1.8
   */
  ...
  /**
   * Returns the value to which the specified key is mapped, or {@code defaultValue} if the map
   ...
   * @return the value to which the specified key is mapped, or {@code defaultValue} if the map
   *     contains no mapping for the key
   * @throws ClassCastException if the key is of an inappropriate type for the map (optional)
   * @throws NullPointerException if the specified key is null and the map does not permit null keys
   *     (optional)
   * @since 1.8
   */
  ...
  /**
   * Performs the given action for each entry in the map until all entries have been processed or
   ...
   * results of the given remapping function, or removes if the result is {@code null}. This method
   * may be of use when combining multiple mapped values for a key. For example, to either create or
   ...
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation is equivalent to performing the following steps for the {@code map},
   ...
   * modifies the passed map during computation and, if appropriate, reporting an error.
   * Non-concurrent implementations should override this method and, on a best-effort basis, throw a
   * {@code ConcurrentModificationException} if it is detected that the remapping function modifies
   * the map during computation. Concurrent implementations should override this method and, on a
   * best-effort basis, throw an {@code IllegalStateException} if it is detected that the remapping
   * function modifies the map during computation and as a result computation would never complete.
   *
   * <p>The default implementation makes no guarantees about synchronization or atomicity properties
   * of this method. Any implementation providing atomicity guarantees must override this method and
   * document its concurrency properties. In particular, all implementations of subinterface {@link
   * java.util.concurrent.ConcurrentMap} must document whether the remapping function is applied
   ...
   *     stored in the map (optional)
   * @throws NullPointerException if the specified key is null and the map does not support null
  ...
  /**
   * If the specified key is not already associated with a value (or is mapped to {@code null}),
   * attempts to compute its value using the given mapping function and enters it into the passed
   * {@code map} unless {@code null}.
   *
   * <p>If the mapping function returns {@code null} no mapping is recorded. If the mapping function
   * itself throws an (unchecked) exception, the exception is rethrown, and no mapping is recorded.
   * The most common usage is to construct a new object serving as an initial mapped value or
   ...
   * }</pre>
   *
   * <p>Or to implement a multi-value map, {@code Map<K,Collection<V>>}, supporting multiple values
   * per key:
   *
   * <pre>{@code
   ...
   *
   * <p><b>Implementation Requirements:</b><br>
   * The default implementation is equivalent to the following steps for the {@code map}, then
   ...
   * }</pre>
   *
   * <p>The default implementation makes no guarantees about detecting if the mapping function
   * modifies the passed map during computation and, if appropriate, reporting an error.
   * Non-concurrent implementations should override this method and, on a best-effort basis, throw a
   * {@code ConcurrentModificationException} if it is detected that the mapping function modifies
   * the map during computation. Concurrent implementations should override this method and, on a
   * best-effort basis, throw an {@code IllegalStateException} if it is detected that the mapping
   * function modifies the map during computation and as a result computation would never complete.
   *
   * <p>The default implementation makes no guarantees about synchronization or atomicity properties
   * of this method. Any implementation providing atomicity guarantees must override this method and
   * document its concurrency properties. In particular, all implementations of subinterface {@link
   * java.util.concurrent.ConcurrentMap} must document whether the mapping function is applied once
   ...
   *     stored in the map
   * @throws NullPointerException if a specified key or newValue is null, and the map does not
   *     permit null keys or values
   * @throws NullPointerException if oldValue is null and the map does not permit null values
   *     (optional)
   ...
   * @throws UnsupportedOperationException if the {@code set} operation is not supported by the
   ...
   *     value is null, and the map does not permit null values
   * @throws ClassCastException if a replacement value is of an inappropriate type for the map
   *     (optional)
   * @throws NullPointerException if function or a replacement value is null, and the map does not
  ...
  public static <K, V> void replaceAll(
      Map<K, V> map, BiFunction<? super K, ? super V, ? extends V> function) {
    Objects.requireNonNull(map);
   ...
   * null} if there is no current mapping). For example, to either create or append a {@code String}
   ...
   *
   * <p>If the remapping function returns {@code null}, the mapping is removed (or remains absent if
   * initially absent). If the remapping function itself throws an (unchecked) exception, the
   ...
   * @throws UnsupportedOperationException if the {@code remove} operation is not supported by the
   *     map (optional)
   * @throws ClassCastException if the key or value is of an inappropriate type for the map
   *     (optional)
   * @throws NullPointerException if the specified key or value is null, and the map does not permit
  ...
  /** A place for the static interface methods of the Java 8 {@link Map.Entry} interface. */
  public static final class Entry {
    ...
    /**
     * Returns a comparator that compares {@link Map.Entry} by key using the given {@link
     * Comparator}.
     *
    ...
    /**
     * Returns a copy of the given {@code Map.Entry}. The returned instance is not associated with
     * any map. The returned instance has the same characteristics as instances returned by the
     * {@link Maps#entry Maps::entry} method.
     *
     * <p><b>API Note:</b><br>
     * An instance obtained from a map's entry-set view has a connection to that map. The {@code
     * copyOf} method may be used to create a {@code Map.Entry} instance, containing the same key
     * and value, that is independent of any map.
     *
     * <p><b>Implementation Note:</b> If the given entry was obtained from a call to {@code copyOf}
     * or {@code Maps::entry}, calling {@code copyOf} will generally not create another copy.
     *
  ...
  /**
   * Returns an unmodifiable map containing zero mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
  ...
  /**
   * Returns an unmodifiable map containing a single mapping. See <a
   * href="#unmodifiable">Unmodifiable Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing two mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing three mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing four mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing five mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing six mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing seven mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  public static <K, V> Map<K, V> of(
    ...
    return new ImmutableCollections.MapN<K, V>(
        k1, v1, k2, v2, k3, v3, k4, v4, k5, v5, k6, v6, k7, v7);
  }

  /**
   * Returns an unmodifiable map containing eight mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing nine mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing ten mappings. See <a href="#unmodifiable">Unmodifiable
   * Maps</a> for details.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
  ...
  /**
   * Returns an unmodifiable map containing keys and values extracted from the given entries. The
   * entries themselves are not stored in the map. See <a href="#unmodifiable">Unmodifiable Maps</a>
   * for details.
   *
   * <p><b>API Note:</b><br>
   ...
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
   * @param entries {@code Map.Entry}s containing the keys and values from which the map is
   *     populated
   * @return a {@code Map} containing the specified mappings
   * @throws IllegalArgumentException if there are any duplicate keys
   * @throws NullPointerException if any entry, key, or value is {@code null}, or if the {@code
   *     entries} array is {@code null}
   * @see Maps#entry Maps.entry()
   * @since 9
   */
  public static <K, V> Map<K, V> ofEntries(Map.Entry<? extends K, ? extends V>... entries) {
    if (entries.length == 0) {
      // implicit null check of entries array
      @SuppressWarnings("unchecked")
  ...
  /**
   * Returns an unmodifiable {@link java.util.Map.Entry} containing the given key and value. These
   * entries are suitable for populating {@code Map} instances using the {@link Maps#ofEntries
   * Maps.ofEntries()} method. The {@code Entry} instances created by this method have the following
   * characteristics:
   *
   * <ul>
   *   <li>They disallow {@code null} keys and values. Attempts to create them using a {@code null}
   *       key or value result in {@code NullPointerException}.
   *   <li>They are unmodifiable. Calls to {@link Map.Entry#setValue Entry.setValue()} on a returned
   *       {@code Entry} result in {@code UnsupportedOperationException}.
   *   <li>They are not serializable.
   *   <li>They are <a href="../lang/package-summary.html#Value-based-Classes">value-based</a>.
   *       Callers should make no assumptions about the identity of the returned instances. This
   ...
   *
   * <p><b>API Note:</b><br>
   * For a serializable {@code Entry}, see {@link java.util.AbstractMap.SimpleEntry} or {@link
  ...
  /**
   * Returns an <a href="#unmodifiable">unmodifiable Map</a> containing the entries of the given
   * Map. The given Map must not be null, and it must not contain any null keys or values. If the
   * given Map is subsequently modified, the returned Map will not reflect such modifications.
   *
   * <p><b>Implementation Note:</b> If the given Map is an <a href="#unmodifiable">unmodifiable
   * Map</a>, calling copyOf will generally not create a copy.
   *
   * @param <K> the {@code Map}'s key type
   * @param <V> the {@code Map}'s value type
