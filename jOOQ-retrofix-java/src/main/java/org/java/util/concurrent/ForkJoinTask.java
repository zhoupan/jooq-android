 ...
 *
 * <p>A "main" {@code ForkJoinTask} begins execution when it is explicitly submitted to a {@link
 * ForkJoinPool}, or, if not already engaged in a ForkJoin computation, commenced in the {@link
 * ForkJoinPool#commonPool()} via {@link #fork}, {@link #invoke}, or related methods. Once started,
 * it will usually in turn start other subtasks. As indicated by the name of this class, many
 * programs using {@code ForkJoinTask} employ only methods {@link #fork} and {@link #join}, or
 ...
 *
 * <p>A {@code ForkJoinTask} is a lightweight form of {@link Future}. The efficiency of {@code
 * ForkJoinTask}s stems from a set of restrictions (that are only partially statically enforceable)
 * reflecting their main use as computational tasks calculating pure functions or operating on
 * purely isolated objects. The primary coordination mechanisms are {@link #fork}, that arranges
 * asynchronous execution, and {@link #join}, that doesn't proceed until the task's result has been
 * computed. Computations should ideally avoid {@code synchronized} methods or blocks, and should
 * minimize other blocking synchronization apart from joining other tasks or using synchronizers
 * such as Phasers that are advertised to cooperate with fork/join scheduling. Subdividable tasks
 * should also not perform blocking I/O, and should ideally access variables that are completely
 * independent of those accessed by other running tasks. These guidelines are loosely enforced by
 * not permitting checked exceptions such as {@code IOExceptions} to be thrown. However,
 ...
 * latter.
 *
 * <p>It is possible to define and use ForkJoinTasks that may block, but doing so requires three
 ...
 * performance.
 *
 * <p>The primary method for awaiting completion and extracting results of a task is {@link #join},
 * but there are several variants: The {@link Future#get} methods support interruptible and/or timed
 ...
 *
 * <p>In the most typical usages, a fork-join pair act like a call (fork) and return (join) from a
 * parallel recursive function. As is the case with other forms of recursive calls, returns (joins)
 * should be performed innermost-first. For example, {@code a.fork(); b.fork(); b.join(); a.join();}
 * is likely to be substantially more efficient than joining {@code a} before {@code b}.
 *
 * <p>The execution status of tasks may be queried at several levels of detail: {@link #isDone} is
 * true if a task completed in any way (including the case where a task was cancelled without
 * executing); {@link #isCompletedNormally} is true if a task completed without cancellation or
 * encountering an exception; {@link #isCancelled} is true if the task was cancelled (in which case
 * {@link #getException} returns a {@link java.util.concurrent.CancellationException}); and {@link
 * #isCompletedAbnormally} is true if a task was either cancelled or encountered an exception, in
 * which case {@link #getException} will return either the encountered exception or {@link
 * java.util.concurrent.CancellationException}.
 *
 * <p>The ForkJoinTask class is not usually directly subclassed. Instead, you subclass one of the
 * abstract classes that support a particular style of fork/join processing, typically {@link
 * org.java.util.concurrent.RecursiveAction} for most computations that do not return results,
 * {@link org.java.util.concurrent.RecursiveTask} for those that do, and {@link
 * org.java.util.concurrent.CountedCompleter} for those in which completed actions trigger other
 ...
 *
 * <p>Method {@link #join} and its variants are appropriate for use only when completion
 * dependencies are acyclic; that is, the parallel computation can be described as a directed
 * acyclic graph (DAG). Otherwise, executions may encounter a form of deadlock as tasks cyclically
 * wait for each other. However, this framework supports other methods and techniques (for example
 * the use of {@link java.util.concurrent.Phaser}, {@link #helpQuiesce}, and {@link #complete}) that
 * may be of use in constructing custom subclasses for problems that are not statically structured
 * as DAGs. To support such usages, a ForkJoinTask may be atomically <em>tagged</em> with a {@code
 * short} value using {@link #setForkJoinTaskTag} or {@link #compareAndSetForkJoinTaskTag} and
 * checked using {@link #getForkJoinTaskTag}. The ForkJoinTask implementation does not use these
 * {@code protected} methods or tags for any purpose, but they may be of use in the construction of
 * specialized subclasses. For example, parallel graph traversals can use the supplied methods to
 * avoid revisiting nodes/tasks that have already been processed. (Method names for tagging are
 * bulky in part to encourage definition of methods that reflect their usage patterns.)
 *
 * <p>Most base support methods are {@code final}, to prevent overriding of implementations that are
 ...
 *
 * <p>ForkJoinTasks should perform relatively small amounts of computation. Large tasks should be
 * split into smaller subtasks, usually via recursive decomposition. As a very rough rule of thumb,
 * a task should perform more than 100 and less than 10000 basic computational steps, and should
 * avoid indefinite looping. If tasks are too big, then parallelism cannot improve throughput. If
 * too small, then memory and internal task maintenance overhead may overwhelm processing.
 *
 * <p>This class provides {@code adapt} methods for {@link Runnable} and {@link Callable}, that may
 * be of use when mixing execution of {@code ForkJoinTasks} with other kinds of tasks. When all
 * tasks are of this form, consider using a pool constructed in <em>asyncMode</em>.
 *
 * <p>ForkJoinTasks are {@code Serializable}, which enables them to be used in extensions such as
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
   ...
   *
   * <p>These control bits occupy only (some of) the upper half (16 bits) of status field. The lower
   ...
   * values.
   *
   * <p>The exception table has a fixed capacity.
   */
  ...
  static <T extends Throwable> void uncheckedThrow(Throwable t) throws T {
    if (t != null)
      ...
      throw (T) t;
    else throw new Error("Unknown Exception");
  }
   ...
   * from {@link #get()} in that abnormal completion results in {@code RuntimeException} or {@code
   * Error}, not {@code ExecutionException}, and that interrupts of the calling thread do
   ...
   *
   * <p>This method may be overridden in subclasses, but if so, must still ensure that these
   * properties hold. In particular, the {@code cancel} method itself must not throw exceptions.
   *
   * <p>This method is designed to be invoked by <em>other</em> tasks. To terminate the current
   * task, you can just return or throw an unchecked exception from its computation method, or
   ...
   *     Error}, the actual exception thrown will be a {@code RuntimeException} with cause {@code
   *     ex}.
   */
   ...
   * {@link #setRawResult} (or {@code null} by default) will be returned as the result of subsequent
  ...
  public final V get() throws InterruptedException, ExecutionException {
    ...
    if (s == EXCEPTIONAL) throw new ExecutionException(getThrowableException());
    return getRawResult();
  }
  ...
  /**
   * Possibly executes tasks until the pool hosting the current task {@linkplain
   * ForkJoinPool#isQuiescent is quiescent}. This method may be of use in designs in which many
   * tasks are forked, but none are explicitly joined, instead executing them until all are
   * processed.
   */
   ...
   *
   * <p>Upon completion of this method, {@code isDone()} reports {@code false}, and {@code
   * getException()} reports {@code null}. However, the value returned by {@code getRawResult} is
   * unaffected. To clear this value, you can invoke {@code setRawResult(null)}.
   */
  public void reinitialize() {
    if ((status & DONE_MASK) == EXCEPTIONAL) clearExceptionalCompletion();
    else status = 0;
  }
   ...
   *
   * <p>This method returns {@code null} if and only if {@link #inForkJoinPool} returns {@code
   * false}.
   *
   ...
   * method, this task is guaranteed to have completed. This method may return false otherwise, to
   * indicate that this task is not necessarily complete (or is not known to be complete), for
   * example in asynchronous actions that require explicit invocations of completion methods. This
   * method may also throw an (unchecked) exception to indicate abnormal exit. This method is
   * designed to support extensions, and should not in general be called otherwise.
   *
   ...
   * be used as visit markers in tasks operating on graphs, as in methods that check: {@code if
   * (task.compareAndSetForkJoinTaskTag((short)0, (short)1))} before processing, otherwise exiting
   * because the node has already been visited.
   *
  ...
  public final boolean compareAndSetForkJoinTaskTag(short expect, short update) {
    for (int s; ; ) {
      if ((short) (s = status) != expect) return false;
      if (U.compareAndSwapInt(this, STATUS, s, (s & ~SMASK) | (update & SMASK))) return true;
    }
  }
  ...
  /**
   * Returns a new {@code ForkJoinTask} that performs the {@code run} method of the given {@code
   * Runnable} as its action, and returns a null result upon {@link #join}.
   *
  ...
  /**
   * Returns a new {@code ForkJoinTask} that performs the {@code run} method of the given {@code
   * Runnable} as its action, and returns the given result upon {@link #join}.
   *
   * @param runnable the runnable action
  ...
  /**
   * Returns a new {@code ForkJoinTask} that performs the {@code call} method of the given {@code
   * Callable} as its action, and returns its result upon {@link #join}, translating any checked
