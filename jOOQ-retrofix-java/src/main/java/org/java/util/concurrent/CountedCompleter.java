 ...
 * zero, but may be (atomically) changed using methods {@link #setPendingCount}, {@link
 * #addToPendingCount}, and {@link #compareAndSetPendingCount}. Upon invocation of {@link
 * #tryComplete}, if the pending action count is nonzero, it is decremented; otherwise, the
 ...
 *
 * <p>A concrete CountedCompleter class must define method {@link #compute}, that should in most
 * cases (as illustrated below), invoke {@code tryComplete()} once before returning. The class may
 * also optionally override method {@link #onCompletion(CountedCompleter)} to perform an action upon
 * normal completion, and method {@link #onExceptionalCompletion(Throwable, CountedCompleter)} to
 * perform an action upon any exception.
 *
 * <p>CountedCompleters most often do not bear results, in which case they are normally declared as
 * {@code CountedCompleter<Void>}, and will always return {@code null} as a result value. In other
 * cases, you should override method {@link #getRawResult} to provide a result from {@code join(),
 * invoke()}, and related methods. In general, this method should return the value of a field (or a
 * function of one or more fields) of the CountedCompleter object that holds the result upon
 * completion. Method {@link #setRawResult} by default plays no role in CountedCompleters. It is
 * possible, but rarely applicable, to override this method to maintain other objects or fields
 * holding result data.
 *
 * <p>A CountedCompleter that does not itself have a completer (i.e., one for which {@link
 * #getCompleter} returns {@code null}) can be used as a regular ForkJoinTask with this added
 ...
 * ForkJoinTask#completeExceptionally(Throwable)} or upon exceptional completion of method {@code
 * compute}. Upon any exceptional completion, the exception may be relayed to a task's completer
 * (and its completer, and so on), if one exists and it has not otherwise already completed.
 * Similarly, cancelling an internal CountedCompleter has only a local effect on that completer, so
 * is not often useful.
 *
 * <p><b>Sample Usages.</b>
 *
 * <p><b>Parallel recursive decomposition.</b> CountedCompleters may be arranged in trees similar to
 * those often used with {@link org.java.util.concurrent.RecursiveAction}s, although the
 * constructions involved in setting them up typically vary. Here, the completer of each task is its
 * parent in the computation tree. Even though they entail a bit more bookkeeping, CountedCompleters
 * may be better choices when applying a possibly time-consuming operation (that cannot be further
 * subdivided) to each element of an array or collection; especially when the operation takes a
 * significantly different amount of time to complete for some elements than others, either because
 * of intrinsic variation (for example I/O) or auxiliary effects such as garbage collection. Because
 * CountedCompleters provide their own continuations, other tasks need not block waiting to perform
 * them.
 *
 * <p>For example, here is an initial version of a utility method that uses divide-by-two recursive
 ...
 * subtask (a "tail call"), the call to {@code tryComplete()} can be optimized away, at the cost of
 * making the pending count look "off by one".
 *
 * <pre>{@code
 * public void compute() {
 *   if (hi - lo >= 2) {
 ...
 *
 * <p><b>Searching.</b> A tree of CountedCompleters can search for a value or property in different
 * parts of a data structure, and report a result in an {@link
 * java.util.concurrent.atomic.AtomicReference AtomicReference} as soon as one is found. The others
 * can poll the result to avoid unnecessary work. (You could additionally {@linkplain #cancel
 ...
 * }</pre>
 *
 * In this example, as well as others in which tasks have no other effects except to {@code
 * compareAndSet} a common result, the trailing unconditional invocation of {@code tryComplete}
 * could be made conditional ({@code if (result.get() == null) tryComplete();}) because no further
 * bookkeeping is required to manage completions once the root task completes.
 *
 * <p><b>Recording subtasks.</b> CountedCompleter tasks that combine results of multiple subtasks
 ...
 * count is zero upon invocation of {@code tryComplete}, or (2) by any of its subtasks when they
 * complete and decrement the pending count to zero. The {@code caller} argument distinguishes
 * cases. Most often, when the caller is {@code this}, no action is necessary. Otherwise the caller
 * argument can be used (usually via a cast) to supply a value (and/or links to other values) to be
 * combined. Assuming proper use of pending counts, the actions inside {@code onCompletion} occur
 * (once) upon completion of a task and its subtasks. No additional synchronization is required
 * within this method to ensure thread safety of accesses to fields of this task or other completed
 * tasks.
 *
 * <p><b>Completion Traversals</b>. If using {@code onCompletion} to process completions is
 * inapplicable or inconvenient, you can use methods {@link #firstComplete} and {@link
 * #nextComplete} to create custom traversals. For example, to define a MapReducer that only splits
 * out right-hand tasks in the form of the third ForEach example, the completions must cooperatively
 ...
 * }</pre>
 *
 * <p><b>Triggers.</b> Some CountedCompleters are themselves never forked, but instead serve as bits
 * of plumbing in other designs; including those in which the completion of one or more async tasks
...
public abstract class CountedCompleter<T> extends ForkJoinTask<T> {
   ...
   * not equal to {@code this}, then it is typically a subtask that may contain results (and/or
  ...
  /**
   * If the pending count is nonzero, decrements the count; otherwise invokes {@link
   * #onCompletion(CountedCompleter)} and then similarly tries to complete this task's completer, if
   ...
   * The given rawResult is used as an argument to {@link #setRawResult} before invoking {@link
   * #onCompletion(CountedCompleter)} or marking this task as complete; its value is meaningful only
   * for classes overriding {@code setRawResult}. This method does not modify the pending count.
   *
   * <p>This method may be useful when forcing completion as soon as any one (versus all) of several
   * subtask results are obtained. However, in the common (and recommended) case in which {@code
   * setRawResult} is not overridden, this effect can be obtained more simply using {@link
   * #quietlyCompleteRoot()}.
   *
  ...
  public final CountedCompleter<?> nextComplete() {
    CountedCompleter<?> p;
    if ((p = completer) != null) return p.firstComplete();
    else {
      quietlyComplete();
      return null;
    }
