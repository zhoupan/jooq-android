 ...
 *
 * <p>A {@code ForkJoinPool} differs from other kinds of {@link ExecutorService} mainly by virtue of
 ...
 *
 * <p>A static {@link #commonPool()} is available and appropriate for most applications. The common
 ...
 *
 * <p>For applications that require separate or custom pools, a {@code ForkJoinPool} may be
 * constructed with a given target parallelism level; by default, equal to the number of available
 * processors. The pool attempts to maintain enough active (or available) threads by dynamically
 * adding, suspending, or resuming internal worker threads, even if some tasks are stalled waiting
 * to join others. However, no such adjustments are guaranteed in the face of blocked I/O or other
 * unmanaged synchronization. The nested {@link ManagedBlocker} interface enables extension of the
 * kinds of synchronization accommodated. The default policies may be overridden using a constructor
 * with parameters corresponding to those documented in class {@link ThreadPoolExecutor}.
 *
 * <p>In addition to execution and lifecycle control methods, this class provides status check
 * methods (for example {@link #getStealCount}) that are intended to aid in developing, tuning, and
 ...
 *
 * <p>As is the case with other ExecutorServices, there are three main task execution methods
 ...
 *   <li>{@code java.util.concurrent.ForkJoinPool.common.threadFactory} - the class name of a {@link
 *       ForkJoinWorkerThreadFactory}. The {@linkplain ClassLoader#getSystemClassLoader() system
 ...
 *       {@link UncaughtExceptionHandler}. The {@linkplain ClassLoader#getSystemClassLoader() system
 *       class loader} is used to load this class.
 ...
 *
 * <p>Upon any error in establishing these settings, default parameters are used. It is possible to
 ...
 *
 * <p><b>Implementation Note:</b> This implementation restricts the maximum number of running
 * threads to 32767. Attempts to create pools with greater than the maximum number result in {@code
 * IllegalArgumentException}.
 *
 * <p>This implementation rejects submitted tasks (that is, by throwing {@link
 * RejectedExecutionException}) only when the pool is shut down or internal resources have been
 * exhausted.
 *
 * @since 1.7
 * @author Doug Lea
...
public class ForkJoinPool extends AbstractExecutorService {
  ...
  static AccessControlContext contextWithPermissions(Permission... perms) {
    ...
    for (Permission perm : perms) permissions.add(perm);
  ...
  private static final class DefaultForkJoinWorkerThreadFactory
    ...
    private static final AccessControlContext ACC =
        contextWithPermissions( // new RuntimePermission("setContextClassLoader"), // streamsupport
            // changed
  ...
  // queue has owner thread
  static final int OWNED = 1;

  // fifo queue or access mode
  ...
  static final class WorkQueue {
    ...
    // versioned, negative: queued, 1: locked
    volatile int phase;
    ...
    final ForkJoinTask<?>[] growArray() {
      ...
      if (oldA != null && (oldMask = oldSize - 1) > 0 && (t = top) - (b = base) > 0) {
        ...
        do {
          ...
          if (x != null && U.compareAndSwapObject(oldA, offset, x, null)) a[b & mask] = x;
        } while (++b != t);
        MemBar.storeFence();
      }
      return a;
    ...
    final void localPopAndExec(int limit) {
      for (; ; ) {
        ...
        if ((a = array) != null && b != s && (al = a.length) > 0) {
          int index = (al - 1) & --s;
          long offset = ((long) index << ASHIFT) + ABASE;
          ForkJoinTask<?> t = (ForkJoinTask<?>) getAndSetObject(a, offset, null);
          if (t != null) {
            top = s;
            MemBar.storeFence();
            t.doExec();
    ...
    final void tryRemoveAndExec(ForkJoinTask<?> task) {
      ForkJoinTask<?>[] wa;
      int s, wal;
      if ( // traverse from top
      base - (s = top) < 0 && (wa = array) != null && (wal = wa.length) > 0) {
    ...
    final int localHelpCC(CountedCompleter<?> task, int limit) {
      ...
      if (task != null && (status = task.status) >= 0) {
        for (; ; ) {
          ...
          if ((status = task.status) < 0 || !help || (limit != 0 && --limit == 0)) break;
        }
      }
      return status;
    }
    ...
    final boolean trySharedUnpush(ForkJoinTask<?> task) {
      ...
      if ((a = array) != null && (al = a.length) > 0) {
        ...
        if (t == task && U.compareAndSwapInt(this, PHASE, 0, QLOCK)) {
          if ( // "lgtm[java/constant-comparison]"
          top == s + 1 && array == a && U.compareAndSwapObject(a, offset, task, null)) {
            popped = true;
            top = s;
          }
        ...
        for (; ; ) {
          ...
          if ((a = array) != null && b != s && (al = a.length) > 0) {
            ...
            if (o instanceof CountedCompleter) {
              ...
              for (CountedCompleter<?> f = t; ; ) {
                if (f != task) {
                  if ((f = f.completer) == null) break;
                } else {
                  if (U.compareAndSwapInt(this, PHASE, 0, QLOCK)) {
                    if ( // "lgtm[java/constant-comparison]"
                    top == s && array == a && U.compareAndSwapObject(a, offset, t, null)) {
                      help = true;
                      top = s - 1;
                    }
                    U.putOrderedInt(this, PHASE, 0);
  ...
  // collects worker nsteals
  volatile long stealCount;

  // milliseconds before dropping if idle
  final long keepAlive;

  // next worker index
  int indexSeed;

  // min, max threads packed as shorts
  final int bounds;
  ...
  // for worker thread string; sync lock
      ...
      synchronized (lock) {
        ...
        if (ws != null && (n = ws.length) > 1) {
          ...
          for (int probes = n >>> 1; ; ) {
            // find empty slot
            WorkQueue q;
            if ((q = ws[i]) == null || q.phase == QUIET) break;
            else if (--probes == 0) {
              // resize below
              i = n | 1;
              break;
          ...
          if (i < n) ws[i] = w;
            ...
            for (int j = 0; j < n; ++j) {
              // copy external queue
              WorkQueue v;
              if ( // position may change
              (v = ws[j]) != null) as[v.id & am & SQMASK] = v;
              if (++j >= n) break;
              // copy worker
              as[j] = ws[j];
            }
  ...
  final void deregisterWorker(ForkJoinWorkerThread wt, Throwable ex) {
    ...
    if (w != null)
      ...
      w.cancelAll();
    if ( // possibly replace worker
    !tryTerminate(false, false)
        && w != null
        && // avoid repeated failures
        w.array != null) signalWork();
    if ( // help clean on way out
    ex == null) ForkJoinTask.helpExpungeStaleExceptions();
    else
      // rethrow
  ...
  final void signalWork() {
    for (; ; ) {
      ...
      (c = ctl) >= 0L) break;
        ...
        // no idle workers
        if ( // too few workers
        (c & ADD_WORKER) != 0L) tryAddWorker(c);
        break;
        ...
        if (sp == vp && U.compareAndSwapLong(this, CTL, c, nc)) {
          v.phase = np;
          if (v.source < 0) LockSupport.unpark(vt);
          break;
        }
      }
    }
    ...
    if ((t = (short) (c >>> TC_SHIFT)) >= 0) {
      ...
      (int) (c >> RC_SHIFT) - (short) (bounds & SMASK) > 0) {
        ...
        if (unstable || tc != 0 || ctl != c)
          ...
          if ((sat = saturate) != null && sat.test(this)) return -1;
          else if (bc < pc) {
            // lagging
            // for retry spins
            Thread.yield();
  ...
  final void runWorker(WorkQueue w) {
    ...
    if ( // initial nonzero seed
    r == 0) r = 1;
    // avoid unneeded signals
    int lastSignalId = 0;
      ...
      for (int n = ws.length, j = n, m = n - 1; j > 0; --j) {
        ...
        (i = r & m) >= 0
          ...
          if (t != null && b++ == q.base && U.compareAndSwapObject(a, offset, t, null)) {
            ...
            t.doExec();
            if ( // run remaining locals
            (w.id & FIFO) != 0) w.localPollAndExec(POLL_LIMIT);
            else w.localPopAndExec(POLL_LIMIT);
  ...
  final void helpQuiescePool(WorkQueue w) {
    ...
    for (int source = prevSrc, released = -1; ; ) {
      // -1 until known
      WorkQueue[] ws;
      if (fifo != 0) w.localPollAndExec(0);
      else w.localPopAndExec(0);
      if (released == -1 && w.phase >= 0) released = 1;
        ...
        for (int n = ws.length, j = n, m = n - 1; j > 0; --j) {
          ...
          if ((i = (r - j) & m) >= 0 && i < n && (q = ws[i]) != null) {
            if ((b = q.base) - q.top < 0 && (a = q.array) != null && (al = a.length) > 0) {
              ...
              quiet = empty = false;
              break;
            } else if ((q.source & QUIET) == 0) quiet = false;
          }
        }
      }
      if (quiet) {
        if (released == 0) getAndAddLong(this, CTL, RC_UNIT);
        w.source = prevSrc;
        break;
      } else if (empty) {
        if (source != QUIET) w.source = source = QUIET;
        if (released == 1) {
          // decrement
          released = 0;
    ...
    while ((mode & STOP) == 0 && (ws = workQueues) != null && (n = ws.length) > 0) {
      ...
      for (int k = origin, oldSum = 0, checkSum = 0; ; ) {
        ...
        if ((k = (k + step) & m) == origin) {
          if (oldSum == (oldSum = checkSum)) break rescan;
          checkSum = 0;
        }
      }
    }
  ...
  final ForkJoinTask<?> nextTaskFor(WorkQueue w) {
    ForkJoinTask<?> t;
    if (w != null && (t = (w.id & FIFO) != 0 ? w.poll() : w.pop()) != null) return t;
    else return pollScan(false);
  }
  ...
  private <T> ForkJoinTask<T> externalSubmit(ForkJoinTask<T> task) {
    ...
    if (((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
        ...
        && (q = w.workQueue) != null) q.push(task);
    else externalPush(task);
    return task;
  }
   ...
   *
   * <p>In a steady state strict (tree-structured) computation, each thread makes available for
   ...
   *
   * <p>The minimum useful constant is just 1. But using a value of 1 would require immediate
   ...
   *
   * <p>So, users will want to use values larger (but not much larger) than 1 to both smooth over
   ...
   *
   * <p>When all threads are active, it is on average OK to estimate surplus strictly locally. In
  ...
  private boolean tryTerminate(boolean now, boolean enable) {
    ...
    while (((md = mode) & SHUTDOWN) == 0) {
      if ( // cannot shutdown
      !enable || this == common) return false;
      else U.compareAndSwapInt(this, MODE, md, md | SHUTDOWN);
    }
    while (((md = mode) & STOP) == 0) {
      // try to initiate termination
      if (!now) {
        // check if quiescent & empty
        for (long oldSum = 0L; ; ) {
          // repeat until stable
          boolean running = false;
          long checkSum = ctl;
          WorkQueue[] ws = workQueues;
          if ((md & SMASK) + (int) (checkSum >> RC_SHIFT) > 0) running = true;
          else if (ws != null) {
            WorkQueue w;
          ...
          else if (workQueues == ws && oldSum == (oldSum = checkSum)) break;
        }
      }
      if ((md & STOP) == 0) U.compareAndSwapInt(this, MODE, md, md | STOP);
    }
    while (((md = mode) & TERMINATED) == 0) {
      // help terminate others
      for (long oldSum = 0L; ; ) {
        // repeat until stable
        WorkQueue[] ws;
        WorkQueue w;
      ...
      if ((md & TERMINATED) != 0) break;
      else if ((md & SMASK) + (short) (ctl >>> TC_SHIFT) > 0) break;
   ...
   * java.lang.Runtime#availableProcessors}, using defaults for all other parameters (see {@link
   * #ForkJoinPool(int, ForkJoinWorkerThreadFactory, Thread.UncaughtExceptionHandler, boolean, int,
   * int, int, Predicate, long, TimeUnit)}).
   *
   ...
   * Thread.UncaughtExceptionHandler, boolean, int, int, int, Predicate, long, TimeUnit)}).
   *
   ...
   * {@link #ForkJoinPool(int, ForkJoinWorkerThreadFactory, Thread.UncaughtExceptionHandler,
   ...
   *     common pool, use {@code 256} plus the {@code parallelism} level. (By default, the common
   *     pool allows a maximum of 256 spare threads.) Using a value (for example {@code
   *     Integer.MAX_VALUE}) larger than the implementation's total thread limit has the same effect
   *     as using this limit (which is the default).
   ...
   *     unexecuted tasks may exist, new threads are constructed, up to the given maximumPoolSize.
   *     For the default value, use {@code 1}, that ensures liveness. A larger value might improve
   *     throughput in the presence of blocked activities, but might not, due to increased overhead.
   *     A value of zero may be acceptable when submitted tasks cannot have dependencies requiring
   *     additional threads.
   * @param saturate if non-null, a predicate invoked upon attempts to create more than the maximum
   *     total allowed threads. By default, when a thread is about to block on a join or {@link
   *     ManagedBlocker}, but cannot be replaced because the maximumPoolSize would be exceeded, a
   *     {@link RejectedExecutionException} is thrown. But if this predicate returns {@code true},
   *     then no exception is thrown, so the pool continues to operate with fewer than the target
   *     number of runnable threads, which might not ensure progress.
  ...
  public ForkJoinPool(
    ...
    n |= n >>> 8;
    n |= n >>> 16;
    // power of two, including space for submission queues
    n = (n + 1) << 1;
  ...
  ForkJoinPool(byte forCommonPoolOnly) {
    ...
    long c =
        ((((long) (-parallelism) << TC_SHIFT) & TC_MASK)
            | (((long) (-parallelism) << RC_SHIFT) & RC_MASK));
  ...
  public void execute(Runnable task) {
    ...
    ForkJoinTask<?> job;
    if ( // avoid re-wrap
    task instanceof ForkJoinTask<?>) job = (ForkJoinTask<?>) task;
  ...
  public boolean isQuiescent() {
    for (; ; ) {
      ...
      if ((md & (STOP | TERMINATED)) != 0) return true;
        ...
        if ((ws = workQueues) != null) {
          for (int i = 1; i < ws.length; i += 2) {
            if ((v = ws[i]) != null) {
              if ((v.source & QUIET) == 0) return false;
              --tc;
            }
          }
        }
        if (tc == 0 && ctl == c) return true;
      }
    }
  }

  /**
   * Returns an estimate of the total number of completed tasks that were executed by a thread other
   * than their submitter. The reported value underestimates the actual total number of steals when
   * the pool is not quiescent. This value may be useful for monitoring and tuning fork/join
   * programs: in general, steal counts should be high enough to keep threads busy, but low enough
   * to avoid overhead and contention across threads.
   *
  ...
  public long getStealCount() {
    ...
    if ((ws = workQueues) != null) {
      for (int i = 1; i < ws.length; i += 2) {
        if ((w = ws[i]) != null) count += (long) w.nsteals & 0xffffffffL;
      }
    }
    return count;
  }
  ...
  public String toString() {
    ...
    String level =
        ((md & TERMINATED) != 0
            ...
            : (md & STOP) != 0
                ? "Terminating"
                : (md & SHUTDOWN) != 0 ? "Shutting down" : "Running");
   ...
   * new tasks will be accepted. Invocation has no effect on execution state if this is the {@link
   * #commonPool()}, and no additional effect if already shut down. Tasks that are in the process of
   ...
   * occurs, or the current thread is interrupted, whichever happens first. Because the {@link
   * #commonPool()} never terminates until program shutdown, when applied to the common pool, this
   * method is equivalent to {@link #awaitQuiescence(long, TimeUnit)} but always returns {@code
   * false}.
   *
  ...
  /**
   * If called by a ForkJoinTask operating in this pool, equivalent in effect to {@link
   * ForkJoinTask#helpQuiesce}. Otherwise, waits and/or attempts to assist performing tasks until
   * this pool {@link #isQuiescent} or the indicated timeout elapses.
   *
   * @param timeout the maximum time to wait
   * @param unit the time unit of the timeout argument
  ...
  public boolean awaitQuiescence(long timeout, TimeUnit unit) {
    ...
    if ((thread instanceof ForkJoinWorkerThread)
      ...
      for (long startTime = System.nanoTime(); ; ) {
        ForkJoinTask<?> t;
        if ((t = pollScan(false)) != null) t.doExec();
        else if (isQuiescent()) return true;
        else if ((System.nanoTime() - startTime) > nanos) return false;
        else
          // cannot block
          Thread.yield();
      }
   ...
   *
   * <p>A {@code ManagedBlocker} provides two methods. Method {@link #isReleasable} must return
   ...
   * invocation.
   *
   * <p>For example, here is a ManagedBlocker based on a ReentrantLock:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Here is a class that possibly blocks waiting for an item on a given queue:
   *
   * <pre>{@code
   ...
   * necessary to ensure sufficient parallelism while the current thread is blocked in {@link
   * ManagedBlocker#block blocker.block()}.
   *
   * <p>This method repeatedly calls {@code blocker.isReleasable()} and {@code blocker.block()}
   * until either method returns {@code true}. Every call to {@code blocker.block()} is preceded by
   * a call to {@code blocker.isReleasable()} that returned {@code false}.
   *
   * <p>If not running in a ForkJoinPool, this method is behaviorally equivalent to
   *
   * <pre>{@code
        ...
        for (; ; ) {
          ...
          if ((a = w.array) != null && (d = b - s) < 0 && (al = a.length) > 0) {
            ...
            if (blocker.isReleasable()) break;
              ...
              if (t == null) {
                if (d == -1) break;
              } else if (!isInstanceOfAsynCompTask(t)) break;
  ...
  static {
    ...
    try {
      ...
      if (p != null) commonMaxSpares = Integer.parseInt(p);
    } catch (Exception ignore) {
    }
    ...
    // attempt to load CompletableFuture.AsynchronousCompletionTask
    {
      Class<?> c = null;
      try {
        c = Class.forName("java8.util.concurrent.CompletableFuture$AsynchronousCompletionTask");
      } catch (Exception ign) {
        // ignore
      } finally {
        ACTCLASS = c;
