...
/**
 * This class consists of {@code static} utility methods for operating on objects, or checking
 * certain conditions before operation. These utilities include {@code null}-safe or {@code
 * null}-tolerant methods for computing the hash code of an object, returning a string for an
 * object, comparing two objects, and checking if indexes or sub-range values are out-of-bounds.
 *
 * <p><b>API Note:</b><br>
 * Static methods such as {@link Objects#checkIndex}, {@link Objects#checkFromToIndex}, and {@link
 * Objects#checkFromIndexSize} are provided for the convenience of checking if values corresponding
 * to indexes and sub-ranges are out-of-bounds.
 *
 * @since 1.7
 */
public final class Objects {
   ...
   * Consequently, if both arguments are {@code null}, {@code true} is returned. Otherwise, if the
   * first argument is not {@code null}, equality is determined by calling the {@link Object#equals
   * equals} method of the first argument with the second argument of this method. Otherwise, {@code
   * false} is returned.
   *
   * @param a an object
   ...
   *
   * <p><b>API Note:</b><br>
   * This method exists to be used as a {@link org.java.util.function.Predicate}, {@code
   * filter(Objects::isNull)}
   *
   * @param obj a reference to be checked against {@code null}
   * @return {@code true} if the provided reference is {@code null} otherwise {@code false}
   * @see org.java.util.function.Predicate
   * @since 1.8
   */
  ...
  /**
   * Checks if the {@code index} is within the bounds of the range from {@code 0} (inclusive) to
   * {@code length} (exclusive).
   *
   * <p>The {@code index} is defined to be out-of-bounds if any of the following inequalities is
   * true:
   *
   * <ul>
   *   <li>{@code index < 0}
   *   <li>{@code index >= length}
  ...
  public static int checkIndex(int index, int length) {
    if (index < 0 || index >= length) {
      throw new IndexOutOfBoundsException(
          String.format("Index %d out-of-bounds for length %d", index, length));
    }
    return index;
  }

  /**
   * Checks if the sub-range from {@code fromIndex} (inclusive) to {@code toIndex} (exclusive) is
   * within the bounds of range from {@code 0} (inclusive) to {@code length} (exclusive).
   *
   * <p>The sub-range is defined to be out-of-bounds if any of the following inequalities is true:
   *
   * <ul>
   *   <li>{@code fromIndex < 0}
  ...
  /**
   * Checks if the sub-range from {@code fromIndex} (inclusive) to {@code fromIndex + size}
   * (exclusive) is within the bounds of range from {@code 0} (inclusive) to {@code length}
   * (exclusive).
   *
   * <p>The sub-range is defined to be out-of-bounds if any of the following inequalities is true:
   *
  ...
  public static int checkFromIndexSize(int fromIndex, int size, int length) {
    if ((length | fromIndex | size) < 0 || size > length - fromIndex) {
      throw new IndexOutOfBoundsException(
          String.format(
              "Range [%d, %<d + %d) out-of-bounds for length %d", fromIndex, size, length));
    }
    return fromIndex;
  }
