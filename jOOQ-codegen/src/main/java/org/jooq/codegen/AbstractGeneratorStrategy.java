...
public abstract class AbstractGeneratorStrategy implements GeneratorStrategy {
  ...
  @Override
  public final String getGlobalReferencesFileName(
  ...
  @Override
  public final File getGlobalReferencesFile(
      Definition container, Class<? extends Definition> objectType) {
    String dir = getTargetDirectory();
  ...
  public final String getFullJavaIdentifier(Definition definition) {
    ...
    if (definition instanceof EmbeddableDefinition) {
      ...
      if (getInstanceFields()) sb.append(getFullJavaIdentifier(e.getReferencingTable()));
      else sb.append(getFullJavaClassName(e.getReferencingTable()));
    }
    ...
    // Columns
      ...
      if (getInstanceFields()) sb.append(getFullJavaIdentifier(e.getContainer()));
      else sb.append(getFullJavaClassName(e.getContainer()));
      ...
      if (getTargetLanguage() == KOTLIN)
        sb.append(
            getGlobalReferencesJavaPackageName(definition.getSchema(), definition.getClass()));
      else
        sb.append(
            getGlobalReferencesFullJavaClassName(definition.getSchema(), definition.getClass()));
    }
  ...
  @Override
  public final String getGlobalReferencesFullJavaClassName(
      Definition container, Class<? extends Definition> objectType) {
    return getGlobalReferencesJavaPackageName(container, objectType)
        + "."
  ...
  final String getFixedJavaIdentifier(Definition definition) {
    ...
    if (definition instanceof IdentityDefinition)
      return "IDENTITY_"
          + getJavaIdentifier(((IdentityDefinition) definition).getColumn().getContainer());

    // [#2032] Intercept default Catalog
    else if (definition instanceof CatalogDefinition
        && ((CatalogDefinition) definition).isDefaultCatalog()) return "DEFAULT_CATALOG";

    // [#2089] Intercept default schema
    else if (definition instanceof SchemaDefinition
        && ((SchemaDefinition) definition).isDefaultSchema()) return "DEFAULT_SCHEMA";
    else return null;
  }
  ...
  final String getFixedJavaClassName(Definition definition) {
    ...
    if (definition instanceof CatalogDefinition
        && ((CatalogDefinition) definition).isDefaultCatalog()) return "DefaultCatalog";

    // [#2089] Intercept default schema
    else if (definition instanceof SchemaDefinition
        && ((SchemaDefinition) definition).isDefaultSchema()) return "DefaultSchema";
    else return null;
  }
}
