...
class GenerationUtil {
  ...
  static final Pattern TYPE_REFERENCE_PATTERN =
      Pattern.compile(
          "^((?:[\\p{L}_$][\\p{L}\\p{N}_$]*\\.)*[\\p{L}_$][\\p{L}\\p{N}_$]*)((?:<.*>|\\[.*])*)$");
  static final Pattern PLAIN_GENERIC_TYPE_PATTERN =
      Pattern.compile(
          "[<\\[]((?:[\\p{L}_$][\\p{L}\\p{N}_$]*\\.)*[\\p{L}_$][\\p{L}\\p{N}_$]*)[>\\]]");
  ...
  private static final Set<String> SCALA_KEYWORDS =
      unmodifiableSet(
          new HashSet<>(
              asList(
                  "abstract",
  ...
  private static final Set<Character> SCALA_WHITESPACE =
      unmodifiableSet(
          new HashSet<>(asList((char) 0x0020, (char) 0x0009, (char) 0x000D, (char) 0x000A)));

  private static final Set<Character> SCALA_PARENTHESES =
      unmodifiableSet(new HashSet<>(asList('(', ')', '[', ']', '{', '}')));

  private static final Set<Character> SCALA_DELIMITER =
      unmodifiableSet(new HashSet<>(asList('`', '\'', '"', '.', ';', ',')));

  private static final Set<String> WINDOWS_FORBIDDEN =
      unmodifiableSet(
          new HashSet<>(
              asList(
  ...
  private static Boolean isScalaOperator(char c) {
    return (c >= 0x0020
            ...
            && !Character.isDigit(c)
            && !SCALA_DELIMITER.contains(c)
            && !SCALA_PARENTHESES.contains(c)
            && !SCALA_WHITESPACE.contains(c))
        || Character.getType(c) == Character.MATH_SYMBOL /* Sm */
        || Character.getType(c) == Character.OTHER_SYMBOL /* So */;
  }
  ...
  /**
   * Take a name and escape it if it is a Windows forbidden name like <code>CON</code> or <code>AUX
   * </code>.
   *
  ...
  /**
   * Take a literal (e.g. database column) and make it a Java identifier to be used without
   * case-change as an enum identifier
   *
   * <p>[#959] These literals are escaped if they collide with reserved words. This implementation
   * is meant as a fix for [#959]. These types of collisions have to be generally reviewed again,
   * when allowing for more control over generated source code, as of [#408][#911]
   *
   * <p>
   */
  public static String convertToIdentifier(String literal, Language language) {
    ...
    if (language == KOTLIN && KOTLIN_KEYWORDS.contains(literal)) return "`" + literal + "`";

    // [#4703] If an identifier consists only of underscores add one more. _ -> __, __ -> ___
    if (UNDERSCORE_PATTERN.matcher(literal).matches()) return literal + "_";

    StringBuilder sb = new StringBuilder();

    if ("".equals(literal))
      if (language == SCALA) return "`_`";
      else if (language == KOTLIN) return "`_`";
      else return "_";

    for (int i = 0; i < literal.length(); i++) {
      char c = literal.charAt(i);

      // [#5424] Scala setters, by convention, end in "property_=", where "=" is an operator and "_"
      // precedes it
      if (language == SCALA
          && i == literal.length() - 1
          && literal.length() >= 2
          && literal.charAt(i - 1) == '_'
          && isScalaOperator(c)) sb.append(c);
      ...
      else if (language == KOTLIN && !isKotlinIdentifierPart(c)) sb.append(escape(c));

      // TODO: Should we do this for Scala as well?
      else if (language == KOTLIN && !Character.isJavaIdentifierPart(c)) return "`" + literal + "`";
      ...
      else if (language == KOTLIN && c == '$') return "`" + literal + "`";
      else sb.append(c);
    }
  ...
  private static String escape(char c) {
    if (c == ' ' || c == '-' || c == '.') return "_";
    else return "_" + Integer.toHexString(c);
  }
  ...
  static String getSimpleJavaType(String qualifiedJavaType) {
    if (qualifiedJavaType == null) return null;
      ...
      default:
        {
          ...
          if ("ARRAY".equalsIgnoreCase(t)) return name("OTHER");

          // This is for backwards compatibility
          else return name(t.replaceFirst("(?i: ARRAY)", ""));
        }
    }
  }

  static ExpressionType expressionType(String expression) {
    if (TYPE_REFERENCE_PATTERN.matcher(expression).matches()) return CONSTRUCTOR_REFERENCE;
