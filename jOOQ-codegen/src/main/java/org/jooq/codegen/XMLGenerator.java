...
public class XMLGenerator extends AbstractGenerator {
  ...
  public void generate(Database db) {
    ...
    TextWriter out =
        new TextWriter(getStrategy().getFile("information_schema.xml"), targetEncoding);
    log.info("");
    ...
    for (CatalogDefinition c : db.getCatalogs()) {
      ...
      for (SchemaDefinition s : c.getSchemata()) {
        ...
        if (generateCommentsOnSchemas()) schema.setComment(s.getComment());
        ...
        for (TableDefinition t : s.getTables()) {
          ...
          table.setTableType(
              t.isView()
                  ? TableType.VIEW
                  : t.isTemporary() ? TableType.GLOBAL_TEMPORARY : TableType.BASE_TABLE);

          if (generateCommentsOnTables()) table.setComment(t.getComment());
          ...
          if (t.isView()) {
            ...
            if (generateSourcesOnViews()) view.setViewDefinition(t.getSource());
          ...
          for (ColumnDefinition co : t.getColumns()) {
            ...
            if (generateCommentsOnColumns()) column.setComment(co.getComment());
            ...
            if (co.isIdentity()) column.setIdentityGeneration("YES");
        ...
        for (IndexDefinition i : db.getIndexes(s)) {
          ...
          if (generateCommentsOnKeys()) index.setComment(i.getComment());
        ...
        for (UniqueKeyDefinition u : db.getKeys(s)) {
          ...
          if (generateCommentsOnKeys()) tc.setComment(u.getComment());
        ...
        for (ForeignKeyDefinition f : db.getForeignKeys(s)) {
          ...
          if (generateCommentsOnKeys()) tc.setComment(f.getComment());
        ...
        for (CheckConstraintDefinition ch : db.getCheckConstraints(s)) {
          ...
          if (generateCommentsOnKeys()) tc.setComment(ch.getComment());
        ...
        for (SequenceDefinition se : db.getSequences(s)) {
          ...
          if (generateCommentsOnSequences()) sequence.setComment(se.getComment());
  ...
  private void exportRoutine(
      InformationSchema is, RoutineDefinition r, String catalogName, String schemaName) {
    ...
    if (generateCommentsOnRoutines()) routine.setComment(r.getComment());
    ...
    for (ParameterDefinition p : r.getAllParameters()) {
      if (p != r.getReturnValue()) {
        ...
        if (r.getPackage() != null) parameter.setSpecificPackage(r.getPackage().getName());

        parameter.setSpecificName(specificName);
        parameter.setOrdinalPosition(i++);
        parameter.setParameterName(p.getName());

        if (generateCommentsOnParameters()) parameter.setComment(p.getComment());

        boolean in = r.getInParameters().contains(p);
        boolean out = r.getOutParameters().contains(p);

        if (in && out) parameter.setParameterMode(ParameterMode.INOUT);
        else if (in) parameter.setParameterMode(ParameterMode.IN);
        else if (out) parameter.setParameterMode(ParameterMode.OUT);
