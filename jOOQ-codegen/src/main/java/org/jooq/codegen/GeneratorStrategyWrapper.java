...
/**
 * A wrapper for generator strategies preventing some common compilation errors resulting from badly
...
class GeneratorStrategyWrapper extends AbstractGeneratorStrategy {
  ...
  /**
   * [#182] Method name disambiguation is important to avoid name clashes due to pre-existing
   * getters / setters in super classes
   */
  private String disambiguateMethod(Definition definition, String method) {
    ...
    if (definition instanceof AttributeDefinition) {
      ...
      else reserved = reservedColumns(TableRecordImpl.class, 0);
    }
    ...
    if (reserved != null) {
      if (reserved.contains(method)) return method + "_";
      ...
      if (method.startsWith("set")) {
        ...
        if (reserved.contains("get" + base) || reserved.contains("is" + base)) return method + "_";
      }
    }
  ...
  /**
   * [#182] Find all column names that are reserved because of the extended class hierarchy of a
   * generated class
   */
  private Set<String> reservedColumns(Class<?> clazz, int length) {
    ...
    if (result == null) {
      ...
      for (Class<?> c : clazz.getInterfaces()) result.addAll(reservedColumns(c, length));

      for (Method m : clazz.getDeclaredMethods())
        if (m.getParameterTypes().length == length) result.add(m.getName());
      ...
      if (getTargetLanguage() == Language.SCALA)
        for (Field f : clazz.getDeclaredFields()) result.add(f.getName());
    }
  ...
  public List<String> getJavaClassImplements(Definition definition, Mode mode) {
    ...
    if (mode == Mode.INTERFACE && generator.generateSerializableInterfaces())
  ...
  public String getJavaClassName(Definition definition, Mode mode) {
    ...
    if (definition instanceof TableDefinition
        ...
        && mode == Mode.RECORD) return Record.class.getSimpleName();
  ...
  public String getJavaPackageName(Definition definition, Mode mode) {
    ...
    // [#1150] Intercept Mode.RECORD calls for tables
    if (!generator.generateRecords()
        && mode == Mode.RECORD
        && definition instanceof TableDefinition) return Record.class.getPackage().getName();
  ...
  public String getJavaMemberName(Definition definition, Mode mode) {
    String identifier =
        convertToIdentifier(delegate.getJavaMemberName(definition, mode), getTargetLanguage());
