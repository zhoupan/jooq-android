 ...
 *
 * <p>It takes its configuration parameters from an XML file passed in either as a JAXB-annotated
 * {@link Configuration} object, or from the file system when passed as an argument to {@link
 * #main(String[])}.
 *
 * <p>See <a href="http://www.jooq.org/xsd/">http://www.jooq.org/xsd/</a> for the latest XSD
 * specification.
 *
 * @author Lukas Eder
 */
public class GenerationTool {
   ...
   *
   * <p>If set, all classes are loaded with this class loader
   */
   ...
   *
   * <p>If set, the configuration XML's <code>&lt;jdbc/&gt;</code> configuration is ignored, and
   * this connection is used for meta data inspection, instead.
   */
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (String file : files) {
      ...
      try {
        ...
        if (in == null && new File(file).exists()) in = new FileInputStream(file);

        if (in == null) {
          log.error(
              "Cannot find "
                  + file
                  + " on classpath, or in directory "
  ...
  private void run0(Configuration configuration) throws Exception {
      ...
      if (property != null) {
        try {
          ...
          log.error(
              ...
              "Unsupported value for system property jooq.codegen.logging: "
                  + property
                  + ". Supported values include: "
                  + Arrays.asList(Logging.values()));
        }
      }
    }
    ...
    if (log.isDebugEnabled()) log.debug("Input configuration", "" + configuration);
    ...
    if (g == null)
      throw new GeneratorException(
          "The <generator/> tag is mandatory. For details, see " + Constants.NS_CODEGEN);

    // [#3669] Optional Database element
    if (g.getDatabase() == null) g.setDatabase(new org.jooq.meta.jaxb.Database());
    ...
    try {
      ...
      if (connection == null) {
        ...
        if (dataSource != null) {
          ...
          if (url != null) {
            ...
            if (j.getDriver() == null) j.setDriver(System.getProperty("jooq.codegen.jdbc.driver"));
            if (j.getUrl() == null) j.setUrl(url);
            if (j.getUser() == null) j.setUser(System.getProperty("jooq.codegen.jdbc.user"));
            ...
            if (j.isAutoCommit() == null) {
              ...
              if (a != null) j.setAutoCommit(Boolean.valueOf(a));
            }
          }
          ...
          if (j != null) {
            try {
              ...
              if (databaseName != null)
                if (databaseName.contains("DDLDatabase")
                    || databaseName.contains("XMLDatabase")
                    || databaseName.contains("JPADatabase"))
                  log.warn(
                      "Error while connecting to database. Note that file based database implementations do not need a <jdbc/> configuration in the code generator.",
                      e);
      ...
      if (matchers != null) {
        ...
        if (g.getStrategy().getName() != null) {
          ...
          if (!DefaultGeneratorStrategy.class.getName().equals(g.getStrategy().getName()))
            log.warn(
                "WARNING: Matchers take precedence over custom strategy. Strategy ignored: "
                    + g.getStrategy().getName());

          g.getStrategy().setName(null);
        }
      } else {
        Class<GeneratorStrategy> strategyClass =
      ...
      Class<? extends Database> databaseClass =
          !isBlank(databaseName)
              ...
              : connection != null ? databaseClass(connection) : databaseClass(j);
      ...
      // For convenience, the catalog configuration can be set also directly in the <database/>
      // element
      if (catalogsEmpty) {
        if (isBlank(d.getInputCatalog()) && !isBlank(d.getOutputCatalog()))
          log.warn(
              "WARNING: /configuration/generator/database/outputCatalog must be paired with /configuration/generator/database/inputCatalog");
        ...
        if (!isBlank(catalog.getInputCatalog())) catalogsEmpty = false;

        // For convenience and backwards-compatibility, the schema configuration can be set also
        // directly
        // in the <database/> element
        if (schemataEmpty) {
          if (isBlank(d.getInputSchema()) && !isBlank(d.getOutputSchema()))
            log.warn(
                "WARNING: /configuration/generator/database/outputSchema must be paired with /configuration/generator/database/inputSchema");
          ...
          if (!isBlank(schema.getInputSchema())) schemataEmpty = false;
        } else {
        ...
        if (!isBlank(d.getInputCatalog()))
          log.warn(
              "WARNING: Cannot combine configuration properties /configuration/generator/database/inputCatalog and /configuration/generator/database/catalogs");
        if (!isBlank(d.getOutputCatalog()))
          log.warn(
              "WARNING: Cannot combine configuration properties /configuration/generator/database/outputCatalog and /configuration/generator/database/catalogs");
        if (!isBlank(d.getInputSchema()))
          log.warn(
              "WARNING: Cannot combine configuration properties /configuration/generator/database/inputSchema and /configuration/generator/database/catalogs");
        if (!isBlank(d.getOutputSchema()))
          log.warn(
              "WARNING: Cannot combine configuration properties /configuration/generator/database/outputSchema and /configuration/generator/database/catalogs");
        if (!schemataEmpty)
          log.warn(
              "WARNING: Cannot combine configuration properties /configuration/generator/database/catalogs and /configuration/generator/database/schemata");
      }

      for (CatalogMappingType catalog : catalogs) {
        if ("".equals(catalog.getOutputCatalog()))
          log.warn(
              "WARNING: Empty <outputCatalog/> should not be used to model default outputCatalogs. Use <outputCatalogToDefault>true</outputCatalogToDefault>, instead. See also: https://github.com/jOOQ/jOOQ/issues/3018");

        // [#3018] If users want the output catalog to be "" then, ignore the actual
        // <outputCatalog/> configuration
        ...
        for (SchemaMappingType schema : catalog.getSchemata()) {
          ...
          if ("".equals(schema.getOutputSchema()))
            log.warn(
                "WARNING: Empty <outputSchema/> should not be used to model default outputSchemas. Use <outputSchemaToDefault>true</outputSchemaToDefault>, instead. See also: https://github.com/jOOQ/jOOQ/issues/3018");

          // [#3018] If users want the output schema to be "" then, ignore the actual
      ...
      database.setForceIntegerTypesOnZeroScaleDecimals(
          !FALSE.equals(d.isForceIntegerTypesOnZeroScaleDecimals()));
      ...
      database.setRegexMatchesPartialQualification(
          !FALSE.equals(d.isRegexMatchesPartialQualification()));
      database.setSqlMatchesPartialQualification(
          !FALSE.equals(d.isSqlMatchesPartialQualification()));
      ...
      if (!isBlank(d.getOrderProvider())) {
        ...
        else log.warn("Order provider must be of type java.util.Comparator: " + orderProvider);
      }
      ...
      if (isBlank(g.getTarget().getEncoding())) g.getTarget().setEncoding(DEFAULT_TARGET_ENCODING);
      ...
      if (!new File(g.getTarget().getDirectory()).isAbsolute())
        g.getTarget()
            .setDirectory(
                new File(configuration.getBasedir(), g.getTarget().getDirectory())
                    .getCanonicalPath());
      ...
      if (g.getTarget().isClean() != null) generator.setTargetClean(g.getTarget().isClean());
      ...
      if (g.getGenerate().isDeprecationOnUnknownTypes() != null)
        generator.setGenerateDeprecationOnUnknownTypes(
            g.getGenerate().isDeprecationOnUnknownTypes());
      ...
      if (g.getGenerate().isConstructorPropertiesAnnotation() != null)
        generator.setGenerateConstructorPropertiesAnnotation(
            g.getGenerate().isConstructorPropertiesAnnotation());
      if (g.getGenerate().isConstructorPropertiesAnnotationOnPojos() != null)
        generator.setGenerateConstructorPropertiesAnnotationOnPojos(
            g.getGenerate().isConstructorPropertiesAnnotationOnPojos());
      if (g.getGenerate().isConstructorPropertiesAnnotationOnRecords() != null)
        generator.setGenerateConstructorPropertiesAnnotationOnRecords(
      ...
      if (g.getGenerate().isRecordsImplementingRecordN() != null)
        generator.setGenerateRecordsImplementingRecordN(
            g.getGenerate().isRecordsImplementingRecordN());
      ...
      if (g.getGenerate().isKotlinSetterJvmNameAnnotationsOnIsPrefix() != null)
        generator.setGenerateKotlinSetterJvmNameAnnotationsOnIsPrefix(
            g.getGenerate().isKotlinSetterJvmNameAnnotationsOnIsPrefix());
      if (g.getGenerate().getGeneratedSerialVersionUID() != null)
        generator.setGenerateGeneratedSerialVersionUID(
            g.getGenerate().getGeneratedSerialVersionUID());
      ...
      if (g.getGenerate().isKeys() != null) generator.setGenerateKeys(g.getGenerate().isKeys());
      ...
      if (g.getGenerate().isJavaBeansGettersAndSetters() != null)
        generator.setGenerateJavaBeansGettersAndSetters(
            g.getGenerate().isJavaBeansGettersAndSetters());
      ...
      if (g.getGenerate().getPrintMarginForBlockComment() != null)
        generator.setGeneratePrintMarginForBlockComment(
            g.getGenerate().getPrintMarginForBlockComment());

      if (!isBlank(d.getSchemaVersionProvider())) generator.setUseSchemaVersionProvider(true);
      if (!isBlank(d.getCatalogVersionProvider())) generator.setUseCatalogVersionProvider(true);
      ...
      logUnused(
          "synthetic identity", "synthetic identities", database.getUnusedSyntheticIdentities());
      logUnused(
          "synthetic primary key",
          "synthetic primary keys",
          database.getUnusedSyntheticPrimaryKeys());
      logUnused(
          "synthetic unique key", "synthetic unique keys", database.getUnusedSyntheticUniqueKeys());
      logUnused(
      ...
      if (connection != null) {
        if (close) {
          ...
          if (ctx != null && ctx.family() == HSQLDB && dataSource == null) ctx.execute("shutdown");
  ...
  private void verifyVersions() {
    ...
    // [#12488] Check if all of jOOQ, jOOQ-meta, jOOQ-codegen are using the same versions and
    // editions
    try {
        ...
        if (!Objects.equals(c1, c2))
          log.warn(
              ...
              "org.jooq.Constants."
                  ...
                  + " vs "
                  + c2
                  + ". Check if you're using the same versions for org.jooq ("
                  + v1
                  + ") and org.jooq.meta ("
                  + v2
                  + ")");
  ...
  private void logUnused(String objectType, String objectTypes, List<?> list) {
    if (!list.isEmpty()
        && Boolean.parseBoolean(System.getProperty("jooq.codegen.logunused", "true"))) {
      unusedLogger.warn(
          "Unused " + objectTypes,
          "There are unused "
              + objectTypes
      ...
      for (Object o : list) unusedLogger.warn("Unused " + objectType, o);
    }
  }
  ...
  private Properties properties(List<Property> properties) {
    ...
    for (Property p : properties) result.put(p.getKey(), p.getValue());
    ...
    try {
      ...
      if (className.startsWith("org.jooq.util.")) {
        ...
        log.warn(
            "Type not found",
            message =
                "Your configured database type was not found: "
                    + className
                    + ".\n"
      ...
      else throw new ClassNotFoundException(message, e);
    }
  }
