...
abstract class AbstractGenerator implements Generator {
  ...
  GeneratedAnnotationType generatedGeneratedAnnotationType =
  ...
  void logGenerationRemarks(Database db) {
    ...
    if (contains(db.getIncludes(), ',') && db.getIncluded().isEmpty())
      log.info(
          "  includes",
          "The <includes/> element takes a Java regular expression, not a comma-separated list. This might be why no objects were included.");

    if (contains(db.getExcludes(), ',') && db.getExcluded().isEmpty())
      log.info(
          "  excludes",
          "The <excludes/> element takes a Java regular expression, not a comma-separated list. This might be why no objects were excluded.");
  }
  ...
  @Override
  public void setGenerateGeneratedAnnotationType(
  ...
  @Override
  public void setGenerateConstructorPropertiesAnnotation(
  ...
  @Override
  public void setGenerateConstructorPropertiesAnnotationOnPojos(
      boolean generateConstructorPropertiesAnnotationOnPojos) {
    this.generateConstructorPropertiesAnnotationOnPojos =
  ...
  public boolean generateConstructorPropertiesAnnotationOnPojos() {
    return TRUE.equals(generateConstructorPropertiesAnnotationOnPojos)
        || generateConstructorPropertiesAnnotation()
            && generateConstructorPropertiesAnnotationOnPojos == null;
  }

  @Override
  public void setGenerateConstructorPropertiesAnnotationOnRecords(
      boolean generateConstructorPropertiesAnnotationOnRecords) {
    this.generateConstructorPropertiesAnnotationOnRecords =
  ...
  public boolean generateConstructorPropertiesAnnotationOnRecords() {
    return TRUE.equals(generateConstructorPropertiesAnnotationOnPojos)
        || generateConstructorPropertiesAnnotation()
            && generateConstructorPropertiesAnnotationOnRecords == null;
  }
  ...
  public boolean generateImmutableInterfaces() {
    return generateImmutableInterfaces
        || (generateInterfaces && (generateImmutablePojos || generatePojosAsJavaRecordClasses));
  }
  ...
  @Override
  public void setGenerateKotlinSetterJvmNameAnnotationsOnIsPrefix(
      boolean generateKotlinSetterJvmNameAnnotationsOnIsPrefix) {
    this.generateKotlinSetterJvmNameAnnotationsOnIsPrefix =
  ...
  @Override
  public void setGenerateGeneratedSerialVersionUID(
  ...
  public void setGenerateNewline(String newline) {
    ...
    this.generateNewline =
        newline == null ? newline : newline.replace("\\r", "\r").replace("\\n", "\n");
  }
  ...
  /**
   * If file is a directory, recursively empty its children. If file is a file, delete it, except if
   * it is in the list of files to keep.
   */
  protected void empty(File file, String suffix, Set<File> keep, Set<File> ignore) {
    ...
    if (file != null) {
      ...
      if (file.getParentFile() == null) {
        log.warn(
            "WARNING: Root directory configured for code generation. Not deleting anything from previous generations!");
        return;
      }
      ...
      if (file.isDirectory()) {
        ...
        //         Useful if a catalog / schema was dropped, or removed from code generation, or
        // renamed
        if (childrenAfterDeletion != null && childrenAfterDeletion.length == 0) file.delete();
      } else if (file.getName().endsWith(suffix) && !keep.contains(file)) file.delete();
    }
  }
}
